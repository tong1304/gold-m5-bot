    ...
    try:
        ...
        with _LOCK:
            ...
            if parsed and parsed.get("status") == "failed":
                failed = [x.get("error") for x in parsed.get("results", []) if x.get("status") == "failed" and x.get("error")]
            ...
            _STATE.update({
                "returncode": code,
                "result": parsed,
                "status": "completed" if code == 0 and not (parsed and parsed.get("status") == "failed") else "failed",
                "running": False,
                "finished_at": _now(),
                "error": error,
            })
