"""V12.1 Historical Replay web service: H1 bias + M15 regime + M5 trigger, isolated from live statistics."""
...
def register(app):
    ...
    def replay_start():
        ...
        threading.Thread(target=_worker,args=(symbols,start,end,start_label,end_label),name="v12-mtf-replay",daemon=True).start();return _json({"status":"started","engine_version":"12.1-MTF-H1-M15-M5-REGIME-8-ENGINE-REENTRY","source":"LSE_HISTORICAL_MTF_OHLCV","timeframe_mode":"MTF:H1→M15→M5","start":start_label,"end":end_label},202)
PAGE='''<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V12.1 MTF Historical Replay</title><style>body{font-family:system-ui;background:#0b1220;color:#e8eef8;padding:20px}main{max-width:1100px;margin:auto}.box{background:#121c30;border:1px solid #263654;border-radius:14px;padding:18px;margin:12px 0}button,a{padding:10px 14px;border-radius:8px;background:#172238;color:#fff;text-decoration:none;border:1px solid #33425e;cursor:pointer}.muted{color:#93a4bd}</style></head><body><main><div class="box"><h1>⏪ V12.1 Historical Replay — MTF</h1><p class="muted">LSE Historical · H1 Big Trend/Bias → M15 Trend/Regime Filter → M5 Setup/Entry Trigger · Lookahead-safe · Replay แยกจาก Live statistics</p><p><label>เริ่ม <input id="start" value="2026-08-21"></label> <label>ถึง <input id="end" value="2026-08-24"></label></p><button onclick="start()">▶ เริ่ม Replay</button> <a href="/statistics">📊 Statistics</a><span id="s" class="muted"> สถานะ: idle</span></div><div class="box"><div id="out">ยังไม่มีผล</div></div></main><script>const $=x=>document.getElementById(x);async function start(){let q=new URLSearchParams({symbols:'BTC,GOLD',start:$('start').value,end:$('end').value});await fetch('/api/replay/start?'+q.toString());poll()}async function poll(){let d=await(await fetch('/api/replay/status?ts='+Date.now(),{cache:'no-store'})).json();$('s').textContent='สถานะ: '+d.status;if(d.progress){let p=d.progress;$('out').textContent=(p.event==='data_fetch'?'กำลังโหลด LSE '+(p.timeframe||'')+': '+p.symbol+' '+p.day+' ('+(p.completed||0)+'/'+(p.total||0)+')':'กำลังประมวลผล '+p.symbol+': '+(p.percent??0)+'% · signals '+(p.trades??0)+' · WIN '+(p.wins??0)+' · LOSS '+(p.losses??0)+' · Net R '+(p.net_r??0))}if(d.result?.reports){$('out').innerHTML='<p>MTF: H1 → M15 → M5 · ช่วง: '+d.result.requested_window.start+' → '+d.result.requested_window.end+'</p>'+d.result.data_quality.map(q=>'<p><b>'+q.symbol+'</b> H1: '+q.h1.historical_rows+' · M15: '+q.m15.historical_rows+' · M5 Target: '+q.m5.target_rows+'</p>').join('')+d.result.reports.map(r=>{let p=r.performance||{};return '<h2>'+r.symbol+'</h2><p>Candles: '+(r.candles_evaluated??0)+' · Signals: '+(r.signals??0)+' · WIN: '+(p.wins??0)+' · LOSS: '+(p.losses??0)+' · Win Rate: '+(p.win_rate??0)+'% · Net R: '+(r.net_r??0)+'</p>'}).join('')}else if(d.error)$('out').textContent=d.error;if(d.running)setTimeout(poll,1000)}poll()</script></body></html>'''
