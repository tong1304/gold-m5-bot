    ...
    try:
        ...
        for line in proc.stdout:
            ...
            with _LOCK:
                ...
                _STATE["output"] = _STATE["output"][-100:]
        code = proc.wait()
        parsed = None
        for line in reversed(_STATE["output"]):
            try:
                ...
                continue
        with _LOCK:
            _STATE["returncode"] = code
            _STATE["result"] = parsed
            _STATE["status"] = "completed" if code == 0 else "failed"
...
def register(app):
    ...
    def replay_start():
        ...
        with _LOCK:
            ...
            _STATE.update({"running": True, "status": "running", "start": start, "end": end,
                           "symbol": symbol, "started_at": _now(), "finished_at": None,
                           "returncode": None, "output": [], "result": None, "error": None})
        thread = threading.Thread(target=_worker, args=(start, end, symbol), name="historical-replay", daemon=True)
        thread.start()
        return _json({"status": "started", "message": "เริ่ม Replay ราคาย้อนหลังจริงจาก LSE แล้ว"}), 202


PAGE = r'''<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Historical Replay</title><style>
body{font-family:system-ui,-apple-system,sans-serif;background:#0b1220;color:#e8eef8;margin:0;padding:20px}main{max-width:900px;margin:auto}.box{background:#121c30;border:1px solid #243451;border-radius:14px;padding:18px;margin:12px 0}h1{margin:0 0 6px}.muted{color:#93a4bd}label{display:block;margin:12px 0 6px}input,select,button{width:100%;box-sizing:border-box;background:#172238;color:#fff;border:1px solid #33425e;border-radius:8px;padding:10px}button{cursor:pointer;margin-top:15px;background:#2459a8}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}pre{white-space:pre-wrap;max-height:350px;overflow:auto;background:#09101d;padding:12px;border-radius:8px}.ok{color:#55d68b}.bad{color:#ff6b78}@media(max-width:650px){.grid{grid-template-columns:1fr}}
a{color:#70a7ff}
</style></head><body><main><div class="box"><h1>⏪ Historical Replay</h1><div class="muted">จำลองสัญญาณด้วยราคาย้อนหลังจริงจาก LSE และบันทึก WIN/LOSS ลง Statistics โดยไม่ส่ง Telegram และไม่เปิดออเดอร์</div></div>
<div class="box"><div class="grid"><div><label>เริ่มวันที่</label><input id="start" type="date" value="2026-08-01"></div><div><label>ถึงวันที่</label><input id="end" type="date"></div></div><label>สินทรัพย์</label><select id="symbol"><option value="ALL">BTC + GOLD</option><option value="BTC">BTC</option><option value="GOLD">GOLD</option></select><button onclick="startReplay()">▶ เริ่ม Replay</button></div>
<div class="box"><b id="status">สถานะ: กำลังตรวจสอบ...</b><div id="summary" class="muted" style="margin-top:10px"></div><pre id="log">-</pre><p><a href="/statistics">📊 เปิดหน้าสถิติย้อนหลัง</a></p></div>
</main><script>
const $=x=>document.getElementById(x); const now=new Date(); $('end').value=now.toISOString().slice(0,10);
async function startReplay(){const body={start:$('start').value,end:$('end').value,symbol:$('symbol').value};const r=await fetch('/api/replay/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok){alert(d.message||'เริ่ม Replay ไม่สำเร็จ');return}poll()}
function render(d){$('status').textContent='สถานะ: '+d.status; $('status').className=d.status==='completed'?'ok':d.status==='failed'?'bad':''; const rs=d.result&&d.result.results?d.result.results:[]; $('summary').textContent=rs.map(x=>`${x.symbol}: generated=${x.generated}, inserted=${x.inserted}, WIN=${x.outcomes.WIN}, LOSS=${x.outcomes.LOSS}, OPEN=${x.outcomes.OPEN}, AMBIGUOUS=${x.outcomes.AMBIGUOUS}`).join(' | '); $('log').textContent=(d.output||[]).join('\n')||d.error||'-'}
async function poll(){const r=await fetch('/api/replay/status');const d=await r.json();render(d);if(d.running)setTimeout(poll,2000)}poll();
</script></body></html>'''
''