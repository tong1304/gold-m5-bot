"""Web-triggered V12 historical replay runner for the free Render web service."""
...
def _trade_history(rows):
    trades=[]
    for row in rows or []:
        if row.get("signal") not in ("BUY","SELL") or not row.get("valid"): continue
        levels=row.get("trade_levels") or {};targets=levels.get("tp_levels") or [];prices=[t.get("price") for t in targets if isinstance(t,dict)]
        trades.append({"candle_time":row.get("candle_time"),"signal":row.get("signal"),"strategy":row.get("strategy"),"entry":levels.get("entry"),"sl":levels.get("sl"),"tp":levels.get("tp"),"tp1":prices[0] if len(prices)>0 else levels.get("tp"),"tp2":prices[1] if len(prices)>1 else None,"tp3":prices[2] if len(prices)>2 else None,"rr":levels.get("effective_rr",levels.get("risk_reward")),"result":row.get("result"),"r_multiple":row.get("r_multiple"),"resolved_at":row.get("resolved_at")})
    return trades

def _compact_result(result):
    if not isinstance(result,dict): return result
    compact=copy.deepcopy(result);compact["reports"]=[]
    for report in (result.get("reports") or []):
        item={k:v for k,v in report.items() if k!="rows"};item["trade_history"]=_trade_history(report.get("rows") or []);compact["reports"].append(item)
    return compact

def _public_state(state):
    public=dict(state);public["result"]=_compact_result(state.get("result"));return public
...
def _worker(start,end,symbol):
    ...
    try:
        ...
        code=proc.wait()
        with _LOCK:
            parsed=_parse_result(_STATE["output"]);failed=[x.get("error") for x in (parsed or {}).get("reports",[]) if x.get("status")=="failed" and x.get("error")];status="failed" if code!=0 else (parsed.get("status") if parsed else "failed");error=("Replay process exited with code %s"%code) if code!=0 else ("; ".join(failed) if failed else None);_STATE.update({"returncode":code,"result":_compact_result(parsed),"status":status,"running":False,"finished_at":_now(),"error":error});_PROC=None
...
def register(app):
    ...
    def replay_start():
        ...
        with _LOCK:
            if _STATE["running"]:return _json({"status":"busy","message":"V12 Historical Replay กำลังทำงานอยู่","state":_public_state(_STATE)}),409
