...
def _historical_m5(symbol,start,end,progress=None):
    from lse import LSE
    ...
    # LSE candles accepts calendar dates only. Fetch each calendar day separately
    # so the M5 target window cannot be silently truncated by an API row limit.
    first_day=warmup.date();last_day=(end-pd.Timedelta(nanoseconds=1)).date()
    frames=[];days=[]
    ...
    while day<=last:
        ...
        if progress:progress("data_fetch",symbol=symbol,day=api_start,completed=len(days),total=int((last-day).days)+1)
        raw=client.candles(market,"5m",start=api_start,end=api_end,limit=1000,order="asc")
        ...
        frames.append(frame);days.append(api_start);day+=pd.Timedelta(days=1)
    ...
    gaps=target["datetime"].diff().dropna()/pd.Timedelta(minutes=5);gap_count=int((gaps>1).sum())
    return frame,{"source":"LSE_HISTORICAL_M5_OHLCV","historical_rows":len(frame),"target_m5_rows":len(target),"first_target_candle":str(target.iloc[0].datetime),"last_target_candle":str(target.iloc[-1].datetime),"five_minute_gap_count":gap_count,"warmup_bars":REPLAY_M5_CONTEXT_BARS,"api_start":days[0],"api_end":days[-1],"calendar_days_fetched":len(days)}
def _worker(symbols,start,end,start_label,end_label):
    try:
        from v11.replay_m5 import replay_frames
        reports=[];data_quality=[]
        with _LOCK:_STATE["output"]=["Fetching real LSE historical M5 data..."]
        for symbol in symbols:
            m5,quality=_historical_m5(symbol,start,end,progress=lambda event,**kw:_set_progress(event,**kw));report=replay_frames(m5,None,symbol,start_time=start,end_time=end,progress_callback=lambda event,**kw:_set_progress(event,**kw));report["data_quality"]=quality;reports.append(report);data_quality.append({"symbol":symbol,**quality})
        result={"status":"completed","engine_version":reports[0].get("engine_version") if reports else None,"engine_name":"REGIME-8-ENGINE-REENTRY","source":"LSE_HISTORICAL_M5_OHLCV","lookahead_safe":True,"timeframe_mode":"M5-only","symbols":symbols,"start":str(start),"end":str(end),"requested_window":{"start":start_label,"end":end_label},"data_quality":data_quality,"reports":reports};_persist(result)
