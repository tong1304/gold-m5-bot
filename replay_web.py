...
def _fetch_history(symbol,timeframe,start,end,progress=None):
    ...
    minutes={"5m":5,"15m":15,"1h":60}[timeframe];warmup_bars={"5m":REPLAY_M5_CONTEXT_BARS,"15m":REPLAY_M15_CONTEXT_BARS,"1h":REPLAY_H1_CONTEXT_BARS}[timeframe];warmup=start-timedelta(minutes=minutes*warmup_bars);market={"BTC":"BTC/USD","GOLD":"XAU/USD"}[symbol];client=LSE(api_key=os.environ["LSE_API_KEY"]);first_day=warmup.date();last_day=(end-pd.Timedelta(nanoseconds=1)).date();frames=[];days=[];skipped_days=[];day=pd.Timestamp(first_day);last=pd.Timestamp(last_day)
    while day<=last:
        ...
        try:
            ...
            if progress:progress("data_fetch_skip",symbol=symbol,timeframe=timeframe,day=api_start,reason=str(exc),skipped=len(skipped_days))
        day+=pd.Timedelta(days=1)
    if not frames:raise RuntimeError(f"NO_HISTORICAL_DATA:{symbol}:{timeframe}:all_days_invalid")
