...
Runs replay_signal_history.py in an isolated subprocess so replay cannot mutate live scanner/scheduler globals. No Telegram alert and no live order is sent.
"""
...
_LOCK=threading.RLock(); _STATE={"running":False,"status":"idle","start":None,"end":None,"symbol":"ALL","started_at":None,"finished_at":None,"returncode":None,"output":[],"result":None,"error":None}; _PROC=None; BANGKOK=ZoneInfo("Asia/Bangkok")
...
def _parse_result(lines):
    for line in reversed(lines):
        try:
            ...
            if isinstance(obj,dict) and "status" in obj and "results" in obj:return obj
...
def _worker(start,end,symbol):
    ...
    try:
        ...
        code=proc.wait()
        with _LOCK:
            parsed=_parse_result(_STATE["output"]); failed=[x.get("error") for x in (parsed or {}).get("results",[]) if x.get("status")=="failed" and x.get("error")]
            ...
            _STATE.update({"returncode":code,"result":parsed,"status":status,"running":False,"finished_at":_now(),"error":error}); _PROC=None
