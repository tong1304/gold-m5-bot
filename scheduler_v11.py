...
def _send_15m_system_monitor(now_bkk=None):
    ...
    try:
        import live_price;status=live_price.status();connected="เชื่อมต่อแล้ว" if status.get("connected") else "ขาดการเชื่อมต่อ";authenticated="ยืนยันตัวตนแล้ว" if status.get("authenticated") else "ยังไม่ยืนยันตัวตน";ticks=status.get("ticks_received",0);msg=(f"🟢 <b>สถานะระบบ V12.1 MTF</b>\n\n🕐 {now_bkk.strftime('%d/%m/%Y %H:%M:%S')} (ประเทศไทย)\n⚙️ Engine: <b>{ENGINE_VERSION}</b>\n🧠 Architecture: <b>H1 → M15 → M5 + REGIME + 8 ENGINES + RE-ENTRY</b>\n⏱ Scheduler: <b>ทำงานอยู่</b>\n📡 LSE: <b>{connected}</b>\n🔐 Authentication: <b>{authenticated}</b>\n📥 Live ticks: <b>{ticks:,}</b>\n\n📊 <b>ราคาสินทรัพย์ปัจจุบัน</b>\n{_live_price_line('GOLD')}\n{_live_price_line('BTC')}\n\nℹ️ แจ้งเตือนสถานะระบบ ไม่ใช่สัญญาณ BUY/SELL");result=send_telegram(msg);_LAST_MONITOR_SLOT=slot;logger.warning("[V12 TELEGRAM] แจ้งสถานะทุก 15 นาที slot=%s result=%s timezone=Asia/Bangkok",slot,result);return True
...
def run_scan_cycle():
    ...
    now_utc=datetime.now(UTC);now_bkk=now_utc.astimezone(BANGKOK);results=[];_CYCLE_COUNT+=1;_LAST_CYCLE_AT=now_utc.isoformat();logger.warning("[V12 SCHEDULER] Scan cycle #%s started at %s Bangkok symbols=%s mode=%s",_CYCLE_COUNT,now_bkk.strftime('%Y-%m-%d %H:%M:%S'),_symbols(),TIMEFRAME_MODE)
    for symbol in _symbols():
        try:
            ...
            if _LAST_CLOSED_CANDLE.get(symbol)==closed_key:results.append({"status":"waiting_new_candle","symbol":symbol,"closed_candle":closed_key,"timeframe_mode":TIMEFRAME_MODE});continue
            result=live_scanner_v11.scan_once(symbol);_LAST_CLOSED_CANDLE[symbol]=closed_key;result.update({"trigger":"NEW_CLOSED_M5_CANDLE","candle_consumed":True,"market_session":session,"engine_version":ENGINE_VERSION,"timeframe_mode":TIMEFRAME_MODE,"timeframes":list(TIMEFRAMES)});results.append(result);logger.warning("[V12 SCHEDULER] %s result status=%s strategy=%s side=%s mode=%s",symbol,result.get("status"),result.get("strategy"),result.get("signal"),TIMEFRAME_MODE)
        except Exception as exc:_notify_error(exc,f"การสแกน {symbol}");results.append({"status":"scan_error","symbol":symbol,"error_type":type(exc).__name__,"message":str(exc),"timeframe_mode":TIMEFRAME_MODE,"live_orders_allowed":False})
...
def _loop():
    logger.warning("[V12 SCHEDULER] Thread entered; performing initial MTF scan H1→M15→M5 on latest closed M5 candle")
    if _RUNNING:
        try:run_scan_cycle()
        except Exception as exc:logger.exception("[V12 SCHEDULER] Initial MTF scan failed: %s",exc)
    while _RUNNING:
        wait=_seconds_to_next_boundary();logger.warning("[V12 SCHEDULER] Waiting %.1fs for next closed M5 boundary; mode=%s",wait,TIMEFRAME_MODE);time.sleep(wait)
        if _RUNNING:
            try:run_scan_cycle()
