...
# Single source of truth: scheduler must always report the exact engine version used by the live scanner.
...
def run_scan_cycle():
    ...
    for symbol in _symbols():
        try:
            ...
            result=live_scanner_v11.scan_once(symbol);_LAST_CLOSED_CANDLE[symbol]=closed_key;result.update({"trigger":"NEW_CLOSED_M5_CANDLE","candle_consumed":True,"market_session":session,"engine_version":ENGINE_VERSION,"timeframe_mode":TIMEFRAME_MODE,"timeframes":list(TIMEFRAMES)});results.append(result);logger.warning("[V12 SCHEDULER] %s result status=%s strategy=%s side=%s mode=%s engine=%s",symbol,result.get("status"),result.get("strategy"),result.get("signal"),TIMEFRAME_MODE,ENGINE_VERSION)
