...
def _trace_source(result):
    """Return the actual analysis payload.

    Valid signals from scan_once are wrapped under `setup`, while NO_TRADE
    responses currently merge setup fields at the top level. Normalize both
    forms here so BTC B1-B3 traces are never silently lost in scheduler logs.
    """
...
def _decision_trace_lines(result):
    ...
    for item in trace:
        ...
        sl = evidence.get("sl_price")
        tp = evidence.get("tp_price")
        rr = evidence.get("risk_reward")
        extra = []
        if entry is not None:
            extra.append(f"entry={_fmt_price(entry)}")
        if sl is not None:
            extra.append(f"sl={_fmt_price(sl)}")
        if tp is not None:
            extra.append(f"tp={_fmt_price(tp)}")
        if rr is not None:
            extra.append(f"rr={rr}")
        if reasons:
            extra.append(f"reason={'/'.join(map(str, reasons))}")
        lines.append(
            f"{engine}:{direction}:{status}" + (" " + " ".join(extra) if extra else "")
        )
    return lines


def _log_decision(symbol, result):
    """Log final decision and complete B1-B3/E1-E8 trace without raising."""
    try:
        setup = _trace_source(result)
