...
def main():
    ...
    for symbol in symbols:
        ...
        start=datetime.fromisoformat(args.start).replace(tzinfo=timezone.utc); end=datetime.fromisoformat(args.end).replace(tzinfo=timezone.utc)
        m5=m5[(m5.datetime>=start)&(m5.datetime<end+__import__('pandas').Timedelta(days=1))].reset_index(drop=True)
        m15=m15[(m15.datetime<=m5.iloc[-1].datetime if len(m5) else False)].reset_index(drop=True)
        report=replay.replay_frames(m5,m15,symbol,limit=None); report.update({"start":args.start,"end":args.end,"dry_run":bool(args.dry_run)}); reports.append(report)
