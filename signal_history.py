...
class SignalHistory:
    ...
    def _connect(self):
        ...
        conn.row_factory=sqlite3.Row; conn.execute("PRAGMA busy_timeout=30000"); conn.execute("PRAGMA journal_mode=WAL"); conn.execute("PRAGMA synchronous=NORMAL"); return conn
    def _init_db(self):
    ...
    def record_signal(self,signal):
        levels=signal.get("trade_levels") or {}; signal_id=str(signal.get("signal_id") or "").strip()
        ...
        with self._lock,self._connect() as conn:
            ...
            conn.execute("INSERT OR IGNORE INTO signals (signal_id,symbol,direction,candle_time,created_at,entry,sl,tp,risk_reward,result,telegram_sent,payload_json) VALUES (?,?,?,?,?,?,?,?,?,'OPEN',?,?)",(signal_id,str(signal.get("symbol","")),str(signal.get("signal")),str(signal.get("closed_candle",signal.get("candle_time",""))),created_at,_num(levels.get("entry")),_num(levels.get("sl")),_num(levels.get("tp")),_num(levels.get("risk_reward",levels.get("effective_rr"))),1,json.dumps(signal,ensure_ascii=False,default=str)))
    ...
    def record_no_trade(self,signal):
        ...
        with self._lock,self._connect() as conn:
            before=conn.total_changes
            conn.execute("INSERT OR IGNORE INTO signals (signal_id,symbol,direction,candle_time,created_at,entry,sl,tp,risk_reward,result,r_multiple,resolved_at,telegram_sent,payload_json) VALUES (?,?,'NO_TRADE',?,?,NULL,NULL,NULL,NULL,'NO_TRADE',NULL,?,0,?)",(signal_id,symbol,candle_time,created_at,candle_time,json.dumps(payload,ensure_ascii=False,default=str)))
            return conn.total_changes>before
    ...
    def list_signals(self,days=30,symbol=None,result=None,limit=200):
        ...
        with self._lock,self._connect() as conn:return [dict(row) for row in conn.execute(query,params).fetchall()]
