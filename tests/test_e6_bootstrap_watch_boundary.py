...
def test_bootstrap_does_not_promote_e6_opportunity_watch_into_setup():
    # Mirrors a live developing-auction state: directional pressure and value
    # support exist, but E4 auction confirmation and E7 setup proof are pending.
    upstream = {
        ...
        "E5": _engine("E5", {
            ...
            "available_space_atr_long": 1.20,
            "available_space_atr_short": 0.60,
        }),
    }

    install(pipeline_module)
    result = pipeline_module.analyze_e6({}, upstream)
    ...
    assert output["opportunity_stage"] == "FORMING"
