...
def test_e1_state_contract_is_stable():
    out = analyze_e1(_bars_from_closes([100 + 0.25 * i for i in range(80)]))
    assert out["directional_pressure"] in {"BULLISH", "BEARISH", "NEUTRAL"}
    assert out["market_state"] in {"TREND_UP", "TREND_DOWN", "RANGE", "COMPRESSION", "EXPANSION", "TRANSITION", "UNCLEAR"}
    assert out["professional_reasoning"]["primary_state"] == out["market_state"]
    assert out["professional_reasoning"]["direction"] in {"UP", "DOWN", "NEUTRAL"}


def test_e1_does_not_flip_established_state_on_one_counter_candle():
...
def test_e1_distinguishes_trend_from_volatility_expansion():
    ...
    assert out["market_state"] in {"TREND_DOWN", "EXPANSION"}
    assert out["volatility_state"] == "EXPANDING"


def test_e1_conflict_is_explicitly_preserved():
    ...
    assert out["professional_reasoning"]["conflict_count"] >= 1


def test_e1_evidence_hierarchy_and_authority_are_preserved():
    out = analyze_e1(_bars_from_closes([100 + 0.25 * i for i in range(80)]))
...
def test_e1_confidence_is_lower_when_regime_evidence_conflicts():
