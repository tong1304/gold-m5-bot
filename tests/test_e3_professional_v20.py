...
def test_semantic_state_uses_ordered_swing_sequence_not_label_counts():
    highs = [pivot(10, 110, "SWING_HIGH"), pivot(20, 108, "LH"), pivot(30, 112, "HH")]
    lows = [pivot(15, 100, "SWING_LOW"), pivot(25, 103, "HL"), pivot(35, 101, "LL")]
    ...
    assert result["latest_hh"]["index"] == 30
    assert result["latest_ll"]["index"] == 35


def test_protected_anchor_requires_confirmed_structural_relation():
    highs = [pivot(10, 110, "SWING_HIGH"), pivot(20, 108, "LH")]
    lows = [pivot(15, 100, "SWING_LOW"), pivot(25, 95, "LL")]
...
def test_break_lifecycle_separates_confirmation_acceptance_failure_and_currentness():
    history = [{"event": "CONFIRMED_BOS", "direction": "DOWN", "level": 100.0, "break_candle_index": 10, "acceptance_candle_index": 12, "failure_candle_index": None}]
