...
def _install_test_wrapper():
    ...
    def original(previous, current_by_direction, *, leader="NEUTRAL", competition="UNCONTESTED"):
        return {
            "opportunities": {
                "BUY": {**current_by_direction["BUY"], "state": "WATCHING"},
                "SELL": {**current_by_direction["SELL"], "state": "WATCHING"},
            },
            "leader": leader,
...
def test_fast_early_opportunity_is_carried_to_lifecycle_and_next_candle():
    ...
    result = pipeline.advance_opportunity_directions(
        {},
        {"BUY": {"candidate": True, "direction": "BUY", "setup": "OPPORTUNITY_WATCH", "opportunity_phase": "EARLY_OPPORTUNITY", "opportunity_speed": "FAST"}},
        leader="BUY",
    )
...
def test_standard_early_opportunity_keeps_normal_confirmation_window():
    ...
    result = pipeline.advance_opportunity_directions(
        {},
        {"SELL": {"candidate": True, "direction": "SELL", "setup": "OPPORTUNITY_WATCH", "opportunity_phase": "EARLY_OPPORTUNITY", "opportunity_speed": "STANDARD"}},
        leader="SELL",
    )
...
def test_late_opportunity_is_chase_prohibited_and_waits_for_new_event():
    ...
    result = pipeline.advance_opportunity_directions(
        {},
        {"SELL": {"candidate": True, "direction": "SELL", "setup": "OPPORTUNITY_WATCH", "opportunity_phase": "LATE_OPPORTUNITY"}},
        leader="SELL",
    )
    sell = result["opportunities"]["SELL"]
