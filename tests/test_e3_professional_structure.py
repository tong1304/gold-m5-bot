...
def _pivot(index, price, label):
    return {"index": index, "price": float(price), "label": label, "confirmation_index": index + 2}
...
def test_authority_is_explicit_and_internal_never_promotes_itself():
    ...
    result = _authority(DOWN, UP, "DOWN", "UP", {"confirmed": False}, {}, protected, {}, invalidation)
