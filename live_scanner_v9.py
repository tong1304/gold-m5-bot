...
def _normalize_lse_rows(raw, symbol, timeframe):
    if isinstance(raw, dict):
        ...
        if rows is None: raise RuntimeError(f"LSE_INVALID_RESPONSE: {symbol} {timeframe} missing data rows; keys={list(raw.keys())[:20]}")
    elif isinstance(raw,(list,tuple)): rows=raw
    else: raise RuntimeError(f"LSE_INVALID_RESPONSE: {symbol} {timeframe} unexpected type={type(raw).__name__}")
    ...
    if "datetime" not in frame.columns:
        for candidate in ("timestamp","time","date"):
            if candidate in frame.columns: frame=frame.rename(columns={candidate:"datetime"}); break
    if "datetime" not in frame.columns: raise RuntimeError(f"LSE_INVALID_RESPONSE: {symbol} {timeframe} missing timestamp column; columns={list(frame.columns)}")
...
def _lse_frame(symbol,timeframe,history_points=200):
    market={"BTC":"BTC/USD","GOLD":"XAU/USD"}[symbol]; dataset="crypto" if symbol=="BTC" else "forex"; now=datetime.now(timezone.utc)
    days=max(2,int(history_points*_timeframe_minutes(timeframe)/1440)+2); start=(now-timedelta(days=days)).date().isoformat(); end=(now+timedelta(days=1)).date().isoformat()
    ...
    frame=_normalize_lse_rows(raw,symbol,timeframe); print(f"[{symbol}] LSE RAW DATA: rows={len(frame)} dataset={dataset}",flush=True)
    ...
    frame=_remove_incomplete_last_candle(frame,timeframe); _validate_freshness(frame,symbol,timeframe); print(f"[{symbol}] Latest closed {timeframe} candle: {frame.iloc[-1]['datetime']}",flush=True); return frame
...
def _validate_freshness(frame,symbol,timeframe):
    ...
    latest=pd.Timestamp(frame.iloc[-1]["datetime"]); now=pd.Timestamp(datetime.now(timezone.utc)); age=(now-latest).total_seconds()/60; maximum=_max_age(timeframe)
    print(f"[{symbol}] DATA CHECK {timeframe}: latest={latest.isoformat()} age={age:.1f}m max={maximum:.1f}m",flush=True)
    if age>maximum: raise RuntimeError(f"STALE_MARKET_DATA: {symbol} {timeframe} latest={latest.isoformat()} age={age:.1f}m max={maximum:.1f}m")
...
def _format_telegram(symbol,setup,signal_id):
    direction=setup["signal"]; levels=setup["trade_levels"]; sweep=setup.get("liquidity_event") or {}; mss=setup.get("m5_trigger") or {}; loc=setup.get("location") or {}; h1=setup.get("structure_bias",{}).get("bias","NEUTRAL"); m15=setup.get("m15_structure",{}).get("bias","NEUTRAL"); side="🟢 BUY — ซื้อ" if direction=="BUY" else "🔴 SELL — ขาย"
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        ...
        if not valid:
            payload={**setup,"signal":"NO_TRADE","result":"NO_TRADE","created_at":datetime.now(timezone.utc).isoformat(),"no_trade_reasons":setup.get("rejection_reasons") or []}; recorded=history.record_no_trade(payload); print(f"[{symbol}] V9 NO_TRADE recorded={recorded} reasons={setup.get('rejection_reasons')}",flush=True); return {"status":"no_trade","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":"NO_TRADE","recorded":recorded,**setup}
