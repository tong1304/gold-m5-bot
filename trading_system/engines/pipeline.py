...
SUB_ENGINE_CODES = [
    '1A','1B','1C','1D','1E','1F','1G','2A','2B','2C','2D','2E','2F',
    '3A','3B','3C','3D','3E','3F','4A','4B','4C','4D','4E','4F',
    '5A','5B','5C','5D','5E','5F','6A','6B','6C','6D','6E','6F',
    '7A','7B','7C','7D','7E','7F','8A','8B','8C','8D','8E','8F','8G',
    '9A','9B','9C','9D','9E','9F','9G','9H'
]


def _module_for(code: str) -> str:
    engine, letter = code[0], code[1]
    suffix_map = {
    ...
    return f'trading_system.engines.e{engine}.{suffix_map[code]}'


def run_all(data: dict[str, Any]) -> dict[str, Any]:
    results = {}
    for code in SUB_ENGINE_CODES:
        engine_cls = import_module(_module_for(code)).SubEngine
        result = engine_cls().run(data)
        results[code] = result
        if not result.gate_passed and code == '1A':
            break
    return results
