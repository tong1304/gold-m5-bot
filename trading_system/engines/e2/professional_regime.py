...
class ProfessionalE2Brain(_Base):
    ...
    E9 remains the sole trade-decision authority and 2A-2F remain paused.
    """
    ...
    def _analyse(self, d: dict[str, Any]):
        ...
        position40 = max(0.0, min(1.0, (last - lo40) / max(hi40 - lo40, 1e-12)))
        ...
        # then established trend, failed auction, balance, and finally transition.
        ...
        elif compressed and abs(slope20) < 0.80:
        ...
        if regime == "BREAKOUT":
            ...
            phase, opportunity = ("COMPRESSION" if compressed else "BALANCED"), "RANGE_ROTATION"
        else:
            phase, opportunity = "TRANSITION", "WAIT_FOR_REPRICING"

        # E2 forms its own thesis first; E1 is only a cross-examination channel.
        ...
        if regime == "RANGE" and not compressed:
            counter.append("range is not sufficiently compressed")
        ...
        evidence = [
            ...
            f"position40={position40:.3f}", f"expansion={expansion}",
        ...
        thesis = (f"E2 independently classifies {regime} with direction={direction}. "
                  f"The market is in {phase} and offers {opportunity}; "
                  f"opportunity_state={opportunity_state}.")
        output = {
            ...
            "independence": "E2_FIRST_E1_CROSS_CHECK", "observations": observations,
