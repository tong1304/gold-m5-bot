...
def _reasoning(engine: Any) -> dict[str, Any]:
    output = getattr(engine, "output", {}) or {}
    professional = output.get("professional_reasoning") or {}
    return professional if isinstance(professional, dict) else {}
...
def _status(engine: Any) -> str:
    eid = engine.engine_id; r = _reasoning(engine); output = getattr(engine, "output", {}) or {}
    status = output.get("analysis_status", ""); reasons = set(output.get("analysis_reason_codes", ()) or engine.reason_codes or ())
...
def _engine_answer(engine: Any) -> str:
    eid = engine.engine_id; r = _reasoning(engine); output = getattr(engine, "output", {}) or {}
    if eid == "E1":
        answer = f"{_th(_first(engine, 'direction_bias', 'direction', 'structure_direction', default=output.get('finding')))} / {_th(_first(engine, 'market_state', default=output.get('finding')))}"
    elif eid == "E2":
        answer = f"{_th(_first(engine, 'regime', 'opportunity_regime', default=output.get('finding')))} / {_th(_first(engine, 'preferred_direction', 'direction', default=output.get('finding')))}"
    elif eid == "E3":
        answer = f"{_th(_first(engine, 'structure', 'lifecycle', default=output.get('finding')))} / {_th(_first(engine, 'alignment', 'structure_direction', default=output.get('finding')))}"
    elif eid == "E4":
        answer = f"{_th(_first(engine, 'liquidity_quality', default=output.get('finding')))} / Sweep {_th(_first(engine, 'sweep', 'liquidity_event', default=output.get('finding')))}"
    elif eid == "E5":
        answer = f"{_th(_first(engine, 'location_quality', default=output.get('finding')))} / {_th(_first(engine, 'space', 'available_space', default=output.get('finding')))}"
    elif eid == "E6":
        answer = f"{_th(_first(engine, 'setup_type', 'setup', default=output.get('finding')))} / {_th(_first(engine, 'formation', 'stage', default=output.get('finding')))}"
    elif eid == "E7":
        answer = f"{_th(_first(engine, 'trigger', default=output.get('finding')))} / {_th(_first(engine, 'confirmation', default=output.get('finding')))}"
    elif eid == "E8":
        p = r.get("trade_plan") or output.get("trade_plan") or {}; answer = f"RR 1:{float(p.get('rr_tp2', 0)):.1f}" if p.get("valid") else _th(p.get("reason") or output.get("finding"))
    elif eid == "E9": answer = str(output.get("decision", "NO_TRADE"))
    else: answer = str(output.get("finding") or "ไม่พบข้อมูล")
    # If the brain uses a single finding/conclusion field, preserve it rather than printing empty placeholders.
    if "ไม่พบข้อมูล" in answer:
        fallback = _first(engine, "finding", "conclusion", "analyst_conclusion", "thesis")
        if fallback: answer = _th(fallback)
    reasons = output.get("decision_reasons") or output.get("reason_codes") or engine.reason_codes or ()
...
def _e9_control_lines(result: Any) -> list[str]:
    ...
    r = _reasoning(e9); output = getattr(e9, "output", {}) or {}
    ...
    def pick(*keys):
        for key in keys:
            ...
            if output.get(key) is not None: return output[key]
        return "ไม่พบข้อมูล"
