...
def _engine_answer(engine: Any) -> list[str]:
    e=engine.engine_id; r=engine.output.get("professional_reasoning",{}) or {}; lines=[f"{e} — {ENGINE_THAI_NAMES.get(e,e)}",f"{'✅' if engine.gate_passed else '❌'} {'ผ่าน' if engine.gate_passed else 'ไม่ผ่าน'}"]
    if e=="E1": lines += [f"• คำตอบ: ตลาดอยู่ใน {_fmt(r.get('market_state'))}",f"• Direction Bias: {_fmt(r.get('direction_bias'))}"]
    elif e=="E2": lines += [f"• คำตอบ: เลือก Playbook ตาม {_fmt(r.get('regime'))}",f"• Preferred Direction: {_fmt(r.get('preferred_direction'))}"]
    elif e=="E3": lines += [f"• คำตอบ: Structure = {_fmt(r.get('structure'))}",f"• Direction: {_fmt(r.get('structure_direction'))}",f"• Alignment: {_fmt(r.get('alignment'))}"]
    ...
    elif e=="E6": lines += [f"• คำตอบ: Setup = {_fmt(r.get('setup_type'))}",f"• Direction: {_fmt(r.get('direction'))}",f"• Formation: {_fmt(r.get('formation'))}",f"• Setup Quality: {_fmt(r.get('setup_quality'))}"]
        ...
        p=r.get("trade_plan") or {}; lines += [f"• คำตอบ: Risk Gate = {_fmt(r.get('risk_gate'))}"]
    ...
    elif e=="E9": lines += [f"• คำตอบ: {'อนุมัติ' if engine.output.get('decision') in {'BUY','SELL'} else 'ไม่อนุมัติ'}",f"• Edge Score: {float(engine.output.get('edge_score',0)):.1f}"]
    return lines
...
def format_decision(result: DecisionResult) -> str:
    ...
    lines=[f"{'🟢 BUY' if result.decision=='BUY' else '🔴 SELL'} — {direction}","",f"📊 สินทรัพย์: {result.symbol}",f"⏱ Timeframe: {result.timeframe}","🧠 ระบบคิดแบบ 9-Engine ต่อเนื่องจาก E1 ถึง E9","","━━━━━━━━━━━━━━━━━━","🧠 คำตอบจากแต่ละ Engine","━━━━━━━━━━━━━━━━━━"]
    for engine in result.engines:
        lines += [""] + _engine_answer(engine)
    lines += ["","━━━━━━━━━━━━━━━━━━","🎯 FINAL DECISION","━━━━━━━━━━━━━━━━━━","🟢 E9 อนุมัติการออกออเดอร์",f"📈 Edge Score: {result.score:.1f}","","━━━━━━━━━━━━━━━━━━","📋 Trade Plan","━━━━━━━━━━━━━━━━━━",f"📍 Entry: {plan['entry']}",f"🛑 Stop Loss: {plan['stop_loss']}",f"🎯 Take Profit 1: {plan['take_profit_1']}",f"🎯 Take Profit 2: {plan['take_profit_2']}",f"📐 RR: 1:{plan['rr_tp2']:.1f}"]
    return _validate("\n".join(lines))


def format_startup(symbols: list[str]) -> str:
    return _validate("\n".join(["✅ ระบบ 9-Engine เริ่มทำงาน","","⚙️ ระบบ: PRODUCTION-V2","🧠 โครงสร้าง: E1 → E2 → E3 → E4 → E5 → E6 → E7 → E8 → E9",f"📊 สินทรัพย์: {', '.join(symbols)}","⏱ Timeframe: M5","","🔄 ทุกแท่ง M5 ที่ปิดจะเริ่มวิเคราะห์ใหม่ตั้งแต่ E1","✅ ระบบพร้อมทำงาน"]))
...
def format_status(status: dict[str, Any]) -> str:
    ...
    return _validate("\n".join(["✅ สถานะระบบ PRODUCTION-V2","","🧠 โครงสร้าง: E1 → E2 → E3 → E4 → E5 → E6 → E7 → E8 → E9","⏱ Timeframe: M5","",f"🚨 เวลาแจ้งเตือน: {timestamp_text} (ประเทศไทย)","","📡 ราคาปัจจุบัน:",f"🌕 GOLD: {price_text('GOLD')}",f"🪙 BTC: {price_text('BTC')}","","✅ ระบบทำงานปกติ"]))


def format_critical(message:str, component:str)->str: return _validate(f"🔴 ระบบผิดปกติ\n\n⚠️ ส่วนที่มีปัญหา: {component}\n📌 รายละเอียด: {message}\n\n⛔ กรุณาตรวจสอบระบบ")
