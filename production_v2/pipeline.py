...
from .e1_professional_layer_v10 import analyze_e1_professional_v10
...
class ProductionPipeline:
    ...
    def run(self, market_data: dict[str, Any], *, wait_bars=0, resume_state=None, historical_calibration=None):
        ...
        e1 = _dict_result("E1", analyze_e1_professional_v10(bars)); results["E1"] = e1
        ...
        e9 = results["E9"]; plan = e9.output.get("trade_plan") or {}
        ...
        decision = e9.output.get("decision") if approved else "NO_TRADE"; engines = tuple(results[e] for e in ENGINE_ORDER)
        return DecisionResult(str(snapshot.get("symbol") or "UNKNOWN"), str(snapshot.get("timeframe") or "M5"), decision, approved, e9.score, engines, {"risk_gate":bool(plan.get("valid")) if isinstance(plan,dict) else False,"trade_plan":plan,"engine_state":"TRADE_APPROVED" if approved else "ANALYSIS_COMPLETE_NO_TRADE","cycle_complete":True,"analysis_architecture":"ONE_BRAIN_PER_ENGINE + DIRECT_ANALYZER_INVOCATION","evidence_inputs":{k:list(EVIDENCE_INPUTS.get(k,())) for k in ENGINE_ORDER},"sub_engines":False,"parallel_peer_analysis":False,"e9_decision_authority":True,"learning_mode":"ADVISORY_ONLY","next_evaluation":"NEXT_CLOSED_M5_CANDLE","wait_bars":0,"decision_reasons":list(e9.reason_codes)},tuple(e9.reason_codes))
