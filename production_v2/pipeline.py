        ...
        for engine_id in self.ENGINE_ORDER:
            ...
            if not result.gate_passed:
                e9 = EngineResult("E9", "Execution Decision Engine", False, result.score,
                                  {"decision": "NO_TRADE", "blocked_by": engine_id,
                                   "decision_authority": "E9"}, (f"{engine_id}_GATE_FAILED",))
                engines.append(e9)
                return DecisionResult(symbol, timeframe, "NO_TRADE", False, result.score,
                                      tuple(engines), {"risk_gate": False},
                                      (f"{engine_id}_GATE_FAILED",))

        e9 = run_e9_decision(context, engines)
        engines.append(e9)
        decision = e9.output.get("decision", "NO_TRADE")
        return DecisionResult(symbol, timeframe, decision, e9.gate_passed, e9.score,
                              tuple(engines), {"risk_gate": engines[-2].gate_passed},
