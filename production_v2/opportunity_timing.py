...
STRONG_EVENTS = (
    "FAILED_BREAK_RECLAIM", "SWEEP_RECLAIM", "SWEEP_REJECTION",
    "HIGH_REJECTION", "LOW_REJECTION", "HIGH_ACCEPTANCE", "LOW_ACCEPTANCE",
    "BREAK", "RECLAIM", "LIQUIDITY_INTERACTION", "ACCEPTANCE", "REJECTION",
    "SWEEP",
)
...
def _direction(output: dict[str, Any]) -> str:
    for key in ("direction", "direction_thesis", "thesis_direction", "opportunity_direction"):
        ...
        if value in DIRECTIONS or value.startswith(("BUY ", "BUY_", "BUY:")):
            return "BUY"
        if value in DIRECTIONS or value.startswith(("SELL ", "SELL_", "SELL:")):
            return "SELL"
...
def _age_bars(output: dict[str, Any]) -> int:
    ...
    candidates = [
        output.get("event_age_bars"),
        output.get("bars_waited"),
        anchor.get("age_bars") if isinstance(anchor, dict) else None,
    ]
...
def classify_opportunity_timing(output: dict[str, Any]) -> dict[str, Any]:
    ...
    if event_level is not None and price is not None and event_atr and event_atr > 0:
        displacement_atr = abs(price - event_level) / event_atr

    late_by_age = age >= 2
    late_by_displacement = displacement_atr is not None and displacement_atr >= 0.75
    late = late_by_age or late_by_displacement

    if direction not in DIRECTIONS:
        phase, speed = NEUTRAL, STANDARD
        reasons = ["NO_DIRECTIONAL_EVIDENCE"]
    elif hard_block:
        phase, speed = LATE if late else EARLY, BLOCK if late else SLOW
        reasons = ["HARD_BLOCK_PRESENT"]
    elif confirmed and not late:
        phase, speed = CONFIRMED, STANDARD
        reasons = ["CLOSED_CANDLE_CONFIRMATION_PRESENT"]
    elif late:
        ...
        phase, speed = EARLY, FAST
        reasons = ["HIGH_QUALITY_FRESH_EVENT", "FAST_EVIDENCE_CYCLE_ALLOWED"]
    elif confidence >= 50.0:
        phase, speed = EARLY, STANDARD
        reasons = ["DEVELOPING_EVIDENCE_REQUIRES_STANDARD_CONFIRMATION"]
    else:
        phase, speed = EARLY, SLOW
        reasons = ["LOW_EVIDENCE_REQUIRES_ADDITIONAL_CONFIRMATION"]
