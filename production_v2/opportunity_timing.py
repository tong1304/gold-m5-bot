...
This layer does not authorize trades. It answers three questions for the
professional opportunity radar:
1. Is the opportunity early enough to justify a fast evidence cycle?
2. Has the setup reached confirmation?
3. Has the opportunity become late enough that chasing should slow or stop?
...
def _num(output: dict[str, Any], *keys: str, default: float | None = None) -> float | None:
    for key in keys:
        try: value = float(output.get(key))
...
def _direction(output: dict[str, Any]) -> str:
    for key in ("direction","direction_thesis","thesis_direction","opportunity_direction"):
        ...
        if value in DIRECTIONS or value.startswith(("BUY ","BUY_","BUY:")): return "BUY"
        if value in DIRECTIONS or value.startswith(("SELL ","SELL_","SELL:")): return "SELL"
    finding = _text(output.get("finding"))
    if finding.startswith("BUY "): return "BUY"
    if finding.startswith("SELL "): return "SELL"
    return "NEUTRAL"
...
def classify_opportunity_timing(output: dict[str, Any]) -> dict[str, Any]:
    out = dict(output or {})
    direction = _direction(out); age = _age_bars(out); event = _event(out)
    confidence = _num(out,"opportunity_strength","opportunity_score","confidence","evidence_strength",default=0.0) or 0.0
    if confidence <= 1.0: confidence *= 100.0
    confidence = max(0.0, min(100.0, confidence))
    space = _num(out,"available_space_atr","effective_space_atr","space_atr","available_space_atr_long","available_space_atr_short",default=0.0) or 0.0
    confirmed = _confirmed(out); hard_block = _hard_block(out); strong_event = any(token in event for token in STRONG_EVENTS)
    event_level = _num(out,"event_level"); price = _num(out,"price","current_price","last_price"); event_atr = _num(out,"event_atr_frozen","event_atr","atr")
    ...
    late_by_age = age >= 2; late_by_displacement = displacement_atr is not None and displacement_atr >= LATE_DISPLACEMENT_ATR; late = late_by_age or late_by_displacement
    if direction not in DIRECTIONS:
        ...
        phase, speed = LATE, SLOW; reasons = []
        if late_by_age: reasons.append("EVENT_AGE_REACHED_LATE_WINDOW")
        if late_by_displacement: reasons.append("PRICE_DISPLACEMENT_REACHED_LATE_WINDOW")
        if confirmed: reasons.append("CONFIRMED_BUT_ENTRY_WINDOW_IS_LATE")
    elif strong_event and confidence >= 65.0 and space >= 0.75:
    ...
    if space < 0.75 and direction in DIRECTIONS:
        reasons.append("SPACE_BELOW_MINIMUM_FOR_FAST_PATH")
        if speed == FAST: speed = STANDARD
    return {"phase":phase,"decision_speed":speed,"direction":direction,"event_age_bars":age,"event":event,"evidence_quality":round(confidence,2),"available_space_atr":round(space,4),"displacement_atr":round(displacement_atr,4) if displacement_atr is not None else None,"confirmed":confirmed,"late_by_age":late_by_age,"late_by_displacement":late_by_displacement,"fast_path_eligible":speed == FAST,"chase_prohibited":phase == LATE,"execution_authority":"E9_ONLY","reasons":list(dict.fromkeys(reasons))}


def enrich_timing(output: dict[str, Any]) -> dict[str, Any]:
    out = dict(output or {}); timing = classify_opportunity_timing(out); out["opportunity_timing"] = timing; out["opportunity_phase_speed"] = timing["phase"]; out["opportunity_decision_speed"] = timing["decision_speed"]; out["opportunity_fast_path"] = timing["fast_path_eligible"]; out["opportunity_chase_prohibited"] = timing["chase_prohibited"]; return out
