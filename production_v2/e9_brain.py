...
def _market_control(upstream):
    ...
    def add(direction, source, weight, status="AUTHORITATIVE"):
        if direction in DIRECTIONS:
            votes.append((direction, source, weight)); evidence_quality.append({"source": source, "status": status, "direction": direction, "weight": weight})

    add(_direction(e1.get("pressure"), e1.get("pressure_direction")), "E1_PRESSURE", 3.0)
    if _text(e3.get("structure_integrity", "VALID")) == "VALID":
        add(_direction(e3.get("structure_direction"), e3.get("external_state"), e3.get("structure"), e3.get("finding")), "E3_STRUCTURE", 3.0)
    # E2 describes opportunity, not raw market control. It is useful context but
    # intentionally lower weight than E1/E3 and can never override them alone.
    add(_direction(e2.get("direction"), e2.get("opportunity_direction")), "E2_OPPORTUNITY", 1.5, "CONTEXTUAL")

    # CRITICAL SEMANTIC HARDENING: a pending/unclear auction response is not a
    # market-control vote. E4's finding (for example HIGH_SWEEP_REJECTION) is an
    # event classification, not proof that the response actor controls price.
    e4_state = _text(e4.get("auction_state", e4.get("state")))
    e4_terminal = e4_state in {"CONFIRMED", "TERMINALLY_CONFIRMED", "ACCEPTED", "REJECTED", "RECLAIMED"} or "TERMINAL" in e4_state
    response = _direction(e4.get("response_actor"), e4.get("auction_response"))
    if e4_terminal and response in DIRECTIONS:
        add(response, "E4_CONFIRMED_AUCTION_RESPONSE", 2.5, "CONFIRMED")
    else:
        evidence_quality.append({"source": "E4_AUCTION_RESPONSE", "status": "NON_AUTHORITATIVE_PENDING" if not e4_terminal else "UNRESOLVED", "direction": response, "weight": 0.0})

    # E5 contributes to market control only when an explicit directional
    # repricing direction exists; value-state labels alone are not votes.
    repricing_direction = _direction(e5.get("repricing_direction"))
    if repricing_direction in DIRECTIONS:
        add(repricing_direction, "E5_REPRICING", 1.5, "CONTEXTUAL")
    else:
        evidence_quality.append({"source": "E5_REPRICING", "status": "NON_DIRECTIONAL", "direction": "NEUTRAL", "weight": 0.0})
    ...
    if not total: state, direction, confidence = "UNRESOLVED", "NEUTRAL", 0.0
        ...
        direction = "BUY" if totals["BUY"] > totals["SELL"] else "SELL"; confidence = round(max(totals.values()) / total * 100.0, 2)
        state = f"{direction}-CONTROLLED" if confidence >= 60 else "MIXED"; direction = direction if state != "MIXED" else "NEUTRAL"
    ...
    return {"market_control_state": state, "control_direction": direction, "control_confidence": confidence,
            ...
            "control_evidence": evidence, "control_evidence_quality": evidence_quality,
            "pending_e4_response_excluded": not e4_terminal,
            "authority_rule": "E4_RESPONSE_REQUIRES_TERMINAL_AUCTION_STATE"}
...
def _confirmation(e7):
    ...
    confirmation = _state(e7, ("confirmation_state", "confirmation", "proof_state")); trigger = any(e7.get(k) is True for k in ("trigger_observed", "valid_trigger", "closed_candle_trigger")) or _state(e7, ("trigger_state", "trigger", "entry_trigger")) in PROVEN
...
def _economic(e8):
    ...
    blockers = _dedupe(blockers); state = _state(e8, ("risk_state", "economic_state", "decision_state", "plan_status")); verified = e8.get("verified") is True or e8.get("trade_plan_verified") is True; ready = (state in READY or verified) and not blockers
...
def _hard_conflicts(upstream):
    ...
    for engine_id in ("E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8"):
        ...
        if engine_id == "E3":
            integrity = _text(output.get("structure_integrity", output.get("protected_structure", {}).get("integrity", "VALID")))
            if integrity == "INVALID":
                found.append("STRUCTURE_INTEGRITY_INVALID")
                for code in output.get("protected_structure", {}).get("integrity_reasons", []) if isinstance(output.get("protected_structure"), dict) else []:
                    found.append("PROTECTED_LEVEL_GEOMETRY_INVALID" if code else code)
...
def analyze_e9(snapshot, upstream):
    ...
    gates = {"e6_thesis_present": True, "e6_setup_mature": _setup_state(e6) == "MATURE", "e7_confirmation_proven": confirmation_proven, "e8_economics_ready": economic_ready, "trade_geometry_valid": _plan_valid(e8, direction), "hard_conflict_clear": not hard_conflicts, "market_alignment_clear": alignment != "CONFLICTED", "opportunity_path_clear": not opportunity_blockers}
    missing = []
