...
def _hard_conflicts(upstream):
    ...
    for engine_id in ("E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8"):
        ...
        if engine_id == "E3":
            ...
            if integrity == "INVALID": found.append("STRUCTURE_INTEGRITY_INVALID")
...
def _market_control(upstream):
    ...
    e4_state=_text(e4.get("auction_state", e4.get("state"))); terminal=e4_state in {"CONFIRMED","TERMINALLY_CONFIRMED","ACCEPTED","REJECTED","RECLAIMED"} or "TERMINAL" in e4_state
...
def _supporting_evidence(upstream):
    e2,e4,e5=(_out(upstream.get(k)) for k in ("E2","E4","E5"))
    e2_codes=[c for c in _engine_codes(upstream.get("E2")) if c not in HARD_CONFLICTS and c not in ECONOMIC_BLOCKERS]
    e4_state=_text(e4.get("auction_state",e4.get("state","UNRESOLVED")))
    e4_codes=[] if e4_state in {"CONFIRMED","ACCEPTED","REJECTED","RECLAIMED"} else ["AUCTION_CONFIRMATION_PENDING"]
    e5_codes=[c for c in _engine_codes(upstream.get("E5")) if c not in HARD_CONFLICTS and c not in ECONOMIC_BLOCKERS]
...
def _no_thesis_output(control, structure_lifecycle, e4):
    ...
    return {"decision":"NO_TRADE","final_governance":"NO_THESIS","governance_decision":"NO_THESIS","governance_reason":"NO_SURVIVING_E6_THESIS","governance_blockers":["NO_SURVIVING_E6_THESIS"],"next_required_events":["E6_NEW_SURVIVING_SETUP_THESIS"],"execution_state":"BLOCKED","all_gates_pass":False,"direction":"NEUTRAL","thesis_direction":"NEUTRAL","setup":"UNKNOWN","thesis":"UNRESOLVED","thesis_state":"UNRESOLVED","thesis_lifecycle_source":"E6","setup_state":"UNRESOLVED","structure_lifecycle":structure_lifecycle,"confirmation_state":"NOT_APPLICABLE","economic_state":"NOT_APPLICABLE","economic_blockers":[],"hard_conflicts":[],"supporting_evidence":_supporting_evidence({"E2":None,"E4":EngineResult("E4","E4",False,0,e4,()),"E5":None}),"market_control_state":control["market_control_state"],"control_direction":control["control_direction"],"control_confidence":control["control_confidence"],"control_scores":control["control_scores"],"control_evidence":control["control_evidence"],"dominant_control_evidence":control["dominant_control_evidence"],"control_evidence_quality":control["control_evidence_quality"],"evidence_alignment":"UNRESOLVED","evidence_alignment_reason":"NO_E6_THESIS_TO_ALIGN","proof_summary":{"core_thesis":False,"e7_trigger":"NOT_APPLICABLE","e8_economics":"NOT_APPLICABLE"},"authority_contract":{"market_evidence_owner":"E1-E5","trade_thesis_owner":"E6","trigger_owner":"E7","trade_economics_owner":"E8","final_decision_owner":"E9","e9_may_rewrite_e6_thesis":False,"e9_may_bypass_e7":False,"e9_may_bypass_e8":False},"mandatory_gates":{"core_thesis":False,"closed_candle_trigger":False,"survivable_economics":False,"fatal_veto_clear":True},"opportunity_state":"NO_THESIS","opportunity":{"direction":"NEUTRAL","setup":"UNKNOWN","state":"NO_THESIS","do_not_execute":True},"reason_codes":reasons,"reasons":reasons,"architecture":ARCHITECTURE,"version":VERSION}


def analyze_e9(snapshot, upstream):
    del snapshot
    e2,e4,e6,e7,e8=(_out(upstream.get(k)) for k in ("E2","E4","E6","E7","E8"))
    control=_market_control(upstream); direction,setup,thesis=_e6_identity(e6); lifecycle=classify_lifecycle(e6,e7,e8) if 'classify_lifecycle' in globals() else {"stage":"UNKNOWN"}
    ...
    core_thesis=direction in DIRECTIONS and setup not in NO_THESIS_SETUP and thesis_state in {"HYPOTHESIS","VALIDATING","MATURE"}; fatal_veto=bool(hard_conflicts); economics_survivable=economic_ready and plan_valid and not economic_blockers
    mandatory={"core_thesis":core_thesis,"closed_candle_trigger":confirmation_proven,"survivable_economics":economics_survivable,"fatal_veto_clear":not fatal_veto}
    ...
    elif economic_state=="BLOCKED" or economic_blockers: governance,decision,reason="REJECTED_ECONOMICS","NO_TRADE",economic_blockers[0] if economic_blockers else "TRADE_ECONOMICS_INVALID"
    elif not confirmation_proven or not economics_survivable: governance,decision,reason="WATCH","NO_TRADE","WAITING_FOR_E7_TRIGGER" if not confirmation_proven else "WAITING_FOR_E8_SURVIVABLE_ECONOMICS"
...
def classify_lifecycle(e6,e7,e8):
    ...
    confirmation_state,confirmation_proven=_confirmation(e7); economic_state,economic_ready,_=_economic(e8)
