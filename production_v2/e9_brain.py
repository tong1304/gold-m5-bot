...
def _text(value: Any) -> str:
    if isinstance(value, dict):
        # Never let structured upstream payloads reach set-membership operations.
        # Preserve deterministic, auditable text instead of raising TypeError.
...
def _codes(output: dict[str, Any]) -> list[str]:
    ...
    for key in ("reason_codes", "reasons", "counter_evidence", "blockers", "risk_blockers", "economic_blockers", "conflicts", "invalidations"):
        ...
        if isinstance(value, str):
            ...
            # Dict payloads are evidence objects, not code sets. Extract true flags
            # and scalar code-like values without ever inserting the dict itself.
...
def _trigger_observed(e7: dict[str, Any]) -> bool:
    ...
    if state in {"VALID", "VALIDATED", "CONFIRMED", "PROVEN", "TRADE_READY"}:
        return True
    codes = _codes(e7)
    return any(code in {"VALID_CLOSED_CANDLE_TRIGGER", "TRIGGER_CONFIRMED", "CONFIRMATION_PROVEN"} for code in codes)
...
def _market_control(upstream: dict[str, EngineResult], e6: dict[str, Any]) -> dict[str, Any]:
    ...
    buy_votes = votes.count("BUY"); sell_votes = votes.count("SELL")
    if not votes: dominant_side = "UNKNOWN"
    elif buy_votes == sell_votes: dominant_side = "CONFLICTED"
    else: dominant_side = "BUY" if buy_votes > sell_votes else "SELL"
    consensus = round(max(buy_votes, sell_votes) / len(votes) * 100.0, 2) if votes else 0.0
    taker_side = _direction(taker); responder_side = _direction(responder)
...
def _resolve(direction: str, setup: str, thesis: str, e6: dict[str, Any], e7: dict[str, Any], e8: EngineResult | None, upstream: dict[str, EngineResult]) -> dict[str, Any]:
    ...
    confirmation = _confirmation_state(e7); trigger = _trigger_observed(e7); boundary, plan = _e8_boundary(e8); economic = _economic_blockers(e8); conflicts = _hard_conflicts(upstream)
    ...
    if all_pass:
        decision, decision_state, master_state = direction, "EXECUTE", "EXECUTE"; thesis_state, setup_state = "ESTABLISHED", "TRADE_READY"; confirmation_final, risk_final, execution_state = "PROVEN", "READY", "READY"; next_event = "NONE"
    elif conflicts:
        decision, decision_state, master_state = "NO_TRADE", "REJECT", "REJECTED_HARD_CONFLICT"; thesis_state = "INVALIDATED" if any("INVALIDAT" in c for c in conflicts) else "CONFLICTED"; setup_state = confirmation_final = risk_final = execution_state = "BLOCKED"; next_event = lifecycle["recovery"]
    else:
...
def analyze_e9(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    del snapshot
    e6 = _out(upstream.get("E6")); e7 = _out(upstream.get("E7")); e8 = upstream.get("E8")
    direction, setup, thesis = _e6_identity(e6); resolved = _resolve(direction, setup, thesis, e6, e7, e8, upstream); market_control = _market_control(upstream, e6)
    ...
    for engine_id in ("E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8"):
        engine = upstream.get(engine_id); output = _out(engine); evidence_summary[engine_id] = {"finding": output.get("finding", output.get("state", "UNRESOLVED")), "gate_passed": engine.gate_passed if engine else None, "reason_codes": _engine_codes(engine)}
