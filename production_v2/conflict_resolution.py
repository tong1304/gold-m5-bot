...
"""Non-authoritative cross-brain conflict ledger for the nine-brain market picture."""
...
def build_conflict_ledger(results: dict[str, EngineResult]) -> dict[str, Any]:
    e1, e2, e3, e5, e6, e7, e8 = (_out(results, x) for x in ("E1", "E2", "E3", "E5", "E6", "E7", "E8"))
    conflicts: list[dict[str, Any]] = []

    d1 = _direction(e1, ("trend_state", "pressure", "structure", "market_state", "finding"))
    d2 = _direction(e2, ("direction", "opportunity_direction", "auction_direction", "finding"))
    ...
    directional = [("E1", d1), ("E2", d2), ("E3", d3), ("E6", d6)]
    claimed = [(brain, direction) for brain, direction in directional if direction in DIRECTIONS]
    claimed_directions = {direction for _, direction in claimed}
    if len(claimed_directions) > 1:
        severity = "HIGH" if d6 in DIRECTIONS and any(direction != d6 for _, direction in claimed) else "MEDIUM"
        conflicts.append(_item("DIRECTION_EVIDENCE_CONFLICT", severity, tuple(brain for brain, _ in claimed), "E1/E2/E3/E6_ROLE_BOUNDARIES", "Directional evidence does not converge; E9 must preserve each specialist's evidence and reconcile the disagreement explicitly.", {brain: direction for brain, direction in claimed}))
    ...
    return {"schema": "CROSS_BRAIN_CONFLICT_LEDGER_V2", "authority": "NON_AUTHORITATIVE", "principle": "ONE_MARKET_REALITY;_SPECIALISTS_MAY_DISAGREE_WITHIN_THEIR_BOUNDARIES", "conflicts": conflicts, "summary": {"total": len(conflicts), "blocking_conflicts": blocking, "tensions": tensions, "supportive_relations": supportive, "has_conflict": bool(conflicts), "directional_claims": {brain: direction for brain, direction in claimed}, "directional_convergence": len(claimed_directions) <= 1}}
