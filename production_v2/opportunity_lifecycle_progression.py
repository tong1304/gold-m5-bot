...
def _identity(previous: dict[str, Any], current: dict[str, Any]) -> str:
    ...
    direction = _text(current.get("direction")) or "NEUTRAL"
    setup = _text(current.get("setup") or "OPPORTUNITY") or "OPPORTUNITY"
    event = str(current.get("event_id") or current.get("origin_event_id") or "").strip()
...
def _terminal_result(previous: dict[str, Any], stage: str, current: dict[str, Any]) -> dict[str, Any]:
    ...
    if stage == "REPLACED": state = "REPLACED"
...
def advance_lifecycle_stage(previous: dict[str, Any] | None, current: dict[str, Any] | None) -> dict[str, Any]:
    ...
    previous = dict(previous or {}); current = dict(current or {})
    requested = _requested_stage(current); previous_stage = _text(previous.get("lifecycle_stage")) or "IDLE"
    ...
    if previous_stage in TERMINAL_STAGES:
        ...
        if current_event and previous_event and current_event != previous_event:
            previous = {"stage_history": []}; previous_stage = "IDLE"
    ...
    if requested == "IDLE":
        ...
        if previous_stage in STAGES:
            result["wait_for_stage"] = STAGES[min(STAGE_RANK[previous_stage] + 1, len(STAGES) - 1)]
    ...
    if previous_rank < 0: stage = requested
