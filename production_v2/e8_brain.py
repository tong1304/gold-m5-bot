...
def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    causal = (f"ENTRY={entry:.6f}->CONFIRMATION={confirmation}->STRUCTURAL_STOP={structural_stop if structural_stop is not None else 'NONE'}->STOP={stop if stop is not None else 'NONE'}->"
              f"TARGET={target.get('level') if target.get('level') is not None else 'NONE'}->REAL_RR={real_rr:.3f}->EFFECTIVE_RR={effective_rr:.3f}->"
              f"P={p if p is not None else 'NA'}->EV_R={ev_r if ev_r is not None else 'NA'}->SENSITIVITY={sensitivity.get('state')}->"
