...
ARCHITECTURE = "E8_PROFESSIONAL_TRADE_ECONOMICS_RISK_BRAIN_V17"
VERSION = "17.0"
...
def _target_candidates(levels, direction, entry, atr, e4):
    ...
    for source, level, quality, rank in raw:
        if level is None or not (level > entry if direction == "BUY" else level < entry):
            continue
        d = abs(level - entry); da = d / max(atr, 1e-9); rejection = []
        ...
        out.append({"hierarchy_rank": rank, "source": source, "level": level, "distance": d, "distance_atr": da, "quality": quality, "credible": quality >= TARGET_QUALITY_MIN and not rejection, "rejection": rejection})
...
def _space(e5, target, direction):
    ...
    e5s = _num(e5.get(key)) if e5.get(key) is not None else 0
    ts = _num(target.get("distance_atr")) if target.get("credible") else 0
    vals = []
    if e5s > 0: vals.append(("E5_LOCATION", e5s))
    if ts > 0: vals.append(("TARGET_BARRIER", ts))
    if not vals:
        return {"state": "UNAVAILABLE", "e5_available_space_atr": e5s, "target_barrier_space_atr": ts, "effective_available_space_atr": 0, "space_consistency_delta_atr": None, "space_source": "NO_USABLE_SPACE_EVIDENCE", "space_ok": False, "space_conflict": False}
    effective = min(v for _, v in vals)
    delta = abs(e5s - ts) if len(vals) == 2 else None
    ...
    return {"state": state, "e5_available_space_atr": e5s, "target_barrier_space_atr": ts, "effective_available_space_atr": effective, "space_consistency_delta_atr": delta, "space_source": "MIN(E5_LOCATION,TARGET_BARRIER)" if len(vals) == 2 else vals[0][0], "space_ok": state == "USABLE", "space_conflict": conflict}
...
def _stop_stability(risk, atr, bars):
    ...
    if not prior:
        return {"state": "UNAVAILABLE", "current_stop_atr": current, "ratio_to_prior": None}
    ref = median(prior); normalized = risk / max(ref, 1e-9); ratio = current / max(normalized, 1e-9)
    return {"state": "STABLE" if 0.75 <= ratio <= 1.35 else "UNSTABLE", "current_stop_atr": current, "prior_atr_median": ref, "prior_normalized_stop_atr": normalized, "ratio_to_prior": ratio}
...
def _survival(bars, entry, direction, atr, risk):
    ...
    adverse.sort(); maximum = max(adverse); med = median(adverse); p95 = adverse[min(len(adverse) - 1, int((len(adverse) - 1) * 0.95))]; margin = risk / max(atr, 1e-9) - p95
...
def _execution(snapshot, atr):
    spread = _first_num(snapshot, ("spread", "spread_price", "current_spread")) or 0
    slippage = _first_num(snapshot, ("slippage", "slippage_price", "expected_slippage")) or 0
    total = max(0, spread + slippage)
...
def _volatility(bars, atr):
    ...
    if atr <= 0 or len(series) < 2:
        return {"state": "INVALID", "last_range_atr": 0, "expansion_ratio": 0, "atr_stability": "INVALID", "atr_drift": 0}
    lr = max(0, _num(bars[-1].get("high")) - _num(bars[-1].get("low"))); pr = max(0, _num(bars[-2].get("high")) - _num(bars[-2].get("low")))
    recent = mean(series[-5:]) if len(series) >= 5 else mean(series); baseline = mean(series[-min(len(series), ATR_PERIOD):]); drift = recent / max(baseline, 1e-9); expansion = lr / max(pr, 1e-9)
    return {"state": "EXPANSION_EXTREME" if lr / atr >= 2.5 else "EXPANSION" if lr / atr >= 1.75 or expansion >= 2 else "COMPRESSION" if lr / atr <= 0.6 else "NORMAL", "last_range_atr": lr / atr, "expansion_ratio": expansion, "atr_stability": "STABLE" if 0.65 <= drift <= 1.5 else "UNSTABLE", "atr_drift": drift}
    ...
    for source, data in sources:
        for key in keys:
            ...
            if sample is not None:
                quality += 25 if sample >= 100 else 15 if sample >= MIN_PROBABILITY_SAMPLE else 0
            quality += min(25, confidence * 0.25)
            if _text(source) == "SNAPSHOT":
                quality -= 15
            historical = _text(source) in {"E6", "E7", "E5", "E4", "E3"} or "histor" in key
            if historical:
                quality += 15
            quality = max(0, min(100, quality))
            return {"state": "AVAILABLE", "value": p, "percent": p * 100, "source": f"{source}.{key}", "sample_size": sample, "confidence_percent": confidence or None, "quality": quality, "quality_state": "STRONG" if quality >= 80 else "ADEQUATE" if quality >= MIN_PROBABILITY_QUALITY else "WEAK", "historical_evidence": historical}
    return {"state": "UNAVAILABLE", "value": None, "percent": None, "source": None, "sample_size": None, "confidence_percent": None, "quality": 0.0, "quality_state": "UNAVAILABLE", "historical_evidence": False}


def _economics(risk, reward, execution_cost, probability):
    if risk <= 0 or reward <= 0:
        return {"state": "UNRESOLVED", "probability": probability.get("value"), "gross_reward_r": 0, "execution_cost_r": 0, "effective_reward_r": 0, "effective_rr": 0, "break_even_probability": 1, "probability_edge": None, "expected_value_r": None, "expected_value_price": None, "edge_class": "UNRESOLVED", "asymmetry": "INVALID"}
    ...
    if p is None:
        return {"state": "UNQUANTIFIED", "probability": None, "gross_reward_r": gross, "execution_cost_r": cost_r, "effective_reward_r": effective, "effective_rr": effective, "break_even_probability": be, "probability_edge": None, "expected_value_r": None, "expected_value_price": None, "edge_class": "PROBABILITY_UNAVAILABLE", "asymmetry": "UNQUANTIFIED"}
    ev = p * effective - (1 - p); ev_price = p * max(0, reward - execution_cost) - (1 - p) * risk; edge = p - be
...
def _sensitivity(entry, stop, target, atr, direction, probability, cost):
    if probability.get("value") is None or atr <= 0 or stop is None or target is None:
        return {"state": "UNAVAILABLE", "scenarios": [], "worst_ev_r": None, "fragility": "UNAVAILABLE"}
    p = probability["value"]; scenarios = []
    cases = (("BASELINE", 0, 0, 0), ("ENTRY_WORST", SENSITIVITY_ENTRY_ATR, 0, 0), ("ENTRY_BEST", -SENSITIVITY_ENTRY_ATR, 0, 0), ("STOP_WORST", 0, SENSITIVITY_STOP_ATR, 0), ("STOP_BEST", 0, -SENSITIVITY_STOP_ATR, 0), ("TARGET_WORST", 0, 0, -SENSITIVITY_TARGET_ATR), ("TARGET_BEST", 0, 0, SENSITIVITY_TARGET_ATR), ("COMBINED_WORST", SENSITIVITY_ENTRY_ATR, SENSITIVITY_STOP_ATR, -SENSITIVITY_TARGET_ATR))
    for name, entry_shift, stop_shift, target_shift in cases:
        en = entry + (entry_shift * atr if direction == "BUY" else -entry_shift * atr)
        st = stop - (stop_shift * atr if direction == "BUY" else -stop_shift * atr)
        tp = target - (target_shift * atr if direction == "BUY" else -target_shift * atr)
        valid_geometry = (st < en < tp) if direction == "BUY" else (tp < en < st)
        risk = abs(en - st); reward = abs(tp - en)
        if not valid_geometry or risk <= 0 or reward <= 0:
            scenarios.append({"scenario": name, "valid_geometry": False, "risk_atr": risk / atr if atr else 0, "reward_atr": reward / atr if atr else 0, "effective_rr": 0, "expected_value_r": None})
            continue
        rr = reward / risk; eff = max(0, rr - cost / risk); ev = p * eff - (1 - p)
        scenarios.append({"scenario": name, "valid_geometry": True, "risk_atr": risk / atr, "reward_atr": reward / atr, "effective_rr": eff, "expected_value_r": ev})
    ...
    return {"state": "ROBUST" if robust else "FRAGILE", "scenarios": scenarios, "worst_ev_r": worst, "fragility": "ROBUST" if robust else "ECONOMICS_SENSITIVITY_FRAGILE", "scenario_count": len(scenarios), "worst_case_rule": "MIN_VALID_SCENARIO_EV_AND_NO_GEOMETRY_BREAK"}
...
def _risk_budget(snapshot, risk):
    ...
    if budget is None and pct is not None and capital is not None:
        budget = capital * pct / 100
    if budget is None:
        return {"state": "UNSPECIFIED", "budget": None, "risk_distance": risk, "utilization": None, "position_size": None, "sizing_state": "NOT_COMPUTABLE", "reason": "NO_RISK_BUDGET"}
    if risk <= 0:
        return {"state": "INVALID", "budget": budget, "risk_distance": risk, "utilization": None, "position_size": None, "sizing_state": "INVALID_RISK", "reason": "NON_POSITIVE_RISK_DISTANCE"}
    point_value = _first_num(snapshot, ("point_value", "contract_value_per_price_unit", "value_per_price_unit", "unit_value"))
    if point_value is None:
        return {"state": "DEFINED_NOT_SIZED", "budget": budget, "risk_distance": risk, "utilization": None, "position_size": None, "sizing_state": "MISSING_POINT_VALUE", "reason": "POSITION_SIZE_REQUIRES_INSTRUMENT_VALUE_PER_PRICE_UNIT"}
    risk_per_unit = risk * point_value
    if risk_per_unit <= 0:
        return {"state": "INVALID", "budget": budget, "risk_distance": risk, "risk_per_unit": risk_per_unit, "utilization": None, "position_size": None, "sizing_state": "INVALID_UNIT_RISK", "reason": "NON_POSITIVE_UNIT_RISK"}
    size = budget / risk_per_unit
    return {"state": "WITHIN_BUDGET", "budget": budget, "risk_distance": risk, "point_value": point_value, "risk_per_unit": risk_per_unit, "utilization": 1.0, "position_size": size, "sizing_state": "COMPUTABLE", "reason": "RISK_BUDGET_DIVIDED_BY_UNIT_RISK"}
...
def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    direction = _direction(e6); setup = _setup(e6); confirmation, confirmation_trace = _confirmation(e7)
    entry = _num(bars[-1].get("close")); atr = _atr(bars); data_valid = entry > 0 and atr > 0
    levels = _levels(e3, e4, e5, bars) if data_valid and direction in {"BUY", "SELL"} else {}
    target = _select_target(levels, direction, entry, atr, e4) if levels else {"source": None, "level": None, "distance_atr": 0, "quality": 0, "hierarchy_rank": None, "credible": False, "rejection": ["NO_LEVEL_MODEL"], "candidate_trace": [], "selection_rule": "UNAVAILABLE"}
    stop_model = _stop(direction, entry, atr, levels) if levels else {"source": None, "level": None, "stop": None, "quality": 0, "candidate_trace": [], "basis": "UNAVAILABLE", "structural": False}
    ...
    risk = abs(entry - stop) if stop is not None else 0; reward = abs(target["level"] - entry) if target.get("level") is not None else 0
    stop_atr = risk / max(atr, 1e-9) if risk else 0; real_rr = reward / risk if risk > 0 and reward > 0 else 0
    stop_stability = _stop_stability(risk, atr, bars) if risk else {"state": "UNAVAILABLE"}
    survival = _survival(bars, entry, direction, atr, risk) if risk else {"state": "UNAVAILABLE"}
    space = _space(e5, target, direction) if levels else {"state": "UNAVAILABLE", "effective_available_space_atr": 0, "space_ok": False, "space_conflict": False}
    ...
    if liq is not None and target.get("level") is not None and min(entry, target["level"]) < liq < max(entry, target["level"]):
        opposing_liquidity = liq; opposing_liquidity_r = abs(liq - entry) / max(risk, 1e-9); counter.append("OPPOSING_LIQUIDITY_ON_TARGET_PATH")
    ...
    gate = {
        ...
        "STOP_GEOMETRY": MIN_STOP_ATR <= stop_atr <= MAX_STOP_ATR and stop_stability.get("state") == "STABLE",
        "SURVIVAL": survival.get("state") == "ROBUST",
        "TARGET_HIERARCHY": bool(target.get("credible")),
        "SPACE": space["space_ok"],
        "RR": real_rr >= MIN_RR,
        "EXECUTION": volatility["state"] not in {"EXPANSION_EXTREME", "INVALID"} and volatility["atr_stability"] == "STABLE" and execution["cost_atr"] <= MAX_EXECUTION_COST_ATR,
        "PROBABILITY": probability["state"] == "AVAILABLE" and probability.get("value", 0) >= MIN_PROBABILITY and probability.get("quality", 0) >= MIN_PROBABILITY_QUALITY and probability.get("sample_size") is not None and probability.get("sample_size", 0) >= MIN_PROBABILITY_SAMPLE and probability.get("historical_evidence", False),
        "EXPECTED_VALUE": economics["state"] == "QUANTIFIED" and _num(economics.get("expected_value_r"), -999) >= MIN_ECONOMIC_EDGE,
        "ECONOMIC_EDGE": economics["edge_class"] == "POSITIVE_EXPECTANCY",
        "SENSITIVITY": sensitivity["state"] == "ROBUST",
        "RISK_BUDGET": risk_budget["state"] == "WITHIN_BUDGET" and risk_budget.get("sizing_state") == "COMPUTABLE",
    }
    ready = all(gate.values())
    lifecycle = {f"{i:02d}_{name}": "PASS" if gate[name] else "FAIL" for i, name in enumerate(gate, 1)}
    lifecycle[f"{len(gate) + 1:02d}_FINAL_RISK_GATE"] = "RISK_READY" if ready else "RISK_NOT_READY"
    ...
    effective_rr = economics.get("effective_rr", 0); be = economics.get("break_even_probability"); p = probability.get("percent"); ev_r = economics.get("expected_value_r"); ev_price = economics.get("expected_value_price")
    observations = [
        f"direction={direction}", f"setup={setup}", f"confirmation={confirmation}", f"entry={entry:.6f}", f"atr={atr:.6f}",
        f"risk_distance_atr={stop_atr:.3f}", f"survival_state={survival.get('state')}", f"target={target.get('level') if target.get('level') is not None else 'NONE'}",
        f"effective_space_atr={space.get('effective_available_space_atr', 0):.3f}", f"real_rr={real_rr:.3f}", f"effective_rr={effective_rr:.3f}",
        f"break_even_probability={be * 100:.2f}%" if be is not None else "break_even_probability=UNAVAILABLE",
        f"probability={p:.2f}%" if p is not None else "probability=UNAVAILABLE", f"probability_quality={probability.get('quality', 0):.1f}",
    ...
    causal = f"ENTRY={entry:.6f}->CONFIRMATION={confirmation}->STRUCTURAL_STOP={structural_stop if structural_stop is not None else 'NONE'}->STOP={stop if stop is not None else 'NONE'}->TARGET={target.get('level') if target.get('level') is not None else 'NONE'}->REAL_RR={real_rr:.3f}->EFFECTIVE_RR={effective_rr:.3f}->P={p if p is not None else 'NA'}->EV_R={ev_r if ev_r is not None else 'NA'}->SENSITIVITY={sensitivity.get('state')}->RISK_BUDGET={risk_budget.get('state')}->POSITION_SIZE={risk_budget.get('position_size')}
    output = {
