...
ARCHITECTURE = "E8_PROFESSIONAL_TRADE_ECONOMICS_RISK_BRAIN_V13"
VERSION = "13.0"
...
def _space(e5: dict[str, Any], target: dict[str, Any], direction: str) -> dict[str, Any]:
    ...
    evidence = []
    if e5_present and e5_space > 0:
        evidence.append(("E5_LOCATION", e5_space))
    if target.get("credible") and target_space > 0:
        evidence.append(("TARGET_BARRIER", target_space))
    if not evidence:
...
def _structural_stop(direction: str, entry: float, atr: float, levels: dict[str, Any]) -> dict[str, Any]:
    if direction == "BUY":
        ...
        if not candidates:
            return {"source": None, "level": None, "stop": entry - FALLBACK_STOP_ATR * atr, "basis": "ATR_FALLBACK_LOWER_CONFIDENCE", "quality": 0.0, "distance_atr": FALLBACK_STOP_ATR, "candidate_trace": []}
        ...
        return {"source": None, "level": None, "stop": None, "basis": "NO_DIRECTION", "quality": 0.0, "distance_atr": 0.0, "candidate_trace": []}
    return {"source": source, "level": level, "stop": stop, "basis": "STRUCTURAL_LEVEL_PLUS_ATR_BUFFER", "quality": quality, "distance_atr": abs(entry - stop) / max(atr, 1e-9), "candidate_trace": candidates}
...
def _adverse_excursion(bars: list[dict[str, Any]], entry: float, direction: str, atr: float, stop_distance: float) -> dict[str, Any]:
    ...
    if not window or atr <= 0:
        return {"state": "UNAVAILABLE", "max_adverse_excursion_atr": 0.0, "median_adverse_excursion_atr": 0.0, "survival_margin_atr": 0.0, "window_bars": 0, "p95_adverse_excursion_atr": 0.0}
    adverse = []
    for bar in window:
        if direction == "BUY":
            adverse.append(max(0.0, entry - _num(bar.get("low"))) / atr)
        elif direction == "SELL":
            adverse.append(max(0.0, _num(bar.get("high")) - entry) / atr)
    adverse.sort()
    max_ae = adverse[-1] if adverse else 0.0
    med_ae = median(adverse) if adverse else 0.0
    p95 = adverse[min(len(adverse) - 1, int((len(adverse) - 1) * 0.95))] if adverse else 0.0
    stop_atr = stop_distance / max(atr, 1e-9)
    survival = stop_atr - max_ae
    state = "ROBUST" if survival >= MIN_SURVIVAL_MARGIN_ATR else "FRAGILE" if survival >= 0 else "NON_SURVIVABLE"
    return {"state": state, "max_adverse_excursion_atr": max_ae, "median_adverse_excursion_atr": med_ae, "p95_adverse_excursion_atr": p95, "survival_margin_atr": survival, "window_bars": len(window)}
...
def _economics(risk_distance: float, reward_distance: float, execution_cost: float, probability: dict[str, Any]) -> dict[str, Any]:
    gross_reward_r = reward_distance / max(risk_distance, 1e-9) if risk_distance > 0 else 0.0
    execution_cost_r = execution_cost / max(risk_distance, 1e-9) if risk_distance > 0 else 0.0
    net_reward_r = max(0.0, gross_reward_r - execution_cost_r)
    break_even_probability = 1.0 / max(1.0 + net_reward_r, 1e-9) if net_reward_r > 0 else 1.0
    p = probability.get("value")
    if p is None:
        return {"state": "STRUCTURAL_ONLY", "probability": None, "gross_reward_r": gross_reward_r, "execution_cost_r": execution_cost_r, "net_reward_r": net_reward_r, "break_even_probability": break_even_probability, "probability_edge": None, "expected_value_r": None, "edge": None, "edge_class": "PROBABILITY_UNAVAILABLE"}
    ev = p * net_reward_r - (1.0 - p)
    probability_edge = p - break_even_probability
    cls = "POSITIVE_EXPECTANCY" if ev >= MIN_ECONOMIC_EDGE and probability_edge > 0 else "MARGINAL_EXPECTANCY" if ev >= 0.0 else "NEGATIVE_EXPECTANCY"
    return {"state": "QUANTIFIED", "probability": p, "gross_reward_r": gross_reward_r, "execution_cost_r": execution_cost_r, "net_reward_r": net_reward_r, "break_even_probability": break_even_probability, "probability_edge": probability_edge, "expected_value_r": ev, "edge": ev, "edge_class": cls}


def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """E8 is the independent trade-economics and survivability brain; E9 remains final authority."""
    ...
    if len(bars) < MIN_BARS:
        obs = [f"closed_candles={len(bars)} minimum_required={MIN_BARS}", "risk_lifecycle=UNRESOLVED", "economic_lifecycle=UNRESOLVED"]
    ...
    stop_model = _structural_stop(direction, entry, atr, levels) if levels else {"source": None, "level": None, "stop": None, "basis": "NO_RISK_MODEL", "quality": 0.0, "distance_atr": 0.0, "candidate_trace": []}
    stop = stop_model.get("stop")
    structural_level = stop_model.get("level")
    structural_source = stop_model.get("source")
    ...
    survival = {"state": "UNAVAILABLE", "max_adverse_excursion_atr": 0.0, "median_adverse_excursion_atr": 0.0, "p95_adverse_excursion_atr": 0.0, "survival_margin_atr": 0.0}
    ...
    if data_valid and direction in {"BUY", "SELL"}:
        ...
        if structural_source is None: counter.append("STOP_LOSS_FALLBACK_LOWER_CONFIDENCE")
        ...
        econ = _economics(risk_distance, reward_distance, execution["total_cost"], probability)
        effective_rr = econ["net_reward_r"]
        plan = {
            "valid": True, "entry": entry, "direction": direction, "stop_loss": stop, "structural_stop": structural_level,
            "invalidation_basis": stop_model.get("basis"), "invalidation_source": structural_source,
            "stop_validity": "STRUCTURAL" if structural_source else "FALLBACK_LOWER_CONFIDENCE", "stop_quality": stop_model.get("quality", 0.0),
            "stop_candidate_trace": stop_model.get("candidate_trace", []), "take_profit_1": tp1, "take_profit_2": target_level,
            "target_source": target.get("source"), "target_quality": target.get("quality", 0.0), "target_hierarchy_rank": target.get("hierarchy_rank"),
            "target_distance_atr": target.get("distance_atr", 0.0), "target_candidate_trace": target.get("candidate_trace", []), "target_rejection": target.get("rejection", []),
            "risk_distance": risk_distance, "risk_distance_atr": stop_atr, "reward_distance": reward_distance,
            "reward_distance_atr": reward_distance / max(atr, 1e-9) if reward_distance else 0.0, "available_space": reward_distance,
            "available_space_atr": space["effective_available_space_atr"], "e5_available_space_atr": space["e5_available_space_atr"],
            "target_barrier_space_atr": space["target_barrier_space_atr"], "space_consistency_delta_atr": space["space_consistency_delta_atr"],
            "space_source": space["space_source"], "space_state": space["state"], "real_rr": real_rr, "effective_rr": effective_rr,
            "rr_tp1": reward_distance * TP1_FRACTION / max(risk_distance, 1e-9) if risk_distance else 0.0, "rr_tp2": real_rr,
            "min_required_rr": MIN_RR, "break_even_probability": econ["break_even_probability"], "probability": probability.get("value"),
            "probability_percent": probability.get("percent"), "probability_source": probability.get("source"), "expected_value_r": econ.get("expected_value_r"),
            "economic_edge": econ.get("edge_class"), "probability_edge": econ.get("probability_edge"), "net_reward_r": econ.get("net_reward_r"),
            "asymmetric_payoff": real_rr >= MIN_RR, "stop_distance_atr": stop_atr, "stop_stability": stop_stability["state"],
            "survival_state": survival["state"], "max_adverse_excursion_atr": survival["max_adverse_excursion_atr"],
            "median_adverse_excursion_atr": survival["median_adverse_excursion_atr"], "p95_adverse_excursion_atr": survival["p95_adverse_excursion_atr"],
            "survival_margin_atr": survival["survival_margin_atr"], "risk_buffer_atr": RISK_ATR_BUFFER,
            "structural_breach": structural_breach, "opposing_liquidity": liquidity_level if opposing else None, "opposing_liquidity_r": liquidity_r,
        }
    else:
        probability = _probability(e3, e4, e5, e6, e7, snapshot)
        econ = _economics(0.0, 0.0, execution["total_cost"], probability)
    ...
    lifecycle = {
        "01_DATA_INTEGRITY": "PASS" if gate_data else "FAIL", "02_DIRECTION": "PASS" if gate_direction else "FAIL",
        "03_SETUP_CONFIRMATION": "PASS" if gate_setup and gate_confirmation else "FAIL", "04_STRUCTURAL_INVALIDATION": "PASS" if gate_invalidation else "FAIL",
        "05_STOP_VALIDITY": "PASS" if structural_source else "FAIL", "06_STOP_SURVIVABILITY": "PASS" if gate_stop else "FAIL",
        "07_TARGET_HIERARCHY": "PASS" if gate_target else "FAIL", "08_SPACE_VALIDATION": "PASS" if gate_space else "FAIL",
        "09_EFFECTIVE_RR": "PASS" if gate_rr else "FAIL", "10_EXECUTION_VOLATILITY": "PASS" if gate_execution else "FAIL",
        "11_EXPECTED_VALUE": "PASS" if gate_expected_value else "FAIL", "12_ECONOMICS": "PASS" if gate_economics else "FAIL",
    ...
    observations = [
        ...
        f"stop={stop:.6f}" if stop is not None else "stop=NONE", f"stop_validity={plan.get('stop_validity', 'NONE')}",
        f"risk_distance_atr={plan.get('risk_distance_atr', 0.0):.3f}", f"target={target_level:.6f}" if target_level is not None else "target=NONE",
        f"target_source={target.get('source') or 'NONE'}", f"target_quality={target.get('quality', 0.0):.1f}",
        f"target_hierarchy_rank={target.get('hierarchy_rank', 'NONE')}", f"reward_distance_atr={plan.get('reward_distance_atr', 0.0):.3f}",
        f"effective_space_atr={space.get('effective_available_space_atr', 0.0):.3f}", f"real_rr={real_rr:.3f}",
        f"effective_rr={plan.get('effective_rr', 0.0):.3f}", f"break_even_probability={econ.get('break_even_probability', 1.0) * 100:.2f}%",
        f"probability={probability.get('percent') if probability.get('percent') is not None else 'UNAVAILABLE'}",
        f"probability_source={probability.get('source') or 'NONE'}", f"expected_value_r={econ.get('expected_value_r') if econ.get('expected_value_r') is not None else 'UNAVAILABLE'}",
        f"economic_edge={econ.get('edge_class')}", f"max_adverse_excursion_atr={survival.get('max_adverse_excursion_atr', 0.0):.3f}",
        f"p95_adverse_excursion_atr={survival.get('p95_adverse_excursion_atr', 0.0):.3f}", f"survival_margin_atr={survival.get('survival_margin_atr', 0.0):.3f}",
        f"survival_state={survival.get('state')}", f"risk_lifecycle={lifecycle['13_RISK_GATE']}", f"economic_lifecycle={state}",
    ]
    ...
    output = {
        **base, "state": state, "economic_state": state, "risk_lifecycle": lifecycle["13_RISK_GATE"], "economic_lifecycle": state,
        ...
        "dynamic_target": target, "probability_evidence": probability, "trade_economics": econ, "lifecycle": lifecycle,
        ...
        "supporting_evidence": support, "counter_evidence": counter, "missing_evidence": missing, "observations": observations,
        "invalidation": ["closed-candle structural invalidation", "fallback-only stop", "stop geometry instability", "recent adverse excursion consumes survival margin", "effective space below minimum", "conflicting space evidence", "effective RR below minimum", "opposing liquidity blocks target path", "volatility or execution cost invalidates economics", "entry confirmation not proven", "target barrier not credible", "negative quantified expectancy"],
        "professional_reasoning": {
            "entry_stop_target_chain": f"ENTRY={entry:.6f} -> INVALIDATION={structural_level if structural_level is not None else 'NONE'} -> STOP={stop if stop is not None else 'NONE'} -> TARGET={target_level if target_level is not None else 'NONE'} -> REWARD={reward_distance:.6f} -> REAL_RR={real_rr:.3f} -> EFFECTIVE_RR={plan.get('effective_rr', 0.0):.3f}",
            "structural_stop_reasoning": f"source={structural_source or 'NONE'}; quality={stop_model.get('quality', 0.0):.1f}; basis={stop_model.get('basis')}; candidates={stop_model.get('candidate_trace', [])}",
            "target_hierarchy_reasoning": f"selected={target.get('source') or 'NONE'} rank={target.get('hierarchy_rank', 'NONE')} quality={target.get('quality', 0.0):.1f}; hierarchy precedes distance; candidate_trace={target.get('candidate_trace', [])}",
            "space_reasoning": f"effective={space.get('effective_available_space_atr', 0.0):.3f} ATR; minimum={MIN_SPACE_ATR:.2f}; source={space.get('space_source')}; conflict={space.get('space_conflict')}",
            "stop_survivability": f"state={survival.get('state')}; max_AE={survival.get('max_adverse_excursion_atr', 0.0):.3f} ATR; p95_AE={survival.get('p95_adverse_excursion_atr', 0.0):.3f} ATR; margin={survival.get('survival_margin_atr', 0.0):.3f} ATR; stop={plan.get('risk_distance_atr', 0.0):.3f} ATR",
            "rr_reasoning": f"gross_reward_R={econ.get('gross_reward_r', 0.0):.3f}; execution_cost_R={econ.get('execution_cost_r', 0.0):.3f}; net_reward_R={econ.get('net_reward_r', 0.0):.3f}; effective_RR={plan.get('effective_rr', 0.0):.3f}; required={MIN_RR:.2f}",
            "probability_reasoning": f"state={probability.get('state')}; probability={probability.get('percent') if probability.get('percent') is not None else 'UNAVAILABLE'}%; source={probability.get('source') or 'NONE'}; E8 never fabricates probability",
            "expected_value_reasoning": f"state={econ.get('state')}; break_even_probability={econ.get('break_even_probability', 1.0) * 100:.2f}%; probability_edge={econ.get('probability_edge') if econ.get('probability_edge') is not None else 'UNAVAILABLE'}; EV_R={econ.get('expected_value_r') if econ.get('expected_value_r') is not None else 'UNAVAILABLE'}; class={econ.get('edge_class')}",
            "economic_veto": "PASS" if risk_ready else "VETO: " + "; ".join(counter + missing + ["ECONOMICS_NOT_READY"]),
            "lifecycle": lifecycle,
            "8E_stop_validity": "Structural invalidation must be meaningful before ATR buffering; fallback is explicitly non-ready.",
            ...
            "8K_expected_value": f"probability={probability.get('percent') if probability.get('percent') is not None else 'UNAVAILABLE'}%; break_even={econ.get('break_even_probability', 1.0) * 100:.2f}%; EV_R={econ.get('expected_value_r') if econ.get('expected_value_r') is not None else 'UNAVAILABLE'}; class={econ.get('edge_class')}",
