...
def _direction(e6: dict[str, Any]) -> str:
    for raw in (e6.get("direction"), e6.get("direction_thesis"), e6.get("thesis_direction")):
        value = _text(raw)
        if value in {"BUY", "BULLISH", "UP", "LONG", "BUYERS", "TREND_UP"}: return "BUY"
        if value in {"SELL", "BEARISH", "DOWN", "SHORT", "SELLERS", "TREND_DOWN"}: return "SELL"
    prefix = _text(e6.get("finding")).split()
    if prefix and prefix[0] in {"BUY", "BULLISH", "UP", "LONG"}: return "BUY"
    if prefix and prefix[0] in {"SELL", "BEARISH", "DOWN", "SHORT"}: return "SELL"
    return "NEUTRAL"
...
def _confirmation(e7: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    for key in ("confirmation", "confirmation_state", "trigger_state", "proof_state"):
        if e7.get(key) not in (None, ""): observed.append(_text(e7[key]))
    proof = e7.get("proof_gates")
    if isinstance(proof, dict):
        for key in ("confirmation", "closed_candle_confirmation", "follow_through"):
            value = proof.get(key)
            if value is True or value in {"PASS", "CONFIRMED", "PROVEN", "VALID", "VALIDATED"}: observed.append("CONFIRMED")
            elif value is False or value in {"FAIL", "PENDING", "UNAVAILABLE", "NOT_PROVEN"}: observed.append("NOT_CONFIRMED")
    for key in ("confirmed", "confirmation_proven", "closed_candle_confirmed"):
        if key in e7: observed.append("CONFIRMED" if bool(e7[key]) else "NOT_CONFIRMED")
    reasons = [_text(x) for x in (e7.get("reason_codes") or e7.get("reasons") or [])]
    if any(x in {"PROOF_GATES_INCOMPLETE", "VALID_CLOSED_CANDLE_TRIGGER_MISSING", "TRIGGER_OBSERVED_NOT_AUTOMATIC_CONFIRMATION", "LIQUIDITY_RECLAIM_LEVEL_REQUIRED"} for x in reasons): return "NOT_CONFIRMED", observed + reasons
    if any(x in {"CONFIRMATION_PROVEN", "CAUSAL_FOLLOW_THROUGH_PROVEN"} for x in reasons): return "CONFIRMED", observed + reasons
...
def _target_candidates(levels: dict[str, Any], direction: str, entry: float, atr: float, e4: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for source, level, base_quality, rank in directional:
        distance = abs(level - entry); distance_atr = distance / max(atr, 1e-9); quality = base_quality; rejection: list[str] = []
        if distance_atr < MIN_TARGET_CLEARANCE_ATR: rejection.append("CLEARANCE_TOO_SMALL")
        if distance_atr > MAX_TARGET_EXTENSION_ATR: rejection.append("EXTENSION_TOO_FAR")
        if source in {"STRUCTURE_HIGH_20", "STRUCTURE_LOW_20"}: quality = min(quality, SECONDARY_TARGET_QUALITY)
    ...
    candidates.sort(key=lambda x: (x["hierarchy_rank"], x["distance"]))
    return candidates
...
def _dynamic_target(levels: dict[str, Any], direction: str, entry: float, atr: float, e4: dict[str, Any]) -> dict[str, Any]:
    ...
    if not credible: return {"source": None, "level": None, "distance": 0.0, "distance_atr": 0.0, "quality": 0.0, "credible": False, "rejection": ["NO_CREDIBLE_OPPOSING_BARRIER"], "candidate_trace": candidates, "selection_rule": "HIERARCHY_THEN_NEAREST_CREDIBLE_BARRIER"}
...
def _space(e5: dict[str, Any], target: dict[str, Any], direction: str) -> dict[str, Any]:
    ...
    e5_present = e5.get(key) is not None; e5_space = _num(e5.get(key)) if e5_present else 0.0
    ...
    values = [v for _, v in evidence]; effective = min(values); delta = abs(e5_space - target_space) if len(values) == 2 else None
    conflict = delta is not None and delta >= SPACE_CONFLICT_ATR; state = "CONFLICTED" if conflict else ("CONSTRAINED" if effective < MIN_SPACE_ATR else "USABLE")
...
def _volatility(bars: list[dict[str, Any]], atr: float) -> dict[str, Any]:
    ...
    if atr <= 0 or len(series) < 2: return {"state": "INVALID", "last_range_atr": 0.0, "expansion_ratio": 0.0, "atr_stability": "INVALID", "atr_drift": 0.0}
    last_range = max(0.0, _num(bars[-1].get("high")) - _num(bars[-1].get("low"))); prev_range = max(0.0, _num(bars[-2].get("high")) - _num(bars[-2].get("low")))
    last_range_atr = last_range / atr; expansion_ratio = last_range / max(prev_range, 1e-9); recent = mean(series[-5:]) if len(series) >= 5 else mean(series); baseline = mean(series[-min(len(series), ATR_PERIOD):]); drift = recent / max(baseline, 1e-9)
    atr_stability = "STABLE" if 0.65 <= drift <= 1.50 else "UNSTABLE"
    state = "EXPANSION_EXTREME" if last_range_atr >= MAX_LAST_RANGE_ATR else "EXPANSION" if last_range_atr >= MODERATE_EXPANSION_ATR or expansion_ratio >= 2.0 else "COMPRESSION" if last_range_atr <= 0.60 else "NORMAL"
...
def _stop_stability(risk: float, atr: float, bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not prior: return {"state": "UNAVAILABLE", "current_stop_atr": current, "prior_atr_median": None, "prior_normalized_stop_atr": None, "ratio_to_prior": None}
    reference = median(prior); prior_normalized = risk / max(reference, 1e-9); ratio = current / max(prior_normalized, 1e-9); state = "STABLE" if STOP_STABILITY_MIN_RATIO <= ratio <= STOP_STABILITY_MAX_RATIO else "UNSTABLE"
...
def _execution(snapshot: dict[str, Any], atr: float) -> dict[str, Any]:
    spread = _first_num(snapshot, ("spread", "spread_price", "current_spread")) or 0.0; slippage = _first_num(snapshot, ("slippage", "slippage_price", "expected_slippage")) or 0.0; total = spread + slippage
...
def _structural_stop(direction: str, entry: float, atr: float, levels: dict[str, Any]) -> dict[str, Any]:
    if direction == "BUY":
        candidates = [("PROTECTED_LOW", levels.get("protected_low"), 100.0), ("STRUCTURE_LOW_20", levels.get("structure_low_20"), 80.0)]
        candidates = [(s, v, q) for s, v, q in candidates if v is not None and v < entry]
        if not candidates: return {"source": None, "level": None, "stop": entry - FALLBACK_STOP_ATR * atr, "basis": "ATR_FALLBACK_LOWER_CONFIDENCE", "quality": 0.0, "distance_atr": FALLBACK_STOP_ATR}
        source, level, quality = min(candidates, key=lambda x: abs(entry - x[1])); stop = level - RISK_ATR_BUFFER * atr
    elif direction == "SELL":
        candidates = [("PROTECTED_HIGH", levels.get("protected_high"), 100.0), ("STRUCTURE_HIGH_20", levels.get("structure_high_20"), 80.0)]
        candidates = [(s, v, q) for s, v, q in candidates if v is not None and v > entry]
        if not candidates: return {"source": None, "level": None, "stop": entry + FALLBACK_STOP_ATR * atr, "basis": "ATR_FALLBACK_LOWER_CONFIDENCE", "quality": 0.0, "distance_atr": FALLBACK_STOP_ATR}
        source, level, quality = min(candidates, key=lambda x: abs(entry - x[1])); stop = level + RISK_ATR_BUFFER * atr
    else: return {"source": None, "level": None, "stop": None, "basis": "NO_DIRECTION", "quality": 0.0, "distance_atr": 0.0}
    return {"source": source, "level": level, "stop": stop, "basis": "STRUCTURAL_LEVEL_PLUS_ATR_BUFFER", "quality": quality, "distance_atr": abs(entry - stop) / max(atr, 1e-9)}
...
def _adverse_excursion(bars: list[dict[str, Any]], entry: float, direction: str, atr: float, stop_distance: float) -> dict[str, Any]:
    ...
    if not window or atr <= 0: return {"state": "UNAVAILABLE", "max_adverse_excursion_atr": 0.0, "median_adverse_excursion_atr": 0.0, "survival_margin_atr": 0.0, "window_bars": 0}
    adverse = []
    for bar in window:
    ...
    max_ae = max(adverse) if adverse else 0.0; med_ae = median(adverse) if adverse else 0.0; survival = stop_distance / max(atr, 1e-9) - max_ae
...
def _probability(e3: dict[str, Any], e4: dict[str, Any], e5: dict[str, Any], e6: dict[str, Any], e7: dict[str, Any], snapshot: dict[str, Any]) -> dict[str, Any]:
    for source, mapping in (("E6", e6), ("E7", e7), ("E5", e5), ("E4", e4), ("E3", e3), ("SNAPSHOT", snapshot)):
        for key in ("historical_probability", "win_probability", "success_probability", "trade_probability", "probability", "estimated_probability"):
            if key in mapping:
                raw = _num(mapping.get(key), -1.0)
                if 0.0 <= raw <= 1.0: return {"state": "AVAILABLE", "value": raw, "percent": raw * 100.0, "source": f"{source}.{key}"}
                if 1.0 < raw <= 100.0: return {"state": "AVAILABLE", "value": raw / 100.0, "percent": raw, "source": f"{source}.{key}"}
...
def _economics(real_rr: float, risk_distance: float, reward_distance: float, execution_cost: float, probability: dict[str, Any]) -> dict[str, Any]:
    gross = reward_distance / max(risk_distance, 1e-9) if risk_distance > 0 else 0.0; cost_r = execution_cost / max(risk_distance, 1e-9) if risk_distance > 0 else 0.0; net_reward = max(0.0, gross - cost_r); p = probability.get("value")
    if p is None: return {"state": "STRUCTURAL_ONLY", "probability": None, "gross_reward_r": gross, "execution_cost_r": cost_r, "net_reward_r": net_reward, "expected_value_r": None, "edge": None, "edge_class": "PROBABILITY_UNAVAILABLE"}
    ev = p * net_reward - (1.0 - p); cls = "POSITIVE_EXPECTANCY" if ev >= MIN_ECONOMIC_EDGE else "MARGINAL_EXPECTANCY" if ev >= 0.0 else "NEGATIVE_EXPECTANCY"
    return {"state": "QUANTIFIED", "probability": p, "gross_reward_r": gross, "execution_cost_r": cost_r, "net_reward_r": net_reward, "expected_value_r": ev, "edge": ev, "edge_class": cls}


def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """E8 validates structural risk, target hierarchy, survivability and economics; E9 remains final authority."""
    bars = list(snapshot.get("bars") or []); e3, e4, e5, e6, e7 = (_evidence(upstream.get(k)) for k in ("E3", "E4", "E5", "E6", "E7"))
    ...
    if len(bars) < MIN_BARS:
        obs = [f"closed_candles={len(bars)} minimum_required={MIN_BARS}", "risk_lifecycle=UNRESOLVED", "economic_lifecycle=UNRESOLVED", "risk_gate=RISK_NOT_READY", "missing_evidence=SUFFICIENT_CLOSED_CANDLE_DATA"]
        return EngineResult("E8", NAME, False, 0.0, {**base, "state": "UNRESOLVED", "economic_state": "UNRESOLVED", "risk_lifecycle": "UNRESOLVED", "economic_lifecycle": "UNRESOLVED", "risk_gate": "RISK_NOT_READY", "trade_plan": {}, "observations": obs, "supporting_evidence": obs, "counter_evidence": ["INSUFFICIENT_CLOSED_CANDLE_DATA"], "missing_evidence": ["SUFFICIENT_CLOSED_CANDLE_DATA"], "gate_matrix": {}}, ("INSUFFICIENT_DATA",))

    direction, setup = _direction(e6), _setup(e6); confirmation, confirmation_trace = _confirmation(e7); entry, atr = _num(bars[-1].get("close")), _atr(bars); vol, execution, liq = _volatility(bars, atr), _execution(snapshot, atr), _liquidity(e4)
    support: list[str] = []; counter: list[str] = []; missing: list[str] = []; data_valid = entry > 0 and atr > 0
    ...
    plan: dict[str, Any] = {}; space = {"state": "UNAVAILABLE", "effective_available_space_atr": 0.0, "e5_available_space_atr": 0.0, "target_barrier_space_atr": 0.0, "space_consistency_delta_atr": None, "space_source": "UNAVAILABLE", "space_ok": False, "space_conflict": False}; stop_stability = {"state": "UNAVAILABLE", "current_stop_atr": 0.0, "ratio_to_prior": None}; survival = {"state": "UNAVAILABLE", "max_adverse_excursion_atr": 0.0, "median_adverse_excursion_atr": 0.0, "survival_margin_atr": 0.0}; risk_distance = reward_distance = real_rr = 0.0; target_level = None; liquidity_r = 0.0; opposing = False

    if data_valid and direction in {"BUY", "SELL"}:
        risk_distance = abs(entry - stop) if stop is not None else 0.0; stop_atr = risk_distance / max(atr, 1e-9); stop_stability = _stop_stability(risk_distance, atr, bars); survival = _adverse_excursion(bars, entry, direction, atr, risk_distance); space = _space(e5, target, direction); target_level = target.get("level"); reward_distance = abs(target_level - entry) if target_level is not None else 0.0; real_rr = reward_distance / max(risk_distance, 1e-9) if target_level is not None and risk_distance > 0 else 0.0
        liquidity_level = levels.get("liquidity_event_level"); opposing = liquidity_level is not None and target_level is not None and min(entry, target_level) < liquidity_level < max(entry, target_level); liquidity_r = abs(liquidity_level - entry) / max(risk_distance, 1e-9) if opposing and risk_distance > 0 else 0.0
    ...
    probability = _probability(e3, e4, e5, e6, e7, snapshot); econ = _economics(real_rr, risk_distance, reward_distance, execution["total_cost"], probability)
    ...
    gate_data = data_valid; gate_direction = direction in {"BUY", "SELL"}; gate_setup = setup.upper() not in {"UNKNOWN", "NONE", "UNRESOLVED"}; gate_confirmation = confirmation == "CONFIRMED"; gate_invalidation = not structural_breach
    gate_stop = bool(plan) and structural_source is not None and MIN_STOP_ATR <= plan.get("stop_distance_atr", 99.0) <= MAX_STOP_ATR and stop_stability["state"] == "STABLE" and survival["state"] == "ROBUST"
    gate_target = bool(target.get("credible")) and target.get("distance_atr", 0.0) <= MAX_TARGET_EXTENSION_ATR; gate_space = space.get("space_ok", False); gate_rr = bool(plan) and plan.get("effective_rr", 0.0) >= MIN_RR
    ...
    lifecycle = {"01_DATA_INTEGRITY": "PASS" if gate_data else "FAIL", "02_DIRECTION": "PASS" if gate_direction else "FAIL", "03_SETUP_CONFIRMATION": "PASS" if gate_setup and gate_confirmation else "FAIL", "04_STRUCTURAL_INVALIDATION": "PASS" if gate_invalidation else "FAIL", "05_STOP_VALIDITY": "PASS" if structural_source else "FAIL", "06_STOP_SURVIVABILITY": "PASS" if gate_stop else "FAIL", "07_TARGET_HIERARCHY": "PASS" if gate_target else "FAIL", "08_SPACE_VALIDATION": "PASS" if gate_space else "FAIL", "09_EFFECTIVE_RR": "PASS" if gate_rr else "FAIL", "10_EXECUTION_VOLATILITY": "PASS" if gate_execution else "FAIL", "11_EXPECTED_VALUE": "PASS" if gate_expected_value else "FAIL", "12_ECONOMICS": "PASS" if gate_economics else "FAIL", "13_RISK_GATE": "RISK_READY" if gate_economics else "RISK_NOT_READY"}
    hard_failure = any(x in critical for x in counter) or bool(missing); state = "ATTRACTIVE" if gate_economics else "CONDITIONAL" if plan and not hard_failure else "UNATTRACTIVE" if plan else "UNRESOLVED"; risk_ready = gate_economics; score = 95.0 if risk_ready else 65.0 if state == "CONDITIONAL" else 30.0 if state == "UNATTRACTIVE" else 15.0
    ...
    output = {**base, "state": state, "economic_state": state, "risk_lifecycle": lifecycle["13_RISK_GATE"], "economic_lifecycle": state, "risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY", "direction": direction, "setup": setup, "confirmation": confirmation, "confirmation_trace": confirmation_trace, "trade_plan": plan, "risk_model": {"atr": atr, "atr_period": ATR_PERIOD, "volatility_state": vol["state"], "last_range_atr": vol["last_range_atr"], "expansion_ratio": vol["expansion_ratio"], "atr_stability": vol["atr_stability"], "atr_drift": vol["atr_drift"], "stop_stability": stop_stability, "survival": survival, "spread": execution["spread"], "slippage": execution["slippage"], "execution_cost_atr": execution["cost_atr"], "structure_lookback": STRUCTURE_LOOKBACK, "min_stop_atr": MIN_STOP_ATR, "max_stop_atr": MAX_STOP_ATR, "min_space_atr": MIN_SPACE_ATR, "min_rr": MIN_RR, "risk_buffer_atr": RISK_ATR_BUFFER, "max_liquidity_risk_r": MAX_LIQUIDITY_RISK_R, "max_execution_cost_atr": MAX_EXECUTION_COST_ATR}, "structural_evidence": {**levels, "structural_breach": structural_breach, "invalidation_source": structural_source, "stop_model": stop_model}, "liquidity_evidence": liq, "location_evidence": {"e5_available_space_atr_long": _num(e5.get("available_space_atr_long")), "e5_available_space_atr_short": _num(e5.get("available_space_atr_short")), "effective_available_space_atr": space.get("effective_available_space_atr", 0.0), "target_barrier_space_atr": space.get("target_barrier_space_atr", 0.0), "space_consistency_delta_atr": space.get("space_consistency_delta_atr"), "space_source": space.get("space_source"), "space_state": space.get("state"), "space_ok": space.get("space_ok", False), "space_conflict": space.get("space_conflict", False)}, "dynamic_target": target, "probability_evidence": probability, "trade_economics": econ, "lifecycle": lifecycle, "gate_matrix": {"8A_data_integrity": "PASS" if gate_data else "FAIL", "8B_direction_validation": "PASS" if gate_direction else "FAIL", "8C_setup_confirmation_gate": "PASS" if gate_setup and gate_confirmation else "FAIL", "8D_structural_invalidation": "PASS" if gate_invalidation else "FAIL", "8E_stop_validity": "PASS" if structural_source else "FAIL", "8F_stop_survivability": "PASS" if gate_stop else "FAIL", "8G_target_hierarchy": "PASS" if gate_target else "FAIL", "8H_available_space": "PASS" if gate_space else "FAIL", "8I_effective_rr": "PASS" if gate_rr else "FAIL", "8J_volatility_execution": "PASS" if gate_execution else "FAIL", "8K_expected_value": "PASS" if gate_expected_value else "FAIL", "8L_trade_economics": state, "8M_final_risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY"}, "supporting_evidence": support, "counter_evidence": counter, "missing_evidence": missing, "observations": observations, "invalidation": ["closed-candle structural invalidation", "structural stop unavailable or fallback-only", "stop geometry becomes unstable versus recent ATR regime", "recent adverse excursion consumes the survival margin", "effective available space collapses below minimum", "space evidence materially conflicts", "effective RR falls below minimum", "opposing or external liquidity blocks the target path", "volatility makes the stop non-survivable", "execution cost becomes excessive", "entry confirmation is not proven", "target barrier is not credible or is too far for M5 execution", "quantified expected value becomes negative"], "professional_reasoning": {"entry_stop_target_chain": f"ENTRY={entry:.6f} -> INVALIDATION={structural_level if structural_level is not None else 'NONE'} -> STOP={stop if stop is not None else 'NONE'} -> TARGET={target_level if target_level is not None else 'NONE'} -> REWARD={reward_distance:.6f} -> RR={real_rr:.3f} -> EFFECTIVE_RR={plan.get('effective_rr', 0.0):.3f}", "structural_stop_reasoning": f"source={structural_source or 'NONE'}; quality={stop_model.get('quality', 0.0):.1f}; basis={stop_model.get('basis')}", "target_hierarchy_reasoning": f"selected={target.get('source') or 'NONE'} rank={target.get('hierarchy_rank', 'NONE')} quality={target.get('quality', 0.0):.1f}; structural hierarchy first, distance second; farther targets cannot manufacture RR", "space_reasoning": f"effective={space.get('effective_available_space_atr', 0.0):.3f} ATR minimum={MIN_SPACE_ATR:.2f} ATR source={space.get('space_source')}", "stop_survivability": f"state={survival.get('state')}; max_adverse_excursion={survival.get('max_adverse_excursion_atr', 0.0):.3f} ATR; survival_margin={survival.get('survival_margin_atr', 0.0):.3f} ATR; stop={plan.get('risk_distance_atr', 0.0):.3f} ATR", "rr_reasoning": f"gross_reward={reward_distance:.6f} / risk={risk_distance:.6f} = {real_rr:.3f}; execution_cost_r={econ.get('execution_cost_r', 0.0):.3f}; effective_rr={plan.get('effective_rr', 0.0):.3f}; required={MIN_RR:.2f}", "probability_reasoning": f"state={probability.get('state')}; value={probability.get('percent') if probability.get('percent') is not None else 'UNAVAILABLE'}%; source={probability.get('source') or 'NONE'}; E8 never fabricates probability", "expected_value_reasoning": f"state={econ.get('state')}; expected_value_r={econ.get('expected_value_r') if econ.get('expected_value_r') is not None else 'UNAVAILABLE'}; edge={econ.get('edge_class')}", "economic_veto": "PASS" if risk_ready else "VETO: " + "; ".join(counter + missing + ["ECONOMICS_NOT_READY"]), "lifecycle": lifecycle, "8E_stop_validity": "Structural invalidation must be meaningful before ATR buffering; fallback is explicitly non-ready.", "8F_stop_survivability": f"state={survival.get('state')}; stop_stability={stop_stability['state']}; current_stop_atr={stop_stability.get('current_stop_atr', 0.0):.3f}", "8G_target_hierarchy": f"selected={target.get('level', 'NONE')} source={target.get('source', 'NONE')} rank={target.get('hierarchy_rank', 'NONE')} quality={target.get('quality', 0.0):.1f}", "8I_effective_rr": f"real_rr={real_rr:.3f}; effective_rr={plan.get('effective_rr', 0.0):.3f}; execution_cost_atr={execution['cost_atr']:.3f}", "8K_expected_value": f"probability={probability.get('percent') if probability.get('percent') is not None else 'UNAVAILABLE'}%; EV_R={econ.get('expected_value_r') if econ.get('expected_value_r') is not None else 'UNAVAILABLE'}; class={econ.get('edge_class')}", "8M_final_risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY"}, "decision_path": "E8 validates structural risk, target hierarchy, survivability and trade economics only; E9 retains final trade authority."}
