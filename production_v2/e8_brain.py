...
ARCHITECTURE = "E8_PROFESSIONAL_TRADE_ECONOMICS_RISK_BRAIN_V8"
VERSION = "8.0"
...
def _dynamic_target(levels: dict[str, Any], direction: str, entry: float, atr: float, e4: dict[str, Any]) -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        if source == "STRUCTURE_HIGH_20" or source == "STRUCTURE_LOW_20":
            quality -= 8.0
            rejection.append("SECONDARY_STRUCTURE_TARGET")
...
def _space_analysis(e5: dict[str, Any], target_meta: dict[str, Any], entry: float, direction: str) -> dict[str, Any]:
    key = "available_space_atr_long" if direction == "BUY" else "available_space_atr_short"
    raw_e5 = e5.get(key)
    e5_space = _num(raw_e5) if raw_e5 is not None else 0.0
    target_space = _num(target_meta.get("distance_atr")) if target_meta.get("credible") else 0.0
    evidence_present = raw_e5 is not None
    ...
    return {
        ...
        "space_evidence_present": evidence_present,
        "space_conflict": conflict,
        "space_source": "MIN(E5_LOCATION,CREDIBLE_TARGET_BARRIER)" if sources else "NO_USABLE_SPACE_EVIDENCE",
        "minimum_required_atr": MIN_SPACE_ATR,
        "space_ok": effective >= MIN_SPACE_ATR and not conflict,
        "entry": entry,
    }
...
def _volatility_state(bars: list[dict[str, Any]], atr: float) -> dict[str, Any]:
    ...
    drift = recent_atr / max(baseline_atr, 1e-9)
    stability = "STABLE" if 0.65 <= drift <= 1.50 else "UNSTABLE"
    return {"state": state, "last_range_atr": ratio, "expansion_ratio": expansion_ratio, "atr_stability": stability, "atr_drift": drift}
...
def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """Validate trade survivability/economics. E9 remains final trade authority."""
    ...
    liquidity = _e4_liquidity_quality(e4)
    support: list[str] = []
    counter: list[str] = []
    missing: list[str] = []
    plan: dict[str, Any] = {}
    ...
    target_meta = _dynamic_target(levels, direction, entry, atr, e4) if levels else {"source": None, "level": None, "distance_atr": 0.0, "quality": 0.0, "credible": False, "rejection": ["NO_TARGET_INPUTS"], "candidate_trace": []}
    ...
    if direction == "BUY":
        ...
        valid = [(n, v) for n, v in invalidations if v is not None and v < entry]
    elif direction == "SELL":
    ...
    if direction in {"BUY", "SELL"} and data_valid:
        ...
        space = _space_analysis(e5, target_meta, entry, direction)
        ...
        opposing_liquidity = liquidity_level is not None and target is not None and min(entry, target) < liquidity_level < max(entry, target)
        liquidity_r = abs(liquidity_level - entry) / max(risk, 1e-9) if opposing_liquidity else 0.0
        ...
        if not space["space_ok"]:
            counter.append("EFFECTIVE_SPACE_BELOW_MINIMUM" if space["effective_available_space_atr"] < MIN_SPACE_ATR else "SPACE_EVIDENCE_CONFLICT")
        if space["space_consistency_delta_atr"] is not None and space["space_consistency_delta_atr"] >= SPACE_CONFLICT_ATR and "SPACE_EVIDENCE_CONFLICT" not in counter:
        ...
        plan = {
            ...
            "target_rejection": target_meta.get("rejection", []),
            "risk_distance": risk,
            ...
            "opposing_liquidity": liquidity_level if opposing_liquidity else None,
    ...
    critical = {
        "RISK_DATA_INVALID", "NO_VALID_DIRECTION", "STRUCTURAL_INVALIDATION_BREACHED", "STOP_TOO_TIGHT_FOR_VOLATILITY", "STOP_TOO_WIDE_FOR_ECONOMICS",
        "NO_USABLE_STRUCTURAL_TARGET", "EFFECTIVE_SPACE_BELOW_MINIMUM", "REAL_RR_BELOW_MINIMUM", "OPPOSING_LIQUIDITY_PATH_RISK",
        "EXTERNAL_LIQUIDITY_PATH_RISK", "LIQUIDITY_AUCTION_NOT_TERMINALLY_CONFIRMED", "LOW_INFORMATION_LIQUIDITY_RISK", "VOLATILITY_RISK_HIGH",
        "ATR_STABILITY_RISK", "EXECUTION_COST_TOO_HIGH", "DYNAMIC_TARGET_NOT_USABLE", "TARGET_TOO_FAR_FOR_M5_EXECUTION", "SPACE_EVIDENCE_CONFLICT",
    ...
    economic_ready = (
        data_valid and direction in {"BUY", "SELL"} and bool(plan) and bool(plan.get("valid"))
        and target_meta.get("credible") and plan.get("real_rr", 0.0) >= MIN_RR
        and MIN_STOP_ATR <= plan.get("stop_distance_atr", float("inf")) <= MAX_STOP_ATR
        and space.get("space_ok", False) and not structural_breach
        and volatility["state"] not in {"EXPANSION_EXTREME", "INVALID"}
        and volatility["atr_stability"] == "STABLE" and execution["cost_atr"] <= MAX_EXECUTION_COST_ATR
        and not any(reason in critical for reason in counter) and not missing
    )
    if economic_ready:
    ...
    output = {
        **base,
        ...
        "risk_model": {
            ...
            "min_space_atr": MIN_SPACE_ATR, "min_rr": MIN_RR, "risk_buffer_atr": RISK_ATR_BUFFER,
            "max_liquidity_risk_r": MAX_LIQUIDITY_RISK_R, "max_execution_cost_atr": MAX_EXECUTION_COST_ATR,
        },
        "structural_evidence": {**levels, "structural_breach": structural_breach, "invalidation_source": structural_source},
        ...
        "location_evidence": {
            ...
            "space_source": space.get("space_source"), "space_ok": space.get("space_ok", False), "space_conflict": space.get("space_conflict", False),
        },
        ...
        "invalidation": [
            ...
            "space evidence materially conflicts", "real RR falls below minimum", "opposing or external liquidity blocks the target path",
            "volatility makes the stop non-survivable", "execution cost becomes excessive", "entry confirmation is not proven",
        ...
        "professional_reasoning": {
            ...
            "8E_liquidity_risk": "Liquidity is a path risk only when it can obstruct the actual entry-to-target route; internal pending liquidity has lower informational weight.",
            "8F_available_space": f"effective_space_atr={space.get('effective_available_space_atr', 0.0):.3f}; source={space.get('space_source')}; conflict={space.get('space_conflict', False)}",
            "8G_dynamic_target": f"selected_target={plan.get('take_profit_2', 'NONE')} source={plan.get('target_source', 'NONE')} quality={plan.get('target_quality', 0.0):.1f}; farther targets never manufacture RR.",
