...
ARCHITECTURE = "E8_PROFESSIONAL_TRADE_ECONOMICS_RISK_BRAIN_V10"
VERSION = "10.0"
...
def _confirmation(e7: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if any(x in {"PROOF_GATES_INCOMPLETE", "VALID_CLOSED_CANDLE_TRIGGER_MISSING",
    ...
    return ("CONFIRMED" if any(x in {"CONFIRMED", "PROVEN", "VALIDATED"} for x in observed)
            else "NOT_CONFIRMED"), observed + reasons
...
def _levels(e3: dict[str, Any], e4: dict[str, Any], e5: dict[str, Any],
    ...
    return {
        ...
        "liquidity_event_level": _first_num(e4, ("event_level", "liquidity_level", "nearest_liquidity",
...
def _dynamic_target(levels: dict[str, Any], direction: str, entry: float, atr: float,
                    e4: dict[str, Any]) -> dict[str, Any]:
    # Causal hierarchy: nearest opposing barrier first. E8 never skips a nearer
    # credible barrier merely to manufacture a better RR with a farther target.
    if direction == "BUY":
        raw = [
...
def _space(e5: dict[str, Any], target: dict[str, Any], direction: str) -> dict[str, Any]:
    ...
    if target_present and target_space > 0:
    ...
    if not evidence:
        return {
            ...
            "effective_available_space_atr": 0.0,
            "space_consistency_delta_atr": None,
            "space_evidence_present": e5_present or target_present,
            "space_conflict": False,
            "space_source": "NO_USABLE_SPACE_EVIDENCE",
    ...
    return {
        ...
        "space_evidence_present": e5_present or target_present,
        "space_conflict": conflict,
        "space_source": "MIN(E5_LOCATION,TARGET_BARRIER)" if len(values) == 2 else evidence[0][0],
        "minimum_required_atr": MIN_SPACE_ATR,
        "space_ok": state == "USABLE",
    }
...
def _volatility(bars: list[dict[str, Any]], atr: float) -> dict[str, Any]:
    ...
    if last_range_atr >= MAX_LAST_RANGE_ATR:
        state = "EXPANSION_EXTREME"
    elif last_range_atr >= MODERATE_EXPANSION_ATR or expansion_ratio >= 2.0:
        state = "EXPANSION"
    elif last_range_atr <= 0.60:
        state = "COMPRESSION"
    else:
        state = "NORMAL"
...
def _stop_stability(risk: float, atr: float, bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ratio = current / max(risk / max(reference, 1e-9), 1e-9)
    state = "STABLE" if STOP_STABILITY_MIN_RATIO <= ratio <= STOP_STABILITY_MAX_RATIO else "UNSTABLE"
    return {"state": state, "current_stop_atr": current, "prior_atr_median": reference,
...
def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """E8 validates trade economics/survivability; E9 retains final authority."""
    ...
    if len(bars) < MIN_BARS:
        return EngineResult(
            "E8", NAME, False, 0.0,
            {**base, "state": "UNRESOLVED", "economic_state": "UNRESOLVED",
             "risk_gate": "RISK_NOT_READY", "trade_plan": {},
             "supporting_evidence": [], "counter_evidence": ["INSUFFICIENT_CLOSED_CANDLE_DATA"],
    ...
    target = _dynamic_target(levels, direction, entry, atr, e4) if levels else {
        "source": None, "level": None, "distance_atr": 0.0, "quality": 0.0,
        "credible": False, "rejection": ["NO_TARGET_INPUTS"], "candidate_trace": [],
    }

    structural_source = structural_level = None
    if direction == "BUY":
        valid = [("PROTECTED_LOW", levels.get("protected_low")),
                 ("STRUCTURE_LOW_20", levels.get("structure_low_20"))]
    elif direction == "SELL":
        valid = [("PROTECTED_HIGH", levels.get("protected_high")),
                 ("STRUCTURE_HIGH_20", levels.get("structure_high_20"))]
    else:
        valid = []
    valid = [(n, v) for n, v in valid if v is not None and
             ((direction == "BUY" and v < entry) or (direction == "SELL" and v > entry))]
    if valid:
        valid.sort(key=lambda x: abs(entry - x[1]))
        structural_source, structural_level = valid[0]

    structural_breach = bool(
        structural_level is not None and
        ((direction == "BUY" and entry <= structural_level) or
         (direction == "SELL" and entry >= structural_level))
    )
    ...
    plan: dict[str, Any] = {}
    space = {
        "state": "UNAVAILABLE", "effective_available_space_atr": 0.0,
        "e5_available_space_atr": 0.0, "target_barrier_space_atr": 0.0,
        "space_consistency_delta_atr": None, "space_source": "UNAVAILABLE",
    ...
    if data_valid and direction in {"BUY", "SELL"}:
        if direction == "BUY":
            stop = structural_level - RISK_ATR_BUFFER * atr if structural_level is not None else entry - FALLBACK_STOP_ATR * atr
        else:
            stop = structural_level + RISK_ATR_BUFFER * atr if structural_level is not None else entry + FALLBACK_STOP_ATR * atr

        risk = abs(entry - stop)
        stop_atr = risk / max(atr, 1e-9)
        stop_stability = _stop_stability(risk, atr, bars)
        ...
        opposing = (
            liquidity_level is not None and target_level is not None and
            min(entry, target_level) < liquidity_level < max(entry, target_level)
        )
        liquidity_r = abs(liquidity_level - entry) / max(risk, 1e-9) if opposing else 0.0

        if opposing:
            counter.append("OPPOSING_LIQUIDITY_ON_TARGET_PATH")
            if liq["externality"] == "EXTERNAL":
            ...
            if liq["auction_state"] == "PENDING":
                counter.append("LIQUIDITY_AUCTION_NOT_TERMINALLY_CONFIRMED")
            if liq["information"] == "LOW_INFORMATION":
        ...
        if stop_stability["state"] == "UNSTABLE":
            counter.append("STOP_GEOMETRY_UNSTABLE")
        if target_level is not None and real_rr < MIN_RR:
            counter.append("REAL_RR_BELOW_MINIMUM")
        if not target.get("credible"):
            counter.append("DYNAMIC_TARGET_NOT_USABLE")
        if target.get("distance_atr", 0.0) > MAX_TARGET_EXTENSION_ATR:
            counter.append("TARGET_TOO_FAR_FOR_M5_EXECUTION")
        if not space["space_ok"]:
            counter.append("EFFECTIVE_SPACE_BELOW_MINIMUM")
        if space["space_conflict"]:
        ...
        plan = {
            ...
            "direction": direction,
            "stop_loss": stop,
            "structural_stop": structural_level,
            "invalidation_basis": "STRUCTURAL_LEVEL_PLUS_ATR_BUFFER" if structural_level is not None else "ATR_FALLBACK_LOWER_CONFIDENCE",
            "invalidation_source": structural_source,
            "take_profit_1": entry + reward * TP1_FRACTION if direction == "BUY" else entry - reward * TP1_FRACTION,
            "take_profit_2": target_level,
            "target_source": target.get("source"),
            "target_quality": target.get("quality", 0.0),
            "target_distance_atr": target.get("distance_atr", 0.0),
            "target_candidate_trace": target.get("candidate_trace", []),
            "target_rejection": target.get("rejection", []),
            "risk_distance": risk,
            "reward_distance": reward,
            "available_space": reward,
            ...
            "space_source": space["space_source"],
            "space_state": space["state"],
            "real_rr": real_rr,
            "rr_tp1": reward * TP1_FRACTION / max(risk, 1e-9),
            "rr_tp2": real_rr,
            "asymmetric_payoff": real_rr >= MIN_RR,
            "stop_distance_atr": stop_atr,
            "stop_stability": stop_stability["state"],
            "risk_buffer_atr": RISK_ATR_BUFFER,
            "structural_breach": structural_breach,
            "opposing_liquidity": liquidity_level if opposing else None,
            "opposing_liquidity_r": liquidity_r,
        }
        support += [
            f"entry={entry:.6f}", f"final_stop={stop:.6f}",
            f"target={target_level:.6f}" if target_level is not None else "target=NONE",
            f"target_source={target.get('source')}",
            f"target_quality={target.get('quality', 0.0):.1f}",
            f"space_state={space['state']}",
            f"effective_space_atr={space['effective_available_space_atr']:.3f}",
            f"real_rr={real_rr:.3f}",
            f"stop_distance_atr={stop_atr:.3f}",
            f"stop_stability={stop_stability['state']}",
        ]
    else:
        counter.append("RISK_MODEL_UNAVAILABLE")

    if vol["state"] == "EXPANSION_EXTREME":
        counter.append("VOLATILITY_RISK_HIGH")
    elif vol["state"] == "EXPANSION":
        counter.append("VOLATILITY_EXPANSION_RISK")
    if vol["atr_stability"] == "UNSTABLE":
        counter.append("ATR_STABILITY_RISK")
    if execution["cost_atr"] > MAX_EXECUTION_COST_ATR:
        counter.append("EXECUTION_COST_TOO_HIGH")
    if vol["state"] == "COMPRESSION" and plan.get("target_distance_atr", 0.0) >= 2.5:
        counter.append("COMPRESSION_TARGET_REALIZATION_RISK")

    critical = {
        "RISK_DATA_INVALID", "NO_VALID_DIRECTION", "STRUCTURAL_INVALIDATION_BREACHED",
        "STOP_TOO_TIGHT_FOR_VOLATILITY", "STOP_TOO_WIDE_FOR_ECONOMICS",
        "STOP_GEOMETRY_UNSTABLE", "NO_USABLE_STRUCTURAL_TARGET",
        "EFFECTIVE_SPACE_BELOW_MINIMUM", "REAL_RR_BELOW_MINIMUM",
        "OPPOSING_LIQUIDITY_PATH_RISK", "EXTERNAL_LIQUIDITY_PATH_RISK",
        "LIQUIDITY_AUCTION_NOT_TERMINALLY_CONFIRMED", "LOW_INFORMATION_LIQUIDITY_RISK",
        "VOLATILITY_RISK_HIGH", "ATR_STABILITY_RISK", "EXECUTION_COST_TOO_HIGH",
        "DYNAMIC_TARGET_NOT_USABLE", "TARGET_TOO_FAR_FOR_M5_EXECUTION",
        "SPACE_EVIDENCE_CONFLICT", "COMPRESSION_TARGET_REALIZATION_RISK",
    }
    ...
    economic_ready = (
        bool(plan) and target.get("credible") and plan.get("real_rr", 0.0) >= MIN_RR
        and MIN_STOP_ATR <= plan.get("stop_distance_atr", 99.0) <= MAX_STOP_ATR
        and space.get("space_ok", False)
        and not structural_breach
        and vol["state"] not in {"EXPANSION_EXTREME", "INVALID"}
        and vol["atr_stability"] == "STABLE"
        and stop_stability["state"] == "STABLE"
        and execution["cost_atr"] <= MAX_EXECUTION_COST_ATR
        and not any(x in critical for x in counter)
        and not missing
    )
    ...
    output = {
        **base,
        ...
        "risk_model": {
            ...
            "volatility_state": vol["state"], "last_range_atr": vol["last_range_atr"],
            "expansion_ratio": vol["expansion_ratio"], "atr_stability": vol["atr_stability"],
            "atr_drift": vol["atr_drift"],
            "stop_stability": stop_stability,
            "spread": execution["spread"], "slippage": execution["slippage"],
            ...
            "min_space_atr": MIN_SPACE_ATR, "min_rr": MIN_RR,
            "risk_buffer_atr": RISK_ATR_BUFFER,
            "max_liquidity_risk_r": MAX_LIQUIDITY_RISK_R,
            "max_execution_cost_atr": MAX_EXECUTION_COST_ATR,
        },
        "structural_evidence": {**levels, "structural_breach": structural_breach,
                                "invalidation_source": structural_source},
        ...
        "location_evidence": {
            ...
            "space_ok": space.get("space_ok", False),
            "space_conflict": space.get("space_conflict", False),
        },
        ...
        "gate_matrix": {
            ...
            "8E_liquidity_risk": "PASS" if not any(x in counter for x in {
                "OPPOSING_LIQUIDITY_PATH_RISK", "EXTERNAL_LIQUIDITY_PATH_RISK",
                "LIQUIDITY_AUCTION_NOT_TERMINALLY_CONFIRMED", "LOW_INFORMATION_LIQUIDITY_RISK"
            }) else "FAIL",
            "8F_available_space": "PASS" if space.get("space_ok") else "FAIL",
            "8G_dynamic_target": "PASS" if target.get("credible") and "TARGET_TOO_FAR_FOR_M5_EXECUTION" not in counter else "FAIL",
            "8H_real_rr": "PASS" if plan.get("real_rr", 0.0) >= MIN_RR else "FAIL",
            "8I_volatility_execution": "PASS" if (
                vol["state"] not in {"EXPANSION_EXTREME", "INVALID"}
                and vol["atr_stability"] == "STABLE"
                and stop_stability["state"] == "STABLE"
                and execution["cost_atr"] <= MAX_EXECUTION_COST_ATR
            ) else "FAIL",
            "8J_trade_economics": state,
        ...
        "invalidation": [
            ...
            "stop geometry becomes unstable versus recent ATR regime",
            "effective available space collapses below minimum",
            "space evidence materially conflicts",
        ...
        "professional_reasoning": {
            ...
            "8E_liquidity_risk": "Liquidity is evaluated on the actual entry-to-target path; pending or low-information liquidity cannot be treated as clean target space.",
            "8F_available_space": f"state={space.get('state')}; effective_space_atr={space.get('effective_available_space_atr', 0.0):.3f}; source={space.get('space_source')}; conflict={space.get('space_conflict', False)}",
            "8G_dynamic_target": f"selected_target={plan.get('take_profit_2', 'NONE')} source={plan.get('target_source', 'NONE')} quality={plan.get('target_quality', 0.0):.1f}; nearest credible barrier is honored and farther targets cannot manufacture RR.",
            "8H_real_rr": f"real_rr={plan.get('real_rr', 0.0):.3f} minimum={MIN_RR:.2f}",
            "8I_volatility_execution": f"volatility={vol['state']} last_range_atr={vol['last_range_atr']:.3f} atr_drift={vol['atr_drift']:.3f} stop_stability={stop_stability['state']} execution_cost_atr={execution['cost_atr']:.3f}",
            "8J_trade_economics": state,
            "8K_final_risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY",
        },
