...
ARCHITECTURE = "E8_PROFESSIONAL_TRADE_ECONOMICS_RISK_BRAIN_V11"
VERSION = "11.0"
...
def _target_candidates(levels: dict[str, Any], direction: str, entry: float, atr: float, e4: dict[str, Any]) -> list[dict[str, Any]]:
    if direction == "BUY":
        raw = [("RESISTANCE", levels.get("next_resistance"), 92.0, 1), ("LIQUIDITY_EVENT", levels.get("liquidity_event_level"), 80.0, 2), ("PROTECTED_HIGH", levels.get("protected_high"), 90.0, 3), ("STRUCTURE_HIGH_20", levels.get("structure_high_20"), 70.0, 4)]
        directional = [(s, v, q, r) for s, v, q, r in raw if v is not None and v > entry]
    elif direction == "SELL":
        raw = [("SUPPORT", levels.get("next_support"), 92.0, 1), ("LIQUIDITY_EVENT", levels.get("liquidity_event_level"), 80.0, 2), ("PROTECTED_LOW", levels.get("protected_low"), 90.0, 3), ("STRUCTURE_LOW_20", levels.get("structure_low_20"), 70.0, 4)]
        directional = [(s, v, q, r) for s, v, q, r in raw if v is not None and v < entry]
    else:
        return []
...
def _dynamic_target(levels: dict[str, Any], direction: str, entry: float, atr: float, e4: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in candidates:
        if item["credible"]:
            return {**item, "candidate_trace": candidates, "selection_rule": "NEAREST_CREDIBLE_OPPOSING_BARRIER"}
    return {"source": None, "level": None, "distance": 0.0, "distance_atr": 0.0, "quality": 0.0, "credible": False, "rejection": ["NO_CREDIBLE_OPPOSING_BARRIER"], "candidate_trace": candidates, "selection_rule": "NEAREST_CREDIBLE_OPPOSING_BARRIER"}
...
def _space(e5: dict[str, Any], target: dict[str, Any], direction: str) -> dict[str, Any]:
    ...
    target_present = bool(target.get("credible"))
    target_space = _num(target.get("distance_atr")) if target_present else 0.0
    evidence: list[tuple[str, float]] = []
    if e5_present and e5_space > 0: evidence.append(("E5_LOCATION", e5_space))
    if target_present and target_space > 0: evidence.append(("TARGET_BARRIER", target_space))
    if not evidence:
        return {"state": "UNAVAILABLE", "e5_available_space_atr": e5_space, "target_barrier_space_atr": target_space, "effective_available_space_atr": 0.0, "space_consistency_delta_atr": None, "space_evidence_present": e5_present or target_present, "space_conflict": False, "space_source": "NO_USABLE_SPACE_EVIDENCE", "minimum_required_atr": MIN_SPACE_ATR, "space_ok": False}
    ...
    return {"state": state, "e5_available_space_atr": e5_space, "target_barrier_space_atr": target_space, "effective_available_space_atr": effective, "space_consistency_delta_atr": delta, "space_evidence_present": e5_present or target_present, "space_conflict": conflict, "space_source": "MIN(E5_LOCATION,TARGET_BARRIER)" if len(values) == 2 else evidence[0][0], "minimum_required_atr": MIN_SPACE_ATR, "space_ok": state == "USABLE"}
...
def analyze_e8(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """E8 validates risk/economics; E9 remains final trade authority."""
    ...
    target = _dynamic_target(levels, direction, entry, atr, e4) if levels else {"source": None, "level": None, "distance": 0.0, "distance_atr": 0.0, "quality": 0.0, "credible": False, "rejection": ["NO_TARGET_INPUTS"], "candidate_trace": [], "selection_rule": "NEAREST_CREDIBLE_OPPOSING_BARRIER"}

    if direction == "BUY": valid_structural = [("PROTECTED_LOW", levels.get("protected_low")), ("STRUCTURE_LOW_20", levels.get("structure_low_20"))]
    elif direction == "SELL": valid_structural = [("PROTECTED_HIGH", levels.get("protected_high")), ("STRUCTURE_HIGH_20", levels.get("structure_high_20"))]
    else: valid_structural = []
    valid_structural = [(n, v) for n, v in valid_structural if v is not None and ((direction == "BUY" and v < entry) or (direction == "SELL" and v > entry))]
    if valid_structural:
        valid_structural.sort(key=lambda x: abs(entry - x[1]))
        structural_source, structural_level = valid_structural[0]
    else:
        structural_source, structural_level = None, None
    ...
    stop = target_level = None
    liquidity_r = 0.0
    opposing = False

    if data_valid and direction in {"BUY", "SELL"}:
        if direction == "BUY": stop = structural_level - RISK_ATR_BUFFER * atr if structural_level is not None else entry - FALLBACK_STOP_ATR * atr
        else: stop = structural_level + RISK_ATR_BUFFER * atr if structural_level is not None else entry + FALLBACK_STOP_ATR * atr
        ...
        liquidity_r = abs(liquidity_level - entry) / max(risk_distance, 1e-9) if opposing else 0.0
        ...
        plan = {"valid": True, "entry": entry, "direction": direction, "stop_loss": stop, "structural_stop": structural_level, "invalidation_basis": "STRUCTURAL_LEVEL_PLUS_ATR_BUFFER" if structural_level is not None else "ATR_FALLBACK_LOWER_CONFIDENCE", "invalidation_source": structural_source, "take_profit_1": tp1, "take_profit_2": target_level, "target_source": target.get("source"), "target_quality": target.get("quality", 0.0), "target_distance_atr": target.get("distance_atr", 0.0), "target_candidate_trace": target.get("candidate_trace", []), "target_rejection": target.get("rejection", []), "risk_distance": risk_distance, "risk_distance_atr": stop_atr, "reward_distance": reward_distance, "reward_distance_atr": reward_distance / max(atr, 1e-9) if reward_distance else 0.0, "available_space": reward_distance, "available_space_atr": space["effective_available_space_atr"], "e5_available_space_atr": space["e5_available_space_atr"], "target_barrier_space_atr": space["target_barrier_space_atr"], "space_consistency_delta_atr": space["space_consistency_delta_atr"], "space_source": space["space_source"], "space_state": space["state"], "real_rr": real_rr, "rr_tp1": reward_distance * TP1_FRACTION / max(risk_distance, 1e-9) if risk_distance else 0.0, "rr_tp2": real_rr, "min_required_rr": MIN_RR, "asymmetric_payoff": real_rr >= MIN_RR, "stop_distance_atr": stop_atr, "stop_stability": stop_stability["state"], "risk_buffer_atr": RISK_ATR_BUFFER, "structural_breach": structural_breach, "opposing_liquidity": liquidity_level if opposing else None, "opposing_liquidity_r": liquidity_r}
        support.extend([f"entry={entry:.6f}", f"atr={atr:.6f}", f"structural_stop={structural_level:.6f}" if structural_level is not None else "structural_stop=NONE", f"final_stop={stop:.6f}", f"risk_distance={risk_distance:.6f}", f"risk_distance_atr={stop_atr:.3f}", f"target={target_level:.6f}" if target_level is not None else "target=NONE", f"target_source={target.get('source') or 'NONE'}", f"target_quality={target.get('quality', 0.0):.1f}", f"reward_distance={reward_distance:.6f}", f"reward_distance_atr={plan['reward_distance_atr']:.3f}", f"effective_space_atr={space['effective_available_space_atr']:.3f}", f"real_rr={real_rr:.3f}", f"min_rr={MIN_RR:.2f}", f"stop_stability={stop_stability['state']}"])
    else:
    ...
    gate_stop = bool(plan) and MIN_STOP_ATR <= plan.get("stop_distance_atr", 99.0) <= MAX_STOP_ATR and stop_stability["state"] == "STABLE"
    ...
    gate_rr = bool(plan) and real_rr >= MIN_RR
    ...
    gate_economics = gate_target and gate_space and gate_rr and gate_stop and gate_invalidation and gate_execution and not any(x in critical for x in counter) and not missing
    lifecycle = {"01_DATA_INTEGRITY": "PASS" if gate_data else "FAIL", "02_DIRECTION": "PASS" if gate_direction else "FAIL", "03_SETUP_CONFIRMATION": "PASS" if gate_setup and gate_confirmation else "FAIL", "04_INVALIDATION": "PASS" if gate_invalidation else "FAIL", "05_STOP_SURVIVABILITY": "PASS" if gate_stop else "FAIL", "06_TARGET_VALIDATION": "PASS" if gate_target else "FAIL", "07_SPACE_VALIDATION": "PASS" if gate_space else "FAIL", "08_RR_VALIDATION": "PASS" if gate_rr else "FAIL", "09_EXECUTION_VOLATILITY": "PASS" if gate_execution else "FAIL", "10_ECONOMICS": "PASS" if gate_economics else "FAIL", "11_RISK_GATE": "RISK_READY" if gate_economics else "RISK_NOT_READY"}
    ...
    observations = [f"direction={direction}", f"setup={setup}", f"confirmation={confirmation}", f"entry={entry:.6f}", f"atr={atr:.6f}", f"stop={stop:.6f}" if stop is not None else "stop=NONE", f"risk_distance={risk_distance:.6f}", f"risk_distance_atr={plan.get('risk_distance_atr', 0.0):.3f}", f"target={target_level:.6f}" if target_level is not None else "target=NONE", f"target_source={target.get('source') or 'NONE'}", f"target_quality={target.get('quality', 0.0):.1f}", f"reward_distance={reward_distance:.6f}", f"reward_distance_atr={plan.get('reward_distance_atr', 0.0):.3f}", f"effective_space_atr={space.get('effective_available_space_atr', 0.0):.3f}", f"min_space_atr={MIN_SPACE_ATR:.2f}", f"real_rr={real_rr:.3f}", f"min_rr={MIN_RR:.2f}", f"volatility={vol['state']}", f"atr_stability={vol['atr_stability']}", f"stop_stability={stop_stability['state']}", f"execution_cost_atr={execution['cost_atr']:.3f}", f"risk_lifecycle={lifecycle['11_RISK_GATE']}", f"economic_lifecycle={state}"]
    ...
    output = {**base, "state": state, "economic_state": state, "risk_lifecycle": lifecycle["11_RISK_GATE"], "economic_lifecycle": state, "risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY", "direction": direction, "setup": setup, "confirmation": confirmation, "confirmation_trace": confirmation_trace, "trade_plan": plan, "risk_model": {"atr": atr, "atr_period": ATR_PERIOD, "volatility_state": vol["state"], "last_range_atr": vol["last_range_atr"], "expansion_ratio": vol["expansion_ratio"], "atr_stability": vol["atr_stability"], "atr_drift": vol["atr_drift"], "stop_stability": stop_stability, "spread": execution["spread"], "slippage": execution["slippage"], "execution_cost_atr": execution["cost_atr"], "structure_lookback": STRUCTURE_LOOKBACK, "min_stop_atr": MIN_STOP_ATR, "max_stop_atr": MAX_STOP_ATR, "min_space_atr": MIN_SPACE_ATR, "min_rr": MIN_RR, "risk_buffer_atr": RISK_ATR_BUFFER, "max_liquidity_risk_r": MAX_LIQUIDITY_RISK_R, "max_execution_cost_atr": MAX_EXECUTION_COST_ATR}, "structural_evidence": {**levels, "structural_breach": structural_breach, "invalidation_source": structural_source}, "liquidity_evidence": liq, "location_evidence": {"e5_available_space_atr_long": _num(e5.get("available_space_atr_long")), "e5_available_space_atr_short": _num(e5.get("available_space_atr_short")), "effective_available_space_atr": space.get("effective_available_space_atr", 0.0), "target_barrier_space_atr": space.get("target_barrier_space_atr", 0.0), "space_consistency_delta_atr": space.get("space_consistency_delta_atr"), "space_source": space.get("space_source"), "space_state": space.get("state"), "space_ok": space.get("space_ok", False), "space_conflict": space.get("space_conflict", False)}, "dynamic_target": target, "lifecycle": lifecycle, "gate_matrix": {"8A_data_integrity": "PASS" if gate_data else "FAIL", "8B_direction_validation": "PASS" if gate_direction else "FAIL", "8C_setup_confirmation_gate": "PASS" if gate_setup and gate_confirmation else "FAIL", "8D_structural_invalidation": "PASS" if gate_invalidation else "FAIL", "8E_stop_survivability": "PASS" if gate_stop else "FAIL", "8F_available_space": "PASS" if gate_space else "FAIL", "8G_dynamic_target": "PASS" if gate_target else "FAIL", "8H_real_rr": "PASS" if gate_rr else "FAIL", "8I_volatility_execution": "PASS" if gate_execution else "FAIL", "8J_trade_economics": state, "8K_final_risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY"}, "supporting_evidence": support, "counter_evidence": counter, "missing_evidence": missing, "observations": observations, "invalidation": ["closed-candle structural invalidation", "structural stop becomes economically excessive", "stop geometry becomes unstable versus recent ATR regime", "effective available space collapses below minimum", "space evidence materially conflicts", "real RR falls below minimum", "opposing or external liquidity blocks the target path", "volatility makes the stop non-survivable", "execution cost becomes excessive", "entry confirmation is not proven", "target barrier is not credible or is too far for M5 execution"], "professional_reasoning": {"entry_stop_target_chain": f"ENTRY={entry:.6f} -> INVALIDATION={structural_level if structural_level is not None else 'NONE'} -> STOP={stop if stop is not None else 'NONE'} -> TARGET={target_level if target_level is not None else 'NONE'} -> REWARD={reward_distance:.6f} -> RR={real_rr:.3f}", "target_hierarchy": "nearest credible opposing barrier; farther targets cannot manufacture RR", "space_reasoning": f"effective={space.get('effective_available_space_atr', 0.0):.3f} ATR minimum={MIN_SPACE_ATR:.2f} ATR source={space.get('space_source')}", "rr_reasoning": f"reward={reward_distance:.6f} / risk={risk_distance:.6f} = {real_rr:.3f}; required={MIN_RR:.2f}", "stop_survivability": f"stop={plan.get('risk_distance_atr', 0.0):.3f} ATR range={MIN_STOP_ATR:.2f}-{MAX_STOP_ATR:.2f} ATR; stability={stop_stability['state']}", "economic_veto": "PASS" if risk_ready else "VETO: " + "; ".join(counter + missing + ["ECONOMICS_NOT_READY"]), "lifecycle": lifecycle, "8A_data_integrity": "PASS" if gate_data else "FAIL", "8B_direction_validation": "PASS" if gate_direction else "FAIL", "8C_setup_confirmation_gate": "PASS" if gate_setup and gate_confirmation else "FAIL", "8D_structural_invalidation": "Protected structure defines invalidation; ATR is the survival buffer.", "8E_stop_survivability": f"state={stop_stability['state']}; current_stop_atr={stop_stability.get('current_stop_atr', 0.0):.3f}; ratio_to_prior={stop_stability.get('ratio_to_prior')}", "8F_available_space": f"state={space.get('state')}; effective_space_atr={space.get('effective_available_space_atr', 0.0):.3f}; source={space.get('space_source')}; conflict={space.get('space_conflict', False)}", "8G_dynamic_target": f"selected={target.get('level', 'NONE')} source={target.get('source', 'NONE')} quality={target.get('quality', 0.0):.1f}; selection={target.get('selection_rule')}", "8H_real_rr": f"real_rr={real_rr:.3f} minimum={MIN_RR:.2f}", "8I_volatility_execution": f"volatility={vol['state']} last_range_atr={vol['last_range_atr']:.3f} atr_drift={vol['atr_drift']:.3f} execution_cost_atr={execution['cost_atr']:.3f}", "8J_trade_economics": state, "8K_final_risk_gate": "RISK_READY" if risk_ready else "RISK_NOT_READY"}, "decision_path": "E8 validates survivability/economics only; E9 retains final trade authority."}
