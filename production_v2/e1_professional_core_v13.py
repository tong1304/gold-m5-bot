...
def _incomplete(reason: str, valid: int, invalid: int) -> dict[str, Any]:
    return {
        ...
        "architecture": "E1_SINGLE_PROFESSIONAL_BRAIN_V13",
        ...
        "counter_pressure": "NONE",
        "dominant_direction": "NEUTRAL",
        "directional_state": "UNRESOLVED",
        "market_phase": "UNRESOLVED",
        ...
        "professional_reasoning": {
            ...
            "primary_thesis": {"direction": "NEUTRAL", "status": "UNRESOLVED", "supporting_evidence": [], "counter_evidence": []},
        ...
        "e1_contract_version": "PROFESSIONAL_MARKET_STATE_V13",
        "e1_engine_version": "PROFESSIONAL_MARKET_STATE_V13",
...
def analyze_e1_professional_v13(bars: list[dict[str, Any]] | None) -> dict[str, Any]:
    ...
    # Arbitration: structural evidence plus persistent long-horizon evidence
    # are primary. EMA is context and can support, but never overrule them.
    ...
    if counter_direction in DIRECTIONS:
        if sd == counter_direction:
            counter_items.append("COUNTER_TREND_STRUCTURE_PRESENT")
        if pressure == counter_direction:
            counter_items.append("SHORT_HORIZON_COUNTER_PRESSURE")
        if recent_pressure == counter_direction:
            counter_items.append("RECENT_COUNTER_PRESSURE")
        if context_flip:
            counter_items.append("CONTEXT_FLIP_REQUIRES_PERSISTENT_REPRICING")
    ...
    if not counter_items:
        counter_items = ["NO_MATERIAL_COUNTER_EVIDENCE"]

    # Transition is a commitment test, not a signal. A counter-trend observation
    # alone can never commit the state. All independent repricing dimensions must agree.
    ...
    transition_checks = {
        "STRUCTURAL_REPRICING": candidate_persistent_structure,
        "LONG_HORIZON_REPRICING": candidate_long,
        "PRESSURE_REPRICING": candidate_pressure,
        "EMA_REPRICING": candidate_ema,
        "BOS_REPRICING": candidate_bos,
    ...
    if state == "TREND_UP" and compression:
        volatility_state = "CONTRACTING"
    elif state == "TREND_DOWN" and compression:
        volatility_state = "CONTRACTING"
    elif expansion:
    ...
    reasons = []
    if structure_alignment == "COUNTER_TREND":
        reasons.append("COUNTER_TREND_STRUCTURE_CANNOT_AUTO_FLIP_STATE")
    reasons.append(f"DOMINANT_BASIS={basis}")
    reasons.append("DATA_INTEGRITY_VALIDATED")
    if persistent_long:
        reasons.append("LONG_HORIZON_PERSISTENCE_CONFIRMED")
    reasons.append("EMA_AS_CONTEXT_NOT_AUTHORITY")
    if compression:
        reasons.append("VOLATILITY_COMPRESSION_DETECTED")
    if transition_confirmed:
        reasons.append("PERSISTENT_STRUCTURAL_REPRICING_CONFIRMED")
    elif dominant in DIRECTIONS:
        reasons.append("TRANSITION_REQUIRES_PERSISTENT_REPRICING")
    ...
    invalidation = []
    if dominant == "UP":
        invalidation = ["PERSISTENT_DOWN_STRUCTURAL_REPRICING", "LONG_HORIZON_DOWN_PERSISTENCE"]
    elif dominant == "DOWN":
        invalidation = ["PERSISTENT_UP_STRUCTURAL_REPRICING", "LONG_HORIZON_UP_PERSISTENCE"]
    else:
        invalidation = ["ESTABLISH_RELIABLE_DOMINANT_DIRECTION"]
    ...
    reasoning_trace = [
        f"QUESTION -> {QUESTION}",
        f"PRIMARY_STATE -> {state}",
        f"DOMINANT_DIRECTION -> {dominant}",
        f"DOMINANT_BASIS -> {basis}",
        f"STRUCTURE_ALIGNMENT -> {structure_alignment}",
        f"COUNTER_EVIDENCE -> {counter_direction}:{counter_strength:.3f}",
        f"TRANSITION -> {'CONFIRMED' if transition_confirmed else 'NOT_COMMITTED'}",
    ]
    ...
    return {
        ...
        "architecture": "E1_SINGLE_PROFESSIONAL_BRAIN_V13",
        "market_state": state,
        "trend_state": dominant if dominant in DIRECTIONS else "NONE",
        ...
        "counter_pressure": counter_direction if counter_items != ["NO_MATERIAL_COUNTER_EVIDENCE"] else "NONE",
        ...
        "professional_reasoning": {
            ...
            "primary_thesis": {
                ...
                "supporting_evidence": [basis, "LONG_HORIZON_PERSISTENCE" if persistent_long else "LONG_HORIZON_UNRESOLVED", f"STRUCTURE={structure[\"state\"]}"],
                "counter_evidence": counter_items,
            },
            "confidence_model": {
                "support": support,
                "counter_evidence": counter_strength,
                "structure": structure["quality"],
                "persistence": long_persistence,
                "stability": stability,
            },
            "invalidation": {"conditions": invalidation},
            "transition_commitment": transition_checks,
        },
        "e1_contract_version": "PROFESSIONAL_MARKET_STATE_V13",
        "e1_engine_version": "PROFESSIONAL_MARKET_STATE_V13",
        "e1_trade_authority": False,
