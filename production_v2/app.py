...
def _current_opportunity_input(result, candle: str) -> dict:
    engines = {engine.engine_id: engine.output or {} for engine in result.engines}; e6 = engines.get("E6", {}); e7 = engines.get("E7", {}); e8 = engines.get("E8", {}); e9 = engines.get("E9", {}); e4 = engines.get("E4", {}); reconciliation = reconcile_causal_evidence(engines)
    ...
    # E6 owns setup/opportunity state. Use its proof requirements before the
    # broader reconciliation projection so the lifecycle cannot regress to an
    # older generic wait list.
    e6_pending_direction, e6_pending_setup, e6_pending_missing, _ = _e6_pending_thesis(engines)
    pending_direction, pending_setup, pending_missing, reconciliation_wait_for = _pending_upstream_thesis(engines)
    if e6_pending_setup:
        pending_direction, pending_setup, pending_missing = e6_pending_direction, e6_pending_setup, e6_pending_missing
        reconciliation_wait_for = []
    elif not pending_setup:
        pending_direction, pending_setup, pending_missing, reconciliation_wait_for = _pending_upstream_thesis(engines)
    ...
    if real_setup:
        ...
        direction = pending_direction; setup = pending_setup; thesis_status = "CONFIRMED" if reconciliation.get("state") == "THESIS_CONFIRMED_SETUP_NOT_FORMED" else "FORMING"; candidate = True; lifecycle_source = "E6_OPPORTUNITY_WATCH" if e6_pending_setup else "OPPORTUNITY_SCOUT"; wait_for = list(dict.fromkeys(pending_missing or reconciliation_wait_for or ["NEXT_CLOSED_M5_CANDLE"]));
    ...
    return {"candidate":candidate,"direction":direction,"setup":setup,"ready":ready,"invalidated":invalidated,"executed":executed,"thesis_status":thesis_status,"candle":candle,"event_id":event_id,"origin_event_id":e6.get("origin_event_id") or event_id,"lifecycle_source":lifecycle_source,"upstream_evidence":pending_missing,"wait_for":wait_for}
