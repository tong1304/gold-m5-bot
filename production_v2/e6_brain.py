...
def analyze_e6(snapshot:dict[str,Any],upstream:dict[str,EngineResult])->EngineResult:
    ...
    c=[float(x["close"]) for x in bars]; h=[float(x["high"]) for x in bars]; l=[float(x["low"]) for x in bars]
    atr=max(mean(abs(c[i]-c[i-1]) for i in range(max(1,len(c)-14),len(c))),1e-9); price=c[-1]; e20=_ema(c,20); e50=_ema(c,50)
    ...
    state=str(e1.get("market_state","UNCLEAR")).upper(); pressure=str(e1.get("directional_pressure","NONE")).upper(); struct=str(e3.get("finding",e3.get("structure_state",""))).upper(); loc=str(e5.get("finding",e5.get("location_state",""))).upper()
    buy=sum([state=="TREND_UP",pressure=="UP",price>e20>e50,"UP" in struct]); sell=sum([state=="TREND_DOWN",pressure=="DOWN",price<e20<e50,"DOWN" in struct])
    ...
    setup="BREAKOUT" if breakout else "TREND_PULLBACK" if direction in {"BUY","SELL"} and pullback else "IMPULSE_CONTINUATION" if direction in {"BUY","SELL"} and impulse else "NONE"
    supporting=[f"E1_state={state}",f"E1_pressure={pressure}",f"E3_structure={struct}",f"impulse={impulse}",f"breakout={breakout}",f"pullback={pullback}"]
    counter=[]; missing=[]
    ...
    thesis="UNRESOLVED" if direction=="NEUTRAL" else f"{direction}_{setup}" if setup!="NONE" else f"{direction}_NO_MATURE_SETUP"
    # A setup remains a valid developing thesis even when downstream evidence is unfavorable.
    ...
    score=85 if maturity=="MATURE" else 55 if maturity=="DEVELOPING" else 25
    return EngineResult("E6",NAME,gate,score,{**base,"state":maturity,"setup":setup,"direction":direction,"maturity":maturity,"thesis":thesis,"supporting_evidence":supporting,"counter_evidence":counter,"missing_evidence":missing,"invalidation":["directional evidence reverses","setup structure breaks","new closed candle invalidates setup"]},() if gate else tuple(counter+missing or ["SETUP_NOT_MATURE"]))
