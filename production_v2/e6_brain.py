...
def _direction(e1: dict[str, Any], e2: dict[str, Any], e3: dict[str, Any], e4: dict[str, Any]) -> tuple[str, list[str], list[str], str]:
    ...
    explicit = [x for x in explicit if x != "NEUTRAL"]
    ...
    if explicit and all(x == explicit[0] for x in explicit):
        ...
        direction = pressure
    elif auction_direction != "NEUTRAL":
    ...
    source = "AUCTION" if auction_direction == direction else "CONTEXT"
...
def _location(e5: dict[str, Any]) -> tuple[bool, float, float]:
    ...
    if long_space or short_space:
        constrained = constrained or max(long_space, short_space) < MIN_SPACE_ATR
...
def _candidate_scan(bars: list[dict[str, Any]], atr: float, direction: str, e3: dict[str, Any], e4: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    rng = max(h - l, 1e-9)
    pos = (c - l) / rng
    directional_close = pos <= 0.40 if reversal_dir == "SELL" else pos >= 0.60 if reversal_dir == "BUY" else False
    if liquidity_event and reversal_dir == direction:
        ...
        if directional_close:
            ...
            rejected.append("LIQUIDITY_REVERSAL:missing_directional_close")
    ...
    if len(bars) >= BREAKOUT_LOOKBACK + 4:
        ...
        if prior_break and retest and hold:
            found.append({"name": "BREAKOUT_RETEST", "evidence": ["PRIOR_CLOSED_BREAKOUT", "LEVEL_RETEST", "RETEST_HOLD"], "strength": 88})
        else:
            missing = []
    ...
    if bodies[idx] >= 0.8 * atr and impulse_dir == direction and follow_dir == direction and dclose:
        found.append({"name": "IMPULSE_CONTINUATION", "evidence": ["DIRECTIONAL_PRIOR_IMPULSE", "DIRECTIONAL_FOLLOW_THROUGH"], "strength": 76})
    else:
