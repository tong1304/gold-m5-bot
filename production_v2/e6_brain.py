...
def _payload(upstream: dict[str, Any], key: str) -> dict[str, Any]: return _out(upstream.get(key))
...
def _e2_unresolved(e2: dict[str, Any]) -> bool:
    ...
    return finding in {"UNRESOLVED","UNPROVEN","AMBIGUOUS","WAIT","EMERGING","PENDING","DEVELOPING"} or state in {"UNRESOLVED","UNPROVEN","AMBIGUOUS","WAIT","EMERGING","PENDING","DEVELOPING"} or maturity in {"UNPROVEN","EMERGING","DEVELOPING"} or "OPPORTUNITY IS DEVELOPING" in finding or "OPPORTUNITY IS EMERGING" in finding or "OPPORTUNITY IS UNPROVEN" in finding
...
def _e5_space(e5: dict[str, Any], direction: str) -> float:
    ...
    try:
        value=float(e5.get(key,0.0) or 0.0); return value if value==value else 0.0
...
def _causal_opportunity(upstream: dict[str, EngineResult]) -> dict[str, Any] | None:
    ...
    e1d=_direction_value(e1.get("directional_pressure",e1.get("pressure"))); e2d=_e2_direction(e2); internal=_direction_value(e3.get("internal_state")); external=_direction_value(e3.get("external_state",e3.get("direction"))); e4d=_e4_direction(e4); event=_text(e4.get("event",e4.get("finding"))); auction_state=_text(e4.get("auction_state",e4.get("state"))); unresolved=_e2_unresolved(e2)
    if _e3_invalidated(e3): return None
    if not any(token in event for token in ("ACCEPTANCE","REJECTION","SWEEP","FAILED_BREAK","BREAK","RECLAIM","LIQUIDITY_INTERACTION")): return None
    if e1d in {"BUY","SELL"}: direction=e1d
    elif external in {"BUY","SELL"}: direction=external
    elif e2d in {"BUY","SELL"}: direction=e2d
    else: direction=e4d
    counter=[]
    ...
    internal_status="ALIGNED" if internal==direction else "UNRESOLVED"; missing_internal=[]
    if internal=="MIXED": internal_status="UNRESOLVED_COUNTERFLOW"; counter.append("E3_INTERNAL_COUNTER_EVIDENCE"); missing_internal.extend(["E3_INTERNAL_EVIDENCE_UNRESOLVED","E3_INTERNAL_STRUCTURE_ALIGNMENT"])
    ...
    if e1d==direction: support.insert(0,"E1_DIRECTIONAL_CORE")
    if external==direction: support.append("E3_EXTERNAL_STRUCTURE_SUPPORT")
    if internal==direction: support.append("E3_INTERNAL_STRUCTURE_SUPPORT")
    if e2d==direction: support.append("E2_DIRECTIONAL_ANCHOR")
    if favorable: support.append("E5_LOCATION_VALUE_SUPPORT")
    return {"direction":direction,"family":family,"space":round(space,4),"support":list(dict.fromkeys(support)),"missing":list(dict.fromkeys(missing)),"counter":list(dict.fromkeys(counter)),"event":event,"event_id":str(e4.get("event_id") or e4.get("event_candle_id") or ""),"internal_status":internal_status,"terminal":terminal,"e2_confirmed":not unresolved and e2d==direction}
...
def _invalidated(original: EngineResult,reason:str)->EngineResult:
    out=dict(original.output or {}); out.update({"architecture":ARCHITECTURE,"version":VERSION,"state":"INVALIDATED","setup_state":"INVALIDATED","opportunity_stage":"INVALIDATED","setup":"NO_SETUP","setup_family":"","candidate_type":"NONE","direction":"NEUTRAL","direction_thesis":"NEUTRAL","thesis_direction":"NEUTRAL","thesis_status":"INVALIDATED","setup_exists":False,"watch_only":False,"trade_ready":False,"gate_passed":False,"finding":f"E6 invalidated: {reason}.","thesis":"Prior E6 state cannot survive invalid upstream structure.","supporting_evidence":[],"counter_evidence":[reason],"hard_conflicts":[reason],"missing_proof":["NEW_VALID_STRUCTURE"],"next_required_event":"NEW_VALID_STRUCTURE","wait_for":"NEW_VALID_STRUCTURE","candidate_identity":"","opportunity_id":"","event_id":"","available_space_atr":0.0,"reason_codes":[reason],"reasons":[reason],"e6_causal_gate":"INVALIDATED","e6_thesis_proven":False,"invalidated":True,"upstream_evidence_lost":True,"causal_evidence_lost":True,"lifecycle_state":"INVALIDATED","invalidation_reason":reason})
    return EngineResult("E6","Setup Brain",False,0.0,out,(reason,))


def _absent(original: EngineResult)->EngineResult:
    out=dict(original.output or {}); out.update({"architecture":ARCHITECTURE,"version":VERSION,"state":"NO_SETUP","setup_state":"NO_SETUP","opportunity_stage":"ABSENT","setup":"NO_SETUP","setup_family":"","candidate_type":"NONE","direction":"NEUTRAL","direction_thesis":"NEUTRAL","thesis_direction":"NEUTRAL","thesis_status":"ABSENT","setup_exists":False,"watch_only":False,"trade_ready":False,"gate_passed":False,"finding":"No surviving causal opportunity thesis from E1-E5; legacy setup output is suppressed.","thesis":"E6 cannot create an independent setup without surviving upstream causal evidence.","supporting_evidence":[],"counter_evidence":[],"hard_conflicts":[],"missing_proof":["E1_E2_E3_E4_E5_CAUSAL_OPPORTUNITY"],"next_required_event":"NEW_CAUSAL_OPPORTUNITY_FROM_E1_E5","wait_for":"NEW_CAUSAL_OPPORTUNITY_FROM_E1_E5","candidate_identity":"","opportunity_id":"","event_id":"","available_space_atr":0.0,"reason_codes":["NO_CAUSAL_OPPORTUNITY"],"reasons":["NO_CAUSAL_OPPORTUNITY"],"e6_causal_gate":"ABSENT","e6_thesis_proven":False,"invalidated":False,"upstream_evidence_lost":False,"causal_evidence_lost":False,"lifecycle_state":"NO_SETUP"})
...
def analyze_e6(market_data: dict[str,Any],upstream: dict[str,EngineResult])->EngineResult:
    ...
    if _e3_invalidated(e3): return _invalidated(base,"E3_STRUCTURE_INVALIDATED")
    candidate=_causal_opportunity(upstream)
    if candidate is None: return _absent(base)
    thesis_ready=candidate["terminal"] and candidate["e2_confirmed"] and candidate["internal_status"]=="ALIGNED" and candidate["space"]>=MIN_SPACE_ATR and not candidate["counter"]
