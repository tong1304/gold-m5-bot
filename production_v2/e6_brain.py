...
ARCHITECTURE = "E6_PROFESSIONAL_SETUP_FORMATION_BRAIN_V17"
VERSION = "17.0"
MIN_BARS = 60
ATR_PERIOD = 14
MIN_SPACE_ATR = 0.75
...
SETUP_FAMILIES = (
    "LIQUIDITY_REVERSAL", "BREAKOUT_RETEST", "TREND_PULLBACK", "BREAKOUT", "IMPULSE_CONTINUATION",
)
...
def _direction_evidence(e1: dict[str, Any], e2: dict[str, Any], e3: dict[str, Any], e4: dict[str, Any]):
    ...
    if len(unique) > 1:
        ...
        direction, source = next(iter(unique)), "E1_E3_E4_CONVERGENCE" if len(votes) >= 2 else "INDEPENDENT_EVIDENCE"
    elif e2_dir != "NEUTRAL":
...
def analyze_e6(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """Form and challenge setup hypotheses; blockers describe maturity, not whether a setup can be diagnosed."""
    ...
    # Hard veto = direction itself cannot be trusted. Maturity blockers must NOT erase a
    # diagnosable setup: a professional trader can say "this setup is forming but not proven".
    hard_direction_veto = direction == "NEUTRAL" or "DIRECTIONAL_EVIDENCE_CONFLICT" in formation_counter or "E2_DIRECTION_DISAGREEMENT" in formation_counter
    ...
    if not hard_direction_veto:
        if auction_dir == direction and any(x in event for x in ("SWEEP_REJECTION", "FAILED_BREAK_RECLAIM")):
            ...
            base = 92.0 if stage == "MATURE" else 78.0 if stage == "VALIDATING" else 62.0
            candidates.append(_candidate("LIQUIDITY_REVERSAL", direction, evidence, missing, base, stage, ["closed_candle_loses_reclaim", "contrary_closed_candle_acceptance", "protected_level_break"], {"causal_event": 30.0, "response": 20.0 if response == direction else 0.0, "terminal": 18.0 if terminal else 0.0}))
        else:
        ...
        if confirmed_break and e3_external == direction:
            ...
            missing = [] if (retest or acceptance) else ["controlled_retest_or_closed_candle_acceptance_after_break"]
            ...
            name = "BREAKOUT_RETEST" if retest else "BREAKOUT"
        ...
        if direction == "NEUTRAL":
            ...
            rejected.append("DIRECTIONAL_THESIS: independent directional evidence conflicts; setup family selection suspended")
    ...
    if len(candidates) >= 2 and candidates[0]["strength"] - candidates[1]["strength"] < 8.0:
        formation_counter.append("SETUP_HYPOTHESES_TOO_CLOSE")
        formation_counter = list(dict.fromkeys(formation_counter))
        candidates[0]["missing"].append("clear_hypothesis_separation")
    ...
    blockers: list[str] = []
    if space < MIN_SPACE_ATR:
        blockers.append(f"INSUFFICIENT_{direction}_STRUCTURAL_SPACE")
    if location_constrained:
        blockers.append("LOCATION_CONSTRAINT")
    if extension_risk:
        blockers.append("LOCATION_EXTENSION_RISK")
    blockers.extend(formation_counter)
    blockers = list(dict.fromkeys(blockers))
    missing = list(selected["missing"])
    next_required = list(missing)
    if space < MIN_SPACE_ATR:
        next_required.append(f"wait for at least {MIN_SPACE_ATR:.2f} ATR {direction.lower()}-side structural space")
