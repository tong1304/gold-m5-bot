...
ARCHITECTURE = "E6_PROFESSIONAL_SETUP_FORMATION_BRAIN_V21"
VERSION = "21.0"
MIN_BARS = 60
ATR_PERIOD = 14
MIN_SPACE_ATR = 0.75
...
def _atr(bars: list[dict[str, Any]]) -> float:
    ...
    for i, candle in enumerate(sample):
        ...
        if i == 0:
            ...
            continue
        prev_close = _num(sample[i - 1].get("close"))
        ranges.append(max(high - low, abs(high - prev_close), abs(low - prev_close)))
    return mean(ranges[-ATR_PERIOD:]) if ranges else 0.0


def _auction(e4: dict[str, Any]) -> tuple[str, str, bool, bool, int, str]:
    ...
    if "HIGH_SWEEP_REJECTION" in event or "HIGH_FAILED_BREAK_RECLAIM" in event:
        ...
        direction = "SELL"
    return event, state, terminal, pending, age, direction


def _directional_thesis(e1: dict[str, Any], e2: dict[str, Any], e3: dict[str, Any], e4: dict[str, Any]) -> tuple[str, list[str], list[str], str]:
    sources = {
    ...
    _, _, _, _, _, auction_direction = _auction(e4)
    sources["E4_AUCTION"] = auction_direction
...
def analyze_e6(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """Reason about setup formation only; never manufacture an execution decision."""
    bars = list(snapshot.get("bars") or [])
    if len(bars) < MIN_BARS:
        return _result(
            state="NO_SETUP", setup="NONE", direction="NEUTRAL", stage="SEARCHING", maturity="UNRESOLVED",
            thesis="Setup reasoning is deferred until sufficient closed-candle evidence exists.",
            quality=0, confidence=100, exists=False, ready=False, supporting=[],
            counter=[f"CLOSED_CANDLES_BELOW_MINIMUM={MIN_BARS}"], missing=["sufficient_closed_candle_data"],
            next_required=[f"wait for at least {MIN_BARS} valid closed candles"],
            invalidation=["insufficient_history"], candidates=[], rejected=list(SETUP_FAMILIES),
            trace={"summary": "insufficient closed-candle history", "decision": "DEFER"},
            ledger=[_evidence("DATA", f"closed_candles={len(bars)}", "CONSTRAINT", "HIGH")],
        )
    ...
    try:
        ...
        return _result(
            state="NO_SETUP", setup="NONE", direction="NEUTRAL", stage="SEARCHING", maturity="UNRESOLVED",
            thesis="Setup reasoning is deferred because closed-candle OHLC data is invalid.",
            quality=0, confidence=100, exists=False, ready=False, supporting=[],
            counter=["INVALID_MARKET_DATA"], missing=["valid_closed_candle_ohlc"],
            next_required=["provide valid closed-candle OHLC values"], invalidation=["invalid_market_data"],
            candidates=[], rejected=list(SETUP_FAMILIES),
            trace={"summary": "invalid closed-candle data", "decision": "DEFER"},
            ledger=[_evidence("DATA", "closed-candle OHLC validation failed", "CONSTRAINT", "HIGH")],
        )
    ...
    direction, direction_support, direction_counter, direction_source = _directional_thesis(e1, e2, e3, e4)
    event, auction_state, terminal, pending, age, auction_direction = _auction(e4)
    ...
    if direction != "NEUTRAL":
        ledger.extend([
            ...
            _evidence("E4", f"event={event or 'NONE'}; direction={auction_direction}; state={auction_state or 'UNKNOWN'}"),
    ...
    if pending:
        ...
        ledger.append(_evidence("E4", "auction evidence is pending and is not terminal confirmation", "COUNTER", "HIGH"))
    ...
    if not hard_veto and auction_direction == direction and "ACCEPTANCE" in event:
        acceptance_context = any(x in value_response for x in ("ACCEPTED_ABOVE_VALUE", "ACCEPTANCE_ABOVE_VALUE", "ACCEPTED_BELOW_VALUE", "ACCEPTANCE_BELOW_VALUE", "EQUILIBRIUM"))
        strength = 64 + (8 if acceptance_context else 0) + (8 if terminal else 0) - (6 if mixed else 0) - (6 if pending else 0)
        missing = []
        if pending:
            missing += ["terminal_auction_confirmation", "closed_candle_follow_through"]
        if mixed:
            missing.append("structural_direction_confirmation")
        if space < MIN_SPACE_ATR:
            missing.append("adequate_structural_space")
        if any(x in opportunity for x in ("UNRESOLVED", "UNPROVEN")):
            missing.append("opportunity_acceptance_follow_through")
        candidates.append({
            "name": "AUCTION_ACCEPTANCE_CONTINUATION", "direction": direction,
            "formation_stage": "VALIDATING", "strength": max(0, min(100, strength)),
            "supporting_evidence": ["directional_auction_event", "value_response_context" if acceptance_context else "auction_location_context"],
            "counter_evidence": list(counter), "missing_evidence": _dedupe(missing),
            "next_required_evidence": _dedupe([
                f"closed-candle {direction} follow-through after auction acceptance",
                "auction transitions from PENDING to terminal confirmation" if pending else "continued acceptance without failure",
                "structure resolves into directional confirmation" if mixed else "maintain structural integrity",
            ]),
            "invalidation": ["acceptance_failure", "reclaim_through_accepted_auction_level"],
        })

    if not hard_veto and auction_direction == direction and any(x in event for x in ("SWEEP_REJECTION", "FAILED_BREAK_RECLAIM")):
        missing = ["terminal_auction_confirmation"] if not terminal else []
        if mixed:
            missing.append("structural_direction_confirmation")
        candidates.append({
            "name": "LIQUIDITY_REVERSAL", "direction": direction,
            "formation_stage": "VALIDATING", "strength": max(0, 66 + (8 if terminal else 0) - (8 if mixed else 0)),
            "supporting_evidence": ["liquidity_event_with_directional_response"],
            "counter_evidence": list(counter), "missing_evidence": _dedupe(missing),
            "next_required_evidence": ["closed-candle rejection holds", "follow-through in reversal direction"],
            "invalidation": ["auction_response_failure", "loss_of_protected_structure"],
        })
    ...
    if not hard_veto and direction == trend and not mixed:
        if "BREAK" in bos and "NO_BREAK" not in bos:
            ...
            candidates.append({
                "name": name, "direction": direction, "formation_stage": "VALIDATING",
                "strength": 58 if terminal else 50,
                "supporting_evidence": ["trend_direction_aligned", f"structure_event={bos}"],
                "counter_evidence": list(counter),
                "missing_evidence": ["closed_candle_follow_through", "acceptance_after_break"],
                "next_required_evidence": ["closed-candle acceptance beyond broken level", "follow-through without immediate reclaim"],
                "invalidation": ["breakout_failure", "reclaim_inside_prior_structure"],
            })
        elif "HL" in sequence or "LH" in sequence:
            candidates.append({
                "name": "TREND_PULLBACK", "direction": direction, "formation_stage": "FORMING", "strength": 42,
                "supporting_evidence": ["directional_structure_present_but_trigger_not_proven"],
                "counter_evidence": list(counter),
                "missing_evidence": ["impulse_then_pullback_sequence", "continuation_trigger"],
                "next_required_evidence": ["confirmed pullback into meaningful structure", "closed-candle continuation trigger"],
                "invalidation": ["loss_of_protected_structure"],
            })

    for c in candidates:
        c["counter_evidence"] = _dedupe(c.get("counter_evidence", []) + counter)
        c["supporting_evidence"] = _dedupe(c.get("supporting_evidence", []))
        c["missing_evidence"] = _dedupe(c.get("missing_evidence", []))
        c["next_required_evidence"] = _dedupe(c.get("next_required_evidence", []))
        c["invalidation"] = _dedupe(c.get("invalidation", []))
    ...
    if not candidates:
        ...
        missing = _dedupe(missing)
        next_required = _dedupe(next_required)
        if len(candidates) > 1 and candidates[0].get("strength", 0) - candidates[1].get("strength", 0) < 8:
            setup = "AMBIGUOUS"
            stage = "FORMING"
            ...
            quality = max(20, best.get("strength", 0) - 10)
            confidence = 70
            rejected = [c["name"] for c in candidates]
        else:
            terminal_for_family = terminal and "ACCEPTANCE" in event
            blockers = set(missing) & {"terminal_auction_confirmation", "closed_candle_follow_through", "structural_direction_confirmation", "opportunity_acceptance_follow_through", "adequate_structural_space", "acceptance_after_break"}
            mature = terminal_for_family and not blockers and not mixed and not hard_veto
            stage = "MATURE" if mature else best.get("formation_stage", "FORMING")
            if stage == "FORMING" and best.get("strength", 0) >= 50:
                stage = "VALIDATING"
            thesis = f"{direction} {setup} is {stage.lower()}: evidence supports a conditional formation thesis, while missing evidence remains explicit."
            quality = best.get("strength", 0)
            confidence = max(45, min(95, 56 + 7 * len(best.get("supporting_evidence", [])) - 5 * len(missing)))

    if direction != "NEUTRAL" and space < MIN_SPACE_ATR and "STRUCTURAL_SPACE_CONSTRAINED" not in counter:
        counter.append("STRUCTURAL_SPACE_CONSTRAINED")
        counter = _dedupe(counter)

    maturity = "MATURE" if stage == "MATURE" else "VALIDATING" if stage == "VALIDATING" else "FORMING"
    ready = (
        exists and setup not in {"NONE", "AMBIGUOUS"} and stage == "MATURE" and direction != "NEUTRAL"
        and space >= MIN_SPACE_ATR and not pending
        and not any(x in opportunity for x in ("UNRESOLVED", "UNPROVEN")) and not mixed
    )
    ...
    selected = next((c for c in candidates if c.get("name") == setup), None)
    if selected:
    ...
    trace = {
        ...
        "auction": {"event": event, "direction": auction_direction, "state": auction_state, "terminal": terminal, "pending": pending, "age_bars": age},
        ...
        "opportunity": {"finding": opportunity or "UNKNOWN", "resolved": not any(x in opportunity for x in ("UNRESOLVED", "UNPROVEN"))},
        "hypothesis_count": len(candidates),
        "selection_rule": "causal setup evidence -> direction -> supporting/counter evidence -> missing evidence -> lifecycle -> invalidation",
        "professional_rule": "Preserve conditional hypotheses, do not convert pending evidence into confirmation, do not manufacture certainty, and never usurp E9 trade authority.",
    }

    return _result(
        state=stage, setup=setup, direction=direction, stage=stage, maturity=maturity, thesis=thesis,
        quality=quality, confidence=confidence, exists=exists, ready=ready,
        supporting=direction_support + (selected.get("supporting_evidence", []) if selected else []),
        counter=counter, missing=missing, next_required=next_required, invalidation=invalidation,
        candidates=candidates, rejected=rejected, trace=trace, ledger=ledger,
    )
