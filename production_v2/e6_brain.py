...
NAME="Setup Brain"; QUESTION="What setup is forming, in what direction, and at what stage?"; ARCHITECTURE="E6_PROFESSIONAL_SETUP_FORMATION_BRAIN_V32"; VERSION="32.0"
...
E2_UNRESOLVED=("UNRESOLVED","UNPROVEN","AMBIGUOUS","WAIT")
...
def _auction(e):
    ...
    terminal=st in {"CONFIRMED","TERMINALLY_CONFIRMED","ACCEPTED","REJECTED","RECLAIMED"} or "TERMINAL" in st
    d="NEUTRAL"
...
def _structure_is_invalidated(e):
    lifecycle=_text(e.get("lifecycle")); invalidation=_text(e.get("invalidation"))
    ...
    explicit_invalidation=invalidation in {"ACTIVE_INVALIDATION","STRUCTURE_INVALIDATED","BULLISH_STRUCTURE_INVALIDATED","BEARISH_STRUCTURE_INVALIDATED"}
    return bool(active_flag or explicit_lifecycle or explicit_invalidation)
def _e2_unresolved(e2):
    finding=_text(e2.get("finding",e2.get("state"))); state=_text(e2.get("opportunity_state",e2.get("opportunity_decision")))
    maturity=_text(e2.get("opportunity_maturity")); regime=_text(e2.get("regime"))
    return finding in E2_UNRESOLVED or state in E2_UNRESOLVED or maturity in {"UNPROVEN","EMERGING"} or regime=="UNRESOLVED"
def _direction(e1,e2,e3,e4):
    ...
    if p!="NEUTRAL" and i!="NEUTRAL" and p==i:d,src=p,"E1_E3_DIRECTIONAL_CORE"
    elif i==x and i!="NEUTRAL":d,src=i,"E3_STRUCTURE_CONVERGENCE"
    elif p!="NEUTRAL" and a["direction"]==p:d,src=p,"E1_E4_DIRECTIONAL_CORE"
    elif a["terminal"] and a["direction"]!="NEUTRAL":d,src=a["direction"],"E4_TERMINAL_AUCTION"
    elif p!="NEUTRAL":d,src=p,"E1_CONTEXT_ONLY"
    elif i!="NEUTRAL":d,src=i,"E3_STRUCTURE_ONLY"
    elif a["direction"]!="NEUTRAL":d,src=a["direction"],"E4_PENDING_HYPOTHESIS"
    else:d,src="NEUTRAL","NO_DIRECTIONAL_THESIS"
    if a["pending"]:c.append("E4_PENDING_DIRECTION_NOT_AUTHORITATIVE")
    e2f=_text(e2.get("finding",e2.get("state"))); e2d=_norm(e2.get("direction",e2.get("opportunity_direction")))
    if e2d!="NEUTRAL" and not any(z in e2f for z in E2_UNRESOLVED):
...
def _candidates(d,a,e1,e3,e5):
    ...
    event_direction=a["direction"]; event_can_drive_setup=event_direction in {"NEUTRAL",d}
...
def analyze_e6(snapshot:dict[str,Any],upstream:dict[str,EngineResult])->EngineResult:
    ...
    a=_auction(e4);d,ds,dc,src=_direction(e1,e2,e3,e4);opp=_text(e2.get("finding",e2.get("state"))) or "UNRESOLVED";e2_unresolved=_e2_unresolved(e2);struct,internal,external=_structure(e3);space=_num(e5.get("available_space_atr_long") if d=="BUY" else e5.get("available_space_atr_short"));location=bool(e5)
    ...
    if e2_unresolved:contradictions+=["E2_OPPORTUNITY_UNRESOLVED"]
    if "SWEEP_RECLAIM" in _text(e3.get("finding")) and a["pending"]:contradictions+=["STRUCTURAL_RECLAIM_WITHOUT_AUCTION_FINALITY"]
    ...
    for c in cands:
        ...
        if e2_unresolved:q-=18;con.append("E2_OPPORTUNITY_UNRESOLVED");miss.append("E2_opportunity_acceptance_follow_through")
        ...
        if c["name"] in {"LIQUIDITY_REVERSAL","AUCTION_ACCEPTANCE_CONTINUATION"} and not a["terminal"]:
            q-=12;con.append("AUCTION_CONFIRMATION_PENDING");miss.append("terminal_auction_confirmation")
        ...
        proof={"context":causal["context"],"event":causal["event"],"response":causal["response"],"structure":causal["structure"],"direction":c["direction"]==d and d!="NEUTRAL","location":location,"space":space>=MIN_SPACE_ATR,"freshness":a["age_bars"]<=MAX_EVENT_AGE_BARS,"opportunity":not e2_unresolved,"auction_finality":a["terminal"] if c["name"] in {"LIQUIDITY_REVERSAL","AUCTION_ACCEPTANCE_CONTINUATION"} else True}
    ...
    hard_unresolved=e2_unresolved or (setup in {"LIQUIDITY_REVERSAL","AUCTION_ACCEPTANCE_CONTINUATION"} and not a["terminal"])
    ...
    elif all(proof.values()) and causal_count==4 and sel["anti_overfit_pass"] and not contradictions and not hard_unresolved:stage=mat="MATURE";q=max(82,sel["causal_score"]);conf=min(96,80+sel["causal_score"]*.14);thesis=f"{d} {setup} is mature: context, event, response and structure form a coherent causal chain."
    elif proof["direction"] and proof["event"] and proof["response"] and sel["anti_overfit_pass"] and not hard_unresolved:stage=mat="VALIDATING";q=sel["causal_score"];conf=72;thesis=f"{d} {setup} is validating: the thesis is alive, but proof gates remain incomplete."
    else:stage=mat="FORMING";q=sel["causal_score"];conf=58;thesis=f"{d} {setup} is a candidate hypothesis only: upstream opportunity/auction proof is incomplete."
    ...
    if e2_unresolved:next_req.append("E2 opportunity acceptance/follow-through before setup can become trade-relevant")
    if not proof.get("auction_finality",True):next_req.append("terminal auction confirmation before liquidity/acceptance setup can validate")
    ...
    ledger=[_evidence("E1",f"observation={_text(e1.get('finding')) or 'NONE'}","OBSERVATION","HIGH"),_evidence("E1",f"interpretation=direction={_norm(e1.get('directional_pressure',e1.get('pressure')))}","INTERPRETATION","MEDIUM"),_evidence("E2",f"observation=opportunity={opp}; unresolved={e2_unresolved}","OBSERVATION","HIGH"),_evidence("E3",f"observation=structure={struct}","STRUCTURE","HIGH"),_evidence("E4",f"observation=event={a['event'] or 'NONE'}","EVENT","HIGH"),_evidence("E4",f"interpretation=auction={a['direction']},state={a['state'] or 'NONE'},terminal={a['terminal']}","INTERPRETATION","HIGH"),_evidence("E5",f"observation=space_atr={space:.4f}","CONSTRAINT","HIGH"),_evidence("E6",f"counter_evidence={','.join(_dedupe(con)) or 'NONE'}","COUNTER","HIGH")]
    trace={"summary":f"E1->E2->E3->E4->E5->conflict_engine->hypothesis_competition->lifecycle={stage}","decision":"DESCRIBE_SETUP_ONLY","selected_hypothesis":setup,"candidate_identity":identity,"candidate_identity_basis":identity_basis,"direction_source":src,"causal_minimum":causal,"causal_minimum_rule":"context + event + response + structure must align for maturity","authority_gates":{"e2_opportunity_required":True,"e4_terminal_auction_required_for_reversal_and_acceptance":True,"e5_space_is_non_overrideable":True},"anti_overfitting":{"pass":sel["anti_overfit_pass"],"rule":"event alone cannot create a mature setup"},"hypothesis_competition":{"primary":setup,"ranked":[{"name":x["name"],"direction":x["direction"],"causal_score":x["causal_score"],"proof_gates":x["proof_gates"],"rejected":x is not sel} for x in scored]},"contradiction_engine":{"direction":_dedupe([x for x in contradictions if "DIRECTION" in x]),"structure":_dedupe([x for x in contradictions if "STRUCTURE" in x]),"auction":_dedupe([x for x in contradictions if "AUCTION" in x]),"opportunity":_dedupe([x for x in contradictions if "OPPORTUNITY" in x]),"location":_dedupe([x for x in contradictions if "LOCATION" in x]),"space":_dedupe([x for x in contradictions if "SPACE" in x])},"thesis_status":"INVALIDATED" if explicit else ("EXPIRED" if stale else ("CANDIDATE_ONLY" if hard_unresolved else "ALIVE")),"lifecycle_rule":"contradiction weakens; explicit invalidating evidence kills; stale event expires; unresolved upstream permission keeps setup candidate-only","evidence_integrity":{"status":"PASS" if all((e1,e2,e3,e4,e5)) else "PARTIAL","upstream_is_source_of_truth":True}}
