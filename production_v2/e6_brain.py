...
def _causal_opportunity(upstream: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if core == "NEUTRAL": core = e2_direction
    if core == "NEUTRAL": return None

    primary_anchor = e1_direction != "NEUTRAL" or e2_direction != "NEUTRAL"
    single_anchor_exception = e1_direction != "NEUTRAL" and external != "NEUTRAL" and e1_direction != external and unresolved and e4_direction == external
    if not primary_anchor and not single_anchor_exception: return None
    ...
    if internal == core:
        ...
        internal_status = "COUNTERFLOW"; counter_evidence.append("E3_INTERNAL_COUNTER_EVIDENCE"); missing_internal_proof.append("E3_INTERNAL_STRUCTURE_ALIGNMENT")
    elif internal == "MIXED":
        internal_status = "UNRESOLVED_COUNTERFLOW"; counter_evidence.append("E3_INTERNAL_COUNTER_EVIDENCE"); missing_internal_proof.extend(["E3_INTERNAL_EVIDENCE_UNRESOLVED", "E3_INTERNAL_STRUCTURE_ALIGNMENT"])
    else:
        internal_status = "UNRESOLVED"; counter_evidence.append("E3_INTERNAL_EVIDENCE_UNRESOLVED"); missing_internal_proof.append("E3_INTERNAL_STRUCTURE_ALIGNMENT")

    if e2_direction != "NEUTRAL" and e2_direction != core: return None
    if e4_direction not in {"NEUTRAL", core}: return None
    ...
    if not directional_event: return None

    space = _e5_space(e5, core)
    value = _text(e5.get("value_state")); location = _text(e5.get("structural_location"))
...
def analyze_e6(market_data: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    legacy=_legacy_analyze_e6(market_data,upstream); opportunity=_causal_opportunity(upstream)
    if opportunity is None: return _no_surviving_causal_thesis(legacy)
    current=_out(legacy); legacy_direction=_direction(current.get("direction")); legacy_setup=_text(current.get("setup")); legacy_has_setup=_text(current.get("state")) not in {"ABSENT","NO_SETUP"} and legacy_setup not in {"","NONE","NO_SETUP","UNKNOWN"}
    if legacy_has_setup and legacy_direction==opportunity["direction"]: return legacy
