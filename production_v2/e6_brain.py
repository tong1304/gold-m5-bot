...
ARCHITECTURE = "E6_PROFESSIONAL_SETUP_FORMATION_BRAIN_V15"
VERSION = "15.0"
MIN_BARS = 60
ATR_PERIOD = 14
MIN_SPACE_ATR = 0.75
...
def _direction_evidence(
    ...
    """Resolve direction conservatively; no single upstream brain may override conflict."""
    ...
    raw_votes = {"E1_PRESSURE": pressure, "E3_EXTERNAL": external, "E4_AUCTION": auction}
    votes = [x for x in raw_votes.values() if x != "NEUTRAL"]
    unique = set(votes)
    supporting = [f"{k}={v}" for k, v in raw_votes.items() if v != "NEUTRAL"]
    if e2_dir != "NEUTRAL":
    ...
    # E2 is corroboration, not an override. A professional formation reasoner refuses
    # promotion when independent structural/auction evidence conflicts.
    if len(unique) == 1:
        direction = next(iter(unique))
        source = "E1_E3_E4_CONVERGENCE" if len(votes) >= 2 else "INDEPENDENT_EVIDENCE"
    elif e2_dir != "NEUTRAL" and not unique:
        direction = e2_dir
        source = "E2_CORROBORATION_ONLY"
    else:
        direction = "NEUTRAL"
        source = "INDEPENDENT_EVIDENCE_CONFLICT" if unique else "INSUFFICIENT_CONVERGENCE"

    if e2_dir != "NEUTRAL" and direction != "NEUTRAL" and e2_dir != direction:
...
def _candidate(name: str, direction: str, evidence: list[str], missing: list[str], strength: float, stage: str, invalidation: list[str]) -> dict[str, Any]:
    return {
        "name": name,
        "direction": direction,
...
def _result(*, state: str, setup: str, direction: str, stage: str, maturity: str, thesis: str,
    ...
    output = {
        ...
        "setup": setup, "setup_family": setup, "direction": direction,
        "stage": stage, "formation_stage": stage, "lifecycle": stage,
        "maturity": maturity, "thesis": thesis, "setup_exists": setup_exists,
        "trade_ready": trade_ready, "trade_readiness": "READY" if trade_ready else "NOT_READY",
        "setup_quality": round(quality, 2), "confidence": round(confidence, 2),
        "candidate_setups": [x["name"] for x in candidates], "candidate_states": candidates,
        "rejected_setups": rejected, "supporting_evidence": list(dict.fromkeys(supporting)),
        "counter_evidence": counter, "missing_evidence": missing,
        "next_required_evidence": next_required, "invalidation": list(dict.fromkeys(invalidation)),
        "observations": [
            f"candidate_setups={','.join(x['name'] for x in candidates) if candidates else 'NONE'}",
            f"selected_setup={setup}", f"selected_direction={direction}", f"selected_stage={stage}",
            f"setup_exists={setup_exists}", f"trade_ready={trade_ready}",
            ...
            f"lifecycle={stage}", f"maturity={maturity}",
        ],
    }
    if trace:
...
def analyze_e6(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    """Infer a causal setup formation thesis from locked E1-E5 evidence only."""
    bars = list(snapshot.get("bars") or [])
    if len(bars) < MIN_BARS:
        return _result(state="NO_SETUP", setup="NONE", direction="NEUTRAL", stage="SEARCHING", maturity="UNRESOLVED",
                       thesis="NO_SETUP: insufficient closed-candle history", quality=0.0, confidence=100.0,
                       setup_exists=False, trade_ready=False, supporting=[],
    ...
    try:
        ...
        return _result(state="NO_SETUP", setup="NONE", direction="NEUTRAL", stage="SEARCHING", maturity="UNRESOLVED",
                       ...
                       missing=["valid_closed_candle_ohlc"], next_required=["provide valid closed-candle OHLC values"],
    ...
    supporting.extend([
        f"E4_EVENT={event or 'NONE'}", f"E4_EVENT_AGE_BARS={event_age}",
        f"E4_AUCTION_STATE={_text(e4.get('auction_state', e4.get('state'))) or 'UNKNOWN'}",
        f"E3_LIFECYCLE={e3_lifecycle or 'UNKNOWN'}", f"E3_EXTERNAL={e3_external}",
        f"E5_LOCATION={_text(e5.get('finding', e5.get('state', 'UNKNOWN'))) or 'UNKNOWN'}",
        f"STRUCTURAL_SPACE_LONG_ATR={long_space:.3f}", f"STRUCTURAL_SPACE_SHORT_ATR={short_space:.3f}",
    ])

    candidates: list[dict[str, Any]] = []
    rejected: list[str] = []

    # Do not invent a setup while independent upstream evidence is conflicting.
    if direction != "NEUTRAL" and not formation_counter:
        if auction_dir == direction and any(x in event for x in ("SWEEP_REJECTION", "FAILED_BREAK_RECLAIM")):
            ...
            candidates.append(_candidate("LIQUIDITY_REVERSAL", direction, evidence, missing,
                                         92.0 if stage == "MATURE" else 78.0 if stage == "VALIDATING" else 62.0,
                                         stage, ["closed_candle_loses_reclaim", "contrary_closed_candle_acceptance", "protected_level_break"]))
        else:
            rejected.append("LIQUIDITY_REVERSAL: causal sweep/rejection or failed-reclaim sequence absent")
        ...
        if confirmed_break and e3_external == direction:
            ...
            stage = "MATURE" if (retest or acceptance) else "VALIDATING"
            candidates.append(_candidate("BREAKOUT_RETEST" if retest else "BREAKOUT", direction, evidence, missing,
                                         88.0 if stage == "MATURE" else 74.0, stage,
        ...
        # A trend label alone is not a pullback. Require an explicit pullback/rejection sequence.
        trend_aligned = e3_external == direction and any(x in e1_trend for x in ("UP", "DOWN", "TREND")) and not structure_mixed
        pullback_evidence = any(x in (e3_finding, e3_lifecycle, opportunity) for x in ("PULLBACK", "RETEST", "REJECTION"))
        if trend_aligned and pullback_evidence:
            evidence = ["E1_TREND_CONTEXT", "E3_STRUCTURE_ALIGNMENT", "PULLBACK_OR_REJECTION_PRESENT"]
            missing = ["closed_candle_continuation_after_pullback"]
            ...
            candidates.append(_candidate("TREND_PULLBACK", direction, evidence, missing, 76.0, "VALIDATING",
                                         ["trend_alignment_lost", "protected_structure_failure", "pullback_breaks_directional_structure"]))
        else:
            rejected.append("TREND_PULLBACK: explicit pullback/rejection sequence not established")

        last = bars[-1]
        body = abs(_num(last.get("close")) - _num(last.get("open")))
        candle_dir = "BUY" if _num(last.get("close")) > _num(last.get("open")) else "SELL" if _num(last.get("close")) < _num(last.get("open")) else "NEUTRAL"
        if body >= 0.8 * atr and candle_dir == direction and e3_external == direction and not structure_mixed:
            candidates.append(_candidate("IMPULSE_CONTINUATION", direction,
                                         ["DIRECTIONAL_IMPULSE", "E3_STRUCTURE_ALIGNMENT"],
                                         ["closed_candle_follow_through_after_impulse"], 66.0, "FORMING",
        ...
        if direction == "NEUTRAL":
            ...
            rejected.append("ALL_SETUP_FAMILIES: independent formation evidence is conflicting or incomplete")
    ...
    if not candidates:
        ...
        if direction == "NEUTRAL":
            ...
            next_required.insert(0, "wait for independent E1/E3/E4 evidence to align")
        if formation_counter:
            missing.extend(formation_counter)
            guidance = {
                "DIRECTIONAL_EVIDENCE_CONFLICT": "wait for independent directional evidence to resolve without override",
                "E2_DIRECTION_DISAGREEMENT": "wait for E2 and independent structure/auction evidence to agree",
                "E2_INTERNAL_DIRECTION_CONFLICT": "wait for E2 to resolve its own directional contradiction",
                "STRUCTURE_MIXED": "wait for confirmed pivots to produce a clean structure",
                "LIQUIDITY_EVENT_PENDING": "wait for the liquidity event to become terminal on a closed candle",
                "OPPORTUNITY_MATURITY_UNPROVEN": "wait for closed-candle acceptance/follow-through proving the opportunity",
                "EXTERNAL_STRUCTURE_COUNTERTREND": "wait for external structure and directional thesis to realign",
            }
            next_required.extend(guidance.get(x, f"resolve {x}") for x in formation_counter)
        return _result(state="NO_SETUP" if direction == "NEUTRAL" else "FORMING", setup="NONE", direction=direction,
                       stage="SEARCHING" if direction == "NEUTRAL" else "FORMING", maturity="UNRESOLVED",
                       thesis=("NO_SETUP: direction is unresolved; no setup family may be promoted" if direction == "NEUTRAL"
                               else f"{direction}: directional thesis exists, but causal setup formation is not established"),
                       quality=0.0 if direction == "NEUTRAL" else 10.0, confidence=82.0,
                       setup_exists=False, trade_ready=False, supporting=supporting, counter=formation_counter,
                       ...
                       trace={"direction_source": direction_source, "direction_inputs": direction_inputs,
                              "formation_counter_evidence": formation_counter, "e3_finding": e3_finding,
                              "e4_event": event, "e4_terminal": terminal, "e4_pending": pending,
                              "e4_event_age_bars": event_age, "e5_location": _text(e5.get("finding", e5.get("state", ""))),
                              "setup_exists": False, "trade_ready": False, "closed_candle_only": True, "lookahead": False})

    selected = candidates[0]
    setup = selected["name"]
    stage = selected["formation_stage"]
    maturity = "MATURE" if stage == "MATURE" else "DEVELOPING"
    setup_exists = True

    readiness_blockers: list[str] = []
    space = long_space if direction == "BUY" else short_space
    if space < MIN_SPACE_ATR:
        readiness_blockers.append(f"INSUFFICIENT_{direction}_STRUCTURAL_SPACE")
        next_space = f"wait for at least {MIN_SPACE_ATR:.2f} ATR {direction.lower()}-side structural space"
    else:
        next_space = ""
    ...
    readiness_blockers.extend(formation_counter)
    readiness_blockers = list(dict.fromkeys(readiness_blockers))

    missing = list(selected["missing"])
    next_required = list(missing)
    if next_space:
        next_required.append(next_space)
    next_required.extend(readiness_blockers)
    next_required.extend(["E7 independent closed-candle entry confirmation", "E8 independent structural/economic survivability"])

    thesis_parts = [f"{direction} {setup}", f"lifecycle={stage}", f"causal_sequence={' -> '.join(selected['evidence'])}"]
    if readiness_blockers:
        thesis_parts.append(f"formation_or_economic_blockers={','.join(readiness_blockers)}")
    thesis = " | ".join(thesis_parts)

    quality = selected["strength"] - min(25.0, 5.0 * len(formation_counter))
    confidence = selected["strength"] + 4.0 - min(25.0, 5.0 * len(formation_counter))
    state = "MATURE" if maturity == "MATURE" and not readiness_blockers else "FORMING"

    return _result(state=state, setup=setup, direction=direction, stage=stage, maturity=maturity, thesis=thesis,
                   quality=quality, confidence=max(55.0, min(96.0, confidence)), setup_exists=setup_exists,
                   trade_ready=False, supporting=supporting + [f"SETUP_THESIS={thesis}"], counter=formation_counter,
                   missing=missing + readiness_blockers, next_required=next_required,
                   invalidation=selected["invalidation"] + ["directional thesis becomes contradictory", "causal setup sequence is invalidated"],
                   candidates=candidates, rejected=rejected,
                   trace={"direction_source": direction_source, "direction_inputs": direction_inputs,
                          "e1_trend_state": e1_trend, "e2_opportunity": opportunity, "e3_finding": e3_finding,
                          "e3_external": e3_external, "e3_lifecycle": e3_lifecycle, "e4_event": event,
                          "e4_terminal": terminal, "e4_pending": pending, "e4_event_age_bars": event_age,
                          "e5_location": _text(e5.get("finding", e5.get("state", ""))), "setup_exists": True,
                          "trade_ready": False, "formation_counter_evidence": formation_counter,
                          "trade_readiness_blockers": readiness_blockers, "closed_candle_only": True, "lookahead": False})
