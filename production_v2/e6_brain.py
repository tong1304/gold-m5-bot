...
def _direction_thesis(
    ...
    if len(unique) == 1 and votes:
        ...
        source = "E1_E3_E4_CONVERGENCE" if len(votes) >= 3 else "DIRECTIONAL_CONVERGENCE"
    elif pressure != "NEUTRAL" and internal == pressure:
        direction = pressure
        source = "E1_E3_DIRECTIONAL_CORE"
    elif internal != "NEUTRAL" and auction["direction"] == internal:
        direction = internal
        source = "E3_E4_DIRECTIONAL_CORE"
    elif pressure != "NEUTRAL" and auction["direction"] == pressure:
        direction = pressure
        source = "E1_E4_DIRECTIONAL_CORE"
    else:
        direction = "NEUTRAL"
...
def _build_result(
    ...
    observations = [
        f"candidate_setup={setup}",
        f"candidate_identity={(trace.get('candidate_identity') or 'NONE')}",
...
def analyze_e6(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    if direction != "NEUTRAL":
        ledger.extend([
            ...
            _evidence("E3", f"structure={structure or 'UNKNOWN'}; internal={internal}; external={external}"),
    ...
    if not hard_veto and auction["direction"] == direction and "ACCEPTANCE" in auction["event"]:
        ...
        add(
            ...
            "closed-candle acceptance at the identified auction level",
        )
    ...
    if not hard_veto and auction["direction"] == direction and any(x in auction["event"] for x in ("SWEEP_REJECTION", "FAILED_BREAK_RECLAIM")):
        add(
            "LIQUIDITY_REVERSAL", "VALIDATING",
            66 + (8 if auction["terminal"] else 0) - (8 if mixed else 0),
            ...
            "liquidity sweep/rejection plus directional response",
        )
    ...
    if not hard_veto and direction == trend and not mixed:
        if "BREAK" in bos and "NO_BREAK" not in bos:
            ...
            add(
                name, "VALIDATING", 58 if auction["terminal"] else 50,
                ...
                "confirmed structural break aligned with prevailing direction",
            )
        else:
            pullback_shape = (direction == "BUY" and sequence_tail in {"HL", "LH"}) or (direction == "SELL" and sequence_tail in {"LH", "HL"})
            if pullback_shape and "HH" in sequence and "LL" not in sequence[-12:]:
                add(
                    "TREND_PULLBACK", "FORMING", 44,
                    ["directional_structure_present", f"sequence_tail={sequence_tail}", "continuation_not_yet_confirmed"],
                    ["impulse_then_pullback_sequence", "pullback_location_validation", "continuation_trigger"],
                    ["confirmed pullback into meaningful structure", "closed-candle continuation trigger"],
                    ["loss_of_protected_structure", "pullback_failure"],
                    "directional structure suggests pullback but causal continuation is not yet proven",
                )
    ...
    if not candidates:
        ...
        missing = list(best["missing_evidence"])
        next_required = list(best["next_required_evidence"])
        if len(candidates) > 1 and best["strength"] - candidates[1]["strength"] < 8:
            ...
            critical_missing = {
                ...
                "pullback_location_validation", "continuation_trigger",
            }
            blockers = critical_missing.intersection(missing)
            mature = (
                best["formation_stage"] == "VALIDATING"
                and not blockers
                and not mixed
                and not hard_veto
    ...
    if anchor > 0:
        invalidation.append(f"candidate_anchor_failure={anchor:.5f}")
    ...
    trace = {
        ...
        "candidate_identity_reason": selected.get("identity_reason") if selected else None,
