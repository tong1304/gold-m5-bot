...
# Dependencies are evidence-only. An engine may reinterpret upstream observations,
# but it can never inherit an upstream decision or gate.
...
def _legacy_context(permitted:dict[str,Any])->dict[str,Any]:
    ...
    Decision and gate authority is deliberately removed from the context.
    """
    ctx={}
...
def _dependency_report(engine_id:str, permitted:dict[str,Any])->dict[str,Any]:
    ...
    return {"required":required,"received":received,"missing":missing,"complete":not missing,"decisions_received":decisions_received,"gates_received":gates_received,"channel":"EVIDENCE_ONLY"}
...
def run_engine(engine_id:str,snapshot:dict[str,Any],evidence_bus:dict[str,Any]|None=None)->EngineResult:
    ...
    # E1-E8 are analysts, not gatekeepers. gate_passed is therefore always False
    # for these results so runtime/API telemetry cannot falsely imply authority.
    return EngineResult(engine_id,ENGINE_NAMES[engine_id],False,score,output,())
