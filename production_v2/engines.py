...
"""Production-V2: parallel evidence architecture.
E1-E8 are independent specialist brains. Their gate_passed values are ignored.
E9 alone synthesizes evidence and decides BUY/SELL/NO_TRADE.
"""
import importlib
...
def _run(code:str,snapshot:dict[str,Any]): return _module(code).SubEngine().run(snapshot)

def run_engine(engine_id:str,snapshot:dict[str,Any])->EngineResult:
    """All specialists in one brain analyze the identical snapshot concurrently."""
    codes=SUB_ENGINE_CODES[engine_id]; results=[]
    with ThreadPoolExecutor(max_workers=len(codes)) as pool:
        fs={pool.submit(_run,c,snapshot):c for c in codes}
    ...
    output={"architecture":"PARALLEL_SPECIALISTS_SHARED_SNAPSHOT","specialists":evidence,"evidence_quality":round(score,2),"decision_authority":"E9_ONLY","gate_semantics":"DISABLED_FOR_E1_E8","professional_reasoning":{"question":_question(engine_id),"conclusion":_conclusion(evidence),"evidence_count":len(evidence)}}
...
def run_all_parallel(snapshot:dict[str,Any])->list[EngineResult]:
    """E1-E8 consume only the shared market snapshot; no E consumes another E."""
    results=[]
    with ThreadPoolExecutor(max_workers=8) as pool:
        fs={pool.submit(run_engine,e,snapshot):e for e in SUB_ENGINE_CODES}
...
def run_e9_decision(snapshot:dict[str,Any],upstream:list[EngineResult])->EngineResult:
    """Only E9 converts the eight independent evidence packages into a decision."""
    ...
    out={"decision":direction if final else "NO_TRADE","decision_authority":"E9","architecture":"PARALLEL_SHARED_SNAPSHOT→E9","evidence_alignment":alignment,"directional_evidence":{"buy":buy,"sell":sell},"confluence":{"setup":setup,"confirmation":confirmation,"economics":economics},"decision_reasons":reasons,"all_evidence_received":sorted(by),"upstream_gates_ignored":True,"gate_semantics":"E9_MASTER_ONLY"}
