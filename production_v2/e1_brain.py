...
OWNERSHIP = {
    "owns": [
        "data_integrity", "volatility_regime", "market_structure_context", "directional_pressure",
        "multi_horizon_alignment", "trend_persistence", "range_regime", "compression_regime",
        "expansion_regime", "regime_transition", "state_stability", "counter_evidence",
        "market_state_invalidation", "market_regime",
    ],
    "does_not_own": [
        "opportunity_setup", "trade_location", "entry_confirmation", "trade_economics",
        "risk_management", "trade_execution", "BUY", "SELL",
    ],
}
...
def _professional_reasoning(
    *, market_state="UNCLEAR", direction="NEUTRAL", directional_pressure="NEUTRAL",
    trend_maturity="NONE", trend_confirmed=False, conflict_detected=False, conflict_count=0,
    classification_reason="", directions=("FLAT", "FLAT", "FLAT"), up_count=0, down_count=0,
    ema_relation="FLAT", ema_gap=0.0, structure_state="MIXED", structure_quality=0.0,
    efficiency10=0.0, efficiency20=0.0, single_counter_candle=False, pressure_score=0.0,
    ...
    return {
        ...
        "directional_consensus": {
            ...
            "confirmed": bool(
                ema_relation in {"UP", "DOWN"}
                and max(up_count, down_count) >= 2
                and trend_confirmed
            ),
            "required_count": 2,
        },
        ...
        "single_counter_candle": bool(single_counter_candle),
        "pressure_score": round(float(pressure_score), 4),
        "structure_alignment": round(float(structure_alignment), 4),
        "trend_score": round(float(trend_score), 4),
        "regime_basis": classification_reason,
    }


def _incomplete(conflicts, evidence, reason):
    return {
        **_base_output(),
        ...
        "professional_reasoning": _professional_reasoning(
            market_state="UNCLEAR",
...
def analyze_e1(bars):
    ...
    if len(valid) < MIN_BARS:
        return _incomplete(
            problems[:6],
            ["valid_candles_below_minimum"],
            "insufficient reliable candles; classification withheld",
        )
    ...
    if atr14 <= 0:
        return _incomplete(
            [*problems[:5], "ATR_INVALID"],
            ["atr_invalid"],
            "ATR invalid; classification withheld",
        )
    ...
    if medium_slope > 0.05 and long_slope > 0.05:
        internal_pressure = "UP"
    elif medium_slope < -0.05 and long_slope < -0.05:
        internal_pressure = "DOWN"
    elif long_slope > 0.05:
        internal_pressure = "UP"
    elif long_slope < -0.05:
        internal_pressure = "DOWN"
    elif directions[1] == directions[2] and directions[1] in {"UP", "DOWN"}:
        internal_pressure = directions[1]
    else:
    ...
    persistence_hits = (
        ...
        if internal_pressure in {"UP", "DOWN"}
        else 0
    )
    ...
    ema_ok = (
        ...
        and (
            (
                internal_pressure == "UP"
                and ema20_slope >= -0.05
                and ema50_slope >= -0.10
            )
            or (
                internal_pressure == "DOWN"
                and ema20_slope <= 0.05
    ...
    ema_conflict = (
        internal_pressure in {"UP", "DOWN"}
        and ema_relation in {"UP", "DOWN"}
        and ema_relation != internal_pressure
    )
    structure_conflict = (
        internal_pressure in {"UP", "DOWN"}
        and structure_direction in {"UP", "DOWN"}
        and structure_direction != internal_pressure
    )
    ...
    # The long horizon is allowed to dominate a short/medium pullback only when
    # it is materially stronger and the EMA regime still agrees with it. This is
    # intentionally asymmetric: a long-horizon trend can absorb a temporary
    # countertrend move, but an EMA regime conflict still blocks promotion.
    long_horizon_dominant = (
        internal_pressure in {"UP", "DOWN"}
        and directions[2] == internal_pressure
        and ema_relation == internal_pressure
        and abs(long_slope) >= 0.80
        and abs(long_slope) >= 1.80 * max(abs(short_slope), abs(medium_slope), 1e-12)
    )

    long_medium_coherent = (
        internal_pressure in {"UP", "DOWN"}
        and directions[1] == internal_pressure
        and directions[2] == internal_pressure
    )
    strong_structure = structure_direction == internal_pressure and structure_quality >= 0.55

    trend_confirmed = (
        (consensus or long_horizon_dominant)
        and ema_ok
        and abs(ema_gap) >= 0.10
        and not ema_conflict
        and not structure_conflict
        and (strong_structure or persistence == 1.0 or long_horizon_dominant)
    )

    # A short-term counter move is counter-evidence, not a regime transition,
    # when the higher-horizon trend remains materially dominant. Transition
    # still requires an independent EMA/structure conflict or true higher-
    # horizon disagreement.
    higher_horizon_reversal = not long_medium_coherent and horizon_conflict and not long_horizon_dominant
    transition_present = (
        not trend_confirmed
        and (
            (ema_conflict and persistence >= 1 / 3)
            or (structure_conflict and persistence >= 1 / 3)
            or (higher_horizon_reversal and _efficiency(closes, 20) < 0.45)
        )
    )

    trs = _true_ranges(valid)
    recent_atr = atr14
    baseline_trs = trs[-64:-14]
    baseline_atr = mean(baseline_trs) if baseline_trs else recent_atr
    atr_ratio = recent_atr / max(baseline_atr, 1e-12)
    compression = atr_ratio < 0.78
    expansion = atr_ratio > 1.10
    efficiency10 = _efficiency(closes, 10)
    efficiency20 = _efficiency(closes, 20)

    deltas = [closes[i] - closes[i - 1] for i in range(1, len(closes))]
    prior_mean = mean(deltas[-6:-1]) if len(deltas) >= 6 else 0.0
    prior_abs = mean(abs(x) for x in deltas[-6:-1]) if len(deltas) >= 6 else 0.0
    last_delta = deltas[-1] if deltas else 0.0
    single_counter_candle = bool(
        prior_mean
        and last_delta
        and prior_mean * last_delta < 0
        and abs(last_delta) > 1.5 * max(prior_abs, 1e-12)
    )

    compression_regime = compression and efficiency20 < 0.35 and not trend_confirmed
    range_regime = (
        efficiency20 < 0.35
        and structure_state == "MIXED"
        and not trend_confirmed
        and not single_counter_candle
        and not compression_regime
    )

    if compression_regime:
        market_state, final_direction, classification_reason = (
            "COMPRESSION", "NEUTRAL", "volatility_compression_with_low_directional_efficiency"
        )
    elif range_regime:
        market_state, final_direction, classification_reason = (
            "RANGE", "NEUTRAL", "balanced_or_rotational_pressure_with_low_directional_efficiency"
        )
    elif transition_present:
        market_state, final_direction, classification_reason = (
            "TRANSITION", internal_pressure, "material_conflict_between_regime_dimensions"
        )
    elif trend_confirmed:
        market_state, final_direction, classification_reason = (
            "TREND_UP" if internal_pressure == "UP" else "TREND_DOWN",
            internal_pressure,
            "persistent_multi_horizon_direction_with_ema_and_structure_coherence",
        )
    elif expansion and internal_pressure in {"UP", "DOWN"} and efficiency10 >= 0.25:
        market_state, final_direction, classification_reason = (
            "EXPANSION", internal_pressure, "volatility_expansion_with_directional_displacement"
        )
    else:
        market_state, final_direction, classification_reason = (
            "UNCLEAR", internal_pressure, "directional_evidence_exists_but_regime_confirmation_is_insufficient"
        )

    pressure_is_directional = (
        internal_pressure in {"UP", "DOWN"}
        and (consensus or strong_structure or persistence == 1.0 or long_medium_coherent or long_horizon_dominant)
    )
    if market_state in {"RANGE", "COMPRESSION"} and not pressure_is_directional:
        final_direction = "NEUTRAL"
    directional_pressure = (
        "BULLISH" if internal_pressure == "UP" and pressure_is_directional
        else "BEARISH" if internal_pressure == "DOWN" and pressure_is_directional
        else "NEUTRAL"
    )
    ...
    # A confirmed long-horizon trend can be developing when only shorter
    # horizons disagree; the macro direction remains intact.
    maturity = (
        "DEVELOPING" if ((long_medium_coherent or long_horizon_dominant) and horizon_conflict)
        else "ESTABLISHED" if trend_confirmed
        else "DEVELOPING" if consensus and ema_ok
        else "DIRECTIONAL_ONLY" if pressure_is_directional
        else "NONE"
    )

    pressure_score = (
        _clamp((max(up_count, down_count) / 3.0) * (0.5 + 0.5 * persistence))
        if pressure_is_directional
        else 0.0
    )
    ...
    confidence = round(
        _clamp(
            ...
            + 0.20 * persistence
            + 0.10 * min(1.0, efficiency20 / 0.70)
            + 0.10 * float(ema_ok)
            - 0.05 * len(conflicts)
        ),
        3,
    ...
    return {
        **_base_output(),
        ...
        "evidence": [
            ...
            f"directional_pressure={directional_pressure}",
            f"price_consensus={max(up_count, down_count)}/3",
...
__all__ = ["analyze_e1", "QUESTION", "MARKET_STATES", "EVIDENCE_HIERARCHY", "OWNERSHIP"]
