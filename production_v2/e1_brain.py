...
def _true_ranges(bars:list[dict[str,Any]])->list[float]:
    ...
    for bar in bars:
        high,low,close=bar["high"],bar["low"],bar["close"]
        result.append(high-low if previous_close is None else max(high-low,abs(high-previous_close),abs(low-previous_close)));previous_close=close
    return result
...
def _structure(bars:list[dict[str,Any]])->tuple[str,float]:
    ...
    for i in range(PIVOT_WING,len(bars)-PIVOT_WING):
        window=bars[i-PIVOT_WING:i+PIVOT_WING+1];high,low=bars[i]["high"],bars[i]["low"]
    ...
    if bullish>=2 and bullish>bearish:return "BULLISH",_clamp(0.58+0.07*bullish)
    if bearish>=2 and bearish>bullish:return "BEARISH",_clamp(0.58+0.07*bearish)
    if hh+hl>=3 and hh+hl>lh+ll:return "BULLISH",0.52
    if lh+ll>=3 and lh+ll>hh+hl:return "BEARISH",0.52
    return "MIXED",0.30

def _base_output():return {"question":QUESTION,"reasoning_role":"MARKET_STATE_ANALYST","trade_decision_authority":False,"decision_authority":"E9_ONLY"}

def _professional_reasoning(*,market_state="UNCLEAR",direction="NEUTRAL",directional_pressure="NEUTRAL",trend_maturity="NONE",trend_confirmed=False,conflict_detected=False,conflict_count=0,classification_reason="",directions=("FLAT","FLAT","FLAT"),up_count=0,down_count=0,ema_relation="FLAT",ema_gap=0.0,structure_state="MIXED",structure_quality=0.0,efficiency10=0.0,efficiency20=0.0,single_counter_candle=False,pressure_score=0.0,structure_alignment=0.0,trend_score=0.0):
    return {"task":"DESCRIBE_MARKET_STATE_ONLY","primary_state":market_state,"market_state":market_state,"direction":direction,"directional_pressure":directional_pressure,"trend_maturity":trend_maturity,"trend_confirmed":bool(trend_confirmed),"conflict_detected":bool(conflict_detected),"conflict_count":int(conflict_count),"classification_reason":classification_reason,"directional_consensus":{"ema":ema_relation,"short":directions[0],"medium":directions[1],"long":directions[2],"up_count":up_count,"down_count":down_count,"confirmed":bool(ema_relation in {"UP","DOWN"} and max(up_count,down_count)>=2 and trend_confirmed),"required_count":2},"independent_evidence":{"ema_gap_atr":round(ema_gap,4),"structure":structure_state,"structure_quality":round(structure_quality,3),"efficiency_10":round(efficiency10,4),"efficiency_20":round(efficiency20,4),"directional_pressure":directional_pressure},"evidence_hierarchy":EVIDENCE_HIERARCHY,"ownership_boundaries":OWNERSHIP,"single_counter_candle":bool(single_counter_candle),"pressure_score":round(float(pressure_score),4),"structure_alignment":round(float(structure_alignment),4),"trend_score":round(float(trend_score),4),"regime_basis":classification_reason}

def _incomplete(conflicts,evidence,reason):return {**_base_output(),"market_state":"UNCLEAR","directional_pressure":"NEUTRAL","trend_state":"NONE","volatility_state":"UNKNOWN","structure_state":"UNCLEAR","structure_quality":0.0,"compression":"UNKNOWN","expansion":"UNKNOWN","transition":"UNKNOWN","confidence":0.0,"evidence":evidence,"conflicts":conflicts,"reasons":[reason],"analysis_status":"INCOMPLETE","professional_reasoning":_professional_reasoning(market_state="UNCLEAR",trend_maturity="UNAVAILABLE",conflict_detected=bool(conflicts),conflict_count=len(conflicts),classification_reason=reason,structure_state="UNCLEAR")}
...
def analyze_e1(bars):
    ...
    closes=[bar["close"] for bar in valid];atr14=_atr(valid,14)
    if atr14<=0:return _incomplete([*problems[:5],"ATR_INVALID"],["atr_invalid"],"ATR invalid; classification withheld")
    ema20=_ema(closes,20);ema50=_ema(closes,50);ema_relation="UP" if ema20[-1]>ema50[-1] else "DOWN" if ema20[-1]<ema50[-1] else "FLAT"
    ema_gap=(ema20[-1]-ema50[-1])/atr14;ema20_slope=_slope(ema20,atr14,5);ema50_slope=_slope(ema50,atr14,5)
    ...
    elif long_slope>0.05:internal_pressure="UP"
    elif long_slope<-0.05:internal_pressure="DOWN"
    ...
    structure_state,structure_quality=_structure(valid);structure_direction="UP" if structure_state=="BULLISH" else "DOWN" if structure_state=="BEARISH" else "NONE"
    ema_ok=internal_pressure in {"UP","DOWN"} and ema_relation==internal_pressure and ((internal_pressure=="UP" and ema20_slope>=-0.05 and ema50_slope>=-0.10) or (internal_pressure=="DOWN" and ema20_slope<=0.05 and ema50_slope<=0.10))
    ...
    conflicts=[]
    if problems:conflicts.append("DATA_QUALITY_ANOMALIES")
    ...
    reasoning_trace=[f"QUESTION -> {QUESTION}",f"STRUCTURE -> {structure_state} quality={structure_quality:.2f}",f"PRESSURE -> {directional_pressure} short={directions[0]} medium={directions[1]} long={directions[2]}",f"PERSISTENCE -> {persistence:.2f}",f"REGIME_CONFIRMATION -> trend_confirmed={trend_confirmed} maturity={maturity}",f"STATE -> {market_state} because={classification_reason}",f"TRANSITION -> {transition}"]
