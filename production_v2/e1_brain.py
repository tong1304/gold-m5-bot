...
# E1 ownership is deliberately narrow. Setup, liquidity, location/space,
# confirmation, economics, risk and execution belong to downstream brains.
E1_OWNERSHIP = {
    "owns": [
        "data_integrity",
        "volatility_regime",
...
def _structure(bars: list[dict[str, Any]]) -> tuple[str, float, dict[str, Any]]:
    ...
    if bullish_pairs >= 2 and bullish_pairs > bearish_pairs:
        state, quality = "BULLISH", min(1.0, 0.65 + 0.08 * bullish_pairs)
    elif bearish_pairs >= 2 and bearish_pairs > bullish_pairs:
        state, quality = "BEARISH", min(1.0, 0.65 + 0.08 * bearish_pairs)
...
def _confidence(
    ...
    quality = (
        0.22 * structure_quality
        + 0.22 * persistence
        + 0.14 * min(1.0, efficiency10 / 0.70)
    ...
    if maturity == "DEVELOPING":
        ...
        quality *= 0.72
    penalty = min(0.30, 0.05 * conflicts)
...
def analyze_e1(bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    It classifies regime from independent evidence. It does not create setups,
    choose entries, size risk, or authorize trades.

    Professional principle:
        direction is evidence; trend is a regime claim.
        A regime claim requires persistence and coherence, not one indicator.
    """
    ...
    if len(valid) < 60:
        return _incomplete(
            ...
            data_problems[:6],
            ["valid_candles_below_minimum", *data_problems[:6]],
        )
    ...
    # 1) Indicator evidence: useful context, never sufficient by itself.
    ...
    # 2) Price structure context. E3 remains the detailed structural authority.
    ...
    # 4) Directional pressure across independent horizons.
    ...
    up_count = price_dirs.count("UP")
    down_count = price_dirs.count("DOWN")
    ...
    # Persistence asks whether the move is material on all relevant horizons,
    # rather than merely pointing in the same direction at the final candle.
    if pressure == "UP":
        aligned = sum((short_slope >= 0.20, medium_slope >= 0.30, long_slope >= 0.45))
    elif pressure == "DOWN":
        aligned = sum((short_slope <= -0.20, medium_slope <= -0.30, long_slope <= -0.45))
    else:
        aligned = 0
    ...
    ema_confirmed = (
        ...
        and (
            (pressure == "UP" and ema20_slope >= -0.05 and ema50_slope >= -0.10)
            or (pressure == "DOWN" and ema20_slope <= 0.05 and ema50_slope <= 0.10)
        )
    )
    ema_conflict = (
        pressure in {"UP", "DOWN"}
    ...
    structural_conflict = (
        ...
        and structure_direction != pressure
    )
    ...
    # Professional classification gates. Trend is deliberately harder to claim
    # than directional pressure. Structure is supporting evidence, not a duplicate
    # of E3's full structural analysis.
    consensus_confirmed = (
        pressure in {"UP", "DOWN"}
        and up_count + down_count >= 2
        and persistence >= 2 / 3
        and ema_confirmed
    )
    ...
    # A single counter candle is information, not a regime reversal.
    single_counter_candle = False
    if pressure in {"UP", "DOWN"} and len(closes) >= 3:
        a = closes[-2] - closes[-3]
        b = closes[-1] - closes[-2]
        single_counter_candle = (
            (pressure == "UP" and b < 0 < a)
            or (pressure == "DOWN" and b > 0 > a)
        )

    # Transition is reserved for active change/conflict, not ordinary imperfection.
    genuine_transition = (
        not trend_confirmed
        and (
            (structural_conflict and persistence >= 1 / 3)
            or (ema_conflict and persistence >= 1 / 3)
            or (horizon_conflict and efficiency20 < 0.45)
        )
    )

    if trend_confirmed:
        trend_maturity = "ESTABLISHED"
    elif pressure in {"UP", "DOWN"} and consensus_confirmed and efficiency20 >= 0.20:
        trend_maturity = "DEVELOPING"
    elif pressure in {"UP", "DOWN"}:
        trend_maturity = "DIRECTIONAL_ONLY"
    else:
        trend_maturity = "NONE"

    # Classification hierarchy: state first, then confidence. No state is created
    # from confidence alone.
    if compression and pressure == "BALANCED":
        market_state, final_pressure, reason = (
            "COMPRESSION", "NEUTRAL", "volatility_compression_dominates_while_direction_is_balanced"
        )
    elif genuine_transition:
        market_state, final_pressure, reason = (
            "TRANSITION",
            pressure if pressure in {"UP", "DOWN"} else "BALANCED",
            "material_evidence_is_changing_or_conflicting_across_regime_dimensions",
        )
    elif trend_confirmed:
        market_state, final_pressure, reason = (
            "TREND_UP" if pressure == "UP" else "TREND_DOWN",
            pressure,
            "direction_persistence_ema_and_structure_are_coherent",
        )
    elif expansion and pressure in {"UP", "DOWN"} and efficiency10 >= 0.25:
        market_state, final_pressure, reason = (
            "EXPANSION", pressure, "volatility_and_price_displacement_are_expanding"
        )
    elif pressure == "BALANCED" and efficiency20 < 0.35:
        market_state, final_pressure, reason = (
            "RANGE", "NEUTRAL", "directional_efficiency_is_low_and_neither_side_controls"
        )
    elif pressure in {"UP", "DOWN"}:
        # Direction exists but has not earned a full trend claim. E1 retains the
        # direction as context while refusing to overstate the regime.
        market_state, final_pressure, reason = (
            "UNCLEAR",
            pressure,
            "directional_evidence_exists_but_regime_confirmation_is_insufficient",
        )
    else:
        market_state, final_pressure, reason = (
            "UNCLEAR", "BALANCED", "directional_evidence_is_balanced"
        )

    if single_counter_candle and market_state in {"TREND_UP", "TREND_DOWN"}:
        reason += ";single_counter_candle_not_enough_to_reverse_established_state"
    ...
    data_quality = max(
        0.0,
        1.0 - min(1.0, len(data_problems) / max(1, len(valid))),
    )
    ...
    if market_state == "TRANSITION":
        thesis = (
            f"Market is transitioning with "
            f"{('bullish' if final_pressure == 'UP' else 'bearish' if final_pressure == 'DOWN' else 'balanced')} "
            "pressure; E1 preserves the conflict instead of inventing certainty."
        )
    elif market_state in {"TREND_UP", "TREND_DOWN"}:
        thesis = (
            f"Market is {('bullish' if final_pressure == 'UP' else 'bearish')} and "
            f"established; persistence={persistence:.2f}, efficiency20={efficiency20:.2f}, "
            f"structure_quality={structure_quality:.2f}."
        )
    elif market_state == "EXPANSION":
        thesis = (
            f"Market is expanding with "
            f"{('bullish' if final_pressure == 'UP' else 'bearish')} pressure; "
            "expansion is recognized without automatically calling it a trend."
        )
        ...
        thesis = (
            f"Market has {pressure_label.lower()} directional pressure, but the evidence is "
            "not coherent enough for a stronger regime claim."
        )
    ...
    evidence = [
        ...
        f"trend_maturity={trend_maturity}",
    ]
    ...
    reasoning_trace = [
        ...
        f"REGIME_CONFIRMATION -> trend_confirmed={trend_confirmed} maturity={trend_maturity}",
    ...
    next_question = (
        ...
        if market_state in {"TREND_UP", "TREND_DOWN", "RANGE", "COMPRESSION", "EXPANSION"}
        else "IS_DIRECTIONAL_PRESSURE_STRONG_ENOUGH_TO_MATTER?"
