...
def _persistence_score(slopes: list[float], pressure: str) -> float:
    if pressure == "UP": hits = sum(s >= t for s, t in zip(slopes, (0.20, 0.25, 0.35, 0.45)))
    elif pressure == "DOWN": hits = sum(s <= t for s, t in zip(slopes, (-0.20, -0.25, -0.35, -0.45)))
    else: return 0.0
    return hits / 4.0

def _long_persistence(slopes: list[float], pressure: str) -> float:
    if pressure == "UP": hits = sum(s >= t for s, t in zip(slopes[1:], (0.25, 0.35, 0.45)))
    elif pressure == "DOWN": hits = sum(s <= t for s, t in zip(slopes[1:], (-0.25, -0.35, -0.45)))
    else: return 0.0
    return hits / 3.0
...
def _hierarchical_state(*, pressure: str, structure_direction: str, structure_quality: float, consensus: float, persistence: float, ema_relation: str, long_consensus: float, long_persistence: float, context_flip: bool, structure_break: bool, single_counter_candle: bool = False) -> dict[str, Any]:
    ...
    strong = consensus >= 0.75 and persistence >= 0.75 and long_consensus >= 0.667 and long_persistence >= 0.667
    ...
    aligned = structure_direction in {"UP", "DOWN"} and pressure == structure_direction and long_consensus >= 0.667 and long_persistence >= 0.667 and structure_quality >= 0.52
    developing = structure_direction in {"UP", "DOWN"} and pressure == structure_direction and consensus >= 0.50 and persistence >= 0.50 and structure_quality >= 0.52
    if confirmed_transition: state, direction, maturity = "TRANSITION", pressure, "TRANSITION"
    elif aligned: state, direction, maturity = ("TREND_UP" if structure_direction == "UP" else "TREND_DOWN"), structure_direction, "ESTABLISHED"
    elif developing: state, direction, maturity = ("TREND_UP" if structure_direction == "UP" else "TREND_DOWN"), structure_direction, "DEVELOPING"
    elif structure_direction in {"UP", "DOWN"}: state, direction, maturity = ("TREND_UP" if structure_direction == "UP" else "TREND_DOWN"), structure_direction, "CHALLENGED"
    else: state, direction, maturity = "UNCLEAR", (pressure if pressure in {"UP", "DOWN"} else "NEUTRAL"), "UNRESOLVED"
    ...
    elif structure_direction in {"UP", "DOWN"}: reason = "structural regime remains dominant but trend confirmation is incomplete"
...
def _structure(b, atr):
    ...
    if bull >= 2 and bull > bear: state, quality = "BULLISH", min(1.0, 0.62 + 0.07 * bull)
    elif bear >= 2 and bear > bull: state, quality = "BEARISH", min(1.0, 0.62 + 0.07 * bear)
    elif hh + hl > lh + ll and hh + hl >= 2: state, quality = "BULLISH", 0.52
    elif lh + ll > hh + hl and lh + ll >= 2: state, quality = "BEARISH", 0.52
    else: state, quality = "MIXED", 0.30
    direction = "UP" if state == "BULLISH" else "DOWN" if state == "BEARISH" else "NEUTRAL"
    last = b[-1]["close"]; sh = hs[-1][1] if hs else last; sl = ls[-1][1] if ls else last; buf = max(0.15 * atr, 1e-12); pc = b[-2]["close"]
    accepted_up, accepted_down = last > sh + buf and pc > sh + buf, last < sl - buf and pc < sl - buf
    probe_up, probe_down = last > sh + buf and pc <= sh + buf, last < sl - buf and pc >= sl - buf
...
def analyze_e1(bars: list[dict[str, Any]] | None) -> dict[str, Any]:
    ...
    if len(valid) < MIN_BARS:
        return {**base, "market_state": "UNCLEAR", "directional_pressure": "NEUTRAL", "directional_state": "UNRESOLVED", "trend_state": "NONE", "volatility_state": "UNKNOWN", "structure_state": "UNCLEAR", "structure_quality": 0.0, "range_state": "UNKNOWN", "compression": "UNKNOWN", "expansion": "UNKNOWN", "transition": "UNKNOWN", "transition_stage": "UNKNOWN", "confidence": 0.0, "evidence": [f"valid_candles={len(valid)}", f"invalid_candles={bad}"], "observations": [], "conflicts": [], "reasons": ["DATA_QUALITY_INSUFFICIENT"], "analysis_status": "INCOMPLETE"}
    closes = [x["close"] for x in valid]; atr14, atr50 = _atr(valid, 14), _atr(valid, 50)
    if atr14 <= 0 or atr50 <= 0:
        return {**base, "market_state": "UNCLEAR", "directional_pressure": "NEUTRAL", "directional_state": "UNRESOLVED", "trend_state": "NONE", "volatility_state": "UNKNOWN", "structure_state": "UNCLEAR", "structure_quality": 0.0, "confidence": 0.0, "reasons": ["ATR_INVALID"], "analysis_status": "INCOMPLETE"}
    ema20, ema50 = _ema(closes, 20), _ema(closes, 50); ema_rel = "UP" if ema20[-1] > ema50[-1] else "DOWN" if ema20[-1] < ema50[-1] else "FLAT"
    slopes = [_slope(closes, atr14, n) for n in (5, 10, 20, 40)]; thresholds = (0.15, 0.20, 0.30, 0.40); states = ["UP" if s >= t else "DOWN" if s <= -t else "FLAT" for s, t in zip(slopes, thresholds)]
    up, down = states.count("UP"), states.count("DOWN"); pressure = "UP" if up > down else "DOWN" if down > up else "BALANCED"; consensus = max(up, down) / 4.0
    long_states = states[1:]; long_cons = max(long_states.count("UP"), long_states.count("DOWN")) / 3.0
    persistence, lpersistence = _persistence_score(slopes, pressure), _long_persistence(slopes, pressure)
    prior = _atr(valid[:-14], 50) if len(valid) > 64 else atr50; vr = atr14 / max(prior, 1e-12); vol = "EXPANDING" if vr > 1.10 else "CONTRACTING" if vr < 0.78 else "NORMAL"
    context, recent = _slope(closes, atr14, 30), _slope(closes, atr14, 8); flip = abs(context) >= 0.45 and abs(recent) >= 0.65 and (context > 0) != (recent > 0)
    st = _structure(valid, atr14); transition_break = st["external_bos"] == "CONFIRMED_BOS" and st["bos_direction"] == pressure and st["direction"] != pressure
    ...
    eff20, eff40, eff10 = _eff(closes, 20), _eff(closes, 40), _eff(closes, 10)
    range_score = _clamp(0.35 * (1 if pressure == "BALANCED" else 0) + 0.30 * (1 - _clamp((eff20 + eff40) / 0.9)) + 0.20 * (1 - _clamp(st["quality"] / 0.7)) + 0.15 * (1 - _clamp(abs(ema20[-1] - ema50[-1]) / max(atr14, 1e-12) / 1.2)))
    range_ok = range_score >= 0.62 and eff20 < 0.40 and eff40 < 0.45 and st["state"] == "MIXED"
    compression = vol == "CONTRACTING" and vr < 0.82 and max(eff20, eff40) < 0.55 and (pressure == "BALANCED" or consensus < 0.75)
    expansion = vol == "EXPANDING" and eff10 >= 0.25 and abs(slopes[0]) >= 0.25 and consensus >= 0.50
    state = h["state"]
    if state == "UNCLEAR":
        if expansion: state = "EXPANSION"
        elif compression: state = "COMPRESSION"
        elif range_ok: state = "RANGE"
    direction = h["direction"]; conflicts = []
    ...
    agreement = 0.5 * (1 if st["direction"] == pressure and pressure != "BALANCED" else 0) + 0.5 * long_cons
    confidence = _clamp(0.40 * st["quality"] + 0.25 * lpersistence + 0.20 * long_cons + 0.15 * agreement - 0.12 * len(conflicts))
    if state == "UNCLEAR": confidence = min(confidence, 0.60)
    ...
    if long_cons >= 0.667: supporting.append("LONG_HORIZON_ALIGNMENT")
    if lpersistence >= 0.667: supporting.append("LONG_HORIZON_PERSISTENCE")
