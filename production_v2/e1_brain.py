...
def _atr(bars: list[dict[str, Any]], p: int, start: int | None = None, end: int | None = None) -> float:
    sample = (bars[start:end] if start is not None or end is not None else bars)[-p:]; trs=[]; prev=None
    for b in sample:
...
def _structure(bars: list[dict[str, Any]], atr: float) -> dict[str, Any]:
    ...
    last=bars[-1]["close"]; sh=max((x[1] for x in highs),default=last); sl=min((x[1] for x in lows),default=last); buf=max(.10*atr,1e-12)
...
def analyze_e1(bars: list[dict[str,Any]]|None)->dict[str,Any]:
    ...
    e20s,e50s=_ema(closes,20),_ema(closes,50); e20,e50=e20s[-1],e50s[-1]; ema="UP" if e20>e50 else "DOWN" if e20<e50 else "FLAT"; ema_gap=(e20-e50)/atr14
    ema20_s,ema50_s=_slope(e20s,atr14,5),_slope(e50s,atr14,5); lbs,th=(5,10,20,40),(.15,.20,.30,.40); slopes=[_slope(closes,atr14,n) for n in lbs]
    horizons=["UP" if s>=t else "DOWN" if s<=-t else "FLAT" for s,t in zip(slopes,th)]; up,dn=horizons.count("UP"),horizons.count("DOWN"); long=horizons[1:]; lu,ld=long.count("UP"),long.count("DOWN")
    pressure="UP" if up==4 else "DOWN" if dn==4 else "UP" if lu>ld else "DOWN" if ld>lu else "BALANCED"; consensus,long_consensus=max(up,dn)/4,max(lu,ld)/3
    ...
    eff10,eff20,eff40=(_eff(closes,n) for n in (10,20,40)); st=_structure(valid,atr14); sd="UP" if st["state"]=="BULLISH" else "DOWN" if st["state"]=="BEARISH" else "NEUTRAL"
    ...
    ema_alignment=1.0 if pressure in {"UP","DOWN"} and ema==pressure else 0.0; ema_conflict=pressure in {"UP","DOWN"} and ema in {"UP","DOWN"} and ema!=pressure; structure_conflict=pressure in {"UP","DOWN"} and sd in {"UP","DOWN"} and sd!=pressure; horizon_conflict=up>0 and dn>0
    prior_atr=_atr(valid,50,-64,-14) if len(valid)>=64 else atr50; vol_ratio=atr14/max(prior_atr,1e-12); compression,expansion=vol_ratio<.78,vol_ratio>1.10; volatility="EXPANDING" if expansion else "CONTRACTING" if compression else "NORMAL"
    pressure_score=consensus*(.65+.35*persistence); trend_score=.25*consensus+.25*persistence+.20*structure_alignment+.15*ema_alignment+.10*long_consensus+.05*max(eff20,eff40)
    ...
    contextual=pressure in {"UP","DOWN"} and long_consensus>=.667 and long_persistence>=.667 and structure_alignment>=.75 and ema_alignment==1.0; trend=established or contextual
    ...
    bos_against=st["external_bos"]=="CONFIRMED_BOS" and pressure in {"UP","DOWN"} and st["bos_direction"]!=pressure; persistent_flip=horizon_conflict and consensus>=.75 and persistence>=.75; ema_flip=ema_conflict and context_flip and persistence>=.50; ema_lag=ema_conflict and consensus>=.75 and persistence>=.75 and (abs(slopes[1])>=.20 or abs(slopes[2])>=.30)
    ...
    transition=not trend and (bos_against or ((context_flip or st["external_bos"]=="CONFIRMED_BOS") and (persistent_flip or ema_flip)) or ema_lag); stress=not transition and not trend and pressure in {"UP","DOWN"} and (ema_conflict or structure_conflict or horizon_conflict) and (consensus>=.50 or persistence>=.50)
    conflicts=[]
    ...
    prior_pressure="UP" if _slope(closes[:-1],atr14,5)>.20 else "DOWN" if _slope(closes[:-1],atr14,5)<-.20 else "NEUTRAL"; last_dir="UP" if closes[-1]>valid[-1]["open"] else "DOWN" if closes[-1]<valid[-1]["open"] else "FLAT"; single_counter=prior_pressure in {"UP","DOWN"} and last_dir in {"UP","DOWN"} and last_dir!=prior_pressure and not transition
    range_candidate=pressure=="BALANCED" and eff20<.35 and eff40<.40 and abs(ema_gap)<.85; expansion_candidate=expansion and pressure in {"UP","DOWN"} and eff10>=.25 and abs(slopes[0])>=.25
    ...
    direction="UP" if pressure=="UP" else "DOWN" if pressure=="DOWN" else "NEUTRAL"; directional_state="CONFIRMED" if state in {"TREND_UP","TREND_DOWN"} else "CONFLICTED" if transition else "DEVELOPING" if direction in {"UP","DOWN"} and (consensus>=.50 or persistence>=.25) else "NEUTRAL" if direction=="NEUTRAL" else "UNRESOLVED"; dc={"direction":direction,"confirmed":consensus>=.75,"score":round(consensus,3),"long_horizon_score":round(long_consensus,3),"horizons":horizons,"up_count":up,"down_count":dn,"state":directional_state}
