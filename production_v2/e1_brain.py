...
def analyze_e1(bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(valid) < 30:
        return {
            ...
            "reasoning_trace": ["INSUFFICIENT_DATA -> UNCLEAR"], "analysis_status": "INCOMPLETE",
    ...
    evidence = [
        ...
        f"price_slope_atr={slope_atr:.3f}", f"structure={structure}", f"directional_pressure={pressure}",
        f"recent_vs_baseline_range={recent / max(baseline, 1e-12):.3f}", f"expansion={expansion}",
        f"compression={compression}", f"sweep_high={sweep_high}", f"sweep_low={sweep_low}",
    ]
    conflicts: list[str] = []
    if conflict:
    ...
    if market_state in {"TRANSITION", "UNCLEAR"} and conflicts:
        confidence -= 0.10
    if len(valid) < 50:
        confidence -= 0.03
