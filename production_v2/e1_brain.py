...
def _pivot_structure(bars: list[dict[str, Any]], wing: int = 2) -> tuple[str, float, dict[str, int]]:
    ...
    if hh + hl >= 2 and hh + hl > lh + ll:
        return "BULLISH", 0.52, counts
    if lh + ll >= 2 and lh + ll > hh + hl:
...
def _incomplete(reason: str, evidence: list[str], conflicts: list[str]) -> dict[str, Any]:
    return {
        **_base(), "market_state": "UNCLEAR", "directional_pressure": "NEUTRAL", "trend_state": "NONE",
        "volatility_state": "UNKNOWN", "structure_state": "UNCLEAR", "structure_quality": 0.0,
        "range_state": "UNKNOWN", "compression": "UNKNOWN", "expansion": "UNKNOWN", "transition": "UNKNOWN",
        "regime_stress": "UNKNOWN", "confidence": 0.0, "evidence": evidence, "observations": evidence,
        "conflicts": conflicts, "reasons": [reason],
        ...
        "professional_reasoning": {
            ...
            "direction": "NEUTRAL", "directional_pressure": "NEUTRAL", "trend_maturity": "UNRESOLVED",
            "trend_confirmed": False, "regime_stress": False, "transition_confirmed": False,
            "conflict_detected": bool(conflicts), "conflict_count": len(conflicts), "classification_reason": reason,
            "single_counter_candle": False, "pressure_score": 0.0, "structure_alignment": 0.0, "trend_score": 0.0,
            "directional_consensus": {"confirmed": False, "score": 0.0}, "regime_basis": reason,
            "independent_evidence": {"data_quality": evidence}, "evidence_hierarchy": EVIDENCE_HIERARCHY,
...
def analyze_e1(bars: list[dict[str, Any]] | None) -> dict[str, Any]:
    """Classify market state from closed candles; never create a trade decision."""
    ...
    longer_up, longer_down = horizons[1:].count("UP"), horizons[1:].count("DOWN")

    # The 5-bar horizon is microstructure. It cannot overrule a neutral 10/20/40-bar context.
    if up == 4:
        pressure = "UP"
    elif down == 4:
        pressure = "DOWN"
    elif longer_up > longer_down:
        pressure = "UP"
    elif longer_down > longer_up:
        pressure = "DOWN"
    else:
        pressure = "BALANCED"
    ...
    # If pivots do not exist, strong persistent directional price movement is a structural proxy.
    structural_proxy = structure == "MIXED" and structure_quality <= .30 and persistence >= .75 and max(eff20, eff40) >= .22
    ...
    # Compare current ATR with the preceding volatility regime. This catches compression/expansion
    # even when the last 50 bars have already moved into the new volatility state.
    ...
    trend_score = 0.30 * consensus + 0.25 * persistence + 0.20 * structure_alignment + 0.15 * ema_alignment + 0.10 * max(eff20, eff40)
    trend_candidate = (
        pressure in {"UP", "DOWN"} and consensus >= .75 and persistence >= .50
        and structure_alignment >= .75 and ema_alignment == 1.0 and max(eff20, eff40) >= .22
    )
    ...
    transition_evidence = [label for label, ok in (
        ("CONTEXT_FLIP", context_flip), ("STRUCTURE_BREAK_PROXY", structure_break_proxy),
        ("PERSISTENT_HORIZON_FLIP", persistent_horizon_flip), ("EMA_CONTEXT_FLIP", ema_context_flip),
    ...
    if transition:
        state, reason, maturity = "TRANSITION", "persistent repricing conflicts with slower market context", "TRANSITION"
    elif trend_candidate:
        state, reason, maturity = "TREND_UP" if pressure == "UP" else "TREND_DOWN", "direction, persistence, structure, EMA context and efficiency are coherent", "ESTABLISHED"
    ...
    independent_evidence = {
        ...
        "structure": {"state": structure, "quality": round(structure_quality, 3), "alignment": round(structure_alignment, 3), "counts": pivot_counts},
    ...
    evidence = [
        f"valid_candles={len(valid)}", f"invalid_candles={invalid_count}", f"ema20_vs_ema50={ema_relation}",
        f"ema_gap_atr={ema_gap:.3f}", f"ema20_slope_atr={ema20_slope:.3f}", f"ema50_slope_atr={ema50_slope:.3f}",
        *(f"price_slope_{n}_atr={s:.3f}" for n, s in zip(lookbacks, slopes)), f"multi_horizon={','.join(horizons)}",
        f"directional_consensus={consensus:.3f}", f"persistence={persistence:.3f}", f"efficiency20={eff20:.3f}",
        f"efficiency40={eff40:.3f}", f"structure_counts={pivot_counts}", f"structure_alignment={structure_alignment:.3f}",
        f"pressure_score={pressure_score:.3f}", f"trend_score={trend_score:.3f}", f"volatility_ratio={volatility_ratio:.3f}",
        f"context_flip={context_flip}", f"transition_evidence={transition_evidence}", f"trend_candidate={trend_candidate}",
        f"regime_stress={regime_stress}", f"ema_lag_transition={ema_lag_transition}", f"single_counter_candle={single_counter_candle}",
    ]
    ...
    return {
        **_base(), "market_state": state, "directional_pressure": public_pressure,
        "trend_state": "UP" if state == "TREND_UP" else "DOWN" if state == "TREND_DOWN" else "NONE",
        "volatility_state": volatility, "structure_state": structure, "structure_quality": round(structure_quality, 3),
        "range_state": "RANGE" if range_candidate else "NOT_RANGE", "compression": "PRESENT" if compression else "ABSENT",
        "expansion": "PRESENT" if expansion else "ABSENT", "transition": "PRESENT" if transition else "ABSENT",
        "regime_stress": "PRESENT" if regime_stress else "ABSENT", "confidence": round(max(0.0, min(.99, confidence)), 3),
        "evidence": evidence, "observations": evidence, "conflicts": conflicts, "reasons": [*conflicts, *( ["REGIME_TRANSITION_CONFIRMED"] if transition else ["REGIME_STRESS_ACTIVE"] if regime_stress else ["REGIME_CONFIRMATION_INSUFFICIENT"] if state == "UNCLEAR" else [] )],
        "reasoning_trace": [
            f"QUESTION -> {QUESTION}", f"EVIDENCE_HIERARCHY -> {EVIDENCE_HIERARCHY}",
            f"STRUCTURE -> {structure} quality={structure_quality:.2f} alignment={structure_alignment:.2f}",
            f"PRESSURE -> {direction} score={pressure_score:.2f}", f"VOLATILITY -> {volatility} ratio={volatility_ratio:.2f}",
            f"PERSISTENCE -> {persistence:.2f}", f"TREND_SCORE -> {trend_score:.2f}",
            f"REGIME_CONFIRMATION -> trend_confirmed={trend_candidate} maturity={maturity}",
            f"REGIME_STRESS -> {'PRESENT' if regime_stress else 'ABSENT'}", f"STATE -> {state} because={reason}",
        ...
        "professional_reasoning": {
            ...
            "directional_pressure": public_pressure, "trend_maturity": maturity,
            "trend_confirmed": state in {"TREND_UP", "TREND_DOWN"}, "regime_stress": regime_stress,
            "transition_confirmed": transition, "conflict_detected": bool(conflicts), "conflict_count": len(conflicts),
            "classification_reason": reason, "single_counter_candle": single_counter_candle,
            "pressure_score": round(pressure_score, 3), "structure_alignment": round(structure_alignment, 3),
            "trend_score": round(trend_score, 3), "directional_consensus": directional_consensus,
            "regime_basis": regime_basis, "independent_evidence": independent_evidence,
            "evidence_hierarchy": EVIDENCE_HIERARCHY, "ownership_boundaries": OWNERSHIP,
        },
