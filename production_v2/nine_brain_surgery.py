...
def _gate_state(engine_id: str, output: dict[str, Any], active_invalidations: list[str]) -> str:
    ...
    if engine_id=="E5":
        if active & {"INVALID_TRADE_GEOMETRY","NO_USABLE_STRUCTURAL_TARGET","EFFECTIVE_SPACE_BELOW_MINIMUM"}: return "BLOCKED"
    ...
    if engine_id=="E8":
        if active & {"INVALID_TRADE_GEOMETRY","INVALID_RISK_GEOMETRY","REAL_RR_BELOW_MINIMUM","STOP_QUALITY_TOO_LOW","TARGET_REALISM_TOO_LOW","PROBABILITY_EDGE_NOT_TRUSTWORTHY","NO_USABLE_STRUCTURAL_TARGET"}: return "BLOCKED"
        return "READY" if _text(output.get("risk_state")) in {"READY","RISK_READY","TRADE_READY","VALIDATED","PASS","PASSED"} or _text(output.get("economic_state")) in {"ECONOMICALLY_ACCEPTABLE","TRADE_READY","VALIDATED"} else "PENDING"
...
def harden_engine(engine_id: str, raw_output: dict[str, Any]) -> dict[str, Any]:
    ...
    output=dict(raw_output or {}); active=_active_codes(output); future=_future_invalidations(output); active_invalidations=_explicit_active_invalidations(output)
    gate_state=_gate_state(engine_id,output,active_invalidations); lifecycle=_lifecycle(engine_id,output)
    ...
    if engine_id=="E1" and _text(output.get("analysis_status"))=="COMPLETE":
        output["data_integrity_current"]="VALIDATED"; output["data_integrity_future_failures"]=[x for x in future if "DATA" in x]
    elif engine_id=="E3" and _text(output.get("lifecycle"))=="TRANSITION": output["transition_is_not_invalidation"]=True
    elif engine_id=="E4":
        state=_text(output.get("auction_state")); response=_text(output.get("response_actor") or "UNCLEAR"); output["auction_confirmation_proven"]=state in {"CONFIRMED","TERMINALLY_CONFIRMED","RECLAIMED","ACCEPTED","REJECTED"}
        output["raw_response_actor"]=response
        output["control_actor_eligible"]=bool(output["auction_confirmation_proven"] and response in {"BUYERS","SELLERS","BUYER","SELLER"})
        if not output["control_actor_eligible"]: output["response_actor_for_control"]="UNCLEAR"
        else: output["response_actor_for_control"]=response
        if state=="PENDING": output["auction_confirmation_pending"]=True
    elif engine_id=="E2":
