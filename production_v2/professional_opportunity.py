...
This layer makes conditional opportunities visible inside E1-E9 without
turning them into execution. Each engine keeps its own authority boundary;
E9 remains the only component allowed to authorize BUY/SELL execution.
"""
...
def _direction(output: dict[str, Any]) -> str:
    for value in (output.get("direction"), output.get("structure_direction"), output.get("market_direction"), output.get("pressure"), output.get("bos_direction"), output.get("finding"), output.get("market_state")):
        x = _text(value)
        if x in DIRECTIONS or x.startswith(("BUY ", "BUY_", "BUY:")) or x in {"UP", "BULLISH", "TREND_UP"}:
            return "BUY"
...
def _stage(engine: str, direction: str, output: dict[str, Any], codes: list[str]) -> str:
    ...
    if "INVALIDATED" in state or any(x in text for x in ("INVALIDATED", "HARD_VETO")):
        return "INVALIDATED"
    if engine == "E1":
        market_state = _text(output.get("market_state") or output.get("state"))
        return "STATE_ESTABLISHED" if market_state in {"TREND_UP", "TREND_DOWN", "EXPANSION", "RANGE", "COMPRESSION", "TRANSITION"} else "STATE_WATCH"
    if engine == "E2":
        maturity = _text(output.get("opportunity_maturity"))
        return "REGIME_CONFIRMED" if maturity in {"ACTIONABLE", "CONFIRMED"} else "REGIME_DEVELOPING" if direction in DIRECTIONS else "REGIME_UNRESOLVED"
    if engine == "E3":
        lifecycle = _text(output.get("lifecycle") or output.get("structure_lifecycle"))
        return "STRUCTURE_ESTABLISHED" if lifecycle in {"ESTABLISHED", "CONFIRMED", "BOS_UP", "BOS_DOWN", "CHOCH"} else "STRUCTURE_FORMING" if direction in DIRECTIONS else "STRUCTURE_WATCH"
    if engine == "E4":
        auction = _text(output.get("auction_state") or output.get("auction_phase"))
        return "AUCTION_CONFIRMED" if auction in {"CONFIRMED", "ACCEPTED", "REJECTED", "RECLAIMED", "TERMINALLY_CONFIRMED"} else "AUCTION_PENDING" if direction in DIRECTIONS else "AUCTION_WATCH"
    if engine == "E5":
    ...
    if engine == "E6":
        return "SETUP_MATURE" if any(x in text for x in ("SETUP_CONFIRMED", "TRADE_READY")) else "SETUP_VALIDATING" if direction in DIRECTIONS else "SETUP_WATCH"
    if engine == "E7":
        confirmation = _text(output.get("confirmation_state"))
        return "CONFIRMED" if confirmation in {"PROVEN", "CONFIRMED", "VALIDATED", "TRADE_READY"} else "WAITING_CONFIRMATION" if direction in DIRECTIONS else "CONFIRMATION_WATCH"
    if engine == "E8":
        return "ECONOMICALLY_BLOCKED" if _hard_blocked(codes, output) or any(x in text for x in ("REAL_RR_BELOW_MINIMUM", "TARGET_REALISM_TOO_LOW", "PROBABILITY_EDGE_NOT_TRUSTWORTHY")) else "ECONOMICALLY_VALIDATED" if direction in DIRECTIONS else "ECONOMIC_WATCH"
...
def _conditional_paths(engine: str, direction: str, output: dict[str, Any], stage: str) -> list[str]:
    ...
    if engine == "E2":
        return ["BUY_CONTINUATION_IF_AUCTION_ACCEPTANCE_AND_FOLLOW_THROUGH_PROVE"] if direction == "BUY" else ["SELL_REVERSAL_OR_CONTINUATION_IF_REJECTION_AND_FOLLOW_THROUGH_PROVE"] if direction == "SELL" else ["WAIT_FOR_DIRECTIONAL_AUCTION_INTENT"]
    if engine == "E3":
        return ["BUY_STRUCTURE_IF_PROTECTED_LOW_HOLDS_AND_BOS_OR_HL_SEQUENCE_CONFIRMS"] if direction == "BUY" else ["SELL_STRUCTURE_IF_PROTECTED_HIGH_HOLDS_AND_BOS_OR_LH_SEQUENCE_CONFIRMS"] if direction == "SELL" else ["STRUCTURE_RESOLUTION_IF_PROTECTED_LEVEL_REACTS"]
    if engine == "E4":
    ...
    if engine == "E6":
        return [f"{direction}_{setup or 'SETUP'}_IF_SETUP_SPECIFIC_PROOF_GATES_COMPLETE" if direction in DIRECTIONS else "SETUP_CANDIDATE_IF_CAUSAL_EVIDENCE_CONVERGES"]
    if engine == "E7":
        return [f"{direction}_CONFIRMATION_IF_CLOSED_CANDLE_TRIGGER_AND_FOLLOW_THROUGH_PROVE" if direction in DIRECTIONS else "CONFIRMATION_IF_SETUP_SPECIFIC_TRIGGER_APPEARS"]
    if engine == "E8":
        return [f"{direction}_ECONOMICS_IF_STRUCTURAL_TARGET_STOP_QUALITY_AND_REAL_RR_PASS" if direction in DIRECTIONS else "ECONOMICS_IF_TRADE_GEOMETRY_BECOMES_SURVIVABLE"]
...
def _failure_conditions(engine: str, direction: str, output: dict[str, Any], codes: list[str]) -> list[str]:
    conditions: list[str] = []
    if "INVALIDATED" in " ".join(codes) or "INVALIDATED" in _text(output.get("invalidation")):
        conditions.append("CURRENT_THESIS_INVALIDATED")
...
def enrich_engine(engine: str, output: dict[str, Any], upstream: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    space_quality = _space_quality(space)
    hard_block = _hard_blocked(codes, out)
    ...
    out["professional_opportunity"] = {
        ...
        "evidence_quality": round(confidence * 100.0, 2), "space_atr": space,
        "space_quality": space_quality, "observed_evidence": codes, "counter_evidence": counter,
    ...
    out["opportunity_state"] = state
    out["opportunity_stage"] = stage
    out["opportunity_direction"] = direction
    out["opportunity_next_event"] = NEXT_EVENT.get(engine, "NEXT_CLOSED_CANDLE_UPDATE")
    out["opportunity_score"] = score
    out["opportunity_authority"] = engine
    return out
...
def consolidate(results: dict[str, Any]) -> dict[str, Any]:
    ...
    for engine in ENGINES[:-1]:
        ...
        if op.get("state") == "NO_OPPORTUNITY": continue
        radar.append({"engine": engine, "scope": op.get("scope"), "direction": op.get("direction"), "state": op.get("state"), "stage": op.get("stage"), "score": op.get("score", 0.0), "conditional_paths": op.get("conditional_paths", []), "next_required_event": op.get("next_required_event"), "trade_authorized": False})
    radar.sort(key=lambda x: (x.get("state") == "OPPORTUNITY_EXECUTABLE", x.get("score", 0.0)), reverse=True)
    buy = [x for x in radar if x.get("direction") == "BUY"]
    sell = [x for x in radar if x.get("direction") == "SELL"]
    return {"count": len(radar), "candidates": radar, "best": radar[0] if radar else None, "directional_balance": {"buy_count": len(buy), "sell_count": len(sell), "state": "BUY_DOMINANT" if len(buy) > len(sell) else "SELL_DOMINANT" if len(sell) > len(buy) else "CONFLICTED_OR_BALANCED"}, "execution_authority": "E9_ONLY", "radar_is_non_executable": True}
