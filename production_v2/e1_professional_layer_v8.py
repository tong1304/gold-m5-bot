...
def _transition(dominant:str,ema:str,structure:str,s20:float,s40:float,gap:float,recent:str,base:str)->dict[str,Any]:
    if dominant not in {"UP","DOWN"}:
        return {"score":0,"watch":base in {"WATCH","DETECTED","VALIDATED","PRESENT"},"committed":False,"evidence":[]}
    ...
    if o=="DOWN" and s20<=-MIN_SLOPE_ATR and s40<=-MIN_SLOPE_ATR: score+=1; ev.append("LONG_SLOPES_OPPOSITE")
    if o=="UP" and s20>=MIN_SLOPE_ATR and s40>=MIN_SLOPE_ATR: score+=1; ev.append("LONG_SLOPES_OPPOSITE")
    if o=="DOWN" and gap<=-MIN_GAP_ATR: score+=1; ev.append("EMA_GAP_OPPOSITE")
    if o=="UP" and gap>=MIN_GAP_ATR: score+=1; ev.append("EMA_GAP_OPPOSITE")
...
def _arb(base:str,ema:str,structure:str,s20:float,s40:float,gap:float,recent:str,base_transition:str)->dict[str,Any]:
    if ema in {"UP","DOWN"} and structure==ema:
        dominant=ema; basis="STRUCTURE_EMA_ALIGNMENT"
    elif ema in {"UP","DOWN"} and abs(gap)>=MIN_GAP_ATR and ((ema=="UP" and s20>=MIN_SLOPE_ATR and s40>=MIN_SLOPE_ATR) or (ema=="DOWN" and s20<=-MIN_SLOPE_ATR and s40<=-MIN_SLOPE_ATR)):
        dominant=ema; basis="EMA_LONG_HORIZON_ALIGNMENT"
    elif structure in {"UP","DOWN"} and ((structure=="UP" and s20>=MIN_SLOPE_ATR and s40>=MIN_SLOPE_ATR) or (structure=="DOWN" and s20<=-MIN_SLOPE_ATR and s40<=-MIN_SLOPE_ATR)):
        dominant=structure; basis="STRUCTURE_LONG_HORIZON_ALIGNMENT"
    else:
        dominant="NEUTRAL"; basis="NO_DOMINANT_CONTEXT"
...
def analyze_e1_professional_v8(bars:list[dict[str,Any]]|None)->dict[str,Any]:
    ...
    if len(clean)<50:
        out["analysis_status"]="INCOMPLETE"; out["reasons"]=[*dict.fromkeys([*(out.get("reasons") or []),"INSUFFICIENT_CANDLES_FOR_V8"])]
        return out
    closes=[float(b["close"]) for b in clean]; atr=_atr(clean); e20,e50=_ema(closes,20),_ema(closes,50); gap=(e20-e50)/max(atr,1e-12)
    ema="UP" if e20>e50 else "DOWN" if e20<e50 else "NEUTRAL"; structure=_structure_direction(clean); s20,s40=_slope(closes,atr,20),_slope(closes,atr,40); recent=_recent_pressure(clean,atr); base=str(out.get("market_state") or "UNCLEAR").upper(); bt=str(out.get("transition_status") or out.get("transition") or "NONE").upper()
    a=_arb(base,ema,structure,s20,s40,gap,recent,bt)
