...
The shared picture contains FACT only. Each brain may add only its own
INTERPRETATION and local DECISION. E9 owns the final decision.
Any contract violation is a governance blocker.
"""

ENGINE_ORDER = ("E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8", "E9")
CONTRACT_SCHEMA = "SHARED_MARKET_PICTURE_CONTRACT_V4"
_CLOSURE_KEYS = ("is_closed", "closed", "complete", "is_complete", "closed_candle", "candle_closed")
_TIME_KEYS = ("timestamp", "time")
...
def _bar_is_closed(bar: dict[str, Any]) -> bool:
    flags = [bool(bar[k]) for k in _CLOSURE_KEYS if k in bar]
    if not flags:
...
def _bar_timestamp(bar: dict[str, Any]) -> str | None:
    v = bar.get("timestamp") or bar.get("time")
    return str(v) if v is not None else None
...
def _clean_bars(bars: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in bars or []:
        if not isinstance(raw, dict):
            continue
        # Missing closure proof is a hard failure; an explicitly open candle
        # is excluded before any indicator/fact can be derived from it.
        if not _bar_is_closed(raw):
            continue
...
def _freeze_closed_candle_boundary(market_data: dict[str, Any], bars: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if cutoff_id is not None:
        ...
        identities = {_bar_identity(b): i for i, b in enumerate(bars)}
        if cutoff_id not in identities:
            raise ValueError("DATA_CUTOFF_CANDLE_NOT_FOUND")
        bars = bars[: identities[cutoff_id] + 1]
    elif cutoff_ts is not None:
        cutoff_ts = str(cutoff_ts)
        timestamped = [_bar_timestamp(b) for b in bars]
        if any(ts is None for ts in timestamped):
            raise ValueError("CUTOFF_TIMESTAMP_REQUIRES_TIMESTAMPED_CANDLES")
        bars = [b for b, ts in zip(bars, timestamped) if ts <= cutoff_ts]

    if not bars:
        raise ValueError("NO_CLOSED_CANDLE_EVIDENCE")
    if not _bar_is_closed(bars[-1]):
        raise ValueError("CLOSED_CANDLE_BOUNDARY_VIOLATION")
    last_id = _bar_identity(bars[-1])
    if last_id is None:
        raise ValueError("CANDLE_IDENTITY_REQUIRED")

    market_data["bars"] = list(bars)
    market_data["data_cutoff_candle_id"] = last_id
    market_data["data_cutoff_timestamp"] = market_data.get("data_cutoff_timestamp") or bars[-1].get("candle_close_timestamp") or _bar_timestamp(bars[-1])
...
def build_shared_market_picture(market_data: dict[str, Any]) -> dict[str, Any]:
    ...
    r20h, r20l = _range(bars, 20)
    r50h, r50l = _range(bars, 50)
    p: dict[str, Any] = {
        ...
        "data_cutoff_candle_id": _bar_identity(last),
        "data_cutoff_timestamp": market_data.get("data_cutoff_timestamp") or last.get("candle_close_timestamp") or _bar_timestamp(last),
        "closed_candle_only": True,
        "lookahead_allowed": False,
        "lookahead_detected": False,
        "candle_identity": _bar_identity(last),
        "bar_count": len(bars),
        "current": {k: last.get(k) for k in ("open", "high", "low", "close")},
        "volatility": {"atr14": atr},
        "trend_context": {"ema20": e20, "ema50": e50, "ema20_vs_ema50": "UP" if e20 > e50 else "DOWN" if e20 < e50 else "FLAT", "ema_gap_atr": (e20 - e50) / atr if atr > 0 else 0.0},
        "reference_levels": {"range20_high": r20h, "range20_low": r20l, "range50_high": r50h, "range50_low": r50l},
        "data_integrity": {"valid_ohlc_bars": len(bars), "source_bars": len(source), "sufficient_for_context": len(bars) >= 50, "all_visible_bars_closed": all(_bar_is_closed(b) for b in bars), "closure_boundary": "EXPLICIT_BAR_METADATA"},
    ...
    source_id = f"CANDLE:{p['data_cutoff_candle_id']}"
    p["fact_ledger"] = {"classification": "FACT_ONLY", "authority": "SHARED_MARKET_PICTURE", "interpretation_allowed": False, "fields": _fact_fields(p), "derivation": "DETERMINISTIC_FROM_FROZEN_CLOSED_OHLC_SNAPSHOT", "fingerprint": p["picture_id"], "source_ids": [source_id]}
    p["contract"] = {"schema": CONTRACT_SCHEMA, "picture_id": p["picture_id"], "data_cutoff_candle_id": p["data_cutoff_candle_id"], "data_cutoff_timestamp": p["data_cutoff_timestamp"], "closed_candle_only": True, "lookahead_detected": False, "fact_source_ids": [source_id], "interpretation_source_ids": [], "decision_source_ids": [], "fact_mutation": "FORBIDDEN", "interpretation_location": "BRAIN_OUTPUT_ONLY", "decision_authority": "E9_ONLY"}
    return p
...
def _validate_shared_snapshot(shared: dict[str, Any]) -> None:
    contract = shared.get("contract")
    ledger = shared.get("fact_ledger")
    if not isinstance(contract, dict) or not isinstance(ledger, dict):
        raise ValueError("SHARED_MARKET_PICTURE_CONTRACT_MISSING")
    if shared.get("closed_candle_only") is not True or shared.get("lookahead_detected") is not False:
        raise ValueError("SHARED_BOUNDARY_VIOLATION")
    if contract.get("schema") != CONTRACT_SCHEMA:
        raise ValueError("SHARED_CONTRACT_SCHEMA_MISMATCH")
    if not shared.get("data_cutoff_candle_id") or not shared.get("data_cutoff_timestamp"):
        raise ValueError("DATA_CUTOFF_PROOF_REQUIRED")
    ids = contract.get("fact_source_ids")
    if not isinstance(ids, list) or not ids or any(not isinstance(x, str) or not x for x in ids):
        raise ValueError("FACT_SOURCE_IDS_MISSING")
    if ids != ledger.get("source_ids"):
        raise ValueError("FACT_SOURCE_IDS_MISMATCH")
    if contract.get("picture_id") != shared.get("picture_id") or _picture_id(shared) != shared.get("picture_id"):
        raise ValueError("SHARED_PICTURE_ID_MISMATCH")
    if contract.get("data_cutoff_candle_id") != shared.get("data_cutoff_candle_id"):
        raise ValueError("CUTOFF_CANDLE_MISMATCH")
    if contract.get("data_cutoff_timestamp") != shared.get("data_cutoff_timestamp"):
        raise ValueError("CUTOFF_TIMESTAMP_MISMATCH")
...
def attach_brain_view(engine_id: str, output: dict[str, Any], shared: dict[str, Any]) -> dict[str, Any]:
    ...
    _validate_shared_snapshot(shared)
    ...
    result["market_picture_contract"] = {"schema": CONTRACT_SCHEMA, "picture_id": shared_copy["picture_id"], "data_cutoff_candle_id": shared_copy["data_cutoff_candle_id"], "data_cutoff_timestamp": shared_copy["data_cutoff_timestamp"], "closed_candle_only": True, "lookahead_allowed": False, "lookahead_detected": False, "fact_source_ids": list(shared_copy["contract"]["fact_source_ids"]), "interpretation_source_ids": [engine_id], "decision_source_ids": [engine_id], "fact_authority": "SHARED_MARKET_PICTURE", "fact_mutation": "FORBIDDEN", "interpretation_authority": "BRAIN_ROLE_ONLY", "decision_authority": "E9_ONLY"}
    result["evidence_audit"] = {"facts": {"source": "SHARED_MARKET_PICTURE", "picture_id": shared_copy["picture_id"], "classification": "FACT", "source_ids": list(shared_copy["contract"]["fact_source_ids"]), "fields": dict(shared_copy["fact_ledger"]["fields"])}, "interpretation": {"source": engine_id, "classification": "INTERPRETATION", "source_ids": [engine_id], "role": view["role"], **_interpretation_payload(result)}, "decision": {"source": engine_id, "classification": "DECISION", "source_ids": [engine_id], "scope": view["role"], "value": _local_decision(result), "final_authority": engine_id == "E9"}}
...
def audit_shared_market_picture_contract(outputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for e in covered:
        o = outputs[e]
        s, c, a = o.get("shared_market_picture"), o.get("market_picture_contract"), o.get("evidence_audit")
        if not isinstance(s, dict) or not isinstance(c, dict) or not isinstance(a, dict):
            missing.append(e)
            continue
        declared = c.get("picture_id")
        actual = _picture_id(s)
        pids[e] = str(declared)
        cuts[e] = str(c.get("data_cutoff_candle_id"))
        times[e] = str(c.get("data_cutoff_timestamp"))
        bv: list[str] = []
        if c.get("schema") != CONTRACT_SCHEMA:
            bv.append("SHARED_CONTRACT_SCHEMA_MISMATCH")
        if c.get("closed_candle_only") is not True:
            bv.append("CLOSED_CANDLE_CONTRACT_VIOLATION")
        if c.get("lookahead_allowed") is True or c.get("lookahead_detected") is not False:
            bv.append("LOOKAHEAD_CONTRACT_VIOLATION")
        if s.get("closed_candle_only") is not True or s.get("lookahead_detected") is not False:
            bv.append("SHARED_BOUNDARY_VIOLATION")
        if declared != s.get("picture_id") or actual != s.get("picture_id"):
            bv.append("SHARED_PICTURE_ID_MISMATCH")
        if s.get("data_cutoff_candle_id") != c.get("data_cutoff_candle_id"):
            bv.append("CUTOFF_CANDLE_MISMATCH")
        if s.get("data_cutoff_timestamp") != c.get("data_cutoff_timestamp"):
            bv.append("CUTOFF_TIMESTAMP_MISMATCH")
        ids = c.get("fact_source_ids")
        if not isinstance(ids, list) or not ids:
            bv.append("FACT_SOURCE_IDS_MISSING")
        elif ids != (s.get("contract") or {}).get("fact_source_ids"):
            bv.append("FACT_SOURCE_IDS_MISMATCH")
        for section, classification in (("facts", "FACT"), ("interpretation", "INTERPRETATION"), ("decision", "DECISION")):
            if (a.get(section) or {}).get("classification") != classification:
                bv.append(f"{classification}_CLASSIFICATION_MISSING")
        if not isinstance((c.get("interpretation_source_ids")), list) or not c.get("interpretation_source_ids"):
            bv.append("INTERPRETATION_SOURCE_IDS_MISSING")
        if not isinstance((c.get("decision_source_ids")), list) or not c.get("decision_source_ids"):
            bv.append("DECISION_SOURCE_IDS_MISSING")
        if bv:
            violating.append(e)
            mismatched.append(e)
            violations.extend(f"{e}:{x}" for x in bv)
    issues: list[str] = []
    if missing:
        issues.append("SHARED_PICTURE_CONTRACT_MISSING")
    if len(set(pids.values())) > 1:
        issues.append("MULTIPLE_SHARED_PICTURES")
    if len(set(cuts.values())) > 1:
        issues.append("MULTIPLE_DATA_CUTOFF_CANDLES")
    if len(set(times.values())) > 1:
        issues.append("MULTIPLE_DATA_CUTOFF_TIMESTAMPS")
    issues.extend(sorted(set(violations)))
    if covered and len(covered) != len(ENGINE_ORDER):
        issues.append("INCOMPLETE_BRAIN_COVERAGE")
    return {"schema": "SHARED_MARKET_PICTURE_AUDIT_V3", "passed": not issues, "covered_brains": covered, "expected_brains": list(ENGINE_ORDER), "unique_picture_ids": sorted(set(pids.values())), "unique_cutoff_candle_ids": sorted(set(cuts.values())), "unique_cutoff_timestamps": sorted(set(times.values())), "violating_brains": sorted(set(violating)), "mismatched_brains": sorted(set(mismatched)), "missing_contract_brains": missing, "issues": sorted(set(issues)), "authority": "HARD_GOVERNANCE_INPUT", "decision_authority": "E9_ONLY", "rule": "NO_BRAIN_MAY_USE_OPEN_CANDLE_OR_LOOKAHEAD_FOR_FACT_INTERPRETATION_EVIDENCE_THESIS_OR_DECISION"}
