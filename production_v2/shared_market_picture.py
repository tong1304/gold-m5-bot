...
def _bar_is_closed(bar: dict[str, Any]) -> bool:
    for key in ("is_closed", "closed", "complete", "is_complete", "closed_candle", "candle_closed"):
        if key in bar:
            return bool(bar[key])
    # The production/replay caller invokes one cycle only after a closed candle.
    # An explicit open/false flag is never accepted; an absent flag inherits the
    # caller's frozen-cycle boundary.
    return True
...
def _freeze_closed_candle_boundary(market_data: dict[str, Any], bars: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # Freeze the caller's visible dataset so every brain receives the same list.
...
def attach_brain_view(engine_id: str, output: dict[str, Any], shared: dict[str, Any]) -> dict[str, Any]:
    ...
    # Do not hand a mutable shared object to a brain. Each output gets an
    # isolated copy, while picture_id proves it is the same factual snapshot.
