...
def _trade_alert_ready(result) -> bool:
    risk = getattr(result, "risk", {}) or {}
    alert = risk.get("trade_alert") if isinstance(risk, dict) else None
    return isinstance(alert, dict) and alert.get("state") == "TRADE_ALERT" and alert.get("user_action") == "USER_ACTION_REQUIRED" and alert.get("alert_authorized_by") == "E9" and alert.get("broker_execution") is False


class LiveService:
    def __init__(self):
        self.pipeline = ProductionPipeline(); self.data = LiveMarketData(); self.interval = int(os.getenv("SIGNAL_INTERVAL_SECONDS", "60")); self.status_interval_seconds = int(os.getenv("STATUS_INTERVAL_SECONDS", "900")); self.max_candle_age_seconds = int(os.getenv("MAX_CANDLE_AGE_SECONDS", "600")); self._started = False; self._last_candle: dict[str, str] = {}; self._last_status_slot: str | None = None; self._runtime_errors: dict[str, str] = {}; self._last_prices: dict[str, float] = {}; self._latest_results: dict[str, object] = {}; self._last_no_trade_slot: str | None = None
    ...
    def _send_aligned_no_trade(self) -> None:
        ...
        if not active_results: self._last_no_trade_slot = slot; return
        if any(_trade_alert_ready(result) for result in active_results.values()): self._last_no_trade_slot = slot; return
        try: send_no_trade(dict(active_results), now); self._last_no_trade_slot = slot
    ...
    def _candle_age_seconds(candle: str) -> float | None:
        try:
            timestamp = datetime.fromisoformat(candle.replace("Z", "+00:00")); timestamp = timestamp if timestamp.tzinfo is not None else timestamp.replace(tzinfo=timezone.utc); return max(0.0, (datetime.now(timezone.utc) - timestamp).total_seconds())
        except (TypeError, ValueError): return None

    @staticmethod
    def _reasoning(engine) -> dict:
        output = engine.output or {}; reasoning = output.get("professional_reasoning") or {}; specialists = output.get("specialists") or {}
        if engine.engine_id == "E1":
            market_state = reasoning.get("market_state") or output.get("market_state") or "UNCLEAR"; volatility = reasoning.get("volatility_state") or output.get("volatility_state") or "UNKNOWN"; structure = reasoning.get("structure_state") or output.get("structure_state") or "UNCLEAR"; transition = reasoning.get("transition") or output.get("transition") or "UNKNOWN"; pressure = reasoning.get("directional_pressure") or output.get("directional_pressure") or "BALANCED"; trend_state = reasoning.get("trend_state") or output.get("trend_state") or "NONE"; conclusion = f"MARKET_STATE={market_state}; VOLATILITY={volatility}; STRUCTURE={structure}; PRESSURE={pressure}; TREND_STATE={trend_state}; TRANSITION={transition}"; observations=[]
            brain_evidence=output.get("evidence") or reasoning.get("evidence") or []; observations.extend(str(x) for x in brain_evidence if x) if isinstance(brain_evidence,(list,tuple)) else None; trace=output.get("reasoning_trace") or []; observations.extend(str(x) for x in trace if x) if isinstance(trace,(list,tuple)) else None
            ...
            if isinstance(specialists,dict):
                for item in specialists.values():
                    if isinstance(item,dict): observations.extend(str(x) for x in (item.get("observations") or []) if x)
        ...
        if engine.engine_id == "E2":
            ...
            observations.extend(f"counter_evidence={x}" for x in (output.get("counter_evidence") or []) if x); observations.extend(f"missing_evidence={x}" for x in (output.get("missing_evidence") or []) if x); reasons=list(engine.reason_codes or [])
        ...
        if engine.engine_id == "E4":
            ...
            for key in ("closed_candle_only","no_lookahead","liquidity_side","liquidity_kind","liquidity_taker","response_actor","touches","freshness","event","event_age_bars","actor_identification","auction_state","lifecycle","follow_through_bars","consecutive_confirmation_bars","true_acceptance_gate","contextual_hint_not_authority"):
        ...
        if engine.engine_id == "E5":
            observations=list(output.get("observations") or []); observations.extend(str(x) for x in (output.get("reasoning_trace") or []) if x); observations.append(f"direction={output.get('direction','NEUTRAL')}"); observations.append(f"location_quality={output.get('location_quality','UNKNOWN')}"); observations.append(f"preferred_location={output.get('preferred_location','NONE')}"); reasons=list(engine.reason_codes or [])+list(output.get("counter_evidence") or [])
        ...
        if engine.engine_id == "E6":
            observations=[]; observations.extend(str(x) for x in (output.get("supporting_evidence") or []) if x); observations.extend(f"counter_evidence={x}" for x in (output.get("counter_evidence") or []) if x); observations.extend(f"missing_proof={x}" for x in (output.get("missing_proof") or []) if x); observations.extend(str(x) for x in (output.get("reason_codes") or []) if x)
        ...
        conclusion=reasoning.get("conclusion") or output.get("analyst_conclusion") or output.get("finding") or "UNRESOLVED"; question=reasoning.get("question") or output.get("question") or output.get("specialist_question"); observations=[]; reasons=list(engine.reason_codes or [])
        if isinstance(specialists,dict):
            for item in specialists.values():
                if isinstance(item,dict): observations.extend(item.get("observations") or []); reasons.extend(item.get("reason_codes") or [])
    ...
    def _trace_result(self, alias: str, result) -> None:
        trace=_pipeline_trace_fields(result); print(f"[PRODUCTION V2] {alias} PIPELINE decision={result.decision} state={trace['state']} blocked_by={trace['blocked_by']} wait_bars={trace['wait_bars']} gate={result.gate_passed} engines={len(result.engines)}",flush=True)
        for engine in result.engines:
            if engine.engine_id=="E9":
                reasoning=engine.output.get("professional_reasoning") or {}; alert=engine.output.get("trade_alert") or {}
                print(f"[PRODUCTION V2] {alias} E9 MASTER decision={engine.output.get('decision',result.decision)} reason={engine.output.get('decision_reasons',list(engine.reason_codes))} thesis={reasoning.get('primary_thesis','UNRESOLVED')} setup={((reasoning.get('independent_setup') or reasoning.get('setup') or {}).get('state',reasoning.get('setup_state','UNKNOWN')))} execution={((reasoning.get('execution') or {}).get('state',reasoning.get('execution_state','UNKNOWN')))} alert={alert.get('state','NONE')} user_action={alert.get('user_action','NONE')} conflicts={reasoning.get('conflicts',[])} invalidations={reasoning.get('hard_invalidations',[])}",flush=True); continue
            why=self._reasoning(engine); print(f"[PRODUCTION V2] {alias} {engine.engine_id} ROLE={why['role']} question={why['question']} finding={why['conclusion']} observations={why['observations']} reasons={why['reasons']}",flush=True)
        ...
        while True:
            for alias in self.data.symbols():
                try:
                    ...
                    if self._last_candle.get(alias)==candle: print(f"[PRODUCTION V2] {alias} DUPLICATE_CANDLE candle={candle} action=SKIP_EVALUATION",flush=True); continue
                    self._last_candle[alias]=candle; print(f"[PRODUCTION V2] {alias} LSE M5 new closed candle bars={len(raw.get('bars') or [])} candle={candle}",flush=True); print(f"[PRODUCTION V2] {alias} PIPELINE_STAGE ENTER E6_E7_E8_E9 candle={candle}",flush=True)
                    result=self.pipeline.run(payload); print(f"[PRODUCTION V2] {alias} PIPELINE_STAGE EXIT E6_E7_E8_E9 candle={candle}",flush=True); self._runtime_errors.pop(alias,None); self._latest_results[alias]=result; store.record(result,self._last_prices.get(alias)); self._trace_result(alias,result)
                    if _trade_alert_ready(result):
                        send_decision(result); print(f"[PRODUCTION V2] {alias} TRADE_ALERT state=TRADE_ALERT user_action=USER_ACTION_REQUIRED direction={result.decision} opportunity_id={(result.risk.get('trade_alert') or {}).get('opportunity_id','NONE')}",flush=True)
