...
def _event(bars, z, atr, i):
    ...
    g = _geom(b)
    sweep = atr * INTERACTION_ATR
    reject = atr * REJECTION_ATR
    accept = atr * ACCEPTANCE_ATR

    if z["side"] == "HIGH":
        interacted = b["low"] <= level + reject and b["high"] >= level - reject
        swept = interacted and b["high"] > level + sweep
        rejection = swept and b["close"] <= level + reject and g["upper_wick_ratio"] >= WICK
        failed_break = interacted and prev["close"] > level + accept and b["close"] <= level + reject
        acceptance = interacted and prev["close"] <= level + reject and b["close"] > level + accept
        if failed_break:
        ...
        swept = interacted and b["low"] < level - sweep
        rejection = swept and b["close"] >= level - reject and g["lower_wick_ratio"] >= WICK
        failed_break = interacted and prev["close"] < level - accept and b["close"] >= level - reject
        acceptance = interacted and prev["close"] >= level - reject and b["close"] < level - accept
        if failed_break:
...
def _follow_through(event, bars, atr):
    ...
    if not z or i >= len(bars) - 1:
        return {
            "present": False, "bars": 0, "available_bars": 0, "required_bars": CONFIRM_BARS,
    ...
    present = not invalidated and support >= required
    ...
    return {
        ...
        "rejection_quality": base in {"REJECTION", "FAILED_BREAK_RECLAIM"} and present,
...
# Backward-compatible internal name used by existing E4 contract tests.
...
def _auction(event, bars, atr):
    ...
    elif base == "FAILED_BREAK_RECLAIM":
        state, confirmed, life = "REJECTION_PENDING", False, "PENDING"
    else:
        state, confirmed, life = "INTERACTION_PENDING", False, "PENDING"
    ...
    for eid in ("E1", "E2", "E3"):
        ...
        if o is None:
            if isinstance(p, dict):
                o = p.get("output", p.get("evidence", p))
            else:
                o = p
        text = str(o).upper()
        if any(x in text for x in ("DIRECTION=UP", "TREND_STATE=UP", "PRESSURE=BULLISH", "DIRECTION: UP", "'DIRECTION': 'UP'", '"DIRECTION": "UP"', "DIRECTION': 'BUY")):
            votes.append("UP")
        if any(x in text for x in ("DIRECTION=DOWN", "TREND_STATE=DOWN", "PRESSURE=BEARISH", "DIRECTION: DOWN", "'DIRECTION': 'DOWN'", '"DIRECTION": "DOWN"', "DIRECTION': 'SELL")):
            votes.append("DOWN")
...
def _reasoning(event, auction, counter, invalidation):
    ...
    return {
        ...
        "actor_identification": "OHLC_INFERENCE_ONLY",
        "counter_evidence": counter,
        "invalidation": invalidation,
        "context_corrobation_only": True,
        "context_used": False,
...
def analyze_e4(snapshot=None, evidence_bus=None):
    ...
    obs = [
        ...
        f"freshness={(event.get('zone') or {}).get('freshness','NONE')}", f"event={event.get('type','NONE')}", f"event_age_bars={event.get('age_bars',0)}",
        f"actor_identification={event.get('actor_evidence_type','NONE')}", f"actor_limit={event.get('actor_identification_limit','NONE')}",
        f"auction_state={auction['state']}", f"lifecycle={auction['lifecycle']}", f"follow_through_bars={auction['follow_through_bars']}",
        f"consecutive_confirmation_bars={detail.get('bars',0)}", f"required_confirmation_bars={detail.get('required_bars',CONFIRM_BARS)}", f"confirmation_horizon={detail.get('horizon_bars',0)}",
        "direction_authority=E4_AUCTION_EVIDENCE_ONLY", "upstream_direction_used_as_context_only=True", "confirmation_requires_consecutive_closed_candles=True", "newer_causal_event_supersedes_older_event=True",
    ]
    if confirmed:
        reasons = []
