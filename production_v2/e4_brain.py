...
def _follow(event,bars,atr):
    ...
    if not z or i>=len(bars)-1:return {'present':False,'bars':0,'available_bars':0,'required_bars':CONFIRM_BARS,'horizon_bars':0,'invalidated':False,'expired':False,'reason':'NO_POST_EVENT_CANDLE','checks':[]}
...
def analyze_e4(snapshot=None,evidence_bus=None):
    ...
    if len(bars)<MIN_BARS or atr<=0:
        e={'type':'LIQUIDITY_DATA_INSUFFICIENT','auction_state':'UNRESOLVED','directional_implication':'NEUTRAL','liquidity_taker':'NONE','actor_evidence_type':'NONE'};a={'state':'UNRESOLVED','confirmed':False,'detail':{}};c=['INSUFFICIENT_DATA'];inv=['new closed-candle data'];r=_reasoning(e,a,c,inv)
        return {**base,'state':'UNAVAILABLE','analysis_status':'INCOMPLETE','finding':'LIQUIDITY_DATA_INSUFFICIENT','analyst_conclusion':'LIQUIDITY_DATA_INSUFFICIENT','direction':'NEUTRAL','directional_implication':'NEUTRAL','direction_confirmed':False,'confidence':0.0,'observations':[f'closed_candles={len(bars)}',f'atr14={atr:.6f}'],'liquidity_map':{},'event':e,'auction':a,'auction_state':'UNRESOLVED','follow_through':{'present':False},'follow_through_bars':0,'auction_confirmation':{'confirmed':False},'auction_confirmation_state':'UNRESOLVED','auction_quality':'UNRESOLVED','counter_evidence':c,'invalidation':inv,'reasons':['INSUFFICIENT_CLOSED_CANDLE_DATA'],'independent_thesis':'LIQUIDITY_DATA_INSUFFICIENT -> NO_DIRECTIONAL_THESIS','professional_reasoning':r,'audit':{'closed_candle_only':True,'no_lookahead':True,'actor_identification':'PRICE_ACTION_INFERENCE_ONLY'}}
