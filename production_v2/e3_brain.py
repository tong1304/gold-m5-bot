...
ARCHITECTURE = "E3_SINGLE_PROFESSIONAL_BRAIN_V19"
...
def _label(hp,lp,atr):
    ...
    for i,p,ci in hp:
        lab="SWING_HIGH" if prev is None else "EQH" if abs(p-prev[1])<=tol else "HH" if p>prev[1] else "LH"
        hs.append({"index":i,"price":round(p,8),"label":lab,"confirmation_index":ci}); prev=(i,p)
    prev=None
    for i,p,ci in lp:
        lab="SWING_LOW" if prev is None else "EQL" if abs(p-prev[1])<=tol else "HL" if p>prev[1] else "LL"
...
def _semantic_pair(h,l):
    ...
    hh=_latest(highs,{"HH"}); lh=_latest(highs,{"LH"}); hl=_latest(lows,{"HL"}); ll=_latest(lows,{"LL"})
    up_ok=hh is not None and hl is not None and (lh is None or hh["index"]>=lh["index"]) and (ll is None or hl["index"]>=ll["index"])
    dn_ok=lh is not None and ll is not None and (hh is None or lh["index"]>=hh["index"]) and (hl is None or ll["index"]>=hl["index"])
    if up_ok and not dn_ok:return UP
    if dn_ok and not up_ok:return DOWN
    return MIXED if (up_ok or dn_ok) else NEUTRAL
...
def _protected_structure(direction,h,l):
    highs=sorted(h,key=lambda x:x["index"]); lows=sorted(l,key=lambda x:x["index"])
    if direction==UP:
        impulse=_latest(highs,{"HH"}); candidates=[x for x in lows if impulse and x["index"]<impulse["index"]]
        ideal=_latest(candidates,{"HL"}) if candidates else None; fallback=candidates[-1] if candidates else None; p=ideal
    ...
    if direction==DOWN:
        impulse=_latest(lows,{"LL"}); candidates=[x for x in highs if impulse and x["index"]<impulse["index"]]
        ideal=_latest(candidates,{"LH"}) if candidates else None; fallback=candidates[-1] if candidates else None; p=ideal
...
def _quality(bar,level,direction,atr):
    ...
    close_ok=dist>=BOS_CLOSE_ATR; displacement=body>=BOS_BODY_ATR or (loc>=BOS_CLOSE_LOCATION if direction==UP else loc<=1-BOS_CLOSE_LOCATION)
...
def _current_break(bars,highs,lows,atr,structure,scope="EXTERNAL",idx=None):
    ...
    checks=[]; hh=_latest(highs,{"HH"},idx-1); lh=_latest(highs,{"LH"},idx-1); hl=_latest(lows,{"HL"},idx-1); ll=_latest(lows,{"LL"},idx-1)
    ...
    events=[_event(bars[idx],p,d,atr,e,scope,idx) for p,d,e in checks]; events=[e for e in events if e["confirmed"]]
    ...
    for i in range(len(bars)):
        if active:
            ...
            if age>=FOLLOW_THROUGH_BARS and not active.get("accepted"):
                accepted_closes=all((bars[j]["close"]>=level+RECLAIM_MIN_ATR*atr if d==UP else bars[j]["close"]<=level-RECLAIM_MIN_ATR*atr) for j in range(active["break_candle_index"]+1,i+1))
                if accepted_closes:
...
def _sweep_reclaim(bars,highs,lows,atr,structure):
    ...
    i=len(bars)-1; found=[]
    high_candidates=[x for x in highs if x["confirmation_index"]<i and x["label"] in {"HH","LH","EQH"}]
    low_candidates=[x for x in lows if x["confirmation_index"]<i and x["label"] in {"HL","LL","EQL"}]
    if high_candidates:
        p=high_candidates[-1]; sweep=(bars[i]["high"]-p["price"])/atr; reclaim=(p["price"]-bars[i]["close"])/atr
        if sweep>=SWEEP_MIN_ATR and reclaim>=RECLAIM_MIN_ATR:found.append((reclaim,{"event":"SWEEP_RECLAIM","direction":DOWN,"confirmed":True,"closed_candle_confirmed":True,"level":p["price"],"swing_index":p["index"],"sweep_candle_index":i,"sweep_distance_atr":round(sweep,4),"reclaim_distance_atr":round(reclaim,4),"scope":"EXTERNAL","liquidity_type":"EQUAL_HIGH" if p["label"]=="EQH" else "STRUCTURAL_SWING_HIGH"}))
    if low_candidates:
        p=low_candidates[-1]; sweep=(p["price"]-bars[i]["low"])/atr; reclaim=(bars[i]["close"]-p["price"])/atr
        if sweep>=SWEEP_MIN_ATR and reclaim>=RECLAIM_MIN_ATR:found.append((reclaim,{"event":"SWEEP_RECLAIM","direction":UP,"confirmed":True,"closed_candle_confirmed":True,"level":p["price"],"swing_index":p["index"],"sweep_candle_index":i,"sweep_distance_atr":round(sweep,4),"reclaim_distance_atr":round(reclaim,4),"scope":"EXTERNAL","liquidity_type":"EQUAL_LOW" if p["label"]=="EQL" else "STRUCTURAL_SWING_LOW"}))
...
def _invalidation(bars,structure,protected):
    ...
    atr=max(_atr(bars),1e-12); ok=(structure==UP and bars[-1]["close"]<=level-RECLAIM_MIN_ATR*atr) or (structure==DOWN and bars[-1]["close"]>=level+RECLAIM_MIN_ATR*atr); p=protected.get("protected_high") if structure==DOWN else protected.get("protected_low")
    base.update({"confirmed":ok,"closed_candle_confirmed":True,"source_label":p.get("label") if p else None,"source_index":p.get("index") if p else None,"invalidates_current_external_thesis":ok}); return base
...
def _authority(ext,inte,ec,ic,bos,failure,protected,sweep,invalidation,slope=NEUTRAL,slope_quality=0.0):
    ...
    if inte==ext and ext in {UP,DOWN}:score+=0.20; support.append("INTERNAL_ALIGNS_WITH_EXTERNAL")
    elif inte in {UP,DOWN}:penalties.append("INTERNAL_COUNTER_STRUCTURE_CONTEXT_ONLY")
    if protected.get("anchor_is_ideal"):score+=0.15; support.append("PROTECTED_PRIMARY_STRUCTURE_CONFIRMED")
    elif protected.get("anchor_status")=="PROVISIONAL":penalties.append("PROTECTED_ANCHOR_PROVISIONAL_ONLY")
    ...
    if ext!=ec:penalties.append("COUNT_STATE_DESCRIPTIVE_ONLY")
    score=round(max(0,min(1,score)),4)
    return {"score":score,"level":"HIGH" if score>=0.8 else "MEDIUM" if score>=0.55 else "LOW","support":support,"penalties":penalties,"primary":"EXTERNAL_STRUCTURE_HAS_AUTHORITY; INTERNAL_STRUCTURE_IS_CONTEXT; COUNT_IS_DESCRIPTIVE; CLOSED_CANDLE_INVALIDATION_IS_DECISIVE","authority_basis":"EXTERNAL_STRUCTURE","authority_direction":ext,"decision_rule":"EXTERNAL_FIRST_INTERNAL_CONTEXT_COUNT_DESCRIPTIVE","why":f"External structure {ext} is authoritative; internal structure is contextual; only confirmed protected anchors and closed-candle events can change current authority." if ext in {UP,DOWN} else "No directional external structure is confirmed, so authority remains unresolved.","explanation":"PRIMARY=EXTERNAL_STRUCTURE; INTERNAL=CONTEXT; COUNT=DESCRIPTIVE_ONLY; HISTORICAL_EVENTS_CANNOT_OVERRIDE_CURRENT_STATE"}
...
def analyze_e3(bars):
    ...
    ihl,ill=_label(ih,il,atr); ehl,ell=_label(eh,el,atr); inte,ext=_resolve_structure(ihl,ill),_resolve_external_state(ehl,ell); ic,ec=_count(ihl,ill),_count(ehl,ell); ics,ecs=_counts(ihl,ill),_counts(ehl,ell)
    ...
    sweep=_sweep_reclaim(b,ehl,ell,atr,ext); invalidation=_invalidation(b,ext,protected); life=_lifecycle(eb,failure,history,active,len(b)-1); auth=_authority(ext,inte,ec,ic,eb,failure,protected,sweep,invalidation); state=_state(ext,inte,eb,failure,sweep,invalidation,life)
    reasons=[]
    ...
    conf=min(1.0,0.35+0.60*auth["score"]+(0.08 if eb["confirmed"] else 0.0)); conf=min(conf,0.60) if failure.get("current") or invalidation.get("confirmed") else conf
    ...
    current_state=state; historical_context=life.get("stage"); narrative=f"External={ext}; Internal={inte}; current_event={eb['event']}; lifecycle={life['stage']}; protected={protected.get('anchor_quality')}; invalidation={'CONFIRMED' if invalidation.get('confirmed') else 'NOT_CONFIRMED'}."
    trace={"external_state":ext,"internal_state":inte,"external_count_state":ec,"internal_count_state":ic,"external_bos_confirmed":eb["confirmed"],"internal_bos_confirmed":ib["confirmed"],"internal_bos_has_market_authority":False,"external_is_authority":True,"closed_candle_only":True,"protected_structure_is_invalidation_anchor":protected.get("anchor_is_ideal",False),"protected_level_break_invalidates_current_external_thesis":invalidation["confirmed"],"break_lifecycle_stage":life["stage"],"authority_explanation":auth["explanation"],"authority_basis":auth["authority_basis"],"authority_direction":auth["authority_direction"],"upstream_inputs_used":False,"upstream_direction_used":False,"upstream_decisions_used":False,"upstream_gates_used":False,"count_is_authority":False,"slope_is_structural_authority":False,"current_state":current_state,"historical_context":historical_context,"invalidation_rule":protected.get("invalidation_type"),"structure_narrative":narrative}
