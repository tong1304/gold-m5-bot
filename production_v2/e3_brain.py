...
ARCHITECTURE = 'E3_SINGLE_PROFESSIONAL_BRAIN_V13'
...
def _resolve_structure(h, l):
    """Resolve structure from chronological paired swings, not raw label counts.

    Professional rule: a directional external state needs a confirmed structural
    pair. Counts are descriptive only and can never override that pair. EQ swings
    are neutral liquidity references, not directional confirmation.
    """
    if len(h) < 2 or len(l) < 2:
        return NEUTRAL
    hh, lh = _latest(h, {'HH'}), _latest(h, {'LH'})
    hl, ll = _latest(l, {'HL'}), _latest(l, {'LL'})
    bull = hh and hl and hh['index'] > h[0]['index'] and hl['index'] > l[0]['index']
    bear = lh and ll
    if bull and bear:
        # Choose the most recent complete directional pair. This avoids MIXED
        # merely because an older opposing sequence remains visible.
        bull_time = max(hh['index'], hl['index'])
        bear_time = max(lh['index'], ll['index'])
        return UP if bull_time > bear_time else DOWN
    if bull:
        return UP
    if bear:
        return DOWN

    # One-sided continuation is allowed only when the opposite side has no
    # contradictory confirmed structural label at the same recent horizon.
    s = _directional_score(sorted(h + l, key=lambda x: x['index']))
    if s >= 2.0:
        return UP
    if s <= -2.0:
        return DOWN
...
def _protected_structure(direction, h, l):
    if direction == UP:
        ...
        return {
            ...
            'why_primary': 'Latest confirmed HL is the defended external bullish anchor; internal structure is context until that anchor is invalidated.'
        }
    if direction == DOWN:
        ...
        return {
            ...
            'why_primary': 'Latest confirmed LH is the defended external bearish anchor; internal structure is context until that anchor is invalidated.'
        }
    return {
...
def _current_break(bars, highs, lows, atr, structure, scope='EXTERNAL', idx=None):
    ...
    if idx < 1 or not bars or atr <= 0:
        return {'event': 'NO_BOS', 'direction': NEUTRAL, 'confirmed': False, 'scope': scope}
    prev_close = bars[idx - 1]['close']
    hi = _latest(highs, {'HH', 'LH', 'EQH'}, idx - 1)
    lo = _latest(lows, {'HL', 'LL', 'EQL'}, idx - 1)
    candidates = []
    if hi and bars[idx]['close'] > hi['price'] and prev_close <= hi['price']:
        q = _quality(bars[idx], hi['price'], UP, atr)
        if q['confirmed']:
            event = 'CONFIRMED_CHOCH' if structure == DOWN else 'CONFIRMED_BOS'
            candidates.append((q['distance_atr'], _event(bars[idx], hi, UP, atr, event, scope, idx)))
    if lo and bars[idx]['close'] < lo['price'] and prev_close >= lo['price']:
        q = _quality(bars[idx], lo['price'], DOWN, atr)
        if q['confirmed']:
            event = 'CONFIRMED_CHOCH' if structure == UP else 'CONFIRMED_BOS'
            candidates.append((q['distance_atr'], _event(bars[idx], lo, DOWN, atr, event, scope, idx)))
    ...
    for i in range(len(bars)):
        ...
        if e['confirmed'] and active is None:
            active = {
                ...
                'follow_through_bars': 0, 'accepted': False,
            }
    if active:
...
def _failure(bars, active, atr):
    ...
    d = active['direction']
    return {
        'event': 'FAILED_BOS', 'direction': DOWN if d == UP else UP, 'confirmed': True,
        'closed_candle_confirmed': True, 'level': active['level'],
        'break_candle_index': active['break_candle_index'],
        'failure_candle_index': active.get('failure_candle_index'), 'scope': 'EXTERNAL'
    }
...
def analyze_e3(bars):
    ...
    conf = min(1., .40 + .55 * auth['score'] + (.10 if eb['confirmed'] else 0))
    if ext == MIXED:
        conf = min(conf, .55)
    ...
    evidence = [
        ...
        f'protected_primary_direction={protected["primary_direction"]}',
        ...
        f'invalidation={invalidation["type"]}@{invalidation["level"]}',
        f'slope_context={slope}', f'slope_quality={slope_quality}',
        f'structure_authority={auth["score"]}',
        'count_state_role=DESCRIPTIVE_NOT_AUTHORITY',
        'external_authority_rule=CONFIRMED_PAIRED_SWING_STRUCTURE',
    ]
    ...
    trace = {
        ...
        'slope_context': slope, 'slope_is_structural_authority': False,
        'external_bos_confirmed': eb['confirmed'], 'internal_bos_confirmed': ib['confirmed'],
        'internal_bos_has_market_authority': False, 'external_is_authority': True,
        ...
        'break_lifecycle_stage': life['stage'], 'authority_explanation': auth['explanation'],
        'upstream_inputs_used': False,
        'structure_resolution_method': 'CHRONOLOGICAL_PAIRED_SWING_AUTHORITY_WITH_COUNT_AS_DESCRIPTIVE_ONLY',
    }
    return {
        ...
        'internal_structure': {'state': inte, 'count_state': ic, 'counts': ics},
        'external_structure': {'state': ext, 'count_state': ec, 'counts': ecs},
        'internal_count_state': ic, 'external_count_state': ec,
        'internal_counts': ics, 'external_counts': ecs,
        ...
        'atr14': round(atr, 8), 'closed_candles': len(b), 'bos': eb, 'external_bos': eb['event'],
        'internal_bos': ib['event'], 'external_bos_detail': eb, 'internal_bos_detail': ib,
        'failure': fail, 'structural_failure': fail, 'sweep_reclaim': sweep, 'break_lifecycle': life,
        'break_history': history[-5:], 'protected_structure': protected,
        ...
        'confidence': round(conf, 4), 'evidence': evidence, 'conflicts': conflicts,
