...
ARCHITECTURE = "E3_SINGLE_PROFESSIONAL_BRAIN_V16"
...
def _resolve_structure(h, l):
    points = sorted(h + l, key=lambda x: (x["index"], x["confirmation_index"]))
    d = [x for x in points if x["label"] in {"HH", "HL", "LH", "LL"}]
    if not d:
        return NEUTRAL
    for x in reversed(d):
        if x["label"] in {"HH", "HL"}:
...
def _protected_structure(direction, h, l):
    """Choose a real defended anchor from the chronological external swing map.

    Direction is not enough: the anchor must belong to the impulse that created
    the latest directional swing. If the ideal HH/HL or LH/LL pair is absent,
    keep the anchor but mark it NON_IDEAL rather than returning a false None.
    """
    highs = sorted(h, key=lambda x: x["index"])
    lows = sorted(l, key=lambda x: x["index"])
    if direction == UP:
        hh = _latest(highs, {"HH"})
        if not hh:
            return {"protected_high": None, "protected_low": None, "primary_direction": UP, "primary_level": None, "primary_label": None, "invalidation_level": None, "invalidation_type": "NO_CONFIRMED_PROTECTED_LOW", "anchor_quality": "MISSING", "why_primary": "Bullish direction exists but no confirmed HH is available to anchor the latest bullish impulse."}
        prior_lows = [x for x in lows if x["index"] < hh["index"]]
        anchor = prior_lows[-1] if prior_lows else None
        ideal = anchor if anchor and anchor["label"] == "HL" else None
        if ideal is None and anchor is None:
            return {"protected_high": hh, "protected_low": None, "primary_direction": UP, "primary_level": None, "primary_label": None, "invalidation_level": None, "invalidation_type": "NO_CONFIRMED_PROTECTED_LOW", "anchor_quality": "MISSING", "why_primary": "Latest HH exists but no preceding confirmed swing low exists inside the external map."}
        p = ideal or anchor
        return {"protected_high": hh, "protected_low": p, "primary_direction": UP, "primary_level": p["price"], "primary_label": p["label"], "invalidation_level": p["price"], "invalidation_type": "CLOSED_CANDLE_ACCEPTANCE_BELOW_PROTECTED_LOW", "anchor_quality": "IDEAL" if ideal else "NON_IDEAL", "why_primary": "Latest HH is the directional impulse; the latest preceding external low is the defended anchor. A non-HL label is context, not fabricated bullish confirmation."}
    if direction == DOWN:
        ll = _latest(lows, {"LL"})
        if not ll:
            return {"protected_high": None, "protected_low": None, "primary_direction": DOWN, "primary_level": None, "primary_label": None, "invalidation_level": None, "invalidation_type": "NO_CONFIRMED_PROTECTED_HIGH", "anchor_quality": "MISSING", "why_primary": "Bearish direction exists but no confirmed LL is available to anchor the latest bearish impulse."}
        prior_highs = [x for x in highs if x["index"] < ll["index"]]
        anchor = prior_highs[-1] if prior_highs else None
        ideal = anchor if anchor and anchor["label"] == "LH" else None
        if ideal is None and anchor is None:
            return {"protected_high": None, "protected_low": ll, "primary_direction": DOWN, "primary_level": None, "primary_label": None, "invalidation_level": None, "invalidation_type": "NO_CONFIRMED_PROTECTED_HIGH", "anchor_quality": "MISSING", "why_primary": "Latest LL exists but no preceding confirmed swing high exists inside the external map."}
        p = ideal or anchor
        return {"protected_high": p, "protected_low": ll, "primary_direction": DOWN, "primary_level": p["price"], "primary_label": p["label"], "invalidation_level": p["price"], "invalidation_type": "CLOSED_CANDLE_ACCEPTANCE_ABOVE_PROTECTED_HIGH", "anchor_quality": "IDEAL" if ideal else "NON_IDEAL", "why_primary": "Latest LL is the directional impulse; the latest preceding external high is the defended anchor. A non-LH label is context, not fabricated bearish confirmation."}
    return {"protected_high": _latest(highs, {"HH", "LH"}), "protected_low": _latest(lows, {"HL", "LL"}), "primary_direction": NEUTRAL, "primary_level": None, "primary_label": None, "invalidation_level": None, "invalidation_type": "NO_DIRECTIONAL_INVALIDATION_LEVEL", "anchor_quality": "UNRESOLVED", "why_primary": "External structure is unresolved; no directional anchor has authority."}
...
def _current_break(bars, highs, lows, atr, structure, scope="EXTERNAL", idx=None):
    ...
    checks = []
    if structure == UP:
        if hh and bars[idx]["close"] > hh["price"] and bars[idx - 1]["close"] <= hh["price"]:
            checks.append((hh, UP, "CONFIRMED_BOS"))
        if hl and bars[idx]["close"] < hl["price"] and bars[idx - 1]["close"] >= hl["price"]:
            checks.append((hl, DOWN, "CONFIRMED_CHOCH"))
    elif structure == DOWN:
        if ll and bars[idx]["close"] < ll["price"] and bars[idx - 1]["close"] >= ll["price"]:
            checks.append((ll, DOWN, "CONFIRMED_BOS"))
        if lh and bars[idx]["close"] > lh["price"] and bars[idx - 1]["close"] <= lh["price"]:
            checks.append((lh, UP, "CONFIRMED_CHOCH"))
    events = [_event(bars[idx], p, d, atr, e, scope, idx) for p, d, e in checks]
    events = [x for x in events if x["confirmed"]]
...
def _break_history(bars, highs, lows, atr, structure):
    """Replay break lifecycle without allowing historical failure to become current state."""
    events, active = [], None
    for i in range(len(bars)):
        if active:
            active["follow_through_bars"] = i - active["break_candle_index"]
            level, d = active["level"], active["direction"]
    ...
    if active:
        events.append(dict(active, status="ACCEPTED_BREAK_WITH_FOLLOW_THROUGH" if active["accepted"] else active["status"]))
...
def _failure(bars, active, atr, current_index=None):
    ...
    current_index = len(bars) - 1 if current_index is None else current_index
    fidx = active.get("failure_candle_index")
    current = fidx == current_index
    d = active["direction"]
    return {"event": "FAILED_BOS" if current else "HISTORICAL_FAILED_BOS", "direction": DOWN if d == UP else UP, "confirmed": True, "current": current, "closed_candle_confirmed": True, "level": active["level"], "break_candle_index": active["break_candle_index"], "failure_candle_index": fidx, "scope": "EXTERNAL"}
...
def _sweep_reclaim(bars, highs, lows, atr, structure):
    ...
    i = len(bars) - 1
    candidates = []
    hi, lo = _latest(highs, {"EQH"}, i - 1), _latest(lows, {"EQL"}, i - 1)
    if hi:
        s, r = (bars[i]["high"] - hi["price"]) / atr, (hi["price"] - bars[i]["close"]) / atr
        if s >= SWEEP_MIN_ATR and r >= RECLAIM_MIN_ATR:
            candidates.append((r, {"event": "SWEEP_RECLAIM", "direction": DOWN, "confirmed": True, "closed_candle_confirmed": True, "level": hi["price"], "swing_index": hi["index"], "sweep_candle_index": i, "sweep_distance_atr": round(s, 4), "reclaim_distance_atr": round(r, 4), "scope": "EXTERNAL"}))
    if lo:
        s, r = (lo["price"] - bars[i]["low"]) / atr, (bars[i]["close"] - lo["price"]) / atr
        if s >= SWEEP_MIN_ATR and r >= RECLAIM_MIN_ATR:
            candidates.append((r, {"event": "SWEEP_RECLAIM", "direction": UP, "confirmed": True, "closed_candle_confirmed": True, "level": lo["price"], "swing_index": lo["index"], "sweep_candle_index": i, "sweep_distance_atr": round(s, 4), "reclaim_distance_atr": round(r, 4), "scope": "EXTERNAL"}))
...
def _lifecycle(current, failure, history, active, last_index):
    ...
    if active:
        ...
        return {"stage": "CURRENT_BREAK_ACCEPTED" if accepted else "CURRENT_BREAK_AWAITING_FOLLOW_THROUGH", "current": True, "active": True, "accepted": accepted, "follow_through": accepted, "failure": False, "terminal": False, "age_bars": last_index - active["break_candle_index"], "follow_through_bars": active.get("follow_through_bars", 0), "level": active["level"], "break_candle_index": active["break_candle_index"], "acceptance_candle_index": active.get("acceptance_candle_index")}
    if history:
        x = history[-1]
...
def _invalidation(bars, structure, protected):
    ...
    if not bars or structure not in {UP, DOWN} or level is None:
        return {"direction": structure, "level": level, "type": protected.get("invalidation_type"), "confirmed": False, "closed_candle_confirmed": False, "source_label": protected.get("primary_label"), "source_index": None, "invalidates_current_external_thesis": False}
...
def _authority(ext, inte, ec, ic, bos, failure, protected, sweep, invalidation, slope, slope_quality):
    ...
    if protected.get("primary_level") is not None:
        score += 0.15 if protected.get("anchor_quality") == "IDEAL" else 0.07
        support.append("PROTECTED_PRIMARY_STRUCTURE_IDENTIFIED")
    else:
    ...
    return {"score": score, "level": level, "support": support, "penalties": penalties, "primary": "EXTERNAL_STRUCTURE_HAS_AUTHORITY; INTERNAL_STRUCTURE_IS_CONTEXT; COUNT_IS_DESCRIPTIVE; CLOSED_CANDLE_INVALIDATION_IS_DECISIVE", "explanation": "PRIMARY=EXTERNAL_STRUCTURE; INTERNAL=CONTEXT; COUNT=DESCRIPTIVE_ONLY; HISTORICAL_EVENTS_CANNOT_OVERRIDE_CURRENT_STATE"}
...
def _state(ext, inte, bos, failure, sweep, invalidation, life):
    ...
    if life.get("stage") == "CURRENT_BREAK_ACCEPTED":
        return "BREAK_ACCEPTED"
    if ext in {UP, DOWN} and inte == ext:
        return "CONTINUATION"
...
def analyze_e3(bars):
    ...
    slope, slope_quality = NEUTRAL, 0.0
    state = _state(ext, inte, eb, failure, sweep, invalidation, life)
    auth = _authority(ext, inte, ec, ic, eb, failure, protected, sweep, invalidation, slope, slope_quality)
    reasons = []
    if ext != ec:
    ...
    if protected.get("anchor_quality") == "NON_IDEAL":
        reasons.append("PROTECTED_ANCHOR_NON_IDEAL_BUT_REAL")
    reasons = list(dict.fromkeys(reasons + data[:8]))
    direction = NEUTRAL if invalidation["confirmed"] else failure["direction"] if failure.get("current") else eb["direction"] if eb["confirmed"] else ext if ext in {UP, DOWN} else sweep["direction"] if sweep["confirmed"] else NEUTRAL
    ...
    conf = min(1.0, 0.40 + 0.55 * auth["score"] + (0.10 if eb["confirmed"] else 0.0))
    if ext == MIXED:
        conf = min(conf, 0.55)
