...
ARCHITECTURE = "E3_SINGLE_PROFESSIONAL_BRAIN_V14"
...
def _compress(points, atr, side=None, spacing=2):
    ...
    for p in points:
        if not out or p[0] - out[-1][0] >= spacing:
            ...
            continue
        q = out[-1]
        if abs(p[1] - q[1]) <= tol or (side == "high" and p[1] > q[1]) or (side == "low" and p[1] < q[1]) or side is None:
            out[-1] = p
    return out
...
def _resolve_structure(h, l):
    """Resolve directional structure from chronological swing sequence.

    HH/HL is bullish structure; LH/LL is bearish structure. EQH/EQL are
    liquidity references and never create directional authority. Count state is
    descriptive only. The newest complete directional sequence wins; an older
    opposing sequence does not force MIXED.
    """
    points = sorted(h + l, key=lambda x: (x["index"], x["confirmation_index"]))
    directional = [x for x in points if x["label"] in {"HH", "HL", "LH", "LL"}]
    if not directional:
        return NEUTRAL

    # Evaluate complete pairs from newest backwards. This is deliberately
    # sequence-based rather than a raw count vote.
    highs = [x for x in h if x["label"] in {"HH", "LH"}]
    lows = [x for x in l if x["label"] in {"HL", "LL"}]
    candidates = []
    for hi in highs[-6:]:
        for lo in lows[-6:]:
            if hi["index"] == lo["index"]:
                continue
            if hi["label"] == "HH" and lo["label"] == "HL":
                candidates.append((max(hi["index"], lo["index"]), UP))
            elif hi["label"] == "LH" and lo["label"] == "LL":
                candidates.append((max(hi["index"], lo["index"]), DOWN))
    if candidates:
        return max(candidates, key=lambda x: x[0])[1]

    # One-sided sequence: two recent same-direction structural labels are enough
    # to maintain the directional thesis until the opposite protected anchor is
    # actually broken on a closed candle.
    recent = directional[-4:]
...
def _protected_structure(direction, h, l):
    if direction == UP:
        ...
        return {
            ...
            "why_primary": "Latest confirmed HL is the defended external bullish anchor. Internal structure and liquidity references cannot invalidate it without a closed-candle break.",
        }
    if direction == DOWN:
        primary = _latest(h, {"LH"})
        secondary = _latest(l, {"LL"})
        return {
            ...
            "why_primary": "Latest confirmed LH is the defended external bearish anchor. Internal structure and liquidity references cannot invalidate it without a closed-candle break.",
        }
    return {
        "protected_high": _latest(h, {"HH", "LH"}),
        "protected_low": _latest(l, {"HL", "LL"}),
        "primary_direction": NEUTRAL,
        "primary_level": None,
        "primary_label": None,
        "invalidation_level": None,
        "invalidation_type": "NO_DIRECTIONAL_INVALIDATION_LEVEL",
        "why_primary": "External structure is unresolved; no directional thesis receives protected-level authority.",
    }
...
def _quality(bar, level, direction, atr):
    ...
    close_quality = loc >= BOS_CLOSE_LOCATION if direction == UP else loc <= 1 - BOS_CLOSE_LOCATION
    return {
        "confirmed": dist >= BOS_CLOSE_ATR and (displacement or close_quality),
        "distance_atr": round(max(0, dist), 4),
...
def _current_break(bars, highs, lows, atr, structure, scope="EXTERNAL", idx=None):
    ...
    if idx < 1 or atr <= 0:
        return {"event": "NO_BOS", "direction": NEUTRAL, "confirmed": False, "scope": scope}
    # Only true structural pivots can generate BOS/CHOCH. EQH/EQL are excluded.
    hi_hh = _latest(highs, {"HH"}, idx - 1)
    hi_lh = _latest(highs, {"LH"}, idx - 1)
    lo_hl = _latest(lows, {"HL"}, idx - 1)
    lo_ll = _latest(lows, {"LL"}, idx - 1)
    candidates = []
    checks = []
    if structure == UP:
        if hi_hh and bars[idx]["close"] > hi_hh["price"] and bars[idx - 1]["close"] <= hi_hh["price"]:
            checks.append((hi_hh, UP, "CONFIRMED_BOS"))
        if lo_hl and bars[idx]["close"] < lo_hl["price"] and bars[idx - 1]["close"] >= lo_hl["price"]:
            checks.append((lo_hl, DOWN, "CONFIRMED_CHOCH"))
    elif structure == DOWN:
        if lo_ll and bars[idx]["close"] < lo_ll["price"] and bars[idx - 1]["close"] >= lo_ll["price"]:
            checks.append((lo_ll, DOWN, "CONFIRMED_BOS"))
        if hi_lh and bars[idx]["close"] > hi_lh["price"] and bars[idx - 1]["close"] <= hi_lh["price"]:
            checks.append((hi_lh, UP, "CONFIRMED_CHOCH"))
    for pivot, direction, event in checks:
        q = _quality(bars[idx], pivot["price"], direction, atr)
        if q["confirmed"]:
            candidates.append((q["distance_atr"], _event(bars[idx], pivot, direction, atr, event, scope, idx)))
...
def _sweep_failure(bars, highs, lows, atr, prior_structure=NEUTRAL):
    """Compatibility helper: real BOS followed by a closed-candle reclaim."""
    if not bars or atr <= 0:
        return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
    if prior_structure == UP:
        pivot = _latest(highs, {"HH"})
        direction = UP
    elif prior_structure == DOWN:
        pivot = _latest(lows, {"LL"})
        direction = DOWN
    else:
        return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
    if not pivot or len(bars) < 3:
        return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
    break_idx = None
    for i in range(1, len(bars)):
        if direction == UP and bars[i]["close"] > pivot["price"] + BOS_CLOSE_ATR * atr and bars[i - 1]["close"] <= pivot["price"]:
            break_idx = i
            break
        if direction == DOWN and bars[i]["close"] < pivot["price"] - BOS_CLOSE_ATR * atr and bars[i - 1]["close"] >= pivot["price"]:
            break_idx = i
            break
    if break_idx is None:
        return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
    for i in range(break_idx + 1, len(bars)):
        reclaimed = (direction == UP and bars[i]["close"] <= pivot["price"] - RECLAIM_MIN_ATR * atr) or (direction == DOWN and bars[i]["close"] >= pivot["price"] + RECLAIM_MIN_ATR * atr)
        if reclaimed:
            return {"event": "FAILED_BOS", "direction": DOWN if direction == UP else UP, "confirmed": True, "closed_candle_confirmed": True, "level": pivot["price"], "break_candle_index": break_idx, "failure_candle_index": i, "scope": "EXTERNAL"}
    return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
...
def _break_history(bars, highs, lows, atr, structure):
    ...
    if active:
        events.append(dict(active, status="ACCEPTED_BREAK_WITH_FOLLOW_THROUGH" if active["accepted"] else "BREAK_CONFIRMED_AWAITING_FOLLOW_THROUGH"))
...
def _failure(bars, active, atr):
    if not active or active.get("status") != "FAILED_BREAK_RECLAIMED":
        return {"event": "NO_FAILURE", "direction": NEUTRAL, "confirmed": False}
    d = active["direction"]
    return {"event": "FAILED_BOS", "direction": DOWN if d == UP else UP, "confirmed": True, "closed_candle_confirmed": True, "level": active["level"], "break_candle_index": active["break_candle_index"], "failure_candle_index": active.get("failure_candle_index"), "scope": "EXTERNAL"}
...
def _invalidation(bars, structure, protected):
    ...
    if not bars or structure not in {UP, DOWN} or level is None:
        return {"direction": structure, "level": level, "type": protected.get("invalidation_type", "NO_DIRECTIONAL_INVALIDATION_LEVEL"), "confirmed": False, "closed_candle_confirmed": False, "source_label": protected.get("primary_label"), "source_index": None, "invalidates_current_external_thesis": False}
...
def _authority(ext, inte, ec, ic, bos, failure, protected, sweep, invalidation, slope, slope_quality):
    ...
    return {"score": score, "level": level, "support": support, "penalties": penalties, "primary": "EXTERNAL_STRUCTURE_HAS_AUTHORITY; INTERNAL_STRUCTURE_IS_CONTEXT; COUNT_IS_DESCRIPTIVE; CLOSED_CANDLE_INVALIDATION_IS_DECISIVE", "explanation": "PRIMARY=EXTERNAL_STRUCTURE; support=" + ",".join(support) + "; penalties=" + ",".join(penalties)}
...
def analyze_e3(bars):
    ...
    inte = _resolve_structure(ihl, ill)
    ext = _resolve_structure(ehl, ell)
    ...
    for x in reversed(history):
        if x.get("status") == "FAILED_BREAK_RECLAIMED":
            fail = {"event": "FAILED_BOS", "direction": DOWN if x["direction"] == UP else UP, "confirmed": True, "closed_candle_confirmed": True, "level": x["level"], "break_candle_index": x["break_candle_index"], "failure_candle_index": x.get("failure_candle_index"), "scope": "EXTERNAL"}
            break
    ...
    state = _state(ext, inte, eb, fail, sweep, invalidation, life)
    ...
    if ext == MIXED:
        reasons.append("EXTERNAL_STRUCTURE_UNRESOLVED")
    ...
    elif ext in {UP, DOWN} and inte in {UP, DOWN} and inte != ext:
    ...
    conf = min(1.0, .40 + .55 * auth["score"] + (.10 if eb["confirmed"] else 0))
    if ext == MIXED:
        conf = min(conf, .55)
    if fail["confirmed"] or invalidation["confirmed"]:
        conf = min(conf, .60)
