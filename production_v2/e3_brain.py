...
ARCHITECTURE = "E3_SINGLE_PROFESSIONAL_BRAIN_V30"
...
def _semantic_structure_state(highs, lows):
    highs = sorted(highs, key=lambda x: x["index"])
    lows = sorted(lows, key=lambda x: x["index"])
    hh, lh = _latest(highs, {"HH"}), _latest(highs, {"LH"})
    hl, ll = _latest(lows, {"HL"}), _latest(lows, {"LL"})
    bullish_pair = bool(hh and hl)
    bearish_pair = bool(lh and ll)
    bull_recency = bullish_pair and (not lh or hh["index"] >= lh["index"])
    bear_recency = bearish_pair and (not hh or lh["index"] >= hh["index"])
    if bull_recency and not bear_recency:
        state = UP
    elif bear_recency and not bull_recency:
        state = DOWN
    elif bull_recency or bear_recency:
        state = MIXED
    else:
        state = NEUTRAL
    return {
        "state": state,
        "basis": "ORDERED_SWING_SEQUENCE",
        ...
        "semantic_rule": "LATEST_DIRECTIONAL_SWING_PAIR_AND_RECENCY",
    }
...
def _protected_structure(direction, h, l):
    ...
    if direction == UP:
        ...
        candidates = [x for x in lows if impulse and x["index"] < impulse["index"]]
        ideal = _latest(candidates, {"HL"}) if candidates else None
        fallback = candidates[-1] if candidates else None
        anchor = ideal or fallback
        return {
            ...
            "anchor_quality": "IDEAL" if ideal else "PROVISIONAL" if fallback else "MISSING",
            "anchor_status": "ACTIVE" if ideal else "PROVISIONAL" if fallback else "MISSING",
            "anchor_index": anchor["index"] if anchor else None,
            "anchor_price": anchor["price"] if anchor else None,
            "anchor_is_ideal": bool(ideal),
            "candidate_anchor": fallback,
            "why_primary": "Latest HH defines the bullish impulse; a preceding confirmed HL is the ideal protected low.",
        }
    if direction == DOWN:
        impulse = _latest(lows, {"LL"})
        candidates = [x for x in highs if impulse and x["index"] < impulse["index"]]
        ideal = _latest(candidates, {"LH"}) if candidates else None
        fallback = candidates[-1] if candidates else None
        anchor = ideal or fallback
        return {
            ...
            "anchor_is_ideal": bool(ideal),
            "candidate_anchor": fallback,
            "why_primary": "Latest LL defines the bearish impulse; a preceding confirmed LH is the ideal protected high.",
        }
    return {
...
def _quality(bar, level, direction, atr):
    ...
    return {
        "confirmed": bool(close_ok and displacement),
...
def _structure_at(highs, lows, idx):
    return _resolve_structure(
        [x for x in highs if x["confirmation_index"] <= idx],
        [x for x in lows if x["confirmation_index"] <= idx],
    )
    ...
    for i in range(len(bars)):
        if active:
            ...
            if age >= FOLLOW_THROUGH_BARS and not active.get("accepted"):
                ...
                closes_ok = all(
                    bars[j]["close"] >= level + RECLAIM_MIN_ATR * atr if d == UP else bars[j]["close"] <= level - RECLAIM_MIN_ATR * atr
...
def _failure(bars, active, atr, current_index=None):
    ...
    return {
        "event": "FAILED_BOS" if current else "HISTORICAL_FAILED_BOS",
        ...
        "failure_candle_index": active.get("failure_candle_index"),
        "scope": "EXTERNAL",
    }
...
def _sweep_reclaim(bars, highs, lows, atr, structure):
    ...
    found = []
    candidates = [
        (_latest(highs, {"HH", "LH", "EQH"}, i - 1), DOWN, "high", "STRUCTURAL_SWING_HIGH"),
        (_latest(lows, {"HL", "LL", "EQL"}, i - 1), UP, "low", "STRUCTURAL_SWING_LOW"),
    ]
    for p, direction, side, liquidity_type in candidates:
        ...
        if sweep >= SWEEP_MIN_ATR and reclaim >= RECLAIM_MIN_ATR:
            found.append((reclaim, {
                ...
                "scope": "EXTERNAL",
                "liquidity_type": "EQUAL_HIGH" if p["label"] == "EQH" else "EQUAL_LOW" if p["label"] == "EQL" else liquidity_type,
            }))
...
def _lifecycle(current, failure, history, active, last_index):
    ...
    if active:
        ...
        return {"stage": "CURRENT_BREAK_ACCEPTED" if accepted else "CURRENT_BREAK_AWAITING_FOLLOW_THROUGH", "current": True, "active": True, "accepted": accepted, "follow_through": accepted, "failure": False, "terminal": False, "age_bars": age, "follow_through_bars": active.get("follow_through_bars", 0), "level": active["level"], "break_candle_index": active["break_candle_index"], "acceptance_candle_index": active.get("acceptance_candle_index")}
    if history:
        x = history[-1]
        failed = x.get("status") == "FAILED_BREAK_RECLAIMED"
        accepted = x.get("status") == "ACCEPTED_BREAK_WITH_FOLLOW_THROUGH"
        return {"stage": "HISTORICAL_FAILED_BREAK" if failed else "HISTORICAL_ACCEPTED_BREAK" if accepted else "HISTORICAL_BREAK", "current": False, "active": False, "accepted": accepted, "follow_through": accepted, "failure": failed, "terminal": True, "age_bars": last_index - x["break_candle_index"], "follow_through_bars": x.get("follow_through_bars", 0), "level": x["level"], "break_candle_index": x["break_candle_index"], "failure_candle_index": x.get("failure_candle_index"), "acceptance_candle_index": x.get("acceptance_candle_index")}
...
def _structure_authority(external, internal, protected, current_event, invalidation):
    ...
    if invalidation.get("confirmed"):
        return {"authority": "NONE", "direction": NEUTRAL, "internal_role": "CONTEXT_ONLY", "count_role": "DESCRIPTIVE_ONLY", "reason": "PROTECTED_STRUCTURE_INVALIDATED", "score": 0.0, "internal_state": inte}
    if ext in {UP, DOWN}:
        score = 0.55 + (0.25 if protected.get("anchor_status") == "ACTIVE" else 0.0) + (0.15 if current_event.get("confirmed") else 0.0)
        return {"authority": "EXTERNAL", "direction": ext, "internal_role": "CONTEXT_ONLY", "count_role": "DESCRIPTIVE_ONLY", "reason": "EXTERNAL_STRUCTURE_HAS_PRIMARY_AUTHORITY", "score": round(min(1.0, score), 4), "internal_state": inte}
    return {"authority": "NONE", "direction": NEUTRAL, "internal_role": "CONTEXT_ONLY", "count_role": "DESCRIPTIVE_ONLY", "reason": "NO_CONFIRMED_EXTERNAL_STRUCTURE", "score": 0.0, "internal_state": inte}
...
def analyze_e3(bars):
    ...
    if protected.get("anchor_quality") == "PROVISIONAL":
        reasons.append("PROTECTED_ANCHOR_PROVISIONAL_BUT_NOT_AUTHORIZED")
    if ext == MIXED:
    ...
    if life.get("stage") in {"HISTORICAL_ACCEPTED_BREAK", "HISTORICAL_FAILED_BREAK"}:
        conflicts.append("HISTORICAL_EVENT_NOT_CURRENT_EVENT")
    if ext in {UP, DOWN} and inte in {UP, DOWN} and ext != inte:
        finding = f"{ext}_STRUCTURE_WITH_INTERNAL_CONFLICT"
    elif invalidation.get("confirmed"):
    ...
    confidence = min(1.0, 0.35 + 0.60 * auth["score"] + (0.08 if eb["confirmed"] else 0.0))
    ...
    narrative = f"External={ext} from ordered swing semantics; Internal={inte} from ordered swing semantics; current_event={eb['event']}; lifecycle={life['stage']}; protected={protected.get('anchor_quality')}; authority={auth['authority']}; invalidation={'CONFIRMED' if invalidation.get('confirmed') else 'NOT_CONFIRMED'}. Counts are descriptive only; internal structure cannot override external authority; historical events cannot override the current thesis."
