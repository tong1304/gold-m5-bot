...
ARCHITECTURE = "E3_SINGLE_PROFESSIONAL_BRAIN_V17"
...
def _protected_structure(direction,h,l):
    ...
    if direction==UP:
        ...
        p=ideal or anchor
        return {"protected_high":impulse,"protected_low":p,"primary_direction":UP,"primary_level":p["price"] if p else None,"primary_label":p["label"] if p else None,"invalidation_level":p["price"] if p else None,"invalidation_type":"CLOSED_CANDLE_ACCEPTANCE_BELOW_PROTECTED_LOW","anchor_quality":"IDEAL" if ideal else "NON_IDEAL" if p else "MISSING","why_primary":"Latest HH defines the bullish impulse; the latest preceding confirmed external low is the defended anchor."}
    if direction==DOWN:
...
def _current_break(bars,highs,lows,atr,structure,scope="EXTERNAL",idx=None):
    ...
    checks=[]
    hh=_latest(highs,{"HH"},idx-1); lh=_latest(highs,{"LH"},idx-1); hl=_latest(lows,{"HL"},idx-1); ll=_latest(lows,{"LL"},idx-1)
    if structure==UP:
...
def _break_history(bars,highs,lows,atr,structure):
    ...
    for i in range(len(bars)):
        if active:
            ...
            d=active["direction"]; level=active["level"]
            ...
            if age>=FOLLOW_THROUGH_BARS: active["accepted"]=True; active["acceptance_candle_index"]=i; active["status"]="ACCEPTED_BREAK_WITH_FOLLOW_THROUGH"
...
def _failure(bars,active,atr,current_index=None):
    ...
    i=len(bars)-1 if current_index is None else current_index; current=active.get("failure_candle_index")==i; d=active["direction"]
...
def _sweep_reclaim(bars,highs,lows,atr,structure):
    ...
    i=len(bars)-1; found=[]; hi=_latest(highs,{"EQH"},i-1); lo=_latest(lows,{"EQL"},i-1)
    if hi:
...
def _lifecycle(current,failure,history,active,last_index):
    ...
    if active:
        ...
        if age==0:
            return {"stage":"CURRENT_BREAK_AWAITING_FOLLOW_THROUGH","current":True,"active":True,"accepted":False,"follow_through":False,"failure":False,"terminal":False,"age_bars":0,"follow_through_bars":0,"level":active["level"],"break_candle_index":active["break_candle_index"]}
        return {"stage":"HISTORICAL_ACCEPTED_BREAK" if accepted else "HISTORICAL_BREAK_AWAITING_FOLLOW_THROUGH","current":False,"active":False,"accepted":accepted,"follow_through":accepted,"failure":False,"terminal":True,"age_bars":age,"follow_through_bars":active.get("follow_through_bars",0),"level":active["level"],"break_candle_index":active["break_candle_index"],"acceptance_candle_index":active.get("acceptance_candle_index")}
    if history:
...
def _authority(ext,inte,ec,ic,bos,failure,protected,sweep,invalidation,slope,slope_quality):
    score=0.35 if ext in {UP,DOWN} else 0.0; support=[f"EXTERNAL_{ext}_PRIMARY"] if ext in {UP,DOWN} else []; penalties=[]
    ...
    if protected.get("primary_level") is not None: score+=0.15 if protected.get("anchor_quality")=="IDEAL" else 0.07; support.append("PROTECTED_PRIMARY_STRUCTURE_IDENTIFIED")
    ...
    penalties.append("COUNT_STATE_DESCRIPTIVE_ONLY"); score=round(max(0,min(1,score)),4)
...
def analyze_e3(bars):
    ...
    ihl,ill=_label(ih,il,atr); ehl,ell=_label(eh,el,atr); inte=_resolve_structure(ihl,ill); ext=_resolve_external_state(ehl,ell); ic,ec=_count(ihl,ill),_count(ehl,ell); ics,ecs=_counts(ihl,ill),_counts(ehl,ell)
    ...
    sweep=_sweep_reclaim(b,ehl,ell,atr,ext); invalidation=_invalidation(b,ext,protected); life=_lifecycle(eb,failure,history,active,len(b)-1); auth=_authority(ext,inte,ec,ic,eb,failure,protected,sweep,invalidation,NEUTRAL,0.0); state=_state(ext,inte,eb,failure,sweep,invalidation,life)
    reasons=[]
...
__all__=["analyze_e3","_compress","_bos","_sweep_failure","_current_break","_break_history","_failure","_sweep_reclaim","_state","_resolve_external_state","_protected_structure","_authority","_lifecycle"]
