...
def _semantic_structure_state(highs, lows):
    ...
    for x in events:
        ...
        if label in {"HH", "LH"}:
            last_high = x
            if last_low:
                if label == "HH" and last_low["label"] == "HL": state = UP
                elif label == "LH" and last_low["label"] == "LL": state = DOWN
                else: state = MIXED
        else:
            last_low = x
            if last_high:
                if label == "HL" and last_high["label"] == "HH": state = UP
                elif label == "LL" and last_high["label"] == "LH": state = DOWN
                else: state = MIXED
        transitions.append({"index": x["index"], "label": label, "state_after": state})
    return {"state": state, "basis": "ORDERED_SWING_RELATIONSHIPS", "counts_used_as_authority": False,
            "latest_directional_event": events[-1] if events else None, "latest_hh": _latest(highs, {"HH"}),
            "latest_hl": _latest(lows, {"HL"}), "latest_lh": _latest(highs, {"LH"}), "latest_ll": _latest(lows, {"LL"}),
            ...
            "structural_sequence": "→".join(x["label"] for x in events[-12:]), "transitions": transitions[-12:],
            "semantic_rule": "ORDERED_SWINGS_ONLY;_COUNTS_DESCRIPTIVE_ONLY;_STALE_PAIRS_CANNOT_OVERRIDE_NEWER_LEG"}
...
def _quality(bar, level, direction, atr):
    ...
    rng = max(bar["high"] - bar["low"], 1e-12); body = abs(bar["close"] - bar["open"]) / atr; location = (bar["close"] - bar["low"]) / rng
...
def _event(bar, pivot, direction, atr, event, scope, idx):
    ...
    return {"event": event, "direction": direction, "confirmed": True, "scope": scope, "level": pivot["price"], "swing_index": pivot["index"], "swing_label": pivot["label"], "break_candle_index": idx, "closed_candle_confirmed": True, **q} if q["confirmed"] else {"event": "NO_BOS", "direction": NEUTRAL, "confirmed": False, "scope": scope, "closed_candle_confirmed": True, **q}
...
def _current_break(bars, highs, lows, atr, structure, scope="EXTERNAL", idx=None):
    ...
    hh, lh = _latest(highs, {"HH"}, idx - 1), _latest(highs, {"LH"}, idx - 1)
    hl, ll = _latest(lows, {"HL"}, idx - 1), _latest(lows, {"LL"}, idx - 1)
...
def _break_history(bars, highs, lows, atr, structure):
    ...
    for e in events:
        ...
        if failure_idx is not None or e["break_candle_index"] < len(bars)-1: history.append(x); active = None
        else: active = x
...
def _invalidation(bars, structure, protected):
    ...
    base.update({"confirmed":broken,"closed_candle_confirmed":True,"invalidates_current_external_thesis":broken}); return base
...
def _structure_authority(external, internal, protected, current_event, invalidation):
    ...
    if invalidation.get("confirmed") or ext not in {UP,DOWN} or protected.get("anchor_status")!="ACTIVE":return {"authority":"NONE","direction":NEUTRAL,"internal_role":"CONTEXT_ONLY","count_role":"DESCRIPTIVE_ONLY","reason":"PROTECTED_STRUCTURE_INVALIDATED" if invalidation.get("confirmed") else "NO_VALID_EXTERNAL_AUTHORITY","score":0.0,"internal_state":inte,"authority_is_actionable":False}
    return {"authority":"EXTERNAL","direction":ext,"internal_role":"CONTEXT_ONLY","count_role":"DESCRIPTIVE_ONLY","reason":"EXTERNAL_ORDERED_STRUCTURE_HAS_PRIMARY_AUTHORITY","score":round(min(1.0,0.80+0.15*bool(current_event.get("confirmed"))),4),"internal_state":inte,"authority_is_actionable":True}
...
def analyze_e3(bars):
    ...
    if ext in {UP,DOWN} and inte in {UP,DOWN} and ext!=inte:reasons.append("INTERNAL_STRUCTURE_CONFLICT");conflicts.append("INTERNAL_VS_EXTERNAL_STRUCTURE")
    ...
    if current_liquidity["current"]:reasons.append("CURRENT_LIQUIDITY_EVENT_CONFIRMED")
