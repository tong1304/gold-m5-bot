...
def _clean(bars):
    ...
    for i,bar in enumerate(bars or []):
        ...
        if any(x is None for x in vals): reasons.append(f"bar_{i}_ohlc_invalid"); continue
        o,h,l,c=vals
...
def _tr(b,i):
    if i<=0:return b[i]["high"]-b[i]["low"] if b else 0.0
...
def _atr(b,p=14,end=None):
    ...
    if end<0:return 0.0
    vals=[_tr(b,i) for i in range(max(1,end-p+1),end+1)]
...
def _pivot_records(raw,current_index):
    ...
    for item in raw or []:
        ...
        try:i,p,ci=int(item[0]),float(item[1]),int(item[2])
        except (TypeError,ValueError):continue
        if ci<=current_index:out.append({"index":i,"price":round(p,8),"confirmation_index":ci,"status":"CONFIRMED"})
    return out
...
def _semantic_structure_state(highs,lows):
    ...
    state=NEUTRAL;lh=ll=None;trans=[]
    for x in events:
        if x["label"] in {"HH","LH"}:lh=x
        else:ll=x
        if lh and ll:
            state=UP if lh["label"]=="HH" and ll["label"]=="HL" else DOWN if lh["label"]=="LH" and ll["label"]=="LL" else MIXED
        trans.append({"index":x["index"],"label":x["label"],"state_after":state})
    return {"state":state,"basis":"ORDERED_CAUSAL_SWING_RELATIONSHIPS","counts_used_as_authority":False,"latest_directional_event":events[-1] if events else None,"latest_hh":_latest(highs,{"HH"}),"latest_hl":_latest(lows,{"HL"}),"latest_lh":_latest(highs,{"LH"}),"latest_ll":_latest(lows,{"LL"}),"bullish_pair":bool(lh and ll and lh["label"]=="HH" and ll["label"]=="HL"),"bearish_pair":bool(lh and ll and lh["label"]=="LH" and ll["label"]=="LL"),"structural_sequence":"→".join(x["label"] for x in events[-12:]),"transitions":trans[-12:],"semantic_rule":"ORDERED_SWINGS_ONLY;CONFIRMED_PIVOTS_ONLY;COUNTS_DESCRIPTIVE_ONLY;NEWER_CONFIRMED_LEG_HAS_AUTHORITY"}

def _protected_structure(highs,lows,state):
    if state==UP:return {"protected_high":_latest(highs,{"HH"}),"protected_low":_latest(lows,{"HL"}),"logic":"latest_confirmed_HL_protects_bullish_structure"}
    if state==DOWN:return {"protected_high":_latest(highs,{"LH"}),"protected_low":_latest(lows,{"LL"}),"logic":"latest_confirmed_LH_protects_bearish_structure"}
...
def _current_break(bars,highs,lows,atr,structure,scope="EXTERNAL",idx=None):
    if not bars:return {"event":"NO_BREAK","direction":NEUTRAL,"confirmed":False,"scope":scope}
    i=len(bars)-1 if idx is None else idx
    if i<0 or i>=len(bars):return {"event":"NO_BREAK","direction":NEUTRAL,"confirmed":False,"scope":scope}
    hs=[x for x in highs if x["confirmation_index"]<=i-1];ls=[x for x in lows if x["confirmation_index"]<=i-1]
    candidates=[];high=_latest(hs,{"HH","LH"});low=_latest(ls,{"HL","LL"})
    if high and bars[i]["close"]>high["price"]:candidates.append({"event":"BOS_UP","direction":UP,"level":high["price"],"structure_index":high["index"]})
    if low and bars[i]["close"]<low["price"]:candidates.append({"event":"BOS_DOWN","direction":DOWN,"level":low["price"],"structure_index":low["index"]})
    if not candidates:return {"event":"NO_BREAK","direction":NEUTRAL,"confirmed":False,"closed_candle_confirmed":True,"scope":scope}
    x=max(candidates,key=lambda z:abs(bars[i]["close"]-z["level"]));prior=structure.get("state",NEUTRAL);choch=(prior==DOWN and x["direction"]==UP) or (prior==UP and x["direction"]==DOWN)
...
def _sweep_reclaim(bars,highs,lows,atr):
    ...
    i=len(bars)-1;found=[]
    for p,d,side in [(_latest(highs,{"HH","LH","EQH"}),DOWN,"high"),(_latest(lows,{"HL","LL","EQL"}),UP,"low")]:
...
def analyze_e3(bars):
    ...
    current=len(clean)-1;atr=_atr(clean)
    highs,lows=_label(_compress(_pivot_records(_pivots(clean,"high",ER),current),atr),_compress(_pivot_records(_pivots(clean,"low",ER),current),atr),atr)
    ih,il=_label(_compress(_pivot_records(_pivots(clean,"high",IR),current),atr),_compress(_pivot_records(_pivots(clean,"low",IR),current),atr),atr)
    external=_semantic_structure_state(highs,lows);internal=_semantic_structure_state(ih,il);protected=_protected_structure(highs,lows,external["state"])
    bos=_current_break(clean,highs,lows,atr,external,"EXTERNAL",current);failure=_failure_from_break(bos,clean,atr);sweep=_sweep_reclaim(clean,highs,lows,atr)
    ...
    return {"engine":"E3","architecture":ARCHITECTURE,"question":QUESTION,"status":"OK","decision_authority":"E9_ONLY","trade_decision":None,"as_of_closed_candle":current,"causal":{"lookahead_allowed":False,"future_data_used":False,"confirmation_cutoff":current},"data_quality":{"valid_bars":len(clean),"rejected":reasons,"atr":round(atr,8)},"external_structure":external,"internal_structure":internal,"protected_structure":protected,"bos_choch":bos,"failed_break":failure,"liquidity":sweep,"structure_lifecycle":{"current_structure":external["state"],"bos_stage":"CONFIRMED" if bos.get("confirmed") else "NONE","failure_stage":"FAILED" if failure.get("confirmed") else "NONE","liquidity_stage":sweep.get("lifecycle","NONE"),"last_confirmed_pivot_index":max([x["confirmation_index"] for x in highs+lows],default=None),"as_of_closed_candle":current},"narrative":narrative,"contract":{"return_type":"dict","tuple_normalized":True,"decision_owner":"E9"}}
