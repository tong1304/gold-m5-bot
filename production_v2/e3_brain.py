...
Single structural brain. No trade decision, gate, score, or upstream direction is consumed.
...
MIN_CANDLES=20; INTERNAL_RADIUS=2; EXTERNAL_RADIUS=5; PROMINENCE_ATR=.10; BOS_DISTANCE_ATR=.10; BOS_BODY_ATR=.20; FAILURE_CLOSE_ATR=.05; FAILURE_SWEEP_ATR=.10; EQ_TOLERANCE_ATR=.10
...
def _compress(points,atr,side=None,spacing=2):
    ...
    if side is None and len(points)>=2: side="high" if points[1][1]>=points[0][1] else "low"
    side=side or "high"
    for p in points:
...
def _structure_counts(highs,lows):
    ...
    return (UP if bull>=2 and bull>bear+1 else DOWN if bear>=2 and bear>bull+1 else MIXED if bull or bear else NEUTRAL),c

def _latest_break_candidates(highs,lows,latest_index):
    return ([(float(highs[-1]["price"]),int(highs[-1]["index"]),UP)] if highs else [])+([(float(lows[-1]["price"]),int(lows[-1]["index"]),DOWN)] if lows else [])

def _candle_break_quality(bar,level,direction,atr):
    if atr<=0:return {"valid":False,"distance_atr":0.0,"body_atr":0.0,"close_location":.5,"displacement_ok":False}
    rng=max(bar["high"]-bar["low"],1e-12); body=abs(bar["close"]-bar["open"])/atr; loc=(bar["close"]-bar["low"])/rng; distance=(bar["close"]-level) if direction==UP else level-bar["close"]; close_ok=distance>=BOS_DISTANCE_ATR*atr; directional=loc>=.55 if direction==UP else loc<=.45; displacement=body>=BOS_BODY_ATR or directional
...
def _bos(bars,highs,lows,atr,prior_structure,scope="EXTERNAL"):
    ...
    for level,idx,direction in _latest_break_candidates(highs,lows,len(bars)-1):
        ...
        candidates.append({"event":event,"direction":direction,"confirmed":True,"scope":scope,"level":round(level,8),"swing_index":idx,"break_candle_index":len(bars)-1,"break_distance_atr":q["distance_atr"],"break_body_atr":q["body_atr"],"close_location":q["close_location"],"displacement_ok":q["displacement_ok"],"close_beyond_level":True})
...
def _strength(external,internal,bos,failure,conflicts):
...
def analyze_e3(bars):
    ...
    if failure.get("confirmed"):direction,state,finding=failure["direction"],"STRUCTURE_FAILURE","FAILED_BREAK"
        ...
        direction=eb["direction"];state="CHANGE_OF_CHARACTER" if eb["event"]=="CONFIRMED_CHOCH" else "BREAKOUT_CONFIRMED";finding="BULLISH_CHOCH" if direction==UP and state=="CHANGE_OF_CHARACTER" else "BEARISH_CHOCH" if direction==DOWN and state=="CHANGE_OF_CHARACTER" else "BULLISH_BOS" if direction==UP else "BEARISH_BOS"
    ...
    if failure.get("confirmed"):obs += [f"failure_sweep_distance_atr={failure['sweep_distance_atr']}",f"failure_reclaim_distance_atr={failure['reclaim_distance_atr']}"]
    trace={"closed_candles":len(clean),"atr_period":14,"internal_pivot_window":INTERNAL_RADIUS,"external_pivot_window":EXTERNAL_RADIUS,"pivot_prominence_atr":PROMINENCE_ATR,"bos_close_distance_atr":BOS_DISTANCE_ATR,"bos_body_atr":BOS_BODY_ATR,"failure_close_distance_atr":FAILURE_CLOSE_ATR,"failure_sweep_distance_atr":FAILURE_SWEEP_ATR,"wick_only_break_is_bos":False,"external_structure_is_authority":True,"slope_is_structural_authority":False,"internal_structure":idef,"external_structure":edef,"internal_state":idef,"external_state":edef,"internal_count_state":ist,"external_count_state":est,"internal_bos":ib,"external_bos":eb,"failure":failure,"upstream_data_consumed":False,"decision_authority":"E9_ONLY"}
