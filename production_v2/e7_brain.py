...
ARCHITECTURE = "E7_PROFESSIONAL_SETUP_AWARE_CONFIRMATION_BRAIN_V12"
VERSION = "12.0"
ATR_PERIOD = 14
MIN_BARS = 5
FOLLOW_THROUGH_MAX_AGE = 3
MAX_CAUSAL_EVENT_AGE = 1
...
def _reasoning(*, conclusion: str, thesis: str, observed: list[str], missing: list[str], support: list[str],
    ...
    return {
        ...
        "decision_path": [
            ...
            "Setup-specific confirmation must be causally tied to the relevant auction/structure event.",
            "Liquidity reversal requires a real sweep/rejection and a directional reclaim of the swept level.",
            "A follow-through candle validates only the immediately preceding trigger; future candles are never borrowed.",
            "Stale auction evidence cannot confirm a current setup; causal auction evidence is limited to the event candle or its immediate successor.",
        ...
        "reasoning_trace_version": "E7_V12_CAUSAL_CONFIRMATION_AUDIT",
    }
...
def _empty(snapshot: dict[str, Any], reason: str) -> EngineResult:
    ...
    out = {
        **_base(snapshot), "state": "WAIT", "confirmation": "UNRESOLVED", "trigger_status": "NOT_EVALUATED",
        "direction": "NEUTRAL", "setup": "NONE", "setup_family": "NONE", "candidate_setup_thesis": "",
        "trigger_observed": False, "confirmation_strength": "NONE", "confirmation_score": 0.0,
        "supporting_evidence": [], "counter_evidence": [reason], "observed_evidence": observed,
        "missing_evidence": missing, "next_required_evidence": missing, "next_required_event": next_event,
        ...
        "reasoning_trace": {"conclusion": "Confirmation cannot be evaluated from current context.", "why_not_confirmed": missing, "next_required_event": next_event},
    }
    out["professional_reasoning"] = _reasoning(
...
def _no_setup(snapshot: dict[str, Any], e6o: dict[str, Any], reason: str) -> EngineResult:
    ...
    lifecycle = {
        "state": "NO_SETUP", "trigger": "NOT_ALLOWED", "confirmation": "NOT_APPLICABLE",
        "follow_through": "NOT_APPLICABLE", "invalidation": "CURRENT_THESIS_NOT_ESTABLISHED",
        "next_required_event": next_event,
    }
    reasons = ("NO_SURVIVING_SETUP", "CONFIRMATION_NOT_APPLICABLE", "E7_DID_NOT_CREATE_THESIS")
    ledger = {
        "surviving_setup": {
            "state": "FAIL", "observed": reason, "required": "E6 surviving setup",
            "interpretation": "E7 cannot confirm an absent thesis.",
        }
    }
    out = {
        **_base(snapshot), "state": "NO_SETUP", "confirmation": "NO_SURVIVING_SETUP", "trigger_status": "NOT_ALLOWED",
        ...
        "confirmation_score": 0.0, "supporting_evidence": [], "counter_evidence": [reason],
        "observed_evidence": observed, "missing_evidence": missing, "next_required_evidence": missing,
        "next_required_event": next_event, "invalidation": ["E6 must produce a new setup that survives causal screening"],
        "proof_gates": {"surviving_setup": "FAIL"}, "evidence_ledger": ledger,
        "confirmation_lifecycle": lifecycle, "observations": observed,
        "reasoning_trace": {
            "conclusion": "E6 produced no setup that survives causal screening; E7 will not invent confirmation.",
            "why_not_confirmed": missing, "next_required_event": next_event,
        },
        "reason_codes": list(reasons),
    }
    out["professional_reasoning"] = _reasoning(
        conclusion=out["reasoning_trace"]["conclusion"], thesis=thesis, observed=observed, missing=missing,
        support=[], counter=[reason], invalidation=out["invalidation"], lifecycle=lifecycle,
        next_event=next_event, ledger=ledger, proof=out["proof_gates"],
    )
...
def analyze_e7(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    e6_finding = _text(e6o.get("finding", ""))
    e6_state = _text(e6o.get("state", ""))
    e6_maturity = _text(e6o.get("maturity", e6o.get("setup_stage", ""))) or "UNRESOLVED"
    no_setup_markers = {
        "NONE", "UNKNOWN", "UNRESOLVED", "NO_SETUP", "NO_PLAUSIBLE_SETUP",
        "NO_SURVIVING_SETUP", "SETUP_NOT_TRADE_READY",
    }
    ...
    def rec(name: str, state: str, observed_value: Any, required: Any, interpretation: str) -> str:
        ...
        ledger[name] = {
            "state": state, "observed": observed_value, "required": required, "interpretation": interpretation,
        }
    ...
    structure_conflict = (
        "MIXED" in structure_finding
        or "TRANSITION" in structure_finding
        or (external in {"BUY", "SELL"} and internal in {"BUY", "SELL"} and external != internal)
    )
    ...
    event_causal = auction["age"] <= MAX_CAUSAL_EVENT_AGE
    ...
    rec("auction_causality", "PASS" if event_causal else "FAIL", auction["age"], f"<= {MAX_CAUSAL_EVENT_AGE} bars", "Setup confirmation may use only the event candle or its immediate successor.")
    ...
    liquidity_reversal = "LIQUIDITY_REVERSAL" in setup
    if "AUCTION_ACCEPTANCE" in setup:
        acceptance_event = "ACCEPTANCE" in auction["event"]
        acceptance_proof = acceptance_event and event_direction_ok and event_causal and (auction["terminal"] or response_present)
        setup_gates["auction_acceptance"] = rec("setup.auction_acceptance", "PASS" if acceptance_proof else "PENDING", {"event": auction["event"], "terminal": auction["terminal"], "response": response_present, "causal": event_causal}, "causal acceptance event + directional response/terminal state", "Acceptance must be evidenced, not inferred from a touch.")
    elif liquidity_reversal:
        sweep = any(x in auction["event"] for x in ("SWEEP", "FAILED_BREAK_RECLAIM", "REJECTION"))
        reclaim_level = auction["level"]
        if reclaim_level:
            reclaim_now = current["close"] > reclaim_level if buy else current["close"] < reclaim_level
            reclaim_previous = previous["close"] > reclaim_level if buy else previous["close"] < reclaim_level
            reclaim = bool((trigger and reclaim_now) or (previous_trigger and reclaim_now) or reclaim_previous)
        else:
            reclaim = False
        sweep_proof = sweep and event_direction_ok and event_causal
        setup_gates["liquidity_sweep"] = rec("setup.liquidity_sweep", "PASS" if sweep_proof else "PENDING", {"sweep": sweep, "causal": event_causal, "event": auction["event"]}, "causal sweep/rejection/failed-break event", "A reversal requires evidence that liquidity was actually taken on the causal candle.")
        setup_gates["reclaim_level"] = rec("setup.reclaim_level", "PASS" if reclaim else "PENDING", {"level": reclaim_level, "current_close": current["close"], "previous_close": previous["close"]}, "closed-candle close beyond swept level", "A directional candle is not enough; price must reclaim the level that was swept.")
        setup_gates["reclaim_response"] = rec("setup.reclaim_response", "PASS" if (reclaim and causal_trigger and event_causal) else "PENDING", {"reclaim": reclaim, "causal_trigger": causal_trigger, "event_causal": event_causal}, "causal trigger + level reclaim", "Reclaim must be tied to the sweep/rejection and the setup trigger.")
        ...
        setup_gates["break_event"] = rec("setup.break_event", "PASS" if (bos and event_causal) else "PENDING", {"bos": bos, "causal": event_causal}, "confirmed causal break", "Retest reasoning is invalid without a causal break.")
        setup_gates["retest_response"] = rec("setup.retest_response", "PASS" if (retest and causal_trigger and event_causal) else "PENDING", {"retest": retest, "causal_trigger": causal_trigger, "causal": event_causal}, "causal retest + directional closed-candle response", "The retest must hold/reject and continue.")
    elif "BREAKOUT" in setup:
        bos = _text(e3o.get("bos", e3o.get("break_of_structure"))) in {"BREAK", "BOS", "YES", "CONFIRMED"} or "BREAK" in auction["event"]
        setup_gates["break_event"] = rec("setup.break_event", "PASS" if (bos and event_causal) else "PENDING", {"bos": bos, "causal": event_causal}, "confirmed causal break", "A breakout requires a causal closed-candle break.")
    ...
    terminal_auction_proof = bool(
        ("AUCTION_ACCEPTANCE" in setup or liquidity_reversal)
        and auction["terminal"]
        and event_direction_ok
        and event_causal
        and event_present
    ...
    causal_confirmation = bool(
        ...
        and follow_proven
        and (not liquidity_reversal or event_causal)
    )
    ...
    proof.update({
        ...
        "auction_causal": "PASS" if event_causal else "FAIL",
    ...
    lifecycle = {
        ...
        "invalidation": invalid or "NONE",
        "next_required_event": next_event,
        "setup": setup,
        "direction": direction,
        "event_age_bars": auction["age"],
        "maturity": e6_maturity,
        "terminal_auction_proof": terminal_auction_proof,
        "causal_confirmation": causal_confirmation,
        "terminal_confirmation": terminal_confirmation,
    }
    conclusion = (
        "Closed-candle evidence confirms the setup."
        if confirmation == "CONFIRMED"
        else "Closed-candle evidence invalidates the current setup thesis."
        if confirmation == "INVALIDATED"
        else "The thesis remains a hypothesis; required proof is incomplete."
    )
    reasons: list[str] = []
    if confirmation == "CONFIRMED":
    ...
    reasons.append(
        "CAUSAL_FOLLOW_THROUGH_PROVEN"
        if causal_confirmation
        else "TERMINAL_AUCTION_CONFIRMATION_PROVEN"
        if terminal_confirmation
        else "TRIGGER_OBSERVED_NOT_AUTOMATIC_CONFIRMATION"
        if trigger
        else "VALID_CLOSED_CANDLE_TRIGGER_MISSING"
    )
    if missing:
        reasons.append("MISSING_EVIDENCE_EXPOSED")
    if counter:
        reasons.append("COUNTER_EVIDENCE_PRESENT")
    if terminal_auction_proof:
        reasons.append("TERMINAL_AUCTION_PROOF_PRESENT")
    if liquidity_reversal:
        reasons.append("LIQUIDITY_RECLAIM_LEVEL_REQUIRED")
    if not event_causal and liquidity_reversal:
        reasons.append("CAUSAL_AUCTION_EVENT_REQUIRED")
    reasons += [
        ...
        "PROFESSIONAL_REASONING_EXPOSED", "E7_DID_NOT_CREATE_THESIS",
    ]
    ...
    out = {
        **_base(snapshot),
        ...
        "trigger": {
            ...
            "direction": direction,
            "closed_candle": True,
            "body_atr": round(current["body_atr"], 4),
            "close_position": round(current["close_position"], 4),
            "is_confirmation": False,
        },
        "follow_through": {
            "state": follow_state,
            "required": "subsequent closed-candle evidence",
        ...
        "reasoning_trace": {
            ...
            "counter_evidence": counter,
            "next_required_event": next_event,
            "setup_specific_proof": setup_gates,
        },
