...
ARCHITECTURE = "E7_PROFESSIONAL_SETUP_AWARE_CONFIRMATION_BRAIN_V9"
VERSION = "9.0"
ATR_PERIOD = 14
MIN_BARS = 5
...
def _reasoning(*, conclusion: str, thesis: str, observed: list[str], missing: list[str], support: list[str],
    ...
    return {
        ...
        "decision_path": [
            "E6 supplies a hypothesis; E7 never creates a new setup thesis.",
            "Only completed candles are admissible; the current open candle is excluded.",
            "A trigger is a candidate event and never equals confirmation by itself.",
            "Setup-specific proof must be satisfied before confirmation can pass.",
            "Follow-through is observed from a later closed candle; it is never borrowed from the future.",
            "Counter-evidence and structural invalidation outrank a positive trigger.",
            "E7 reports confirmation evidence; E9 retains trade-decision authority.",
        ],
        "reasoning_trace_version": "E7_V9_EXPLICIT_AUDIT",
    }


def _empty(snapshot: dict[str, Any], reason: str) -> EngineResult:
    observed = [f"context=INSUFFICIENT reason={reason}"]; missing = ["valid directional setup thesis", "valid closed-candle confirmation"]
...
def _setup_family(setup: Any) -> str:
    s = _text(setup)
    return {"TREND_PULLBACK": "TREND_PULLBACK_CONTINUATION", "PULLBACK": "TREND_PULLBACK_CONTINUATION", "IMPULSE_CONTINUATION": "IMPULSE_CONTINUATION", "AUCTION_ACCEPTANCE": "AUCTION_ACCEPTANCE_CONTINUATION"}.get(s, s)
...
def _no_setup(snapshot: dict[str, Any], e6o: dict[str, Any], reason: str) -> EngineResult:
    ...
    observed = [f"e6_setup={_text(e6o.get('setup', e6o.get('setup_family'))) or 'NONE'}", f"e6_direction={_direction(e6o.get('direction', e6o.get('direction_thesis')))}", f"e6_finding={_text(e6o.get('finding', '')) or 'UNRESOLVED'}", f"setup_status={reason}"]
    missing = ["new E6 setup thesis that survives causal screening"]; next_event = "E6_NEW_SURVIVING_SETUP_THESIS"
...
def analyze_e7(snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    if e6_state in {"NO_SETUP", "UNRESOLVED"} and not raw_setup: return _no_setup(snapshot, e6o, "E6_SETUP_UNRESOLVED")

    direction = _direction(e6o.get("direction", e6o.get("direction_thesis"))); thesis = str(e6o.get("candidate_setup_thesis") or e6o.get("thesis") or ""); setup = raw_setup or "UNKNOWN"
    if direction not in {"BUY", "SELL"}: return _no_setup(snapshot, e6o, "E6_DIRECTION_UNRESOLVED")

    buy = direction == "BUY"; atr = max(_atr(bars), 1e-9); current = _candle(bars[-1], bars[-2], atr); previous = _candle(bars[-2], bars[-3], atr); auction = _e4(e4o)
    ...
    def rec(name: str, state: str, observed_value: Any, required: Any, interpretation: str) -> str:
        normalized = _text(state); state = normalized if normalized in VALID_STATES else "UNAVAILABLE"
    ...
    directional = current["bullish"] if buy else current["bearish"]; close_ok = current["close_position"] >= MIN_CLOSE_POSITION if buy else current["close_position"] <= 1.0 - MIN_CLOSE_POSITION
    displacement = directional and current["body_atr"] >= MIN_DISPLACEMENT_ATR and close_ok; engulf = current["bullish_engulf"] if buy else current["bearish_engulf"]
    ...
    protected_high = _num(e3o.get("protected_high")); protected_low = _num(e3o.get("protected_low")); structure_finding = _text(e3o.get("finding", e3o.get("structure_state")))
    external = _direction(e3o.get("external_state", e3o.get("external_count_state"))); internal = _direction(e3o.get("internal_state", e3o.get("internal_count_state")))
    ...
    if buy and protected_low and current["close"] < protected_low: invalid.append(f"CLOSE_BELOW_PROTECTED_LOW={protected_low:.5f}")
    if not buy and protected_high and current["close"] > protected_high: invalid.append(f"CLOSE_ABOVE_PROTECTED_HIGH={protected_high:.5f}")
    ...
    space = _num(e5o.get("available_space_atr_long" if buy else "available_space_atr_short"), 0.0); space_ok = bool(e5o) and space >= MIN_STRUCTURAL_SPACE_ATR
    rec("structural_space", "PASS" if space_ok else "PENDING", round(space, 4), f">={MIN_STRUCTURAL_SPACE_ATR} ATR", "Space is an economic filter, not a confirmation generator.")
    event_direction_ok = auction["direction"] in {"NEUTRAL", direction}; event_fresh = auction["age"] <= FOLLOW_THROUGH_MAX_AGE
    event_present = bool(auction["event"] and "NO_CONFIRMED" not in auction["event"] and "NO_LIQUIDITY" not in auction["event"]); response_present = auction["response_actor"] not in {"", "UNKNOWN", "NONE", "UNCLEAR"}
    ...
    if "AUCTION_ACCEPTANCE" in setup:
        acceptance_event = "ACCEPTANCE" in auction["event"]; acceptance_proof = acceptance_event and event_direction_ok and (auction["terminal"] or response_present)
        ...
        sweep = any(x in auction["event"] for x in ("SWEEP", "FAILED_BREAK_RECLAIM", "REJECTION")); reclaim = trigger and event_direction_ok
        ...
        bos = _text(e3o.get("bos", e3o.get("break_of_structure"))) in {"BREAK", "BOS", "YES", "CONFIRMED"} or "BREAK" in auction["event"]; retest = "RETEST" in auction["event"]
    ...
    previous_trigger = bool((previous["bullish"] if buy else previous["bearish"]) and ((previous["close_position"] >= MIN_CLOSE_POSITION) if buy else (previous["close_position"] <= 1.0 - MIN_CLOSE_POSITION)) and (previous["body_atr"] >= MIN_DISPLACEMENT_ATR or (previous["bullish_engulf"] if buy else previous["bearish_engulf"])))
    follow_now = bool(previous_trigger and directional and close_ok)
    if trigger:
        follow_state = "PENDING_NEXT_CLOSED_CANDLE"; rec("follow_through", "PENDING", "TRIGGER_ON_LATEST_CLOSED_CANDLE", "subsequent closed candle", "The next candle does not exist yet; future evidence cannot be borrowed."); next_event = "NEXT_CLOSED_CANDLE_THESIS_FOLLOW_THROUGH"
    elif follow_now:
        follow_state = "PROVEN_ON_CURRENT_CLOSED_CANDLE"; rec("follow_through", "PASS", True, "current candle follows previous trigger", "The current closed candle confirms continuation after the previous trigger."); next_event = "MONITOR_CONFIRMED_THESIS_INVALIDATION"
    else:
        follow_state = "NOT_PROVEN"; rec("follow_through", "PENDING", "NOT_OBSERVED", "closed-candle follow-through", "Follow-through must be observed after a setup-specific trigger."); next_event = "VALID_CLOSED_CANDLE_TRIGGER"

    if structure_conflict: counter.append("STRUCTURE_COUNTER_EVIDENCE")
    if auction["pending"] and not auction["terminal"]: counter.append("AUCTION_PENDING")
    if space and space < MIN_STRUCTURAL_SPACE_ATR: counter.append("STRUCTURAL_SPACE_CONSTRAINED")
    if buy and current["bearish"]: counter.append("BEARISH_CLOSED_CANDLE_COUNTER_EVIDENCE")
    if not buy and current["bullish"]: counter.append("BULLISH_CLOSED_CANDLE_COUNTER_EVIDENCE")
    counter = _dedupe(counter)

    invalidated = bool(invalid); all_setup_pass = bool(setup_gates) and all(v == "PASS" for v in setup_gates.values())
    hard_counter = any(x in counter for x in ("STRUCTURE_COUNTER_EVIDENCE", "BEARISH_CLOSED_CANDLE_COUNTER_EVIDENCE", "BULLISH_CLOSED_CANDLE_COUNTER_EVIDENCE"))
    follow_proven = follow_now or (auction["terminal"] and event_direction_ok and event_fresh)
    confirmation_allowed = not invalidated and all_setup_pass and structure_aligned and space_ok and not hard_counter and follow_proven

    if invalidated:
        state = "INVALIDATED"; confirmation = "INVALIDATED"; trigger_status = "INVALIDATED"; score = 0.0; next_event = "E6_NEW_SETUP_AFTER_INVALIDATION"; missing.append("new valid setup thesis")
    elif confirmation_allowed:
        state = "CONFIRMED"; confirmation = "CONFIRMED"; trigger_status = "CONFIRMED"; score = 92.0; next_event = "MONITOR_CONFIRMED_THESIS_INVALIDATION"
    else:
        state = "WAIT"; confirmation = "DEVELOPING"; trigger_status = "TRIGGER_OBSERVED" if trigger else "NOT_CONFIRMED"
        pass_count = sum(v == "PASS" for v in ledger.values() if isinstance(v, dict)); score = min(79.0, 40.0 + 4.0 * pass_count)
        if trigger: next_event = "NEXT_CLOSED_CANDLE_THESIS_FOLLOW_THROUGH"
        elif follow_now: next_event = "SETUP_PROOF_AND_RISK_RECHECK"

    proof = dict(setup_gates); proof.update({"trigger": "PASS" if trigger else "FAIL", "structure_alignment": "PASS" if structure_aligned else "FAIL", "structural_space": "PASS" if space_ok else "PENDING", "auction_terminal": "PASS" if auction["terminal"] else "PENDING", "follow_through": "PASS" if follow_proven else "PENDING", "invalidation": "FAIL" if invalidated else "PASS"})
    proof_pass = sum(v == "PASS" for v in proof.values()); proof_total = max(1, len(proof))
    observed = _dedupe(observed + [f"setup={setup}", f"direction={direction}", f"maturity={e6_maturity if 'e6_maturity' in locals() and e6_maturity else 'UNRESOLVED'}", f"trigger={trigger}", f"previous_trigger={previous_trigger}", f"follow_through={follow_state}", f"auction_event={auction['event'] or 'NONE'}", f"auction_state={auction['state'] or 'NONE'}", f"auction_age_bars={auction['age']}", f"current_body_atr={current['body_atr']:.4f}", f"current_close_position={current['close_position']:.4f}", f"proof_pass_ratio={proof_pass}/{proof_total}"])
    missing = _dedupe(missing); support = _dedupe(support); invalid = _dedupe(invalid); next_required = _dedupe(missing)
    lifecycle = {"state": state, "trigger": "OBSERVED" if trigger else "NOT_OBSERVED", "confirmation": confirmation, "follow_through": follow_state, "invalidation": invalid or "NONE", "next_required_event": next_event, "setup": setup, "direction": direction, "event_age_bars": auction["age"]}
    ...
    out = {**_base(snapshot), "state": state, "confirmation": confirmation, "trigger_status": trigger_status, "direction": direction, "setup": setup, "setup_family": setup, "candidate_setup_thesis": thesis, "maturity": e6_maturity if 'e6_maturity' in locals() and e6_maturity else "UNRESOLVED", "trigger_observed": trigger, "confirmation_strength": "HIGH" if confirmation == "CONFIRMED" else "NONE" if confirmation == "INVALIDATED" else "DEVELOPING", "confirmation_score": round(score, 2), "supporting_evidence": support, "counter_evidence": counter, "observed_evidence": observed, "missing_evidence": missing, "next_required_evidence": next_required, "next_required_event": next_event, "invalidation": invalid or ["closed-candle structural break or setup-specific contradiction"], "proof_gates": proof, "evidence_ledger": ledger, "confirmation_lifecycle": lifecycle, "trigger": {"observed": trigger, "direction": direction, "closed_candle": True, "body_atr": round(current["body_atr"], 4), "close_position": round(current["close_position"], 4), "is_confirmation": False}, "follow_through": {"state": follow_state, "required": "subsequent closed-candle evidence"}, "reasoning_trace": {"conclusion": conclusion, "why_not_confirmed": missing, "observed": observed, "counter_evidence": counter, "next_required_event": next_event, "setup_specific_proof": setup_gates}, "observations": observed, "reason_codes": _dedupe(reasons)}
