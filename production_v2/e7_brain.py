...
def analyze_e7(snapshot:dict[str,Any],upstream:dict[str,EngineResult])->EngineResult:
    ...
    if len(bars)<5 or not e6:return EngineResult("E7",NAME,None,0,{**base,"state":"WAIT","confirmation":"UNRESOLVED","evidence":[],"counter_evidence":["MISSING_SETUP_CONTEXT"],"missing_evidence":["closed-candle confirmation"],"invalidation":["new closed candle"]},("INSUFFICIENT_CONTEXT",))
    o=e6.output; direction=str(o.get("direction","NEUTRAL")).upper(); setup=str(o.get("setup","NONE")).upper(); b=bars[-1]; p=bars[-2]; op,hi,lo,cl=[float(b[k]) for k in ("open","high","low","close")]; prev=float(p["close"]); rng=max(hi-lo,1e-9); body=abs(cl-op); atr=max(mean(float(x["high"])-float(x["low"]) for x in bars[-14:]),1e-9); pos=(cl-lo)/rng
    ...
    if not trigger: missing.append("VALID_CLOSED_CANDLE_TRIGGER")
    if liquidity_conflict: counter.append("LIQUIDITY_DIRECTION_CONFLICT")
    confirmation="CONFIRMED" if direction in {"BUY","SELL"} and trigger and not liquidity_conflict else "DEVELOPING" if direction in {"BUY","SELL"} and not counter else "UNRESOLVED"
    gate=confirmation=="CONFIRMED"; score=85 if gate else 45 if confirmation=="DEVELOPING" else 25
