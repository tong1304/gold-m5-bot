...
def _watch_state(e6: dict[str, Any]) -> tuple[str, str] | None:
    setup = _text(e6.get("setup") or e6.get("setup_type") or e6.get("setup_family")); direction = _text(e6.get("direction") or e6.get("thesis_direction") or e6.get("direction_thesis"))
    if setup not in _WATCH_SETUPS or direction not in _DIRECTIONS: return None
    if e6.get("watch_only") is not True or e6.get("trade_ready") is True or e6.get("gate_passed") is True: return None
    return direction, setup


def _has_hard_invalidation(upstream: dict[str, EngineResult]) -> bool:
    for key in ("E1","E2","E3","E4","E5","E6","E7","E8"):
        output = _out(upstream.get(key)); values = output.get("reason_codes") or output.get("reasons") or ()
        if isinstance(values, str): values = (values,)
        if any(_text(value) in _HARD_CONFLICTS for value in values): return True
        if any(_text(output.get(name)).endswith("_INVALIDATED") for name in ("invalidation","state","setup_state")): return True
    return False
...
def _watch_wait_events(e6: dict[str, Any]) -> list[str]:
    ...
    events = [value for value in events if value not in {"E7_CONFIRMATION","E7_VALID_CLOSED_CANDLE_TRIGGER_REQUIRED"}]
...
def _watch_result(direction: str, setup: str, e6: dict[str, Any]) -> EngineResult:
    ...
    timing_input = dict(e6)
    timing = e6.get("opportunity_timing") if isinstance(e6.get("opportunity_timing"), dict) else classify_opportunity_timing(timing_input)
    ...
    if timing_phase == "EARLY_OPPORTUNITY" and timing_speed == "FAST": timing_reason = "EARLY_OPPORTUNITY_FAST_PATH_PENDING"
    reasons = ["E9_FINAL_GOVERNANCE","E6_OPPORTUNITY_WATCH","E6_SETUP_THESIS_REQUIRED","E7_TRIGGER_BLOCKED_UNTIL_E6_THESIS",timing_reason]
    output = {
        "decision":"NO_TRADE","final_governance":"WATCH","governance_decision":"WATCH","governance_reason":timing_reason,
        "governance_blockers":["E6_SETUP_THESIS_REQUIRED"],"next_required_events":wait_events,"execution_state":"BLOCKED",
        ...
        "thesis_state":"HYPOTHESIS","thesis_lifecycle_source":"E6","setup_state":"FORMING","confirmation_state":"NOT_APPLICABLE","economic_state":"NOT_APPLICABLE","economic_blockers":[],"economic_pending":[],"hard_conflicts":[],
        ...
        "architecture":"E9_FINAL_GOVERNANCE_THESIS_TRIGGER_ECONOMICS","watch_boundary":"E6_WATCH_REQUIRES_E6_THESIS_BEFORE_E7_AND_E8",
...
def _restore_governance_layers(result: EngineResult) -> EngineResult:
    out = _out(result); layers = dict(out.get("governance_layers") or {})
    layers.update({"market_control":layers.get("market_control","MARKET_CONTROL"),"thesis_control":layers.get("thesis_control","E6_OWNER"),"proof_control":layers.get("proof_control","E7_CONFIRMATION_AND_E8_ECONOMICS"),"final_governance":layers.get("final_governance","E9_FINAL_AUTHORITY")})
...
def install(e9_module) -> None:
    if getattr(e9_module,"_E9_WATCH_BOUNDARY_INSTALLED",False): return
    ...
    def guarded(snapshot: dict[str, Any], upstream: dict[str, EngineResult]):
        ...
        if watch and e8.get("applicability") == "NOT_APPLICABLE_WITHOUT_SURVIVING_E6_THESIS" and not _has_hard_invalidation(upstream): return _watch_result(watch[0],watch[1],e6)
