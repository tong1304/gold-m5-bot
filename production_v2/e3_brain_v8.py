...
E3 uses closed candles only. External structure is the authority; internal
structure is tactical evidence. Counts and slope corroborate structure but
never override it. BOS requires a confirmed structural level to be cleared
by a closed candle with displacement or strong close location. A protected
level break invalidates the current thesis; it does not automatically create
a new trend. Liquidity sweep/reclaim is kept separate from invalidation.
E3 never makes an execution decision.
"""
...
def _break(bar, level, direction, atr):
    ...
    return {
        ...
        "direction": direction if close_beyond and (displacement_ok or location_ok) else NEUTRAL,
        "level": float(level),
...
def _external_event(bar, external, highs, lows, atr, index):
    ...
    continuation, protected = _protected(external, highs, lows)
    candidates = []
    if continuation:
        direction = UP if external == UP else DOWN
        q = _break(bar, continuation.get("price"), direction, atr)
        if q["confirmed"]:
            candidates.append({**q, "event": "CONFIRMED_BOS", "scope": "EXTERNAL", "swing_index": continuation["index"], "swing_label": continuation["label"]})
    if protected:
        direction = DOWN if external == UP else UP
        q = _break(bar, protected.get("price"), direction, atr)
        if q["confirmed"]:
            candidates.append({**q, "event": "CONFIRMED_CHOCH", "scope": "EXTERNAL", "swing_index": protected["index"], "swing_label": protected["label"]})
    if not candidates:
        return {"event": "NO_BOS", "direction": NEUTRAL, "confirmed": False, "scope": "EXTERNAL"}
    if len(candidates) > 1:
    ...
    candidates[0]["break_candle_index"] = index
    return candidates[0]
...
def analyze_e3(bars):
    ...
    if external == UP and protected_low:
        ...
        if q["confirmed"]:
            protected_break = {**q, "confirmed": True, "event": "PROTECTED_LOW_BREAK", "direction": DOWN, "structural_invalidation": True}
    elif external == DOWN and protected_high:
        q = _break(clean[-1], protected_high["price"], UP, atr)
        if q["confirmed"]:
            protected_break = {**q, "confirmed": True, "event": "PROTECTED_HIGH_BREAK", "direction": UP, "structural_invalidation": True}

    sweep = _v6._sweep_failure(clean, eh, el, atr, external)
    if sweep.get("confirmed"):
        sweep = {**sweep, "type": "LIQUIDITY_SWEEP_RECLAIM", "structural_invalidation": False}
    else:
        sweep = {**sweep, "type": "NONE", "structural_invalidation": False}
    ...
    trace.update({
        "external_is_authority": True, "internal_is_tactical_only": True,
        "counts_are_corroboration_only": True, "slope_is_corroboration_only": True,
        "closed_candle_only": True, "protected_level_break_invalidates_current_thesis": bool(protected_break["confirmed"]),
        "protected_level_break_does_not_create_new_trend": True,
        "liquidity_sweep_is_not_structural_invalidation": True,
        "sequence_quality": seq_q, "sequence_state": seq_state,
        "bos_confirmation_rule": "CLOSE_BEYOND_LEVEL_AND_(BODY_DISPLACEMENT_OR_CLOSE_LOCATION)",
    })

    out = dict(base)
    out.update({
        "architecture": ARCHITECTURE, "finding": finding, "direction": direction,
        "directional_bias": direction, "structural_bias": external if external in {UP, DOWN} else NEUTRAL,
        "structure_state": state, "structure": state, "bos": ext_event,
        "BOS": ext_event.get("event", "NO_BOS"), "BOS_type": ext_event.get("event", "NO_BOS"),
        "BOS_level": ext_event.get("level"), "BOS_candle_index": ext_event.get("break_candle_index"),
        "external_bos": ext_event.get("event", "NO_BOS"), "internal_bos": int_event.get("event", "NO_BOS"),
        "external_bos_detail": ext_event, "internal_bos_detail": int_event,
        "structural_failure": sweep, "failure_type": sweep.get("type", "NONE"), "failure_level": sweep.get("level"),
        ...
        "structure_strength": authority, "strength": authority, "confidence": confidence,
        "reason_codes": reasons, "conflicts": conflicts, "observations": observations, "reasoning_trace": trace,
        "upstream_direction_used": False, "upstream_decisions_used": False, "upstream_gates_used": False,
        "score_used": False, "trade_decision_authority": False, "decision_authority": "E9_ONLY",
        "decision": None, "gate": None, "specialists_active": False, "specialists_status": "PAUSED",
    })
    if data_reasons:
