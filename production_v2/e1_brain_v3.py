...
def analyze_e1(bars: list[dict[str,Any]]|None)->dict[str,Any]:
    ...
    for i,b in enumerate(bars or []):
        ...
        if any(x is None for x in v.values()): problems.append(f"bar_{i}_ohlc_invalid"); continue
        if v["high"]<max(v["open"],v["close"]) or v["low"]>min(v["open"],v["close"]) or v["high"]<v["low"]: problems.append(f"bar_{i}_ohlc_inconsistent"); continue
        valid.append({**b,**v})
    base={"question":QUESTION,"reasoning_role":"MARKET_STATE_ANALYST","trade_decision_authority":False,"decision_authority":"E9_ONLY"}
    if len(valid)<60:
        return {**base,"market_state":"UNCLEAR","directional_pressure":"NEUTRAL","trend_state":"NONE","volatility_state":"UNKNOWN","structure_state":"UNCLEAR","structure_quality":0.0,"compression":"UNKNOWN","expansion":"UNKNOWN","transition":"UNKNOWN","confidence":0.0,"evidence":["valid_candles_below_minimum"],"conflicts":problems[:6],"reasons":["insufficient reliable candles; classification withheld"],"analysis_status":"INCOMPLETE","professional_reasoning":{"task":"DESCRIBE_MARKET_STATE_ONLY","trend_maturity":"UNAVAILABLE","trend_confirmed":False,"classification_reason":"insufficient reliable candles; classification withheld","ownership_boundaries":OWNERSHIP}}
    closes=[b["close"] for b in valid]; a=_atr(valid)
    ...
    e20s,e50s=_ema(closes,20),_ema(closes,50); rel="UP" if e20s[-1]>e50s[-1] else "DOWN" if e20s[-1]<e50s[-1] else "FLAT"; gap=(e20s[-1]-e50s[-1])/a
    es20,es50=_slope(e20s,a,5),_slope(e50s,a,5); ss,ms,ls=(_slope(closes,a,n) for n in (5,10,20))
    dirs=["UP" if ss>.15 else "DOWN" if ss<-.15 else "FLAT","UP" if ms>.20 else "DOWN" if ms<-.20 else "FLAT","UP" if ls>.30 else "DOWN" if ls<-.30 else "FLAT"]
    up,down=dirs.count("UP"),dirs.count("DOWN"); pressure="UP" if up>down else "DOWN" if down>up else "BALANCED"
    aligned=(sum((ss>=.20,ms>=.30,ls>=.45)) if pressure=="UP" else sum((ss<=-.20,ms<=-.30,ls<=-.45)) if pressure=="DOWN" else 0); persistence=aligned/3
    e10,e20=_eff(closes,10),_eff(closes,20); structure,sq=_structure(valid); sd="UP" if structure=="BULLISH" else "DOWN" if structure=="BEARISH" else "FLAT"
    ...
    # Trend is established by coherent evidence, not by one indicator. Efficiency
    # is a quality dimension and must not veto a highly persistent coherent state.
    ...
    atr_ratio=_atr(valid,14)/max(_atr(valid,50),1e-12); compression=atr_ratio<.78; expansion=atr_ratio>1.18
    ...
    maturity="ESTABLISHED" if trend else "DEVELOPING" if pressure in ("UP","DOWN") and consensus and ema_ok else "DIRECTIONAL_ONLY" if pressure in ("UP","DOWN") else "NONE"
    pl="BULLISH" if fp=="UP" else "BEARISH" if fp=="DOWN" else "NEUTRAL"; ts="UP" if state=="TREND_UP" else "DOWN" if state=="TREND_DOWN" else "NONE"; tr="PRESENT" if state=="TRANSITION" else "ABSENT"
    confidence=round(min(.99,max(.0,.45+.25*sq+.20*persistence+.10*min(1,e20/.7)+.10*float(ema_ok)-.05*len(conflicts))),3)
