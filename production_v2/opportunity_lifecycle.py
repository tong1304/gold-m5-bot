...
def advance_opportunity(previous:dict[str,Any]|None,current:dict[str,Any])->dict[str,Any]:
    ...
    oid=_identity(d,setup,event); pid=_text(p.get("opportunity_id")); ps=_text(p.get("state")); age=int(p.get("bars_waited",0) or 0)+(1 if p else 0)
    ...
    if executed:
        return {**base,"state":"EXECUTE","continuity":"E9_AUTHORIZED_EXECUTION","trade_authorized":True,"invalidation_reason":None}
    if invalidated:
    ...
    if p and ps in TERMINAL:
        return {**c,"state":"REPLACED","continuity":"NEW_OPPORTUNITY_AFTER_TERMINAL","previous_opportunity_id":pid,"opportunity_id":oid,"direction":d,"setup":setup,"bars_waited":0,"origin_candle":candle,"last_evaluated_candle":candle,"trade_authorized":False,"invalidation_reason":None}
    if p and d in {"BUY","SELL"} and _text(p.get("direction")) in {"BUY","SELL"} and d!=_text(p.get("direction")):
        return {**c,"state":"REPLACED","continuity":"DIRECTION_CHANGED","previous_opportunity_id":pid,"opportunity_id":oid,"bars_waited":0,"origin_candle":candle,"last_evaluated_candle":candle,"trade_authorized":False,"invalidation_reason":"DIRECTION_CHANGED"}
    ...
    if ready and candidate and oid:
        return {**base,"state":"READY","continuity":"ADVANCING_TO_EXECUTION_GATE","opportunity_id":oid,"direction":d,"setup":setup,"bars_waited":age if p else 0,"origin_candle":p.get("origin_candle") or candle,"invalidation_reason":None}
    if candidate and oid:
        if p and age>5:
            return {**base,"state":"EXPIRED","continuity":"OPPORTUNITY_EXPIRED","opportunity_id":oid,"direction":d,"setup":setup,"bars_waited":age,"wait_for":"NEW_CAUSAL_OPPORTUNITY","invalidation_reason":"WATCH_MAX_AGE_REACHED"}
    ...
    if p:
        return {**base,"state":"INVALIDATED","continuity":"OPPORTUNITY_INVALIDATED","opportunity_id":pid,"bars_waited":age,"wait_for":"NEW_CAUSAL_OPPORTUNITY","invalidation_reason":"THESIS_NOT_PRESERVED"}
