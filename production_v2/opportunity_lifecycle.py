...
def advance_lifecycle(previous:dict[str,Any]|None,current:dict[str,Any],*,bar_id:str,max_watch_bars:int=5)->dict[str,Any]:
    ...
    if p and _text(p.get("direction")) in {"BUY","SELL"} and d in {"BUY","SELL"} and _text(p.get("direction"))!=d:return {**c,"lifecycle_state":"INVALIDATED","previous_opportunity_id":pid,"opportunity_id":pid or oid,"age_bars":age,"wait_for":"NEW_CAUSAL_OPPORTUNITY","invalidation_reason":"DIRECTION_CHANGED"}
