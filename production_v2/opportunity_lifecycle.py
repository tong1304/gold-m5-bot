...
def _base(p: dict[str, Any], c: dict[str, Any], event_id: Any, candle: str, anchor: dict[str, Any]) -> dict[str, Any]:
    return {**p,"last_evaluated_candle":candle,"trade_authorized":False,"event_id":event_id or p.get("event_id"),"origin_event_id":p.get("origin_event_id") or event_id or p.get("origin_event_id"),"causal_event_anchor":anchor}
...
def advance_opportunity(previous: dict[str, Any] | None, current: dict[str, Any]) -> dict[str, Any]:
    ...
    anchor = _anchor(p, c); base = _base(p, c, event_id, candle, anchor)
    ...
    if pending_watch and thesis and d == pd and setup not in WATCH_SETUPS and setup not in {"", "UNKNOWN", "NONE", "NO_SETUP"}:
        state = "READY" if ready else "WAITING"; phase = "EXECUTABLE" if ready else "TRIGGER_PENDING"
    ...
    if candidate and oid:
        ...
        if watch: state, lifecycle, phase = "WATCHING", "OPPORTUNITY_WATCH", "OPPORTUNITY_WATCH"
...
def advance_opportunity_directions(previous: dict[str, Any] | None, current_by_direction: dict[str, dict[str, Any]], *, leader: str = "NEUTRAL", competition: str = "UNCONTESTED") -> dict[str, Any]:
    ...
    for direction in ("BUY", "SELL"):
        current = dict(current_by_direction.get(direction) or {}); current.setdefault("direction", direction)
        if not current.get("candle"): current["candle"] = previous.get("last_evaluated_candle")
    ...
    if leader not in VALID_DIRECTIONS or not any(_text(v.get("direction")) == leader for v in active): leader = "NEUTRAL" if not active else _text(active[0].get("direction"))
