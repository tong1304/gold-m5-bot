...
def advance_opportunity(previous:dict[str,Any]|None,current:dict[str,Any])->dict[str,Any]:
    ...
    if p and pd in {"BUY","SELL"} and d in {"BUY","SELL"} and d!=pd:return {**c,"state":"INVALIDATED","continuity":"OPPORTUNITY_INVALIDATED","previous_opportunity_id":pid,"opportunity_id":pid,"direction":pd,"setup":previous_setup,"bars_waited":age,"origin_candle":p.get("origin_candle") or candle,"last_evaluated_candle":candle,"trade_authorized":False,"invalidation_reason":"DIRECTION_CHANGED"}
