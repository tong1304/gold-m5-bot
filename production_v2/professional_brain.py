...
def _state_values(engine: EngineResult | None) -> set[str]:
    ...
    values=set(); tracked={"state","direction","bias","orientation","market_direction","classification","regime","setup","confirmation","risk_gate","phase"}
    def rec(value):
        if isinstance(value, dict):
            for key,item in value.items():
...
def _independent_setup_maturity(by,direction):
    ...
    supporting=sum((structure,location,setup_evidence,confirmation)); mature=direction in DIRECTIONS and confirmation and supporting>=3
...
def run_professional_e9(context,upstream,historical_calibration=None):
    ...
    if not setup["confirmation"]: reasons.append("ENTRY_CONFIRMATION_NOT_PROVEN")
    reasons.extend(execution["reasons"])
    ready=bool(direction in DIRECTIONS and setup["mature"] and execution["ready"] and thesis_quality>=70 and not conflicts and not invalidations)
