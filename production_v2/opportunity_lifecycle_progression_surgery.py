...
def lifecycle_telemetry(symbol: str, lifecycle: dict[str, Any]) -> str:
    ...
    execution = _text(lifecycle.get("execution_boundary_state") or lifecycle.get("execution_state")) or "NONE"
    return ("[PRODUCTION V2] OPPORTUNITY_LIFECYCLE " f"symbol={_text(symbol) or 'UNKNOWN'} " f"opportunity_id={lifecycle.get('opportunity_id') or 'NONE'} " f"origin_event_id={lifecycle.get('origin_event_id') or 'NONE'} " f"event_id={lifecycle.get('event_id') or 'NONE'} " f"candle={lifecycle.get('last_progression_candle') or lifecycle.get('last_evaluated_candle') or lifecycle.get('stage_candle') or 'NONE'} " f"stage={_text(lifecycle.get('lifecycle_stage')) or 'IDLE'} " f"state={_text(lifecycle.get('state')) or 'UNKNOWN'} " f"wait_for={_text(lifecycle.get('wait_for_stage')) or 'NONE'} " f"terminal={terminal} " f"execution={execution} " f"trade_authorized={1 if _bool(lifecycle.get('trade_authorized')) else 0}")
...
def enrich(result: DecisionResult, market_data: dict[str, Any]) -> DecisionResult:
    ...
    execution = initial_execution_state({"decision": result.decision, "gate_passed": result.gate_passed}) if progressed.get("lifecycle_stage") == "TRADE" else {"state": "NONE", "authorized_by": None, "history": []}
    lifecycle["execution_boundary_state"] = execution["state"]; lifecycle["execution_boundary_history"] = execution["history"]; lifecycle["execution_authorized_by"] = execution["authorized_by"]
    e9["opportunity_lifecycle"] = lifecycle; e9["lifecycle_stage"] = progressed.get("lifecycle_stage"); e9["lifecycle_stage_history"] = progressed.get("stage_history") or []; e9["lifecycle_wait_for_stage"] = progressed.get("wait_for_stage"); e9["lifecycle_terminal_state"] = progressed.get("terminal_stage"); e9["lifecycle_terminal_reason"] = progressed.get("terminal_reason"); e9["opportunity_symbol"] = symbol; e9["execution_boundary_state"] = execution["state"]; e9["execution_boundary_history"] = execution["history"]
    ...
    updated = DecisionResult(symbol, result.timeframe, decision, gate, result.score, tuple(engines), risk, tuple(dict.fromkeys(list(result.reason_codes) + ([f"LIFECYCLE_{progressed.get('lifecycle_stage')}"] if progressed.get("lifecycle_stage") else []))), result.state, result.blocked_by, result.wait_bars, result.execution_state)
...
def install(pipeline_module: Any) -> None:
    ...
    print("[PRODUCTION V2] OPPORTUNITY_LIFECYCLE_PROGRESSION_BINDING stages=WATCH>CONFIRMED>E6_THESIS>E7_CONFIRMED>E8_READY>TRADE terminals=TOO_LATE|EXPIRED|INVALIDATED execution=NONE>ORDER_INTENT>ORDER_SUBMITTED>BROKER_ACCEPTED>POSITION_OPEN", flush=True)
