...
def _current_stage_input(result: DecisionResult, lifecycle: dict[str, Any], market_data: dict[str, Any]) -> dict[str, Any]:
    ...
    direction = _text(e6.get("direction") or e6.get("direction_thesis") or e6.get("thesis_direction") or e4.get("direction") or e3.get("direction") or lifecycle.get("direction"))
    opportunities = lifecycle.get("opportunities") if isinstance(lifecycle.get("opportunities"), dict) else {}
    direction_item = opportunities.get(direction) if direction in {"BUY", "SELL"} and isinstance(opportunities.get(direction), dict) else {}
    lifecycle_direction = _text(lifecycle.get("direction"))
    same_direction_lifecycle = lifecycle_direction == direction
    event_id = e4.get("event_id") or e4.get("auction_event_id") or e4.get("event_candle_id") or direction_item.get("event_id") or direction_item.get("origin_event_id")
    if not event_id and same_direction_lifecycle:
        event_id = lifecycle.get("event_id") or lifecycle.get("origin_event_id")
    origin_event_id = direction_item.get("origin_event_id") or event_id
    if same_direction_lifecycle:
        origin_event_id = lifecycle.get("origin_event_id") or origin_event_id
    timing = e6.get("opportunity_timing") if isinstance(e6.get("opportunity_timing"), dict) else {}
    opportunity_phase = e6.get("opportunity_phase") or timing.get("phase") or direction_item.get("opportunity_phase") or (lifecycle.get("opportunity_phase") if same_direction_lifecycle else None)
    opportunity_speed = e6.get("opportunity_speed") or e6.get("opportunity_decision_speed") or timing.get("decision_speed") or direction_item.get("opportunity_speed") or (lifecycle.get("opportunity_speed") if same_direction_lifecycle else None)
    confirmation_window = e6.get("confirmation_window") or timing.get("confirmation_window") or direction_item.get("confirmation_window") or (lifecycle.get("confirmation_window") if same_direction_lifecycle else None)
    wait_for = e6.get("wait_for") or timing.get("wait_for") or direction_item.get("wait_for") or (lifecycle.get("wait_for") if same_direction_lifecycle else None)
    chase_prohibited = bool(e6.get("chase_prohibited") if "chase_prohibited" in e6 else timing.get("chase_prohibited", direction_item.get("chase_prohibited", lifecycle.get("chase_prohibited", False) if same_direction_lifecycle else False)))
    candidate = direction in {"BUY", "SELL"} and (bool(direction_item.get("opportunity_id")) or _bool(e6.get("watch_only")) or _text(e6.get("candidate_type")) in {"OPPORTUNITY_CANDIDATE", "SETUP_CANDIDATE", "EARLY_OPPORTUNITY_CANDIDATE"} or bool(event_id))
    return {"symbol": _symbol(result, market_data, lifecycle), "direction": direction, "candidate": candidate, "confirmed": e4_state in {"CONFIRMED", "TERMINALLY_CONFIRMED", "ACCEPTED", "RECLAIMED"}, "e4_state": e4_state, "thesis_proven": _bool(e6.get("e6_thesis_proven")) or _text(e6.get("thesis_state") or e6.get("maturity") or e6.get("setup_state")) in {"MATURE", "CONFIRMED", "VALIDATED", "TRADE_READY", "ESTABLISHED"}, "e7_confirmed": e7_state in {"PASS", "PASSED", "CONFIRMED", "TRIGGER_CONFIRMED", "PROVEN", "VALIDATED", "TRADE_READY"}, "e7_confirmation_state": e7_state, "e8_ready": bool(_engine(result, "E8") and _engine(result, "E8").gate_passed) or e8_state in {"PASS", "PASSED", "READY", "APPROVED", "POSITIVE", "RISK_READY", "ECONOMICALLY_ACCEPTABLE", "TRADE_READY"}, "e8_economic_state": e8_state, "e9_trade": e9_decision in {"BUY", "SELL", "TRADE"} and bool(result.gate_passed) and geometry_state not in {"TOO_LATE", "EXPIRED", "INVALID_GEOMETRY", "UNFAVORABLE_RR"}, "execution_state": geometry_state or _text(lifecycle.get("execution_state")), "invalidated": _bool(e6.get("invalidated")) or (same_direction_lifecycle and _text(lifecycle.get("state")) == "INVALIDATED") or bool(invalidation_reason), "invalidation_reason": invalidation_reason, "event_id": event_id, "origin_event_id": origin_event_id, "candle": candle, "opportunity_phase": opportunity_phase, "opportunity_speed": opportunity_speed, "opportunity_decision_speed": opportunity_speed, "confirmation_window": confirmation_window, "wait_for": wait_for, "chase_prohibited": chase_prohibited, "opportunity_timing": dict(timing) if timing else (dict(direction_item.get("opportunity_timing")) if isinstance(direction_item.get("opportunity_timing"), dict) else (dict(lifecycle.get("opportunity_timing")) if same_direction_lifecycle and isinstance(lifecycle.get("opportunity_timing"), dict) else {}))}
