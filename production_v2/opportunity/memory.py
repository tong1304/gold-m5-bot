"""Canonical opportunity-memory storage boundary."""
...
_LOCK = RLock()
_DEFAULT_PATH = "./data/production_v2_opportunity_lifecycle.json"
...
def _ensure_postgres() -> None:
    with _pg_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(f"CREATE TABLE IF NOT EXISTS {_TABLE} (symbol TEXT PRIMARY KEY,state_json TEXT NOT NULL,updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())")
...
def _load_postgres() -> dict[str, dict[str, Any]]:
    ...
    for symbol, state_json in rows:
        try:
            ...
            logger.error("[PRODUCTION V2] invalid lifecycle state symbol=%s error=%s", symbol, exc)
            continue
        if isinstance(state, dict):
            out[str(symbol).upper()] = dict(state)
    return out
...
def load_all() -> dict[str, dict[str, Any]]:
    with _LOCK:
        try:
            ...
            result = {str(s).upper(): dict(v) for s, v in payload.items() if isinstance(v, dict)} if isinstance(payload, dict) else {}
            _clear_error()
            return result
        except Exception as exc:
    ...
    with _LOCK:
        try:
            if _postgres_enabled():
                ...
                with _pg_connect() as conn:
                    with conn.cursor() as cur:
                        cur.execute(f"INSERT INTO {_TABLE} (symbol,state_json,updated_at) VALUES (%s,%s,NOW()) ON CONFLICT (symbol) DO UPDATE SET state_json=EXCLUDED.state_json,updated_at=NOW()", (symbol, json.dumps(dict(state), ensure_ascii=False, sort_keys=True)))
            else:
                path = _path()
