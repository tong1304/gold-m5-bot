...
def _identity(direction:Any,setup:Any,event_id:Any=None)->str:
    d,s=_text(direction),_text(setup); return f"{d}|{s}" if d in VALID_DIRECTIONS and s not in {"","UNKNOWN","NONE","NO_SETUP"} else ""
def _stable_identity(previous:dict[str,Any],direction:str,setup:str,event_id:Any=None)->str:
    pid=_text(previous.get("opportunity_id")); pd=_text(previous.get("direction")); ps=_text(previous.get("setup")); state=_text(previous.get("state")); return pid if pid and pd==direction and direction in VALID_DIRECTIONS and (ps in WATCH_SETUPS or state in ACTIVE_STATES) else _identity(direction,setup,event_id)
