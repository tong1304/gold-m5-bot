...
def _is_invalidated(e3: dict[str, Any]) -> bool: return bool(e3.get("structure_invalidated") is True or e3.get("active_invalidation") is True or _text(e3.get("lifecycle")) == "INVALIDATED" or "STRUCTURE_INVALIDATED" in _text(e3.get("finding")) or "STRUCTURE_INVALIDATED" in _text(e3.get("invalidation")))
def _is_existing_watch(e6: dict[str, Any]) -> bool:
    setup = _text(e6.get("setup") or e6.get("setup_family")); candidate = _text(e6.get("candidate_type")); return (setup in WATCH_SETUPS or candidate == "OPPORTUNITY_CANDIDATE" or e6.get("watch_only") is True) and e6.get("trade_ready") is not True
def _is_no_setup(e6: dict[str, Any]) -> bool:
    setup = _text(e6.get("setup") or e6.get("setup_family")); finding = _text(e6.get("finding")); reasons = {_text(x) for x in (e6.get("reason_codes") or e6.get("reasons") or [])}; return setup in {"", "NO_SETUP", "UNKNOWN", "NONE"} and ("NO_CAUSAL_OPPORTUNITY" in reasons or "NO SURVIVING CAUSAL OPPORTUNITY THESIS" in finding or "NO CAUSAL SETUP HYPOTHESIS" in finding)
def _pending_event(e4: dict[str, Any]) -> bool:
    event = _text(e4.get("event") or e4.get("finding")); state = _text(e4.get("auction_state") or e4.get("auction_phase") or e4.get("state")); return state in PENDING_AUCTION_STATES and any(token in event for token in MEANINGFUL_EVENTS)
