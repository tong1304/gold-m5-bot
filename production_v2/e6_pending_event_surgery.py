...
VERSION = "E6_PENDING_EVENT_SURGERY_V2"
...
def _event_direction(e4: dict[str, Any]) -> str:
    direct = _direction(e4.get("direction") or e4.get("directional_implication"))
...
def _generic_candidate(upstream: dict[str, EngineResult]) -> dict[str, Any] | None:
    ...
    if event_direction in DIRECTIONS: direction = event_direction
    elif e2_direction in DIRECTIONS: direction = e2_direction
    elif external in DIRECTIONS: direction = external
    elif pressure in DIRECTIONS: direction = pressure
    else: return None
    ...
    finding5 = _text(e5.get("finding")); preferred = _text(e5.get("preferred_location")); preferred_direction = "BUY" if preferred == "LONG" else "SELL" if preferred == "SHORT" else "NEUTRAL"; value_state = _text(e5.get("value_state")); space = _space(e5, direction)
...
def _candidate(upstream: dict[str, EngineResult]) -> dict[str, Any] | None:
    # Preserve the existing special handling for failed-break/reclaim events.
    if _failed_break_pending(_payload(upstream, "E4")):
        e1, e2, e3, e4, e5 = (_payload(upstream, key) for key in ("E1", "E2", "E3", "E4", "E5"))
        if _is_invalidated(e3): return None
        pressure = _direction(e1.get("directional_pressure") or e1.get("pressure")); external = _direction(e3.get("external_state") or e3.get("structure_direction") or e3.get("direction")); e2_direction = _direction(e2.get("direction") or e2.get("opportunity_direction")); direction = pressure if pressure in DIRECTIONS else external if external in DIRECTIONS else e2_direction
        if direction in DIRECTIONS:
            finding5 = _text(e5.get("finding")); value_state = _text(e5.get("value_state")); location = _text(e5.get("structural_location")); favorable_location = "FAVORABLE_LOCATION" in finding5 or location in {"AT_SUPPORT", "AT_RESISTANCE"} or value_state in {"DISCOUNT", "PREMIUM", "EQUILIBRIUM"}; space = _space(e5, direction)
            if favorable_location or space > 0:
                counter: list[str] = ["E4_FAILED_BREAK_RECLAIM_PENDING"]
                if external in DIRECTIONS and external != direction: counter.append("E3_EXTERNAL_COUNTERFLOW")
                if e2_direction in DIRECTIONS and e2_direction != direction: counter.append("E2_DIRECTION_COUNTERFLOW")
                missing = ["E4_AUCTION_FOLLOW_THROUGH", "E7_CONFIRMATION"]
                if _text(e2.get("opportunity_maturity") or e2.get("state") or e2.get("opportunity_state")) in {"DEVELOPING", "EMERGING", "PENDING", "UNRESOLVED"}: missing.insert(0, "E2_OPPORTUNITY_CONFIRMATION")
                if _text(e3.get("internal_state")) == "MIXED": missing.append("E3_INTERNAL_STRUCTURE_ALIGNMENT")
                if space < 0.75: missing.append("STRUCTURAL_SPACE_INSUFFICIENT")
                return {"direction": direction, "family": "LIQUIDITY_RESPONSE", "event_id": str(e4.get("event_id") or e4.get("event_candle_id") or ""), "space": round(space, 4), "support": list(dict.fromkeys(["E1_DIRECTIONAL_CORE" if pressure == direction else "E3_EXTERNAL_STRUCTURE_SUPPORT", "E4_FAILED_BREAK_RECLAIM_EVENT", "E5_SPACE_EVIDENCE"])), "counter": list(dict.fromkeys(counter)), "missing": list(dict.fromkeys(missing)), "direction_source": "UPSTREAM"}
