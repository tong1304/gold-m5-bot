...
def _is_concrete_surviving_setup(e6: dict[str, Any]) -> bool:
    ...
    explicit_candidate = _text(e6.get("candidate_type")) == "SETUP_CANDIDATE" and e6.get("watch_only") is False
    ...
    return bool(
        ...
        and (e6.get("setup_exists") is True or state in CONCRETE_SETUP_STATES or _text(e6.get("e6_causal_gate")) == "PASSED" or explicit_candidate or explicit_thesis)
...
def _is_opportunity_watch(e6: dict[str, Any]) -> bool:
    ...
    if _is_concrete_surviving_setup(e6):
        return False
    return bool(e6.get("watch_only") is True or candidate_type == "OPPORTUNITY_CANDIDATE" or setup.startswith(WATCH_PREFIXES))


def _pending_early_confirmation(result: EngineResult, e6o: dict[str, Any]) -> EngineResult:
    """Let E7 evaluate an early opportunity without upgrading it to a trade."""
    out = dict(result.output or {})
    thesis = str(e6o.get("candidate_setup_thesis") or e6o.get("thesis") or "")
    direction = _text(e6o.get("direction") or e6o.get("direction_thesis") or e6o.get("thesis_direction"))
    cleaned = [
        str(x) for x in (out.get("reason_codes") or result.reason_codes or [])
        if _text(x) not in {"E6_OPPORTUNITY_WATCH_NOT_SETUP", "E7_DID_NOT_CREATE_THESIS"}
    ]
    cleaned = list(dict.fromkeys(cleaned + ["E6_EARLY_OPPORTUNITY_CANDIDATE", "E7_EVALUATES_SETUP_CONFIRMATION"]))
    missing = ["E7_SETUP_SPECIFIC_CLOSED_CANDLE_CONFIRMATION"]
    out.update({
        "state": "WAIT",
        ...
        "setup_family": _text(e6o.get("setup_family") or e6o.get("setup")) or "LIQUIDITY_RESPONSE",
        "direction": direction,
        "supporting_evidence": list(dict.fromkeys(list(out.get("supporting_evidence") or []) + ["E6_EARLY_OPPORTUNITY_CANDIDATE"])),
        ...
        "reasoning_trace": {
            **dict(out.get("reasoning_trace") or {}),
            "conclusion": "E6 has identified an early opportunity candidate; E7 is evaluating the next closed-candle confirmation.",
        ...
        "thesis_boundary": {
            ...
            "early_opportunity_candidate": True,
            "watch_is_not_trade": True,
            "enforced": True,
        },
    })
...
def enforce_e6_thesis_boundary(original_e7, snapshot: dict[str, Any], upstream: dict[str, EngineResult]) -> EngineResult:
    ...
    early_candidate = _is_early_opportunity_candidate(e6o)
    print(
        ...
        f"concrete={concrete} early_candidate={early_candidate}",
        flush=True,
    )
    if early_candidate:
        result = original_e7(snapshot, upstream)
        return _pending_early_confirmation(result, e6o)
    if _is_opportunity_watch(e6o):
        result = original_e7(snapshot, upstream)
        out = dict(result.output or {})
        out.update({
            "state": "WAIT","confirmation": "UNRESOLVED","confirmation_state": "NOT_APPLICABLE","trigger_status": "NOT_ALLOWED","trigger_observed": False,"confirmation_strength": "NONE","confirmation_score": 0.0,"trade_decision_authority": False,"candidate_setup_thesis": "","setup": "NONE","setup_family": "NONE","supporting_evidence": [],"counter_evidence": ["E6_OPPORTUNITY_WATCH_IS_NOT_A_SETUP_THESIS"],"missing_evidence": ["E6_SURVIVING_SETUP_THESIS", "E7_SETUP_SPECIFIC_CLOSED_CANDLE_CONFIRMATION"],"next_required_evidence": ["E6_SURVIVING_SETUP_THESIS", "E7_SETUP_SPECIFIC_CLOSED_CANDLE_CONFIRMATION"],"next_required_event": "E6_SURVIVING_SETUP_THESIS","reason_codes": ["CONFIRMATION_NOT_APPLICABLE", "E7_DID_NOT_CREATE_THESIS", "E6_OPPORTUNITY_WATCH_NOT_SETUP"],"confirmation_lifecycle": {"state": "WAIT","trigger": "NOT_ALLOWED","confirmation": "NOT_APPLICABLE","follow_through": "NOT_APPLICABLE","invalidation": "NONE","next_required_event": "E6_SURVIVING_SETUP_THESIS"},"reasoning_trace": {"conclusion": "E6 is carrying an opportunity watch, not a surviving trade setup thesis; E7 cannot confirm it.","why_not_confirmed": ["E6_OPPORTUNITY_WATCH_NOT_SETUP"],"next_required_event": "E6_SURVIVING_SETUP_THESIS"},"thesis_boundary": {"e6_owns_thesis": True,"e7_may_confirm_only_surviving_setup": True,"watch_is_not_setup": True,"enforced": True},
        })
        out["professional_reasoning"] = {**dict(out.get("professional_reasoning") or {}),"conclusion": out["reasoning_trace"]["conclusion"],"hypothesis": "","missing_evidence": out["missing_evidence"],"next_required_event": out["next_required_event"]}
