...
def install(module: Any) -> None:
    if getattr(module, "_PROFESSIONAL_E5_DIRECT_V2", False):
        return
    ...
    def analyze_e5(*args: Any, **kwargs: Any):
        ...
        if isinstance(result, dict):
            output = dict(result)
            rebuild = lambda out: out
        else:
            output = dict(getattr(result, "output", {}) or {})
            rebuild = lambda out: result.__class__(result.engine_id, result.name, result.gate_passed, result.score, out, result.reason_codes)
    ...
    module._PROFESSIONAL_E5_DIRECT_V2 = True
