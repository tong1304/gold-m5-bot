"""V12.1 Statistics UI: MTF H1/M15/M5."""
...
def _payload():
    live=_live();state=_replay();result=state.get("result");base={"engine_version":ENGINE_VERSION,"engine_name":"REGIME-8-ENGINE-REENTRY","source_contract":"V12.1","timeframe_mode":"MTF:H1→M15→M5","timeframes":["H1","M15","M5"],"mtf_policy":"H1_BIAS_M15_REGIME_M5_TRIGGER","live_signals":live,"live_signal_count":len(live),"live_orders_allowed":False}
    if state.get("running"):return {**base,"status":"running","running":True,"source":"LSE_HISTORICAL_MTF_OHLCV","message":"V12.1 MTF Historical Replay กำลังประมวลผล","backtest_window":state.get("request")}
    if not result:return {**base,"status":"no_replay","running":False,"source":"LSE_HISTORICAL_MTF_OHLCV","message":"ยังไม่มีผล V12.1 MTF Historical Replay","replay_trades":[],"performance":None,"strategies":{},"backtest_window":None}
    trades=[]
