...
def _bangkok_display_time(value):
    """Convert every ISO timestamp to Bangkok time; never expose UTC offset in UI."""
    if not value:return "—"
    try:
        text=str(value).strip()
        dt=datetime.fromisoformat(text.replace("Z","+00:00"))
        # Historical/LSE timestamps are UTC when they arrive without an explicit offset.
        if dt.tzinfo is None:dt=dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(BANGKOK).strftime("%d/%m/%Y %H:%M:%S")
    except (TypeError,ValueError):
        return str(value).replace("+00:00","").replace("+00","")

def _no_trade_reason(payload):
    """Return the recorded rejection reason for a NO_TRADE signal."""
...
def _live():
    try:
        ...
        for row in history.list_signals(days=3650,limit=1000):
            ...
            l=p.get("trade_levels") or {};r=p.get("regime") or {}
            raw_time=p.get("candle_time") or p.get("closed_candle") or x.get("candle_time") or x.get("created_at")
            x.update({"engine_version":p.get("engine_version") or ENGINE_VERSION,"strategy":p.get("strategy") or "NONE","engine":p.get("engine") or "NONE","regime":r.get("regime") if isinstance(r,dict) else r,"setup_id":p.get("setup_id") or x.get("setup_key"),"entry_type":p.get("entry_type"),"setup_score":(p.get("setup_score") or {}).get("score") if isinstance(p.get("setup_score"),dict) else p.get("setup_score"),"entry":l.get("entry",x.get("entry")),"sl":l.get("sl",x.get("sl")),"tp":l.get("tp",x.get("tp")),"rr":l.get("risk_reward",l.get("effective_rr",x.get("risk_reward"))),"candle_time":raw_time,"display_time":_bangkok_display_time(raw_time),"no_trade_reason":_no_trade_reason(p),"timeframe_mode":"M5-only"});out.append(x)
        return out
...
def _payload():
    ...
    for report in result.get("reports") or []:
        for t in report.get("trade_history") or []:
            item={**t,"symbol":report.get("symbol")}
            item["display_time"]=_bangkok_display_time(item.get("candle_time") or item.get("created_at"))
            trades.append(item)
