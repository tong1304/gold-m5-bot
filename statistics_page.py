...
def _payload():
    ...
    if state.get("running"):return {**base,"status":"running","running":True,"source":"LSE_HISTORICAL_M5_OHLCV","message":"V12 M5-only Historical Replay กำลังประมวลผล"}
    if not result:return {**base,"status":"no_replay","running":False,"source":"LSE_HISTORICAL_M5_OHLCV","message":"ยังไม่มีผล V12 M5-only Historical Replay","replay_trades":[],"performance":None,"strategies":{}}
    trades=[]
    ...
    return {**base,"status":"ok","running":False,"source":"LSE_HISTORICAL_M5_OHLCV","performance":{"trades":len(trades),"decided":d,"wins":w,"losses":l,"win_rate":round(100*w/d,2) if d else 0.0,"net_r":net,"expectancy_r":round(net/d,4) if d else 0.0},"strategies":{},"replay_trades":sorted(trades,key=lambda x:str(x.get("candle_time") or ""),reverse=True)}
def register(app):
