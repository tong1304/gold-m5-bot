...
def _gold_market_status(now_utc):
    ...
    weekday = now_utc.weekday(); current = now_utc.time()
    if weekday == 5: return False, "WEEKEND_CLOSED"
    if weekday == 6: return (current >= sunday_open, "OPEN" if current >= sunday_open else "SUNDAY_CLOSED")
    if weekday == 4: return (current < friday_close, "OPEN" if current < friday_close else "FRIDAY_CLOSED")
...
def _notify_scheduler_error(exc, context="Scheduler"):
    ...
    message = ("❌ <b>ระบบ Scheduler ขัดข้อง</b>\n\n" f"🕐 เวลา: {now_bkk} (กรุงเทพฯ)\n"
    ...
    try:
        live_scanner = _get_live_scanner(); result = live_scanner.engine.send_telegram(message)
    except Exception:
        try:
            import engine_v5 as engine; result = engine.send_telegram(message)
...
def _get_closed_candle_key(symbol):
    ...
    if df.empty: raise RuntimeError(f"ยังไม่มีแท่ง M5 ที่ปิดแล้วสำหรับ {symbol}")
...
def _send_price_heartbeat(now_bkk):
    ...
    for symbol in _symbols():
        ...
        if not market_open: lines.append(f"⏸ {symbol}: ตลาดปิด ({session})"); continue
        if live_scanner is None: lines.append(f"❌ {symbol}: ดึงราคาจาก Twelve Data ไม่ได้"); continue
        try:
            price, _ = live_scanner.BINANCE.fetch_price(DISPLAY_TO_MARKET[symbol])
            lines.append(f"📊 {symbol}: <b>{price:,.8f}</b>")
        except Exception as exc:
            feed_ok = False; logger.exception("Twelve Data price monitor failed for %s", symbol)
            lines.append(f"❌ {symbol}: ดึงราคาจาก Twelve Data ไม่ได้"); lines.append(f"   └ {type(exc).__name__}: {str(exc)}")
    lines.extend(["", "✅ Scheduler ทำงาน", "✅ Twelve Data Price Feed" if feed_ok else "⚠️ Twelve Data Price Feed มีข้อผิดพลาด", "✅ Telegram Monitor", "", "ℹ️ ข้อความนี้เป็นการทดสอบระบบ", "⛔ ไม่ใช่สัญญาณ BUY/SELL"])
    try:
        if live_scanner is not None: result = live_scanner.engine.send_telegram("\n".join(lines))
        else:
...
def run_scan_cycle():
    ...
    try: live_scanner = _get_live_scanner()
        ...
        logger.warning("[%s] Twelve Data scanner unavailable; skip signal scans: %s", ",".join(symbols), exc)
        for symbol in symbols:
            results.append({"status":"twelve_data_unavailable","symbol":symbol,"market_symbol":DISPLAY_TO_MARKET[symbol],"error_type":type(exc).__name__,"message":str(exc),"telegram_alert_sent":False,"live_orders_allowed":False,"scan_skipped":True})
        if heartbeat is not None: results.append({"status":"price_heartbeat","heartbeat":heartbeat,"timezone":"Asia/Bangkok"})
        logger.warning("[HEARTBEAT] Scheduler cycle END: Twelve Data unavailable; heartbeat=%s", heartbeat); return results
    for symbol in symbols:
        try:
            logger.warning("[%s] Scan step START", symbol)
            if symbol not in live_scanner.SUPPORTED_SYMBOLS: raise RuntimeError(f"ไม่รองรับสินทรัพย์: {symbol}")
            market_open, session = _asset_market_status(symbol, now_utc)
            if not market_open:
                results.append({"status":"market_closed","symbol":symbol,"market_symbol":DISPLAY_TO_MARKET[symbol],"session":session,"telegram_alert_sent":False,"live_orders_allowed":False,"scan_skipped":True,"message":"ตลาดปิดตามเวลาทำการ ระบบข้ามการวิเคราะห์และรอ Session ถัดไป"}); continue
            closed_key = _get_closed_candle_key(symbol); previous = _LAST_CLOSED_CANDLE.get(symbol)
            if previous == closed_key:
                results.append({"status":"waiting_new_candle","symbol":symbol,"timeframe":"M5","closed_candle":closed_key,"message":"ยังไม่มีแท่ง M5 ใหม่ปิด ระบบรอแท่งถัดไป","telegram_alert_sent":False,"live_orders_allowed":False}); continue
            result = live_scanner.scan_once(symbol)
            telegram_sent = bool(isinstance(result, dict) and result.get("telegram_alert_sent")); valid_signal = bool(isinstance(result, dict) and result.get("valid"))
            if valid_signal and not telegram_sent: logger.warning("[%s] VALID signal detected but Telegram alert was not confirmed; candle=%s will be retried", symbol, closed_key)
            else: _LAST_CLOSED_CANDLE[symbol] = closed_key
            result["trigger"] = "NEW_CLOSED_M5_CANDLE"; result["candle_consumed"] = not (valid_signal and not telegram_sent); result["market_session"] = session; results.append(result)
        except Exception as exc:
            logger.exception("[%s] Scan failed", symbol); _notify_scheduler_error(exc, context=f"การสแกน {symbol}")
