"""Multi-Strategy live scanner adapter."""
import os, logging
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        ...
        if not valid:
            ...
            payload={**setup,"signal":"NO_TRADE","result":"NO_TRADE","created_at":datetime.now(timezone.utc).isoformat(),"no_trade_reasons":reasons,"rejection_reasons":reasons}
            recorded=_base.history.record_no_trade(payload)
        ...
        if setup_key in _ALERTED_SIGNAL_KEYS or _base.history.get(signal_id):
            return {"status":"duplicate_suppressed","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":signal,"strategy":setup.get("strategy"),"regime":setup.get("regime"),"signal_id":signal_id,"setup_key":setup_key}
        payload={"signal_id":signal_id,"symbol":symbol,"signal":signal,"closed_candle":candle_time,"created_at":datetime.now(timezone.utc).isoformat(),"engine_version":engine.ENGINE_VERSION,"replay":False,"strategy":setup.get("strategy"),"regime":setup.get("regime"),"strategy_candidates":setup.get("strategy_candidates"),"pattern_signal":signal,"m5_direction":signal,"v10_setup":setup,"structure_bias":setup.get("structure_bias"),"location":setup.get("location"),"liquidity_event":setup.get("liquidity_event"),"m5_trigger":setup.get("m5_trigger"),"pullback":setup.get("pullback"),"target_liquidity":setup.get("target_liquidity"),"rejection_reasons":setup.get("rejection_reasons"),"trade_levels":setup["trade_levels"],"mtf":{"H1":{"bias":setup.get("structure_bias",{}).get("decision")},"M15":{"bias":setup.get("m15_structure",{}).get("bias")},"M5":signal}}
