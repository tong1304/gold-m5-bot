...
def _format_telegram(symbol, setup, signal_id):
    direction = setup["signal"]; levels = setup["trade_levels"]; loc = setup.get("location") or {}
    h1 = setup.get("structure_bias", {}); pattern = (setup.get("pattern") or {}).get("name", "-")
    ind = setup.get("indicator_context") or {}; side = "🟢 BUY — ซื้อ" if direction == "BUY" else "🔴 SELL — ขาย"
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        ...
        suffix = signal if valid else "NO_TRADE"; signal_id = f"{symbol}-{str(candle_time).replace(':','').replace('-','').replace(' ','-')}-{suffix}"; setup["signal_id"] = signal_id
        if not valid:
            ...
            logger.warning("[%s] %s NO_TRADE recorded=%s reasons=%s", symbol, engine.ENGINE_VERSION, recorded, reasons)
            return {"status":"no_trade","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":"NO_TRADE","recorded":recorded,"rejection_reasons":reasons,"no_trade_reasons":reasons,**setup}
