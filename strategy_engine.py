"""Multi-Strategy Engine v10.3 for BTC and XAU/USD.
M15 context -> M5 structure -> strategy evaluation -> closed M5 trigger.
H1 is intentionally unused. No weighted confluence.
"""
...
STRATEGY_REGIMES={
"BTC":{"TREND_PULLBACK":{"TREND_UP","TREND_DOWN"},"BREAKOUT_RETEST":{"TREND_UP","TREND_DOWN","BREAKOUT","VOLATILITY_EXPANSION"},"RANGE_BREAKOUT":{"RANGE","BREAKOUT"},"MOMENTUM":{"TREND_UP","TREND_DOWN","VOLATILITY_EXPANSION"},"VOLATILITY_BREAKOUT":{"BREAKOUT","VOLATILITY_EXPANSION"}},
"GOLD":{"TREND_PULLBACK":{"TREND_UP","TREND_DOWN"},"BREAKOUT_RETEST":{"TREND_UP","TREND_DOWN","BREAKOUT","VOLATILITY_EXPANSION"},"EMA_PULLBACK":{"TREND_UP","TREND_DOWN"},"LIQUIDITY_SWEEP":{"RANGE","BREAKOUT","VOLATILITY_EXPANSION"},"SR_REVERSAL":{"RANGE","BREAKOUT"},"VOLATILITY_BREAKOUT":{"BREAKOUT","VOLATILITY_EXPANSION"}}}
def _num(v,d=0.0):
    try:
...
def _regime(m15,m5):
    ...
    c15=_num(x.close.iloc[-1]); e20=_num(_ema(x,20).iloc[-1]); e50=_num(_ema(x,50).iloc[-1]); a15=_atr(x,14); atr15=_num(a15.iloc[-1]); med15=_num(a15.dropna().tail(30).median()); ratio=atr15/med15 if med15>0 else 0; s15=_structure(x,100)
    direction="BUY" if c15>e20>e50 and s15["bias"]=="BUY" else "SELL" if c15<e20<e50 and s15["bias"]=="SELL" else "NEUTRAL"
    a5=_atr(y,14); atr5=_num(a5.iloc[-1]); med5=_num(a5.dropna().tail(30).median()); vr=atr5/med5 if med5>0 else 0; prior=y.iloc[:-1].tail(20); hi=_num(prior.high.max()); lo=_num(prior.low.min()); close=_num(y.close.iloc[-1]); range_atr=(hi-lo)/max(atr5,1e-9)
...
def _trend_pullback(m5,d):
    ...
    touch=any((_num(x.low.iloc[i])<=_num(e20.iloc[i])+a*.35) if d=="BUY" else (_num(x.high.iloc[i])>=_num(e20.iloc[i])-a*.35) for i in range(max(0,len(x)-10),len(x))); aligned=(last["close"]>_num(e20.iloc[-1])>_num(e50.iloc[-1])) if d=="BUY" else (last["close"]<_num(e20.iloc[-1])<_num(e50.iloc[-1])); confirm=(last["bull"] and last["body_ratio"]>=.25) if d=="BUY" else (last["bear"] and last["body_ratio"]>=.25)
...
def _liquidity_sweep(m5,d):
    x=m5.tail(30).reset_index(drop=True); a=_num(_atr(x,14).iloc[-1]); last=_candle(x); p=x.iloc[:-1].tail(12); hi=_num(p.high.max()); lo=_num(p.low.min()); return (last["low"]<lo-a*.05 and last["close"]>lo and last["bull"]) if d=="BUY" else (last["high"]>hi+a*.05 and last["close"]<hi and last["bear"])
def _sr_reversal(m5,d):
    x=m5.tail(40).reset_index(drop=True); a=_num(_atr(x,14).iloc[-1]); last=_candle(x); p=x.iloc[:-1].tail(20); hi=_num(p.high.max()); lo=_num(p.low.min()); return (last["low"]<=lo+a*.20 and last["close"]>lo and last["lower"]>=last["body"]*1.2) if d=="BUY" else (last["high"]>=hi-a*.20 and last["close"]<hi and last["upper"]>=last["body"]*1.2)
...
def _candidate_order(symbol,regime): return list(BTC_STRATEGIES if symbol=="BTC" else GOLD_STRATEGIES)
def _candidate_directions(strategy,regime,m5,regime_direction):
    if regime_direction in ("BUY","SELL"): return [regime_direction]
    x=m5.tail(21).reset_index(drop=True); last=_candle(x); p=x.iloc[:-1].tail(20); hi=_num(p.high.max()); lo=_num(p.low.min())
...
def _diagnose(strategy,m5,d):
    ...
    if strategy=="TREND_PULLBACK":
        e20=_ema(x,20); e50=_ema(x,50); aligned=(last["close"]>_num(e20.iloc[-1])>_num(e50.iloc[-1])) if d=="BUY" else (last["close"]<_num(e20.iloc[-1])<_num(e50.iloc[-1])); touch=any((_num(x.low.iloc[i])<=_num(e20.iloc[i])+a*.35) if d=="BUY" else (_num(x.high.iloc[i])>=_num(e20.iloc[i])-a*.35) for i in range(max(0,len(x)-10),len(x))); confirm=(last["bull"] and last["body_ratio"]>=.25) if d=="BUY" else (last["bear"] and last["body_ratio"]>=.25)
...
def _log_evaluations(symbol,regime,tested):
    ...
    for item in tested: logger.warning("[%s] STRATEGY EVAL | strategy=%s | direction=%s | status=%s | reason=%s",symbol,item["strategy"],item["direction"],item["status"],item["reason"])
    passed=[f'{x["strategy"]}:{x["direction"]}' for x in tested if x["status"]=="PASS"]
    logger.warning("[%s] STRATEGY EVALUATION SUMMARY | PASS=%s | FAIL=%d | NOT_APPLICABLE=%d",symbol,passed or "NONE",sum(x["status"]=="FAIL" for x in tested),sum(x["status"]=="NOT_APPLICABLE" for x in tested))
def analyze(m5,m15,h1=None,symbol="BTC"):
    ...
    regime=_regime(m15,m5); rn=regime.get("name","NEUTRAL"); direction=regime.get("direction","NEUTRAL"); tested=[]
    for strategy in _candidate_order(symbol,rn):
        applicable=rn in STRATEGY_REGIMES[symbol][strategy]
        if not applicable:
            tested.append({"strategy":strategy,"direction":direction if direction in ("BUY","SELL") else "NEUTRAL","passed":False,"status":"NOT_APPLICABLE","reason":[f"REGIME_{rn}_NOT_SUPPORTED"],"direction_attempts":[]}); continue
        directions=_candidate_directions(strategy,rn,m5,direction); attempts=[]; winner=None
        for d in directions:
            diag_ok,reasons=_diagnose(strategy,m5,d); passed=bool(diag_ok and _FUNCS[strategy](m5,d)); attempts.append({"direction":d,"passed":passed,"reason":["SETUP_VALID"] if passed else reasons})
            if passed: winner=d; break
        if winner:
            tested.append({"strategy":strategy,"direction":winner,"passed":True,"status":"PASS","reason":["SETUP_VALID"],"direction_attempts":attempts})
            _log_evaluations(symbol,rn,tested)
            return {"signal":winner,"valid":True,"strategy":strategy,"regime":rn,"regime_detail":regime,"strategy_candidates":tested,"analysis_window":{"m15_context_bars":100,"m5_structure_bars":50,"m5_setup_bars":20,"m5_trigger_bars":3},"trigger_candle_count":3,"rejection_reasons":[]}
        best=next((a for a in attempts if a["reason"]),attempts[0]); dirs="/".join(a["direction"] for a in attempts)
        tested.append({"strategy":strategy,"direction":dirs,"passed":False,"status":"FAIL","reason":best["reason"],"direction_attempts":attempts})
    _log_evaluations(symbol,rn,tested)
    return {"signal":"NO_TRADE","valid":False,"strategy":"NONE","regime":rn,"regime_detail":regime,"strategy_candidates":tested,"analysis_window":{"m15_context_bars":100,"m5_structure_bars":50,"m5_setup_bars":20,"m5_trigger_bars":3},"rejection_reasons":["NO_STRATEGY_SETUP"]}
