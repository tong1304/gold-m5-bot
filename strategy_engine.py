...
Design: broad context -> market structure -> strategy selection -> 1-3 candle trigger.
No weighted confluence. Exactly one strategy may become active for a setup.
Only closed candles are evaluated by callers.
"""
...
def _regime(m15, m5):
    ...
    a = _atr(x,14).iloc[-1]; med = _atr(x,14).dropna().tail(30).median(); ratio = a/med if _num(med) > 0 else 0
    s15 = _structure(x, 100); direction = "BUY" if c15 > e20 > e50 and s15["bias"] == "BUY" else "SELL" if c15 < e20 < e50 and s15["bias"] == "SELL" else "NEUTRAL"
...
def _trend_pullback(m5, direction):
    x=m5.tail(30).reset_index(drop=True); e20=_ema(x,20); e50=_ema(x,50); c=_num(x.close.iloc[-1]); a=_num(_atr(x,14).iloc[-1]); recent=x.tail(10); touched=False
    for _,r in recent.iterrows():
        if direction=="BUY" and _num(r.low) <= _num(e20.loc[r.name]) + a*0.35: touched=True
        if direction=="SELL" and _num(r.high) >= _num(e20.loc[r.name]) - a*0.35: touched=True
    last=_candle(x,-1); aligned=(c>_num(e20.iloc[-1])>_num(e50.iloc[-1])) if direction=="BUY" else (c<_num(e20.iloc[-1])<_num(e50.iloc[-1]))
    confirm=last["bull"] and last["body_ratio"]>=0.25 if direction=="BUY" else last["bear"] and last["body_ratio"]>=0.25
...
def _breakout_retest(m5,direction):
    x=m5.tail(30).reset_index(drop=True); a=_num(_atr(x,14).iloc[-1]); last=_candle(x,-1); prior=x.iloc[:-1]
...
def _range_breakout(m5,direction):
    x=m5.tail(30).reset_index(drop=True); a=_num(_atr(x,14).iloc[-1]); prior=x.iloc[:-1].tail(20); last=_candle(x,-1); hi=_num(prior.high.max()); lo=_num(prior.low.min())
...
def _momentum(m5,direction):
    x=m5.tail(12).reset_index(drop=True); a=_num(_atr(x,14).iloc[-1]); last=_candle(x,-1); move=_num(x.close.iloc[-1])-_num(x.close.iloc[-6]); threshold=max(a*1.0,1e-9)
...
def _volatility_breakout(m5,direction):
    x=m5.tail(45).reset_index(drop=True); aa=_atr(x,14); a=_num(aa.iloc[-1]); med=_num(aa.dropna().tail(30).median()); last=_candle(x,-1); prior=x.iloc[:-1].tail(20); hi=_num(prior.high.max()); lo=_num(prior.low.min())
    expansion=a/max(med,1e-9)>=1.25
...
def _liquidity_sweep(m5,direction):
    ...
    if direction=="BUY": return last["low"] < lo-a*.05 and last["close"] > lo and last["bull"]
    return last["high"] > hi+a*.05 and last["close"] < hi and last["bear"]
...
def _sr_reversal(m5,direction):
    ...
    if direction=="BUY": return last["low"]<=lo+a*.20 and last["close"]>lo and last["lower"]>=last["body"]*1.2
    return last["high"]>=hi-a*.20 and last["close"]<hi and last["upper"]>=last["body"]*1.2


def _passes(strategy,m5,direction):
    return {
        "TREND_PULLBACK": _trend_pullback(m5,direction),
        "BREAKOUT_RETEST": _breakout_retest(m5,direction),
        "RANGE_BREAKOUT": _range_breakout(m5,direction),
        "MOMENTUM": _momentum(m5,direction),
        "VOLATILITY_BREAKOUT": _volatility_breakout(m5,direction),
        "EMA_PULLBACK": _ema_pullback(m5,direction),
        "LIQUIDITY_SWEEP": _liquidity_sweep(m5,direction),
        "SR_REVERSAL": _sr_reversal(m5,direction),
    }[strategy]
...
def analyze(m5, m15, h1, symbol):
    ...
    regime=_regime(m15,m5); direction=regime.get("direction")
    if direction not in ("BUY","SELL"):
        return {"signal":"NO_TRADE","valid":False,"strategy":"NONE","regime":regime["name"],"regime_detail":regime,"rejection_reasons":["NO_DIRECTIONAL_REGIME"]}
    order=_candidate_order(symbol,regime["name"])
    tested=[]
    for strategy in order:
        passed=_passes(strategy,m5,direction); tested.append({"strategy":strategy,"passed":bool(passed)})
        if passed:
            return {"signal":direction,"valid":True,"strategy":strategy,"regime":regime["name"],"regime_detail":regime,"strategy_candidates":tested,"analysis_window":{"m15_context_bars":100,"m5_structure_bars":50,"m5_setup_bars":20,"m5_trigger_bars":3},"trigger_candle_count":3,"rejection_reasons":[]}
    return {"signal":"NO_TRADE","valid":False,"strategy":"NONE","regime":regime["name"],"regime_detail":regime,"strategy_candidates":tested,"analysis_window":{"m15_context_bars":100,"m5_structure_bars":50,"m5_setup_bars":20,"m5_trigger_bars":3},"rejection_reasons":["NO_STRATEGY_SETUP"]}
