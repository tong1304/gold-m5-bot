...
The module name remains v8 for backward compatibility with scheduler imports.
The actual signal engine is V9.
"""
...
def _load_frames(symbol):
    history_points=max(100,int(os.getenv("LIVE_SIGNAL_HISTORY","200")))
    frames={}
    ...
    for tf in ("1h","15m"):
        if frames[tf].iloc[-1]["datetime"]>latest:
            frames[tf]=frames[tf].iloc[:-1].reset_index(drop=True)
    return frames
