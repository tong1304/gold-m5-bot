"""Structure V8 - standalone live/replay signal engine for BTC + GOLD.
...
def _ema_bias(df):
    if df is None or len(df) < 50:
        return "NEUTRAL"
    c = pd.to_numeric(df.close, errors="coerce")
    e20_series = c.ewm(span=20, adjust=False).mean()
    ...
    if last < e20 and e20 <= e50:
        return "SELL"
    slope = _f(e20 - e20_series.iloc[max(0, len(e20_series)-6)])
    return "BUY" if slope > 0 else "SELL" if slope < 0 else "NEUTRAL"


def _structure(df, lookback=30):
    if df is None or len(df) < 50:
        return {"bias":"NEUTRAL","high":None,"low":None}
...
def _retest(df, mss, direction, tolerance_atr=0.45):
    if not mss:
        return {"valid":False,"reason":"NO_MSS_BOS"}
    level = _f(mss["level"])
    i = len(df)-1
    r = df.iloc[i]
    tol = _atr(df,i)*tolerance_atr
    if direction == "BUY":
        touched = _f(r.low) <= level+tol
        held = _f(r.close) >= level
        confirm = _f(r.close) > _f(r.open)
    else:
        touched = _f(r.high) >= level-tol
        held = _f(r.close) <= level
        confirm = _f(r.close) < _f(r.open)
...
def execution_price(raw, side):
    adverse = max(_f(SPREAD)/2+_f(SLIPPAGE), 0.0)
    p = _f(raw)
...
def analyze_structure_setup(m5,m15,h1,index=None):
    ...
    levels=build_trade_levels(m5,len(m5)-1,direction,invalidation,target)
...
def resolve_trade(direction,entry,sl,tp,future):
    ...
    rr=abs(float(tp)-float(entry))/risk if risk else 0
...
def remove_incomplete_last_candle(df,timeframe_minutes=5):
    if df is None or len(df)==0 or "datetime" not in df.columns:
        return df
    x=df.copy()
    ts=pd.to_datetime(x["datetime"],utc=True,errors="coerce")
    cutoff=pd.Timestamp.now(tz="UTC").floor(f"{int(timeframe_minutes)}min")
    return x.loc[ts < cutoff].reset_index(drop=True)
...
def evaluate_live_risk_guard(*args, **kwargs):
    ...
    def integer(name, default=0):
        try: return int(kwargs.get(name,default))
    ...
    reasons=[]
    if max_price_jump_atr>0 and price_jump_atr>=max_price_jump_atr: reasons.append("PRICE_JUMP_ATR")
    if max_daily_loss_r>0 and daily_loss_r<=-abs(max_daily_loss_r): reasons.append("MAX_DAILY_LOSS_R")
    if max_consecutive_losses>0 and consecutive_losses>=max_consecutive_losses: reasons.append("MAX_CONSECUTIVE_LOSSES")
    if max_trades_per_day>0 and trades_today>=max_trades_per_day: reasons.append("MAX_TRADES_PER_DAY")
    if max_slippage>0 and slippage>max_slippage: reasons.append("MAX_SLIPPAGE")
    return {"allowed":not reasons,"blocked":bool(reasons),"reasons":reasons,"price_jump_atr":price_jump_atr,"daily_loss_r":daily_loss_r,"consecutive_losses":consecutive_losses,"trades_today":trades_today,"slippage":slippage}
...
def send_telegram(message):
    ...
    if not token or not chat_id:
        return {"success":False,"message":"Telegram credentials are not configured"}
    try:
        response=requests.post(f"https://api.telegram.org/bot{token}/sendMessage",json={"chat_id":chat_id,"text":str(message),"parse_mode":"HTML"},timeout=15)
        response.raise_for_status()
        data=response.json()
        return {"success":bool(data.get("ok")),"message":"Telegram message sent successfully" if data.get("ok") else str(data)}
