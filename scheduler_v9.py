"""V9 scheduler for BTC + GOLD using LSE data.
...
def _record_data_no_trade(symbol,reason):
    try:
        ...
        payload={"signal_id":signal_id,"symbol":symbol,"signal":"NO_TRADE","result":"NO_TRADE","closed_candle":candle_time,"candle_time":candle_time,"created_at":datetime.now(UTC).isoformat(),"engine_version":"V9","rejection_reasons":[str(reason)],"no_trade_reasons":[str(reason)],"data_valid":False}
...
def _system_test(now_bkk):
    ...
    lines=["🧪 <b>ทดสอบระบบ V9 ทุก 15 นาที</b>","",f"🕐 เวลา: {now_bkk.strftime('%d/%m/%Y %H:%M')} (กรุงเทพฯ)",""]; live_ok=True
    try:
    ...
    lines += ["","✅ V9 Scheduler ทำงาน","✅ LSE WebSocket Live Price" if live_ok else "⚠️ LSE WebSocket Live Price มีปัญหา","✅ Telegram Monitor","","ℹ️ การทดสอบนี้ไม่ใช่สัญญาณ BUY/SELL"]
    try:
...
def run_scan_cycle():
    now_bkk=datetime.now(UTC).astimezone(BANGKOK); now_utc=now_bkk.astimezone(UTC); symbols=_symbols(); logger.warning("[HEARTBEAT] V9 Scheduler cycle START: %s | symbols=%s | interval=%ss | provider=LSE",now_bkk.strftime("%d/%m/%Y %H:%M:%S"),symbols,_interval_seconds())
    ...
    for symbol in symbols:
        try:
            ...
            logger.warning("[%s] Fetching closed M5 candles from LSE",symbol); frame=scanner._lse_frame(symbol,"5m",max(100,int(os.getenv("LIVE_SIGNAL_HISTORY","200"))))
            ...
            if _LAST_CLOSED_CANDLE.get(symbol)==closed_key: logger.warning("[%s] WAITING_NEW_CANDLE: %s",symbol,closed_key); results.append({"status":"waiting_new_candle","symbol":symbol,"timeframe":"M5","closed_candle":closed_key,"live_orders_allowed":False}); continue
            logger.warning("[%s] Calling V9 Signal Engine",symbol); result=scanner.scan_once(symbol); _LAST_CLOSED_CANDLE[symbol]=closed_key; result["trigger"]="NEW_CLOSED_M5_CANDLE"; result["candle_consumed"]=True; result["market_session"]=session; logger.warning("[%s] V9 result: signal=%s status=%s recorded=%s",symbol,result.get("signal"),result.get("status"),result.get("recorded")); results.append(result)
        except Exception as exc:
            message=str(exc); logger.exception("[%s] V9 Scan failed",symbol)
            if any(tag in message for tag in ("STALE_MARKET_DATA","DATA_INVALID","LSE_RECENT_DATA_UNAVAILABLE")):
                recorded=_record_data_no_trade(symbol,message); results.append({"status":"data_invalid","symbol":symbol,"signal":"NO_TRADE","recorded":recorded,"reason":message,"data_valid":False,"live_orders_allowed":False})
            else: _notify_error(exc,f"การสแกน V9 {symbol}"); results.append({"status":"scan_error","symbol":symbol,"error_type":type(exc).__name__,"message":message,"live_orders_allowed":False})
    ...
    logger.warning("[HEARTBEAT] V9 Scheduler cycle END: processed=%d symbol(s) | provider=LSE",len(results)); return results
def _seconds_to_next_five_minute(): return max(1,300-(datetime.now(UTC).timestamp()%300))
def _loop():
    global _RUNNING
    logger.warning("[BOOT TEST] Running immediate V9 scan after scheduler startup")
    try: run_scan_cycle()
    except Exception as exc: logger.exception("Immediate V9 scheduler cycle failed"); _notify_error(exc,"รอบทดสอบ V9 ทันทีหลัง Scheduler เริ่ม")
    while _RUNNING:
        wait=_seconds_to_next_five_minute(); logger.warning("[HEARTBEAT] Next scheduled V9 cycle in %.1fs",wait); time.sleep(wait)
        if not _RUNNING: break
        started=time.monotonic()
        try: run_scan_cycle()
        except Exception as exc: logger.exception("Fatal V9 scheduler cycle error"); _notify_error(exc,"รอบการทำงานหลักของ V9 Scheduler")
        elapsed=time.monotonic()-started; wait=_seconds_to_next_five_minute(); logger.warning("[HEARTBEAT] V9 Scheduler cycle returned; elapsed=%.2fs; next_cycle_in=%.1fs; provider=LSE",elapsed,wait)
def start():
    global _RUNNING,_THREAD
    if _RUNNING and _THREAD and _THREAD.is_alive(): return False
    _RUNNING=True; _THREAD=threading.Thread(target=_loop,name="m5-btc-gold-v9-scanner",daemon=True); _THREAD.start(); logger.warning("V9 Signal Scheduler started successfully; thread=%s",_THREAD.name); return True
def stop():
...
def status():
    ...
    return {"running":bool(_RUNNING and _THREAD and _THREAD.is_alive()),"interval_seconds":_interval_seconds(),"symbols":_symbols(),"test_slots":"00,15,30,45","timezone":"Asia/Bangkok","provider":"LSE","engine_version":"V9","live_price":live,"statistics_page":"/statistics","statistics_api":"/api/statistics","market_sessions":{s:{"open":_asset_market_status(s,now)[0],"session":_asset_market_status(s,now)[1]} for s in _symbols()}}
