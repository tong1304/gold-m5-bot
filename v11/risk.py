...
def calculate(m5,direction: str,strategy: str,evidence: dict|None=None,*,rr: float=MIN_RISK_REWARD):
    ...
    raw=num(raw)
    if direction=="BUY": sl=min(raw-a*.10,entry-a*.80); risk=entry-sl
    else: sl=max(raw+a*.10,entry+a*.80); risk=sl-entry
    target_price=evidence.get("target_price")
    if target_price is not None:
        target_price=num(target_price)
        tp=target_price if target_price>0 else (entry+rr*risk if direction=="BUY" else entry-rr*risk)
    else: tp=entry+rr*risk if direction=="BUY" else entry-rr*risk
    if not all(math.isfinite(v) for v in (sl,tp,risk)) or risk<=0 or min(entry,sl,tp)<=0:return {"valid":False,"reason":"INVALID_RISK"}
