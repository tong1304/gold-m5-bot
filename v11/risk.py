...
def _nearest_levels(df,direction,entry,atr):
    ...
    if direction=="BUY":
        below=sorted({v for _,v in supports if v<entry and entry-v>=gap},reverse=True); above=sorted({v for _,v in resistances if v>entry})
        return (below[0] if below else None),above
    above=sorted({v for _,v in resistances if v>entry and v-entry>=gap}); below=sorted({v for _,v in supports if v<entry},reverse=True)
...
def calculate(m5,direction:str,strategy:str,evidence:dict|None=None,*,rr:float=MIN_RISK_REWARD):
    ...
    sl=sl_level-buffer if direction=="BUY" else sl_level+buffer; risk=abs(entry-sl)
    ...
    if first_rr<MIN_RISK_REWARD:return {"valid":False,"reason":"NEAREST_STRUCTURE_RR_BELOW_2","entry":entry,"sl":sl,"risk":risk,"first_tp":first_level,"first_tp_rr":round(first_rr,4),"target_rr":MIN_RISK_REWARD,"strategy":strategy}
