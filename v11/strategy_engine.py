...
def _vwap(x):
    ...
    if "datetime" in x:
        ts=pd.to_datetime(x.datetime,utc=True,errors="coerce");d=ts.dt.date
        return (typ*vol).groupby(d).cumsum()/vol.groupby(d).cumsum()
...
def _result(eid,direction,anchor,evidence,score,trigger_sig):
    return {"status":"PASS","engine":eid,"strategy":ENGINE_NAMES[eid],"direction":direction,"setup_anchor":anchor,"evidence":evidence,"quality":float(max(0,min(100,score))),"trigger_signature":trigger_sig}
...
def evaluate_strategy(engine_id,m5,context,direction):
    ...
    a=_atr(x);last=candle_metrics(x.iloc[-1]);prev=candle_metrics(x.iloc[-2]);s=structure(x,min(80,len(x)));reg=context.get("regime","RANGE");rctx=context
    if eid=="E1":
    ...
    if eid=="E2":
        ...
        e20=ema(x,20).iloc[-1]; anchor=lows[-1][1] if direction=="BUY" else highs[-1][1]
        touched=(x.low.iloc[lows[-1][0]+1:].min()<=e20+.35*a) if direction=="BUY" else (x.high.iloc[highs[-1][0]+1:].max()>=e20-.35*a)
        impulse=abs(num(x.close.iloc[-1])-num(x.close.iloc[-min(6,len(x))]))>=a
        ok=(context.get("direction") in (direction,"NEUTRAL")) and touched and impulse and _confirm(x,direction)
        return _result(eid,direction,anchor,{"ema20":num(e20),"atr":a,"impulse_atr":abs(last["close"]-num(x.close.iloc[-min(6,len(x))]))/max(a,1e-12),"setup_anchor":anchor},86,"E2|continuation|%s|%s"%(direction,last["close"])) if ok else _fail(eid,direction,["IMPULSE_PULLBACK_CONTINUATION_FAILED"])
    if eid in ("E3","E4"):
        look=x.iloc[-21:-1] if len(x)>=22 else x.iloc[:-1];rh=num(look.high.max());rl=num(look.low.min())
        if eid=="E3":
            ok=(direction=="BUY" and last["close"]>rh and last["body_ratio"]>=.60 and _volume_ratio(x)>=1.2) or (direction=="SELL" and last["close"]<rl and last["body_ratio"]>=.60 and _volume_ratio(x)>=1.2)
        ...
        prior=x.iloc[-4:-1];touched=(num(prior.low.min())<=rh+.25*a and last["close"]>rh) if direction=="BUY" else (num(prior.high.max())>=rl-.25*a and last["close"]<rl)
        ok=touched and _confirm(x,direction)
        anchor=rh if direction=="BUY" else rl
    ...
    if eid=="E5":
        atr_ratio=(a/max(num(atr14(x).dropna().iloc[-6]),1e-12)) if len(atr14(x).dropna())>=6 else 1
        ok=last["body"]>=.70*a and last["body_ratio"]>=.65 and _volume_ratio(x)>=1.5 and atr_ratio>=1.05 and ((direction=="BUY" and last["bull"]) or (direction=="SELL" and last["bear"]))
    ...
    if eid=="E6":
        ...
        rejection=(last["lower_wick"]>=last["body"]*.8 and last["close"]>last["open"]) if direction=="BUY" else (last["upper_wick"]>=last["body"]*.8 and last["close"]<last["open"])
        ok=dist>=1.5 and rejection and ((direction=="BUY" and last["close"]<v) or (direction=="SELL" and last["close"]>v))
    ...
    if eid=="E7":
        ...
        sweep=prev["low"]<level and prev["close"]>level if direction=="BUY" else prev["high"]>level and prev["close"]<level
        rejection=prev["lower_wick"]>=max(prev["body"],.15*a) if direction=="BUY" else prev["upper_wick"]>=max(prev["body"],.15*a)
        ok=sweep and rejection and _confirm(x,direction)
    ...
    if eid=="E8":
        rh=num(x.high.iloc[-21:-1].max());rl=num(x.low.iloc[-21:-1].min());anchor=rl if direction=="BUY" else rh
        rejection=(last["low"]<=rl+.20*a and last["bull"] and last["lower_wick"]>=last["body"]*.5) if direction=="BUY" else (last["high"]>=rh-.20*a and last["bear"] and last["upper_wick"]>=last["body"]*.5)
        ok=reg=="RANGE" and rejection
...
def score_setup(result,regime,rr=None):
    ...
    ev=result.get("evidence") or {};score=45.0;components={"setup":45.0}
    if ev.get("volume_ratio",0)>=1.2 or ev.get("volume_ratio",0)>=1.5:score+=10;components["volume"] = 10
    if ev.get("atr_expansion",0)>=1.05:score+=10;components["volatility"] = 10
    if result.get("engine") in ("E1","E2") and regime=="TREND":score+=15;components["regime_fit"]=15
    if result.get("engine") in ("E6","E8") and regime=="RANGE":score+=15;components["regime_fit"]=15
    if result.get("engine") in ("E3","E4","E7") and regime=="TRANSITION":score+=15;components["regime_fit"]=15
...
def evaluate_all_allowed(m5,context):
    engines=context.get("allowed_engines",[]);reg=context.get("regime","RANGE");directions=[context.get("direction")] if context.get("direction") in ("BUY","SELL") else ["BUY","SELL"];out=[]
    for eid in engines:
        for direction in directions:
            result=evaluate_strategy(eid,m5,context,direction)
            if result.get("status")=="PASS":
                result["score_detail"]=score_setup(result,reg);out.append(result)
...
def enrich_selected(result,symbol,regime,candle_time):
    anchor=result.get("setup_anchor"); setup_id=build_setup_id(symbol,regime,result["engine"],result["direction"],anchor)
    trigger=build_trigger_id(result["engine"],result["direction"],candle_time,result.get("trigger_signature",""));return {**result,"setup_id":setup_id,"trigger_id":trigger,"regime":regime,"symbol":symbol}
