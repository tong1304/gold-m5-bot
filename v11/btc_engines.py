...
def _reversal_bull(x):
    ...
    return (_bull(a) and _bear(b) and _num(a.open)<=_num(b.close) and _num(a.close)>=_num(b.open)) or (_bull(a) and lower>=max(body*1.5,_candle_range(a)*.45) and (_num(a.close)-_num(a.low))/_candle_range(a)>=.65)
def _reversal_bear(x):
    if len(x)<2:return False
    a,b=x.iloc[-1],x.iloc[-2];body=_body(a);upper=_num(a.high)-max(_num(a.open),_num(a.close))
    return (_bear(a) and _bull(b) and _num(a.open)>=_num(b.close) and _num(a.close)<=_num(b.open)) or (_bear(a) and upper>=max(body*1.5,_candle_range(a)*.45) and (_num(a.close)-_num(a.low))/_candle_range(a)<=.35)
...
def _evaluate_b1(x,direction):
    ...
    rh,rl=_range_levels(x,20);current=x.iloc[-1];avg=_avg_body(x.iloc[:-1],min(20,len(x)-1));sweep=None
    for i in range(max(1,len(x)-6),len(x)-1):
        c=x.iloc[i]
        if direction=="BUY" and _num(c.low)<rl and _num(c.close)>rl:sweep=c
        if direction=="SELL" and _num(c.high)>rh and _num(c.close)<rh:sweep=c
    if sweep is None:return _fail("B1",direction,"RANGE_SWEEP_FAILED")
    fvg=_recent_fvg(x,direction);swing=_swing_high(x,8) if direction=="BUY" else _swing_low(x,8);displacement=(_num(current.close)>swing and _bull(current) if direction=="BUY" else _num(current.close)<swing and _bear(current)) and _body(current)>=avg*1.5
    if not(displacement and fvg):return _fail("B1",direction,"DISPLACEMENT_OR_FVG_FAILED")
    entry=fvg["top"] if direction=="BUY" else fvg["bottom"];sl=_num(sweep.low)-BTC_POINT_BUFFER if direction=="BUY" else _num(sweep.high)+BTC_POINT_BUFFER;risk=entry-sl if direction=="BUY" else sl-entry
    if risk<=0:return _fail("B1",direction,"INVALID_B1_RISK")
    tp=max(rh,entry+risk*1.5) if direction=="BUY" else min(rl,entry-risk*1.5);rr=(tp-entry)/risk if direction=="BUY" else (entry-tp)/risk
    return _result("B1",direction,rl if direction=="BUY" else rh,{"range_high":rh,"range_low":rl,"sweep_low":_num(sweep.low) if direction=="BUY" else None,"sweep_high":_num(sweep.high) if direction=="SELL" else None,"swing_high":swing if direction=="BUY" else None,"swing_low":swing if direction=="SELL" else None,"fvg":fvg,"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":rr,"point_buffer":BTC_POINT_BUFFER},94,f"B1|range-sweep-displacement|{direction}|{entry}","BUY_LIMIT" if direction=="BUY" else "SELL_LIMIT")
def _evaluate_b2(x,m15,h1,direction):
    ...
    current=x.iloc[-1];fvg=_recent_fvg(x.iloc[:-1],direction)
    if direction=="BUY":
        ...
        if choch and fvg and fvg["width"]>=BTC_FVG_MIN_POINTS and retest and confirm:
            entry=_num(current.close);sl=swing_low-BTC_POINT_BUFFER;tp=entry+abs(entry-sl)*2.0;return _result("B2",direction,swing_low,{"htf_zone_m15":z15,"htf_zone_h1":z1,"m5_swing_low":swing_low,"m15_swing_high":_num(m15.high.tail(20).max()) if m15 is not None else None,"fvg":fvg,"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":2.0},92,f"B2|htf-zone-fvg-retest|BUY|{entry}")
    ...
    if choch and fvg and fvg["width"]>=BTC_FVG_MIN_POINTS and retest and confirm:
        entry=_num(current.close);sl=swing_high+BTC_POINT_BUFFER;tp=entry-abs(entry-sl)*2.0;return _result("B2",direction,swing_high,{"htf_zone_m15":z15,"htf_zone_h1":z1,"m5_swing_high":swing_high,"m15_swing_low":_num(m15.low.tail(20).min()) if m15 is not None else None,"fvg":fvg,"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":2.0},92,f"B2|htf-zone-fvg-retest|SELL|{entry}")
...
def _evaluate_b3(x,direction):
    ...
    if direction=="BUY" and _num(current.low)<=resistance and _bull(current):
        ...
        if risk>0:tp=entry+risk*1.5;return _result("B3",direction,resistance,{"resistance":resistance,"support":support,"breakout_high":_num(breakout.high),"breakout_low":_num(breakout.low),"fvg":fvg,"volume_ratio":_volume_ratio(x),"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":1.5},90,f"B3|volatility-breakout-retest|BUY|{entry}")
    if direction=="SELL" and _num(current.high)>=support and _bear(current):
        ...
        if risk>0:tp=entry-risk*1.5;return _result("B3",direction,support,{"resistance":resistance,"support":support,"breakout_high":_num(breakout.high),"breakout_low":_num(breakout.low),"fvg":fvg,"volume_ratio":_volume_ratio(x),"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":1.5},90,f"B3|volatility-breakout-retest|SELL|{entry}")
...
def evaluate_btc_engines(m5,m15=None,h1=None):
    ...
    for direction in ("BUY","SELL"):
        for eid,fn,args in (("B1",_evaluate_b1,(x,direction)),("B2",_evaluate_b2,(x,m15,h1,direction)),("B3",_evaluate_b3,(x,direction))):
            r=fn(*args);trace.append(f"BTC {eid}:{direction}:{r['status']}"+(f" reason={r['rejection_reasons'][0]}" if r.get('rejection_reasons') else ""));
