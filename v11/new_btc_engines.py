...
BTC_NEW_ENGINE_NAMES={"B1":"RANGE_SWEEP_DISPLACEMENT","B2":"HTF_OB_M5_FVG_RETEST","B3":"VOLATILITY_EXPANSION_BREAKOUT_RETEST"}
...
def _fvg(x,i,direction):
    ...
    if direction=="BUY" and _n(c.low)>_n(a.high):return {"bottom":_n(a.high),"top":_n(c.low),"ce":(_n(a.high)+_n(c.low))/2,"index":i,"type":"BULLISH_FVG"}
    if direction=="SELL" and _n(c.high)<_n(a.low):return {"bottom":_n(c.high),"top":_n(a.low),"ce":(_n(a.low)+_n(c.high))/2,"index":i,"type":"BEARISH_FVG"}
    return None
...
def _fail(eid,direction,reason,e=None):return {"status":"FAIL","engine":eid,"strategy":BTC_NEW_ENGINE_NAMES[eid],"direction":direction,"quality":0.0,"rejection_reasons":[reason],"evidence":e or {}}
def _pass(eid,direction,e,quality,entry_type):return {"status":"PASS","engine":eid,"strategy":BTC_NEW_ENGINE_NAMES[eid],"direction":direction,"setup_anchor":e.get("setup_anchor"),"evidence":e,"quality":float(quality),"trigger_signature":f"{eid}|{direction}|{e.get('entry_price')}","entry_type_hint":entry_type,"rejection_reasons":[]}
def _rr(entry,sl,tp,direction):return ((tp-entry) if direction=="BUY" else (entry-tp))/max(abs(entry-sl),1e-12)

def _b1(x,direction):
    if len(x)<50:return _fail("B1",direction,"INSUFFICIENT_M5_RANGE_CONTEXT")
    z=x.iloc[-49:-1];rh,rl=_n(z.high.max()),_n(z.low.min());a=_atr(x);sweep=None
    ...
    def mss(i):
        c=x.iloc[i]
        return (_n(c.close)>internal and _bull(c)) if direction=="BUY" else (_n(c.close)<internal and _bear(c))
    ci=next((i for i in range(si+1,len(x)-1) if mss(i)),None)
...
def _b2(x,m15,h1,direction):
    ...
    ob1,ob15=_htf_ob(h1,direction),_htf_ob(m15,direction);zone=ob1 or ob15
    ...
    if zone.get("index") is not None and zone.get("index")>=len(htf)-2:return _fail("B2",direction,"HTF_OB_NOT_CONFIRMED")
    hs,ls=_pivots(x,2);fvg=_latest_fvg(x,direction,max(2,len(x)-18),len(x))
    if not fvg:return _fail("B2",direction,"M5_FVG_REQUIRED")
    if direction=="BUY":
        prior_low=x.low.iloc[max(0,len(x)-12):-2].min();sweep=next((i for i in range(len(x)-2,max(2,len(x)-18),-1) if _n(x.iloc[i].low)<_n(x.low.iloc[max(0,i-8):i].min()) and _n(x.iloc[i].close)>_n(x.low.iloc[max(0,i-8):i].min())),None);level=hs[-1][1] if hs else None;choch=level is not None and _n(x.close.iloc[-1])>level;entry=fvg["ce"];sl=_n(x.low.iloc[max(0,fvg["index"]-2):fvg["index"]+1].min())-.15*_atr(x);tp=_n(h1.high.tail(60).max()) if h1 is not None and not h1.empty else entry+4*_atr(x)
    else:
        prior_high=x.high.iloc[max(0,len(x)-12):-2].max();sweep=next((i for i in range(len(x)-2,max(2,len(x)-18),-1) if _n(x.iloc[i].high)>_n(x.high.iloc[max(0,i-8):i].max()) and _n(x.iloc[i].close)<_n(x.high.iloc[max(0,i-8):i].max())),None);level=ls[-1][1] if ls else None;choch=level is not None and _n(x.close.iloc[-1])<level;entry=fvg["ce"];sl=_n(x.high.iloc[max(0,fvg["index"]-2):fvg["index"]+1].max())+.15*_atr(x);tp=_n(h1.low.tail(60).min()) if h1 is not None and not h1.empty else entry-4*_atr(x)
    ...
    return _pass("B2",direction,{"htf_ob":zone,"htf_timeframe":"1H" if ob1 else "15M_FALLBACK","m5_sweep_index":sweep,"m5_choch_level":level,"fvg":fvg,"entry_price":entry,"sl_price":sl,"tp_price":tp,"risk_reward":rr,"setup_anchor":zone["low"]},93,"LIMIT")
...
def _b3(x,direction):
    ...
    e20=ema(x,20);e50=ema(x,50);avg_vol=_n(pd.to_numeric(x.volume.iloc[-21:-1],errors="coerce").mean(),0) if "volume" in x else 0;v=_n(x.volume.iloc[-1],0) if "volume" in x else 0;c=x.iloc[-1]
    if direction=="BUY":resistance=_n(x.high.iloc[-25:-1].max());breakout=_bull(c) and _n(c.close)>_n(upper.iloc[-1]) and _n(c.close)>resistance;align=_n(e20.iloc[-1])>=_n(e50.iloc[-1])
    else:resistance=_n(x.low.iloc[-25:-1].min());breakout=_bear(c) and _n(c.close)<_n(lower.iloc[-1]) and _n(c.close)<resistance;align=_n(e20.iloc[-1])<=_n(e50.iloc[-1])
