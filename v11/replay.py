...
def replay_frames(m5: pd.DataFrame, m15: pd.DataFrame, symbol: str, *, limit: int | None = None):
    ...
    rows=[]
    start=max(60,len(m5)-limit) if limit else 60
    for i in range(start,len(m5)):
        ts=m5.iloc[i].datetime
        # Only M15 candles closed by this M5 candle's close are visible.
        context=m15[m15.datetime <= ts].reset_index(drop=True)
