...
def _flatten_trade(row: dict) -> dict:
    ...
    return {
        ...
        "sl": levels.get("sl"),
        "tp": levels.get("tp"),
        "tp1": levels.get("tp1", levels.get("tp")),
        "tp2": levels.get("tp2"),
        "tp3": levels.get("tp3"),
        "rr": levels.get("risk_reward", levels.get("effective_rr")),
        "risk_reward": levels.get("risk_reward", levels.get("effective_rr")),
        "trade_levels": levels,
...
def _resolve_active(active: dict, candle: pd.Series) -> dict | None:
    ...
    sl = float(active["trade_levels"]["sl"]); tp = float(active["trade_levels"]["tp"])
    hit_sl = low <= sl if direction == "BUY" else high >= sl
    hit_tp = high >= tp if direction == "BUY" else low <= tp
    if hit_sl and hit_tp:
        return {"result": "AMBIGUOUS", "r_multiple": 0.0, "resolved_at": str(candle.datetime)}
    if hit_tp:
        entry = float(active["trade_levels"]["entry"])
        return {"result": "WIN", "r_multiple": round(abs(tp-entry)/abs(entry-sl), 4), "resolved_at": str(candle.datetime)}
    if hit_sl:
        return {"result": "LOSS", "r_multiple": -1.0, "resolved_at": str(candle.datetime)}
    return None
...
def summarize_rows(rows: list[dict]) -> dict:
    ...
    for row in rows:
        ...
        if result not in counts: result="NO_TRADE"
        counts[result]+=1; strategy=str(row.get("strategy") or "NONE")
    ...
    wins,losses=counts["WIN"],counts["LOSS"]; decided=wins+losses; trades=decided+counts["OPEN"]+counts["AMBIGUOUS"]
    gross_profit=round(sum(r for r in decided_r if r>0),4); gross_loss=round(abs(sum(r for r in decided_r if r<0)),4); net_r=round(sum(decided_r),4)
...
def _bounded_context(frame: pd.DataFrame, end_position: int, max_bars: int) -> pd.DataFrame:
    end_position = max(0, min(int(end_position), len(frame))); start = max(0, end_position - max_bars)
...
def replay_frames(m5: pd.DataFrame, m15: pd.DataFrame, symbol: str, *, limit: int|None=None, start_time=None, end_time=None, progress_callback: Callable[..., None]|None=None):
    """Chronological replay using the same one-active-signal/setup-lock policy as Live V11."""
    ...
    context_ends=_m15_context_end_positions(m5,m15); rows=[]; trade_history=[]; total=len(indices); active=None
    progress_counts={"trades":0,"wins":0,"losses":0,"open":0,"ambiguous":0,"net_r":0.0}
    if progress_callback: progress_callback("engine_progress", symbol=symbol, completed=0, total=total, percent=0, **progress_counts)
    for n,i in enumerate(indices, start=1):
        candle=m5.iloc[i]; ts=candle.datetime
        # Resolve only from candles that occur after the active signal. This mirrors Live resolve_open_for_symbol().
        if active:
            outcome=_resolve_active(active,candle)
            if outcome:
                active.update(outcome)
                for trade in reversed(trade_history):
                    if trade.get("signal_id") == active.get("signal_id"):
                        trade.update(outcome); break
                active=None
        if active:
            ...
            setup.update({"candle_time":str(ts),"closed_candle":str(ts),"symbol":symbol,"engine_version":engine.ENGINE_VERSION})
            signal=setup.get("signal"); levels=setup.get("trade_levels") or {}; valid=bool(setup.get("valid")) and signal in ("BUY","SELL") and bool(levels.get("valid")) and float(levels.get("risk_reward",levels.get("effective_rr",0)) or 0) >= 2.0
            setup["valid"]=valid
            if valid:
                setup["setup_key"]=_setup_key(setup)
                signal_id=f"REPLAY-{symbol}-{ts.strftime('%Y%m%dT%H%M%SZ')}-{signal}"
                setup["signal_id"]=signal_id
                active={"signal":signal,"strategy":setup.get("strategy","NONE"),"trade_levels":levels,"setup_key":setup["setup_key"],"signal_id":signal_id}
                trade=_flatten_trade({**setup,"result":"OPEN","r_multiple":0.0,"resolved_at":None,"symbol":symbol})
                trade_history.append(trade)
                rows.append({**setup,"result":"OPEN","r_multiple":0.0,"resolved_at":None})
                progress_counts["trades"] += 1; progress_counts["open"] += 1
            else:
                rows.append({**setup,"signal":"NO_TRADE" if signal not in ("BUY","SELL") else signal,"result":"NO_TRADE","r_multiple":0.0,"resolved_at":None,"lock_reason":None})
        if progress_callback and (n == total or n == 1 or n % 10 == 0):
            decided_rows=[t for t in trade_history if t.get("result") in ("WIN","LOSS")]
            progress_counts["wins"]=sum(t.get("result")=="WIN" for t in trade_history); progress_counts["losses"]=sum(t.get("result")=="LOSS" for t in trade_history); progress_counts["open"]=sum(t.get("result")=="OPEN" for t in trade_history); progress_counts["ambiguous"]=sum(t.get("result")=="AMBIGUOUS" for t in trade_history); progress_counts["net_r"]=round(sum(float(t.get("r_multiple") or 0) for t in decided_rows),4)
