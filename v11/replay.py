...
def _pct(n:int,d:int)->float:return round(100.0*n/d,2) if d else 0.0
...
def summarize_rows(rows:list[dict])->dict:
    counts={k:0 for k in ("WIN","LOSS","OPEN","AMBIGUOUS","NO_TRADE")};decided_r=[];strategies={}
    for row in rows:
        ...
        if result in ("WIN","LOSS"):
            r=float(row.get("r_multiple") or 0.0);decided_r.append(r);s["net_r"]+=r
...
def replay_frames(m5:pd.DataFrame,m15:pd.DataFrame,symbol:str,*,limit:int|None=None,start_time=None,end_time=None,progress_callback:Callable[...,None]|None=None):
    ...
    for n,i in enumerate(indices,start=1):
        ...
        if active:
            ...
            if outcome:
                setup_key_closed=active.get("setup_key")
                signal_id_closed=active.get("signal_id")
                active.update(outcome)
                for trade in reversed(trade_history):
                    if trade.get("signal_id")==signal_id_closed:trade.update(outcome);break
                active=None;last_closed_setup_key=setup_key_closed
        if active:
            ...
            if valid:
                ...
                if last_closed_setup_key and setup["setup_key"]==last_closed_setup_key:
                    ...
                    signal_id=f"REPLAY-{symbol}-{ts.strftime('%Y%m%dT%H%M%SZ')}-{signal}";setup["signal_id"]=signal_id;active={"signal":signal,"strategy":setup.get("strategy","NONE"),"trade_levels":levels,"setup_key":setup["setup_key"],"signal_id":signal_id};trade_history.append(_flatten_trade({**setup,"result":"OPEN","r_multiple":0.0,"resolved_at":None,"symbol":symbol}));rows.append({**setup,"result":"OPEN","r_multiple":0.0,"resolved_at":None})
    ...
    for row in rows:
        trade=outcome_by_id.get(row.get("signal_id"))
        if trade:row.update({"result":trade.get("result"),"r_multiple":trade.get("r_multiple"),"resolved_at":trade.get("resolved_at")})
    summary=summarize_rows(rows)
    # Performance is based on actual replay trades, not repeated locked candle evaluations.
    trade_rows=[]
    for trade in trade_history:
        trade_rows.append(trade)
    trade_summary=summarize_rows([{"strategy":t.get("strategy"),"result":t.get("result"),"r_multiple":t.get("r_multiple",0)} for t in trade_history])
    return {"status":"completed","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"candles_evaluated":len(rows),"signals":len(trade_history),"wins":trade_summary["wins"],"losses":trade_summary["losses"],"ambiguous":trade_summary["ambiguous"],"open":trade_summary["open"],"net_r":trade_summary["net_r"],"performance":trade_summary|{"rows":len(rows),"evaluated_rows":summary["rows"],"locked_rows":sum(r.get("lock_reason") in ("ACTIVE_SIGNAL_LOCK","SAME_SETUP_AFTER_PREVIOUS_SIGNAL") for r in rows)},"rows":rows,"trade_history":trade_rows,"live_orders_allowed":False,"m15_policy":"CLOSED_AT_M5_CLOSE_MINUS_15M","lookahead_safe":True,"warmup_bars":60,"signal_lock_policy":"ONE_ACTIVE_SIGNAL_PER_SYMBOL","setup_lock_policy":"ONE_SIGNAL_PER_SETUP_UNTIL_NEW_SETUP"}
