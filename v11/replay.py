...
def _resolve_trade(trade,candle):
    high=float(candle.high);low=float(candle.low);direction=trade["signal"];levels=trade["trade_levels"];entry=float(levels["entry"]);sl=float(levels["sl"]);tp=float(levels["tp"])
...
def _max_drawdown(values):
    ...
    for value in values:
        equity+=float(value);peak=max(peak,equity);drawdown=max(drawdown,peak-equity)
...
def replay_frames(m5,m15,symbol,*,limit=None,start_time=None,end_time=None,progress_callback:Callable[...,None]|None=None):
    ...
    for n,i in enumerate(indices,start=1):
        ...
        setup=engine.analyze(m5_context,m15_context,symbol,setup_state=state);setup.update({"candle_time":str(ts),"closed_candle":str(ts),"symbol":symbol,"engine_version":engine.ENGINE_VERSION})
        signal=setup.get("signal");valid=bool(setup.get("valid")) and signal in ("BUY","SELL") and bool((setup.get("trade_levels") or {}).get("valid"))
        if valid:
            setup_id=setup.get("setup_id");different_active=any(t.get("setup_id")!=setup_id for t in active)
            if different_active:
                row={**setup,"signal":"NO_TRADE","valid":False,"result":"NO_TRADE","lock_reason":"DIFFERENT_SETUP_ACTIVE"}
            else:
                signal_id=f"REPLAY-{symbol}-{pd.Timestamp(ts).strftime('%Y%m%dT%H%M%SZ')}-{signal}-{setup.get('trigger_id','NONE')}";trade={"signal":signal,"strategy":setup.get("strategy"),"engine":setup.get("engine"),"trade_levels":setup["trade_levels"],"setup_id":setup_id,"trigger_id":setup.get("trigger_id"),"signal_id":signal_id,"result":"OPEN","r_multiple":0.0,"resolved_at":None};active.append(trade);trades.append({**setup,"signal_id":signal_id,"result":"OPEN","r_multiple":0.0,"resolved_at":None});row=trades[-1]
        else:row={**setup,"signal":"NO_TRADE","result":"NO_TRADE","r_multiple":0.0,"lock_reason":None}
        rows.append(row)
        decided=[t for t in trades if t.get("result") in ("WIN","LOSS")];wins=sum(t.get("result")=="WIN" for t in trades);losses=sum(t.get("result")=="LOSS" for t in trades);opens=sum(t.get("result")=="OPEN" for t in trades);amb=sum(t.get("result")=="AMBIGUOUS" for t in trades);net=sum(float(t.get("r_multiple") or 0) for t in decided)
