...
This module deliberately does not replace the existing specialist calculations.
It converts their result into explicit E1-E8 evidence and gives E9 the only
...
def _professional_analyze(original_analyze: Callable[..., dict], *args, **kwargs):
...
def wrap(original_analyze: Callable[..., dict], gold_analyze: Callable[..., dict] | None = None):
    ...
    def analyze(m5, m15=None, symbol=None, index=None, setup_state=None, h1=None):
        ...
        if normalized in ("GOLD", "XAU/USD", "XAU/USDT", "XAU", "XAUUSD") and gold_analyze is not None:
            return _professional_analyze(gold_analyze, m5, m15=m15, symbol=symbol, index=index, setup_state=setup_state, h1=h1)
        return _professional_analyze(original_analyze, m5, m15=m15, symbol=symbol, index=index, setup_state=setup_state, h1=h1)
    return analyze
