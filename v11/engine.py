...
ENGINE_VERSION="11.2-DYNAMIC-STRATEGIES"
FORWARD_BARS=12
...
def detect_m5_direction(m5):
    x=m5.reset_index(drop=True); s=structure(x,min(len(x),80))
    return s["bias"] if s["bias"] in ("BUY","SELL") else "NEUTRAL"
...
def _enrich(result,m5,direction):
    ...
    if result.status=="PASS":
        ev=dict(result.evidence or {}); d.update({"freshness_bars":int(result.freshness_bars or ev.get("freshness_bars",_freshness(m5,direction))),"quality":float(result.quality or ev.get("quality",55.0))})
    return d
...
def analyze(m5,m15,symbol,index=None):
    ...
    registry=get_strategy_registry(symbol); m15_trend=detect_m15_trend(m15); regime=build_regime_context(m5,m15); m5_direction=detect_m5_direction(m5); candidates=[]; passes=[]; directions=[m5_direction,"SELL" if m5_direction=="BUY" else "BUY"] if m5_direction in ("BUY","SELL") else ["BUY","SELL"]
    strategy_ctx={"m15":m15_trend,"regime":regime,"opening_range_minutes":int(__import__('os').getenv("OPENING_RANGE_MINUTES","30"))}
    for name,fn in registry.items():
        best=None
        for direction in directions:
            result=fn(m5,direction,strategy_ctx); best=result
    ...
    if not selected:
        reasons=["NO_M5_STRATEGY_SETUP"] if not passes else ["M15_TREND_NEUTRAL" if m15_trend["direction"]=="NEUTRAL" else "NO_ELIGIBLE_STRATEGY"]
        return {"engine_version":ENGINE_VERSION,"symbol":symbol,"valid":False,"signal":"NO_TRADE","strategy":"NONE","m5_direction":m5_direction,"m15_trend":m15_trend,"regime":regime,"strategy_candidates":candidates,"strategy_passes":passes,"rejection_reasons":reasons,"trade_levels":{"valid":False,"reason":reasons[0]},"data_quality":{"m5":[],"m15":[]},"live_orders_allowed":False}
    ...
    return {"engine_version":ENGINE_VERSION,"symbol":symbol,"valid":valid,"signal":selected["direction"] if valid else "NO_TRADE","strategy":selected["strategy"],"m5_direction":m5_direction,"m15_trend":m15_trend,"regime":regime,"strategy_candidates":candidates,"strategy_passes":passes,"selected_strategy":selected,"rejection_reasons":reasons,"trade_levels":levels,"analysis_window":{"m5_setup_bars":"dynamic<=100","m15_context_bars":100},"data_quality":{"m5":[],"m15":[]},"live_orders_allowed":False}
