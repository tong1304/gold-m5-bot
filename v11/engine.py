...
def _btc_target_rr(engine):
    return {"B1":2.0,"B2":3.0,"B3":1.5}.get(str(engine).upper(),MIN_RISK_REWARD)

def _btc_trade_levels(selected):
    e=selected.get("evidence",{}) or {}
    entry=float(e.get("entry_price",0) or 0);sl=float(e.get("sl_price",0) or 0);primary=float(e.get("tp_price",0) or 0)
    minimum=_btc_target_rr(selected.get("engine"));risk=abs(entry-sl);rr=abs(primary-entry)/max(risk,1e-12);direction=str(selected.get("direction",""))
    ...
    # Stage unusually large RR trades instead of using one distant TP.
    # The engine's primary target remains the final TP; intermediate levels
    # are deterministic R-multiples and never replace the primary target.
    if rr>=8.0:
    ...
    for r in milestones:
        ...
        if (direction=="BUY" and p<primary) or (direction=="SELL" and p>primary):
            tps.append(p)
    tps.append(primary)

    allocations={1:[100],2:[50,50],3:[40,30,30],4:[25,25,25,25]}[len(tps)]
    levels=[
        {"price":p,"risk_reward":abs(p-entry)/risk,"type":f"TP{i+1}","allocation_pct":allocations[i]}
        for i,p in enumerate(tps)
    ]
    return {
        "valid":True,"entry":entry,"sl":sl,"tp":tps[-1],
        "tp1":tps[0],"tp2":tps[1] if len(tps)>1 else None,
        "tp3":tps[2] if len(tps)>2 else None,
        "tp4":tps[3] if len(tps)>3 else None,
        "risk":risk,"risk_reward":levels[-1]["risk_reward"],"effective_rr":levels[-1]["risk_reward"],
        "target_rr":minimum,"minimum_rr":minimum,"tp_levels":levels,"tp_count":len(levels),
        "tp_allocations":allocations,"tp_structure_levels":tps,
...
def analyze(m5,m15=None,symbol=None,index=None,setup_state=None,h1=None):
    ...
    if mb not in ("BUY","SELL"):
        return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","rejection_reasons":["M15_TREND_NOT_CONFIRMED"],"trade_levels":{"valid":False},"decision_trace":[]})
    if hb!=mb:
        return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","rejection_reasons":["H1_M15_TREND_CONFLICT"],"trade_levels":{"valid":False},"decision_trace":[]})
