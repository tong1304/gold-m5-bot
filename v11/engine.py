...
ENGINE_VERSION="12.5-NEW-GOLD-G1-G3-NEW-BTC-B1-B3"
FORWARD_BARS=12
...
def _btc_trade_levels(selected):
    evidence=selected.get("evidence",{}) or {};entry=float(evidence.get("entry_price",0) or 0);sl=float(evidence.get("sl_price",0) or 0);tp=float(evidence.get("tp_price",0) or 0);rr=float(evidence.get("risk_reward",0) or 0);target=_btc_target_rr(selected.get("engine"))
    ...
    return {"valid":True,"entry":entry,"sl":sl,"tp":tp,"tp1":tp,"tp2":None,"tp3":None,"risk":risk,"risk_reward":rr,"effective_rr":rr,"target_rr":target,"minimum_rr":target,"strategy":selected.get("strategy"),"structure_type":"BTC_NEW_ENGINE","structure_level":sl,"sl_buffer":0.0,"tp_levels":[{"price":tp,"risk_reward":rr,"type":"TP1","allocation_pct":100}],"tp_count":1,"tp_allocations":[100],"tp_structure_levels":[tp],"tp_selection":"BTC_NEW_ENGINE_EXACT_SPEC_LEVELS"}
...
def analyze(m5,m15=None,symbol=None,index=None,setup_state=None,h1=None):
    ...
    if q5 or q15 or q1:return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","regime":None,"allowed_engines":list(BTC_STRATEGIES),"rejection_reasons":q5+q15+q1,"trade_levels":{"valid":False},"data_quality":{"m5":q5,"m15":q15,"h1":q1}})
    regime=classify_regime(m5,m15,h1);base.update({"regime":regime,"m5_trend":detect_m5_trend(m5),"h1_bias":regime.get("h1_bias"),"h1_gate":regime.get("h1_gate"),"m15_regime":regime.get("m15_regime"),"allowed_engines":list(BTC_STRATEGIES)})
    is_btc=str(symbol or "").upper() in ("BTC","BTC/USD","BTC/USDT")
    if is_btc:
        candidates,trace=evaluate_new_btc_engines(m5,m15,h1)
        if not candidates:return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","setup_candidates":[],"selected_setup":None,"rejection_reasons":["NO_ALLOWED_NEW_BTC_ENGINE_SETUP"],"trade_levels":{"valid":False},"decision_trace":trace})
    else:
        candidates,trace=evaluate_all_allowed_with_trace(m5,regime)
        if not candidates:return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","setup_candidates":[],"selected_setup":None,"rejection_reasons":["NO_ALLOWED_ENGINE_SETUP"],"trade_levels":{"valid":False},"decision_trace":trace})
    base["decision_trace"]=trace
    selected_candidate=candidates[0] if is_btc else choose_priority_setup(candidates)
    selected=enrich_selected(selected_candidate,symbol,regime.get("regime","UNKNOWN"),str(m5.iloc[-1].get("datetime","")));score=selected.get("score_detail",{})
    ...
    if is_btc:
        target_rr=_btc_target_rr(selected["engine"]);levels=_btc_trade_levels(selected)
    else:
        strategy=selected["strategy"];target_rr=min_rr_for_strategy(strategy);levels=calculate_risk(m5,selected["direction"],strategy,selected.get("evidence"),rr=target_rr)
