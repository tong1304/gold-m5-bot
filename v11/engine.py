...
ENGINE_VERSION="12.6-G1-G3-B1-B3-H1-M15-M5"
FORWARD_BARS=12
...
def analyze(m5,m15=None,symbol=None,index=None,setup_state=None,h1=None):
    is_gold=str(symbol or "").upper() in ("GOLD","XAU/USD","XAU/USDT","XAUUSD")
    if is_gold:
    ...
    regime=classify_regime(m5,m15,h1);base.update({"regime":regime,"m5_trend":detect_m5_trend(m5),"h1_bias":regime.get("h1_bias"),"h1_gate":regime.get("h1_gate"),"m15_regime":regime.get("m15_regime"),"m15_role":"TREND_BIAS_ONLY","allowed_engines":list(BTC_STRATEGIES)})
    ...
    base["decision_trace"]=trace;selected_candidate=candidates[0];selected=enrich_selected(selected_candidate,symbol,regime.get("regime","UNKNOWN"),str(m5.iloc[-1].get("datetime","")));score=selected.get("score_detail",{})
    ...
    target_rr=_btc_target_rr(selected["engine"]);levels=_btc_trade_levels(selected);actual_rr=float(levels.get("risk_reward",levels.get("effective_rr",0)) or 0)
