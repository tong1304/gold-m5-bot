...
def trend_pullback(m5,direction,ctx):
    ...
    c=last["close"];trend=(c>e20.iloc[-1]>e50.iloc[-1]) if direction=="BUY" else (c<e20.iloc[-1]<e50.iloc[-1]);highs,lows=_pivot_points(x)
    if direction=="BUY":
        if not lows:return StrategyResult.fail("TREND_PULLBACK",direction,["NO_PROTECTED_LOW"])
        pivot_i,anchor=lows[-1];post=x.iloc[pivot_i+1:];touched=(not post.empty and num(post.low.min())<=float(e20.iloc[-1])+.35*a);future=[v for _,v in highs if v>c];room=(max(future)-c) if future else a*3
    else:
        if not highs:return StrategyResult.fail("TREND_PULLBACK",direction,["NO_PROTECTED_HIGH"])
        pivot_i,anchor=highs[-1];post=x.iloc[pivot_i+1:];touched=(not post.empty and num(post.high.max())>=float(e20.iloc[-1])-.35*a);future=[v for _,v in lows if v<c];room=(c-min(future)) if future else a*3
    ...
    return StrategyResult.pass_("TREND_PULLBACK",direction,{"support":anchor if direction=="BUY" else None,"resistance":anchor if direction=="SELL" else None,"ema20":num(e20.iloc[-1]),"ema50":num(e50.iloc[-1]),"atr":a,"setup_anchor":anchor,"pivot_index":pivot_i},84,0)

def breakout_retest(m5,direction,ctx):
    x=_x(m5);a=_atr(x);last=candle_metrics(x.iloc[-1]);highs,lows=_pivot_points(x);level=None;breakout_i=None;levels=[v for _,v in (highs if direction=="BUY" else lows)]
    ...
    retest=x.iloc[breakout_i+1:];touched=(num(retest.low.min())<=level+.25*a and last["close"]>level) if direction=="BUY" else (num(retest.high.max())>=level-.25*a and last["close"]<level)
...
def liquidity_sweep(m5,direction,ctx):
    ...
    if direction=="BUY":
        ...
        level=lows[-1][1];sweep=candle_metrics(x.iloc[-2]);confirm=candle_metrics(x.iloc[-1]);ok=sweep["low"]<level and sweep["close"]>level and sweep["lower_wick"]>=max(sweep["body"],a*.15) and confirm["bull"] and confirm["close"]>sweep["high"];evidence={"sweep_level":level,"support":sweep["low"]-.10*a,"atr":a,"setup_anchor":level}
    else:
        if not highs:return StrategyResult.fail("LIQUIDITY_SWEEP",direction,["NO_LIQUIDITY_HIGH"])
        level=highs[-1][1];sweep=candle_metrics(x.iloc[-2]);confirm=candle_metrics(x.iloc[-1]);ok=sweep["high"]>level and sweep["close"]<level and sweep["upper_wick"]>=max(sweep["body"],a*.15) and confirm["bear"] and confirm["close"]<sweep["low"];evidence={"sweep_level":level,"resistance":sweep["high"]+.10*a,"atr":a,"setup_anchor":level}
...
def vwap_momentum_pullback(m5,direction,ctx):
    ...
    last=candle_metrics(x.iloc[-1]);prev=candle_metrics(x.iloc[-2]);v=float(vw.iloc[-1]);c=last["close"];slope=float(vw.iloc[-1]-vw.iloc[0]) if len(vw)>1 else 0.0;highs,lows=_pivot_points(x)
    if direction=="BUY":
        if not lows:return StrategyResult.fail("VWAP_MOMENTUM_PULLBACK",direction,["NO_MOMENTUM_ANCHOR"])
        pivot_i,anchor=lows[-1];momentum=(c-anchor)/max(a,1e-12);ok=c>v and slope>0 and momentum>=1.0 and prev["low"]<=v+.30*a and last["bull"] and last["body_ratio"]>=MIN_BODY_RATIO and c>prev["high"];support=prev["low"]-.10*a
    else:
        if not highs:return StrategyResult.fail("VWAP_MOMENTUM_PULLBACK",direction,["NO_MOMENTUM_ANCHOR"])
        pivot_i,anchor=highs[-1];momentum=(anchor-c)/max(a,1e-12);ok=c<v and slope<0 and momentum>=1.0 and prev["high"]>=v-.30*a and last["bear"] and last["body_ratio"]>=MIN_BODY_RATIO and c<prev["low"];support=prev["high"]+.10*a
    if not ok:return StrategyResult.fail("VWAP_MOMENTUM_PULLBACK",direction,["VWAP_MOMENTUM_PULLBACK_NOT_CONFIRMED"])
    return StrategyResult.pass_("VWAP_MOMENTUM_PULLBACK",direction,{"vwap":v,"vwap_slope":slope,"momentum_atr":momentum,"support":support if direction=="BUY" else None,"resistance":support if direction=="SELL" else None,"atr":a,"setup_anchor":v,"pivot_index":pivot_i},82,0)
