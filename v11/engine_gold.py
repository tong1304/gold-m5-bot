...
ENGINE_VERSION = "12.10-ASSET-REGIME-GATE-SCORE-FILTER-GOLD-BTC"
...
def _trade_levels(selected):
    ev=selected.get("evidence") or {};entry=float(ev.get("entry_price",0) or 0);sl=float(ev.get("sl_price",0) or 0);primary=float(ev.get("tp_price",0) or 0);minimum=MIN_RR.get(selected.get("engine"),2.0)
...
def analyze(m5,m15=None,symbol=None,index=None,setup_state=None,h1=None):
    ...
    regime=classify_regime(m5,m15,h1);base.update({"regime":regime,"m5_regime":regime.get("m5_regime"),"h1_bias":regime.get("h1_bias"),"m15_regime":regime.get("m15_regime"),"m15_trend":regime.get("m15_trend"),"m15_role":"CONTEXT_ONLY","allowed_engines":list(GOLD_ENGINES)})
    candidates,trace=evaluate_asset_strategies("GOLD",m5,regime);base["decision_trace"]=trace
    if not candidates:return _finalize({**base,"valid":False,"signal":"NO_TRADE","strategy":"NONE","setup_candidates":[],"selected_setup":None,"rejection_reasons":["NO_GOLD_STRATEGY_PASSED_CORE_GATE_SCORE_FILTER"],"trade_levels":{"valid":False}})
    selected=candidates[0];sid,tid=_setup_ids(selected,symbol,str(m5.iloc[-1].get("datetime","")));selected={**selected,"setup_id":sid,"trigger_id":tid,"symbol":symbol};score=selected.get("score_detail") or {};base.update({"setup_candidates":candidates,"selected_setup":selected,"strategy":selected["strategy"],"engine":selected["engine"],"setup_id":sid,"trigger_id":tid,"setup_score":score})
