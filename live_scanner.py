...
def _load_frames(symbol):
    market = _market_symbol(symbol); frames = {}
    for tf, minutes in (("1h",60),("15m",15),("5m",5)):
        cfg=_config(symbol,tf); df=BINANCE.fetch_candles(market,tf,cfg["history"]); df=BINANCE.remove_incomplete_last_candle(df,timeframe_minutes=minutes)
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        ...
        for target in (engine,base):
            ...
            target.MIN_RISK_REWARD=max(float(os.getenv("MIN_RISK_REWARD","2.0")),2.0); target.RISK_REWARD=max(float(os.getenv("RISK_REWARD","2.0")),2.0)
        ...
        return {"status":"signal_sent","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":signal,"signal_id":signal_id,"recorded":recorded,"telegram":telegram_result,"setup":setup}