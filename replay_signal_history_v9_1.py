...
def _fetch(symbol, start, end, timeframe="5m", chunk_days=6):
    ...
    while cursor < end:
        ...
        raw = client.candles(SYMBOLS[symbol], timeframe, start=cursor.date().isoformat(), end=chunk_end.date().isoformat(), limit=200, order="desc")
...
def _resample(m5, minutes):
    return (m5.set_index("datetime")[["open","high","low","close","volume"]]
...
def replay_symbol(symbol, start, end, dry_run=False):
    ...
    if len(m5) < 150: raise RuntimeError(f"Not enough LSE M5 history for {symbol}: {len(m5)}")
    ...
    for i in range(100, len(m5)-1):
        ...
        setup["candle_time"]=ts.isoformat(); setup["closed_candle"]=ts.isoformat(); setup["symbol"]=symbol; setup["engine_version"]=engine.ENGINE_VERSION
        signal=setup.get("signal"); levels=setup.get("trade_levels") or {}; rr=float(levels.get("effective_rr",levels.get("risk_reward",0)) or 0)
        ...
        if not valid or key in used:
            ...
            if key in used: reason=["DUPLICATE_SETUP"]
            for r in reason: rejected[r]=rejected.get(r,0)+1
            outcomes["NO_TRADE"]+=1; generated+=1
            payload={**setup,"signal":"NO_TRADE","result":"NO_TRADE","replay":True,"replay_source":"LSE_HISTORICAL_OHLCV","created_at":ts.isoformat(),"no_trade_reasons":reason,"rejection_reasons":reason}
            if not dry_run and history.record_no_trade(payload): inserted+=1
            continue
        used.add(key); generated+=1
        sid=f"REPLAY-V91-{symbol}-{ts.strftime('%Y%m%dT%H%MZ')}-{signal}"
        payload={**setup,"signal_id":sid,"signal":signal,"replay":True,"replay_source":"LSE_HISTORICAL_OHLCV","created_at":ts.isoformat(),"pattern_signal":signal,"m5_direction":signal,"v9_1_setup":setup,"trade_levels":levels}
    ...
    return {"symbol":symbol,"engine_version":engine.ENGINE_VERSION,"provider":"LSE","generated":generated,"inserted":inserted,"outcomes":outcomes,"rejected":dict(sorted(rejected.items(),key=lambda x:x[1],reverse=True)[:20])}
...
def main():
    ...
    start,end=_bounds(a.start,a.end); symbols=["BTC","GOLD"] if a.symbol=="ALL" else [a.symbol]; results=[]
    for s in symbols:
    ...
    print(json.dumps({"status":"dry-run" if a.dry_run and status=="completed" else status,"engine_version":engine.ENGINE_VERSION,"provider":"LSE","start":a.start,"end":a.end,"results":results},ensure_ascii=False,indent=2))
