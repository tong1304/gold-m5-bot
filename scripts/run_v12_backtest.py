...
def historical_m5(client, symbol, start, end):
    ...
    if not isinstance(rows, (list, tuple)):
        raise RuntimeError(f"LSE_INVALID_RESPONSE:{symbol}")
    ...
    if missing:
        raise RuntimeError(f"LSE_INVALID_RESPONSE:{symbol}:missing={missing}")
    ...
    if len(frame) < REPLAY_M5_CONTEXT_BARS + 1:
        raise RuntimeError(f"INSUFFICIENT_HISTORICAL_M5:{symbol}:{len(frame)}")
    return frame


def main():
    start, end = normalize_replay_window(START, END)
    client = LSE(api_key=os.environ["LSE_API_KEY"])
    reports = []
    for symbol in SYMBOLS:
        frame = historical_m5(client, symbol, start, end)
        report = replay_frames(frame, None, symbol, start_time=start, end_time=end)
        reports.append({
            "symbol": symbol,
    ...
    payload = {
        "status": "completed",
        "engine_version": reports[0]["engine_version"] if reports else None,
        "source": "LSE_HISTORICAL_M5_OHLCV",
        "timeframe_mode": "M5-only",
        "lookahead_safe": True,
        "start": str(start),
        "end": str(end),
    ...
    print(json.dumps({"status": payload["status"], "start": payload["start"], "end": payload["end"], "reports": [{k: r[k] for k in ("symbol", "candles_evaluated", "signals", "wins", "losses", "net_r")} for r in reports]}, ensure_ascii=False))
