...
app=Flask(__name__);logger=logging.getLogger("v11_app");SUPPORTED_SYMBOLS=("BTC/USDT","XAU/USDT");SERVICE_LOCK=threading.RLock();_SERVICES_STARTED=False;SCHEDULER_LOCK_FILE=os.getenv("V11_SCHEDULER_LOCK_FILE","/tmp/gold-m5-v11-scheduler.lock")
...
def _startup_probe_worker():
    try:
        ...
        for symbol in ("BTC","GOLD"):
            try:
                frames=live_scanner_v11._load_frames(symbol);m5=frames["5m"];m15=frames["15m"];ok=(m5 is not None and not m5.empty and m15 is not None and not m15.empty);results[symbol]=ok;logger.warning("[V11 STARTUP] LSE_REST_PROBE %s=%s M5=%s M15=%s",symbol,"READY" if ok else "FAILED",len(m5) if m5 is not None else 0,len(m15) if m15 is not None else 0)
            except Exception as exc:results[symbol]=False;logger.error("[V11 STARTUP] LSE_REST_PROBE %s=FAILED error=%s",symbol,exc)
    ...
    except Exception:logger.exception("[V11 STARTUP] LSE REST readiness probe failed")
def _start_runtime_services():
    ...
    with SERVICE_LOCK:
        ...
        if not _acquire_scheduler_lock():logger.warning("[V11 STARTUP] Scheduler lock already owned by another process; this worker will not start runtime services");_SERVICES_STARTED=True;return
        try:
            import live_price;live_started=live_price.start();import scheduler_v11 as scheduler;scheduler_started=scheduler.start();live_status=live_price.status();scheduler_status=scheduler.status();logger.warning("[V11 STARTUP] ENGINE=%s SCHEDULER=%s LIVE_PRICE=%s PROVIDER=LSE",v11_engine.ENGINE_VERSION,"RUNNING" if scheduler_status.get("running") else "NOT_RUNNING","RUNNING" if live_status.get("running") else "NOT_RUNNING");logger.warning("[V11 STARTUP] scheduler_started=%s live_started=%s live_state=%s api_key=%s",scheduler_started,live_started,live_status.get("loop_state"),live_status.get("api_key_configured"));threading.Thread(target=_startup_probe_worker,name="v11-lse-startup-probe",daemon=True).start()
            try:
            ...
            except Exception as exc:logger.warning("[V11 STARTUP] Telegram startup notification failed: %s",exc)
            _SERVICES_STARTED=True
        except Exception:
            try:os.unlink(SCHEDULER_LOCK_FILE)
            except OSError:pass
            logger.exception("[V11 STARTUP] Runtime service startup failed");raise
@app.before_request
...
def health():
    ...
    return _json_response({"status":"ok","service":"gold-m5-bot","engine_version":v11_engine.ENGINE_VERSION,"exchange":"LSE","symbols":["BTC/USD","XAU/USD"],"timeframe":"M5 trigger + M15 trend","analysis_windows":{"M15_context_bars":100,"M5_setup_bars":"dynamic<=100"},"live_price":live,"scheduler":scheduler,"live_orders_allowed":False})
...
def validation():
    ...
    try:
        import live_scanner_v11;m5=live_scanner_v11._lse_frame(mapped,"5m",max(bars,100));m15=live_scanner_v11._lse_frame(mapped,"15m",max(bars,100));report=validate_v11(m5,m15,mapped,limit=bars);report.update({"endpoint":"/validation","request":{"symbol":symbol,"bars":bars}});return _json_response(report)
...
def validation_v92():return _json_response({"status":"deprecated","message":"Legacy V9.2 validation is intentionally isolated from V11. Use /validation.","engine_version":v11_engine.ENGINE_VERSION,"live_orders_allowed":False},410)
