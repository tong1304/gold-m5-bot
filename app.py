...
# กำหนด Timezone เป็น กรุงเทพฯ (UTC+7)
...
def now_bangkok():
    """แสดงผลเวลาปัจจุบันในโซนเวลา Asia/Bangkok (UTC+7)"""
    return datetime.now(BANGKOK_TZ).strftime("%Y-%m-%d %H:%M:%S")


def format_bangkok_time(dt):
    """แปลง Datetime ใดๆ เป็นข้อความเวลา Asia/Bangkok"""
...
def get_market_data(outputsize=CANDLES):
    ...
    # กำหนดให้เวลาที่ได้จาก API มี Timezone UTC
    df["datetime"] = pd.to_datetime(df["datetime"], errors="coerce").dt.tz_localize(timezone.utc)
...
# ============================================================
# REMOVE INCOMPLETE CANDLE (CANDLE CLOSURE CHECK)
# ============================================================

def remove_incomplete_last_candle(df):
    """ตรวจสอบแท่งเทียนล่าสุด หากแท่งปัจจุบันยังไม่ปิดสมบูรณ์ ให้ตัดออกก่อนคำนวณ"""
    if df.empty:
        return df
    ...
    try:
        ...
        close_time = last_time + timedelta(minutes=timeframe_minutes())
        current_time = datetime.now(timezone.utc)

        # ถ้าเวลาปัจจุบันยังไม่ถึงเวลาปิดแท่งเทียน ให้ลบแท่งล่าสุดออก
...
def calculate_indicators(df):
    ...
    df["atr"] = df["atr"].bfill().fillna(1.0)

    # Candle
    df["body"] = (df["close"] - df["open"]).abs()
    df["range"] = (df["high"] - df["low"]).replace(0, np.nan)
    df["body_ratio"] = df["body"] / df["range"]
    df["body_ratio"] = df["body_ratio"].fillna(0).clip(0, 1)

    df["upper_wick"] = df["high"] - df[["open", "close"]].max(axis=1)
    df["lower_wick"] = df[["open", "close"]].min(axis=1) - df["low"]

    # ATR ratio
    atr_average = df["atr"].rolling(50).mean().replace(0, np.nan)
    df["atr_ratio"] = df["atr"] / atr_average
...
def get_support_resistance(df, index, lookback=50):
    start = max(0, index - lookback)
    window = df.iloc[start:index + 1]

    if len(window) < 5:
        return {
            "support": safe_float(df.iloc[index]["low"]),
            "resistance": safe_float(df.iloc[index]["high"])
        }
...
def is_local_high(df, i, left=2, right=2):
    ...
    value = safe_float(df.iloc[i]["high"])
    left_values = [safe_float(df.iloc[j]["high"]) for j in range(i - left, i)]
    right_values = [safe_float(df.iloc[j]["high"]) for j in range(i + 1, i + right + 1)]

    return value >= max(left_values) and value >= max(right_values)
...
def is_local_low(df, i, left=2, right=2):
    ...
    value = safe_float(df.iloc[i]["low"])
    left_values = [safe_float(df.iloc[j]["low"]) for j in range(i - left, i)]
    right_values = [safe_float(df.iloc[j]["low"]) for j in range(i + 1, i + right + 1)]

    return value <= min(left_values) and value <= min(right_values)
...
def detect_patterns(df, i):
    ...
    if body > 0 and lower_wick >= body * 2 and upper_wick <= body and candle_range > 0:
        patterns.append("Hammer")

    if body > 0 and upper_wick >= body * 2 and lower_wick <= body and candle_range > 0:
    ...
    # Double Top / Bottom
    lookback_start = max(2, i - 20)
    pivot_highs, pivot_lows = [], []

    for p in range(lookback_start, i - 1):
        if is_local_high(df, p, 2, 2):
            pivot_highs.append(p)
        if is_local_low(df, p, 2, 2):
            pivot_lows.append(p)

    atr = max(safe_float(row["atr"]), 0.01)
    tolerance = max(atr * 0.35, 0.50)

    if len(pivot_highs) >= 2:
        h1 = safe_float(df.iloc[pivot_highs[-2]]["high"])
        h2 = safe_float(df.iloc[pivot_highs[-1]]["high"])
        if abs(h1 - h2) <= tolerance and c < h2:
            patterns.append("Double Top")

    if len(pivot_lows) >= 2:
        l1 = safe_float(df.iloc[pivot_lows[-2]]["low"])
        l2 = safe_float(df.iloc[pivot_lows[-1]]["low"])
        if abs(l1 - l2) <= tolerance and c > l2:
            patterns.append("Double Bottom")

    # Breakout
    breakout_start = max(0, i - 6)
    breakout_window = df.iloc[breakout_start:i]

    if len(breakout_window) >= 5:
        previous_resistance = safe_float(breakout_window["high"].max())
        previous_support = safe_float(breakout_window["low"].min())

        if c > previous_resistance and c > o:
            patterns.append("Bullish Breakout")

        if c < previous_support and c < o:
    ...
    if ema20 > ema50 and l <= ema20 and c > ema20 and c > o:
        patterns.append("Bullish Pullback")

    elif ema20 < ema50 and h >= ema20 and c < ema20 and c < o:
...
BULLISH_PATTERNS = {
    ...
    "Double Bottom", "Bullish Breakout", "Bullish Pullback"
}

BEARISH_PATTERNS = {
    "Bearish Engulfing", "Shooting Star", "Evening Star",
    "Double Top", "Bearish Breakout", "Bearish Pullback"
}


def get_pattern_direction(patterns):
    bullish = [p for p in patterns if p in BULLISH_PATTERNS]
    bearish = [p for p in patterns if p in BEARISH_PATTERNS]

    if bullish and bearish:
        return {"direction": "CONFLICT", "bullish": bullish, "bearish": bearish, "conflict": True}

    if bullish:
        return {"direction": "BUY", "bullish": bullish, "bearish": [], "conflict": False}

    if bearish:
        return {"direction": "SELL", "bullish": [], "bearish": bearish, "conflict": False}

    return {"direction": "NONE", "bullish": [], "bearish": [], "conflict": False}
...
def calculate_market_regime(df, i):
    ...
    atr = safe_float(row["atr"], 1.0)
    price = safe_float(row["close"])

    trend_strength = (abs(ema20 - ema50) / atr) if atr > 0 else 0.0

    if ema20 > ema50 and price >= ema20 and trend_strength >= 1.0:
        regime = "TREND_UP"
    elif ema20 < ema50 and price <= ema20 and trend_strength >= 1.0:
        regime = "TREND_DOWN"
    elif atr_ratio < 0.70 and trend_strength >= 0.8:
        regime = "LOW_VOLATILITY_TREND_UP" if ema20 >= ema50 else "LOW_VOLATILITY_TREND_DOWN"
    ...
    if regime in ["TREND_UP", "TREND_DOWN"]:
        score = 100.0
    elif regime.startswith("LOW_VOLATILITY"):
        score = 80.0
...
def calculate_location(df, i, direction):
    row = df.iloc[i]
    price = safe_float(row["close"])
    ema20 = safe_float(row["ema20"])
    ema50 = safe_float(row["ema50"])

    levels = get_support_resistance(df, i)
...
def calculate_momentum(df, i, direction):
    row = df.iloc[i]
    ...
    if direction == "BUY":
        if 50 <= rsi <= 68:
            ...
            score = 45.0
            reasons.append("RSI high for BUY")
        elif rsi < 40:
            score = 30.0
            reasons.append("RSI weak for BUY")
        else:
            score = 60.0
    elif direction == "SELL":
        if 32 <= rsi <= 50:
            ...
            score = 45.0
            reasons.append("RSI low for SELL")
        elif rsi > 60:
            score = 20.0
            reasons.append("RSI weak for SELL")
        else:
            score = 60.0

    strength = max(0, min(100, (rsi - 50) * 2)) if direction == "BUY" else max(0, min(100, (50 - rsi) * 2))
...
def calculate_pattern_quality(df, i, patterns, direction):
    row = df.iloc[i]
    body_ratio = safe_float(row["body_ratio"])
    candle_direction = "BUY" if safe_float(row["close"]) > safe_float(row["open"]) else "SELL"
    ...
    for pattern in patterns:
        if direction == "BUY" and pattern in BEARISH_PATTERNS:
            opposite.append(pattern)
        elif direction == "SELL" and pattern in BULLISH_PATTERNS:
            opposite.append(pattern)
    ...
    if direction == candle_direction:
        ...
        score -= 10
        reasons.append("Pattern candle direction weak")

    if opposite:
        score -= 15
        reasons.append("Opposite pattern detected")
    else:
        reasons.append("No opposite pattern")

    score = clamp(score)
    quality = "PASS" if score >= MIN_PATTERN_QUALITY else "FAIL"
...
def calculate_trigger_quality(df, i, direction):
    row = df.iloc[i]
    ...
    if direction == "BUY":
        ...
        if close >= high - atr * 0.20:
            score += 25
        trigger = high + atr * 0.05
    else:
        if close < open_price:
            score += 25
        if close <= low + atr * 0.20:
            score += 25
        trigger = low - atr * 0.05

    if body_ratio < 0.30:
        reasons.append("Weak trigger candle")

    score = clamp(score)

    return {
        "body_ratio": round(body_ratio, 3),
        "signal_close": round_price(close),
        "trigger": round_price(trigger),
        "triggered": False,
        "valid": score >= MIN_TRIGGER_QUALITY,
        "score": round(score, 2),
        "reasons": reasons
    }
...
def calculate_directional_filter(df, i, direction):
    row = df.iloc[i]
    ema20 = safe_float(row["ema20"])
    ema50 = safe_float(row["ema50"])
    close = safe_float(row["close"])
    ...
    if direction == "BUY":
        ...
        score += 15 if close > ema20 else -15
    elif direction == "SELL":
        score += 25 if ema20 < ema50 else -25
        score += 15 if close < ema20 else -15
...
def calculate_score(directional, regime, location, momentum, pattern_quality, trigger_quality):
    score = (
...
def hard_filter(df, i, direction, regime, location, momentum, pattern_quality, trigger_quality, score):
    row = df.iloc[i]
    ...
    checks = {
        ...
        "pattern_quality": pattern_quality["score"] >= (
            SELL_MIN_PATTERN_QUALITY if direction == "SELL" else MIN_PATTERN_QUALITY
        ),
        "trigger": trigger_quality["score"] >= (
            SELL_MIN_TRIGGER_QUALITY if direction == "SELL" else MIN_TRIGGER_QUALITY
        ),
        "score": score >= (
            SELL_MIN_SCORE if direction == "SELL" else MIN_SCORE
        ),
    }

    failed = [key for key, value in checks.items() if not value]
    critical_pass = checks["atr"] and checks["direction"] and checks["market_regime"]
...
def calculate_trade_levels(df, i, direction, entry_price=None):
    row = df.iloc[i]
    setup_close = safe_float(row["close"])

    if entry_price is None:
        entry_price = setup_close

    entry_price = safe_float(entry_price)
    atr = max(safe_float(row["atr"]), MINIMUM_ATR)
    ...
    if direction == "BUY":
        structure_sl = support - atr * 0.20
        atr_sl = entry_price - atr * 1.50
        raw_sl = min(structure_sl, atr_sl)

        risk = entry_price - raw_sl
        risk = max(risk, atr * MIN_STOP_ATR)
        risk = min(risk, atr * MAX_STOP_ATR)

        sl = entry_price - risk
        tp = entry_price + risk * RISK_REWARD
    else:
        structure_sl = resistance + atr * 0.20
        atr_sl = entry_price + atr * 1.50
    ...
    return {
        "entry_reference": round_price(entry_price),
        "setup_close": round_price(setup_close),
        "sl": round_price(sl),
        "tp": round_price(tp),
...
def analyze_candle(df, i, include_trade_levels=True):
    if i < 55:
        return {
            "valid": False,
            "signal": "NO_TRADE",
    ...
    pattern_direction = get_pattern_direction(patterns)
    direction = pattern_direction["direction"]

    # รูปแบบการแสดงผลเวลาเป็นประเทศไทย (Asia/Bangkok)
    setup_candle_time = format_bangkok_time(row["datetime"])
    ...
    if direction == "CONFLICT":
        return {
            ...
            "patterns": patterns,
            "setup_candle": setup_candle_time
        }
    ...
    directional = calculate_directional_filter(df, i, direction)
    regime = calculate_market_regime(df, i)
    location = calculate_location(df, i, direction)
    momentum = calculate_momentum(df, i, direction)
    pattern_quality = calculate_pattern_quality(df, i, patterns, direction)
    trigger_quality = calculate_trigger_quality(df, i, direction)

    score = calculate_score(
        directional, regime, location, momentum, pattern_quality, trigger_quality
    )

    filter_result = hard_filter(
        df, i, direction, regime, location, momentum, pattern_quality, trigger_quality, score
    )
    ...
    if include_trade_levels:
        trade_levels = calculate_trade_levels(
            df, i, direction, entry_price=safe_float(row["close"])
        )

        if trade_levels["risk_reward"] < MIN_RISK_REWARD:
            filter_result["passed"] = False
            if "risk_reward" not in filter_result["failed"]:
                filter_result["failed"].append("risk_reward")

    signal = direction if filter_result["passed"] else "NO_TRADE"
    confidence = score

    return {
        "architecture": [
            "Pattern", "Pattern Quality", "Directional Filter",
            "Market Regime", "Location", "Momentum", "Trigger Quality",
            "Hard Filter", "Score", "NEXT CANDLE ENTRY", "ATR SL/TP", "Realistic Backtest"
        ],
        "symbol": SYMBOL,
        "timeframe": "M5",
        ...
        "directional_filter": {
            **directional,
            "bullish": pattern_direction["bullish"],
            "bearish": pattern_direction["bearish"]
        },
        ...
        "score_percent": score,
        "probability": None,
        "probability_note": "Score is not statistical probability. Historical probability is calculated from backtest statistics.",
        "trend": "UPTREND" if safe_float(row["ema20"]) > safe_float(row["ema50"]) else "DOWNTREND",
        "ema20": round_price(row["ema20"]),
        "ema50": round_price(row["ema50"]),
        "rsi": round(safe_float(row["rsi"]), 1),
        "atr": round(safe_float(row["atr"]), 4),
        "support": location["support"],
        "resistance": location["resistance"],
        "entry_rule": "NEXT CANDLE OPEN",
        "next_candle_entry": (signal in ["BUY", "SELL"]),
        "signal": signal,
        "valid": (signal in ["BUY", "SELL"]),
        "status": "READY" if signal in ["BUY", "SELL"] else "HARD_FILTER_FAILED",
...
def simulate_trade(df, signal_index, direction, setup_levels):
    ...
    levels = calculate_trade_levels(df, signal_index, direction, entry_price=entry)
    ...
    def path(use_break_even):
        ...
        for j in range(entry_index, end_index):
            ...
            if use_break_even and BREAK_EVEN and not moved_to_be and mfe >= BREAK_EVEN_R:
    ...
    return {
        ...
        "setup_time": format_bangkok_time(df.iloc[signal_index]["datetime"]),
        "entry_time": format_bangkok_time(df.iloc[entry_index]["datetime"]),
        "exit_time": format_bangkok_time(df.iloc[actual["exit_index"]]["datetime"]),
        "direction": direction,
        "entry_raw": round_price(raw_entry),
        "entry": round_price(entry),
        "sl": round_price(
            actual["result"] == "TIMEOUT" and original_sl or (
        ...
        "timeout_mfe_ge_1r": (actual["result"] == "TIMEOUT" and actual["mfe_r"] >= 1.0),
        "timeout_mfe_ge_tp": (
            actual["result"] == "TIMEOUT" and actual["mfe_r"] >= safe_float(levels["risk_reward"])
        ),
        "risk": round_price(risk),
...
def calculate_trade_statistics(trades):
    ...
    profit_factor = (
        total_profit_r / total_loss_r if total_loss_r > 0
        else (float("inf") if total_profit_r > 0 else 0)
    )
    profit_factor_no_be = (
        total_profit_r_no_be / total_loss_r_no_be if total_loss_r_no_be > 0
    ...
    longest_losing_streak, current_streak = 0, 0
    for trade in trades:
...
def empirical_probability(trades, group_name="all"):
    ...
    return {
        ...
        "note": "Empirical historical win rate among resolved trades."
    }
...
def run_backtest(df, test_points=200):
    ...
    pattern_candidates, score_passed, hard_filter_passed, skipped_overlapping = 0, 0, 0, 0
    ...
    return {
        ...
        "test_start": format_bangkok_time(df.iloc[start]["datetime"]),
        "test_end": format_bangkok_time(df.iloc[-2]["datetime"]),
        "performance": stats,
...
def home():
    return jsonify({
        "status": "online",
        "version": "3.0",
        "service": "XAU/USD M5 Quality Filtered Next Candle Entry Engine",
        "symbol": SYMBOL,
        "timeframe": "M5",
        "timezone": "Asia/Bangkok (UTC+7)",
        "minimum_score": MIN_SCORE,
        "endpoints": [
            "/", "/health", "/signal", "/backtest",
            "/test-data", "/test-telegram"
        ]
    })
...
def health():
    return jsonify({
        "status": "healthy",
        "service": "XAU/USD Signal Engine v3",
        "symbol": SYMBOL,
        "timeframe": "M5",
        "timezone": "Asia/Bangkok (UTC+7)",
...
def test_data():
    try:
        ...
        raw_latest = df.iloc[-1]

        # ตรวจสอบการปิดแท่งเทียนสมบูรณ์
        closed_df = remove_incomplete_last_candle(df)
        ...
        return jsonify({
            "status": "success",
            "message": "Twelve Data connection working",
            "symbol": SYMBOL,
            "timeframe": "M5",
            "candles": len(df),
            "closed_candles": len(closed_df),
            "latest_raw": {
                "datetime": format_bangkok_time(raw_latest["datetime"]),
                "open": round_price(raw_latest["open"]),
                "high": round_price(raw_latest["high"]),
                "low": round_price(raw_latest["low"]),
                "close": round_price(raw_latest["close"])
            },
            "latest_closed": {
                "datetime": format_bangkok_time(latest["datetime"]),
                "open": round_price(latest["open"]),
                "high": round_price(latest["high"]),
                "low": round_price(latest["low"]),
                "close": round_price(latest["close"])
            }
        })
    except Exception as e:
        return jsonify({
...
def signal():
    try:
        ...
        # กรองเฉพาะแท่งเทียนที่ปิดสมบูรณ์แล้ว
...
<b>Setup Candle Time (BKK):</b> {result['setup_candle']}
"""
...
def backtest_endpoint(points=200):
    try:
        outputsize = max(points + 150, 300)
        df = get_market_data(outputsize)
        df = calculate_indicators(df)
        result = run_backtest(df, points)
        return jsonify(result)
    except Exception as e:
        return jsonify({
            "status": "error",
...
def startup_message():
    """แจ้งเตือนทันทีเมื่อ App เริ่มรันสำเร็จ"""
    message = f"""
<b>🟢 XAU/USD ENGINE v3 STARTED</b>

<b>Status:</b> Bot is online and connected successfully!
<b>Symbol:</b> {SYMBOL}
<b>Timeframe:</b> M5
<b>Timezone:</b> Bangkok (UTC+7)
<b>Start Time:</b> {now_bangkok()}

<b>Engine:</b> Quality Filtered Next Candle Entry Engine
<b>Min Score:</b> {MIN_SCORE} | <b>Min RR:</b> {MIN_RISK_REWARD}
<b>Break Even:</b> {"ON" if BREAK_EVEN else "OFF"}
"""
...
if __name__ == "__main__":
    print("=" * 70)
    print("XAU/USD QUALITY FILTERED NEXT CANDLE ENTRY ENGINE v3")
    print("=" * 70)
    print(f"Symbol       : {SYMBOL}")
    print("Timeframe    : M5")
    print(f"Timezone     : Bangkok (Asia/Bangkok)")
    print(f"Min Score    : {MIN_SCORE}")
    print(f"Min RR       : {MIN_RISK_REWARD}")
    print(f"Twelve Data  : {'OK' if TWELVE_DATA_API_KEY else 'NOT CONFIGURED'}")
    print(f"Telegram     : {'OK' if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID else 'NOT CONFIGURED'}")
    print("=" * 70)

    # ส่งข้อความแจ้งเตือนเมื่อระบบเริ่มทำงานสำเร็จ
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
        try:
            result = startup_message()
            print("Telegram startup notification:", result)
        except Exception as e:
