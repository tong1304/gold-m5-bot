...
Patterns are evidence only. A signal requires a confirmed M5 setup plus aligned
M15/H1 context in live_scanner.py. No pattern is treated as guaranteed.
"""
...
def detect_price_action(df,i):
    ...
    if b.close>b.open and a.close<a.open and b.open<=a.close and b.close>=a.open: out.append({"name":"Bullish Engulfing","category":"PRICE_ACTION","direction":"BUY"})
    if b.close<b.open and a.close>a.open and b.open>=a.close and b.close<=a.open: out.append({"name":"Bearish Engulfing","category":"PRICE_ACTION","direction":"SELL"})
    upper=float(b.high)-max(float(b.open),float(b.close)); lower=min(float(b.open),float(b.close))-float(b.low)
    if lower>=max(bb*2,_rng(b)*.45) and b.close>b.open: out.append({"name":"Hammer / Rejection","category":"PRICE_ACTION","direction":"BUY"})
    if upper>=max(bb*2,_rng(b)*.45) and b.close<b.open: out.append({"name":"Shooting Star / Rejection","category":"PRICE_ACTION","direction":"SELL"})
    if float(b.high)<=float(a.high) and float(b.low)>=float(a.low): out.append({"name":"Inside Bar","category":"PRICE_ACTION","direction":"NEUTRAL"})
    return out

def detect_chart_patterns(df,i,lookback=80):
    if i<25:return []
    s=df.iloc[max(0,i-lookback):i+1].copy(); atr=float(_atr(s).iloc[-1]) if pd.notna(_atr(s).iloc[-1]) else float(np.mean((s.high-s.low).tail(14)))
    if atr<=0:return []
    highs,lows=_pivots(s); out=[]
    # Work with confirmed pivot points only; the current candle is the confirmation candle.
    if len(lows)>=2:
    ...
    if len(lows)>=3:
        ...
        if max(abs(a[1]-b[1]),abs(b[1]-c[1]))<=atr*2 and float(s.close.iloc[-1])>neck: out.append({"name":"Triple Bottom","category":"CHART_PATTERN","direction":"BUY","confirmed":True})
    if len(highs)>=3:
        a,b,c=highs[-3:]; neck=float(s.low.iloc[a[0]+1:c[0]+1].min())
        if max(abs(a[1]-b[1]),abs(b[1]-c[1]))<=atr*2 and float(s.close.iloc[-1])<neck: out.append({"name":"Triple Top","category":"CHART_PATTERN","direction":"SELL","confirmed":True})
    ...
    # Wedges: converging pivot slopes plus breakout confirmation.
    ...
    if len(recent)>=15:
        ...
        # Triangle/flag-style compression with directional breakout.
        ...
        # Ascending/descending triangle approximations.
        ...
        # Flags require a strong impulse followed by a short counter-trend channel and breakout.
    ...
    # Rounding patterns are deliberately conservative: broad curvature + breakout.
...
def detect_smc(df,i,swing=3):
    ...
    if float(row.high)>hp and float(row.close)<hp: out.append({"name":"Liquidity Sweep High","category":"SMC_ICT","direction":"SELL"})
    if float(row.low)<lp and float(row.close)>lp: out.append({"name":"Liquidity Sweep Low","category":"SMC_ICT","direction":"BUY"})
    if float(row.close)>hr: out.append({"name":"Bullish BOS / MSS","category":"SMC_ICT","direction":"BUY"})
    if float(row.close)<lr: out.append({"name":"Bearish BOS / MSS","category":"SMC_ICT","direction":"SELL"})
    if i>=2:
        a,b,c=df.iloc[i-2],df.iloc[i-1],df.iloc[i]
        if float(a.high)<float(c.low): out.append({"name":"Bullish FVG","category":"SMC_ICT","direction":"BUY"})
        if float(a.low)>float(c.high): out.append({"name":"Bearish FVG","category":"SMC_ICT","direction":"SELL"})
    return out
...
def detect_supply_demand(df,i):
    ...
    if impulse>=float(atr)*1.5: out.append({"name":"Demand Impulse / Base Candidate" if b.close>b.open else "Supply Impulse / Base Candidate","category":"SUPPLY_DEMAND","direction":"BUY" if b.close>b.open else "SELL"})
    return out
...
def detect_trend_breakout(df,i):
    ...
    if e20.iloc[i]>e50.iloc[i] and c.iloc[i]>e20.iloc[i]: out.append({"name":"Uptrend / EMA20-50 Alignment","category":"TREND_BREAKOUT","direction":"BUY"})
    if e20.iloc[i]<e50.iloc[i] and c.iloc[i]<e20.iloc[i]: out.append({"name":"Downtrend / EMA20-50 Alignment","category":"TREND_BREAKOUT","direction":"SELL"})
    ph=float(df.high.iloc[i-20:i].max()); pl=float(df.low.iloc[i-20:i].min())
    if float(row.close)>ph: out.append({"name":"20-Bar Breakout","category":"TREND_BREAKOUT","direction":"BUY"})
    if float(row.close)<pl: out.append({"name":"20-Bar Breakdown","category":"TREND_BREAKOUT","direction":"SELL"})
    return out
...
def detect_fibonacci(df,i):
    ...
    for n,r in (("38.2",.382),("50.0",.5),("61.8",.618)):
        level=hi-span*r
        if abs(px-level)<=span*.012:return [{"name":f"Fibonacci {n}% Zone","category":"FIBONACCI_HARMONIC","direction":"BUY" if px<(hi+lo)/2 else "SELL"}]
    return []
...
def detect_indicators_session(df,i):
    ...
    if v<30:return [{"name":"RSI Oversold","category":"INDICATOR_SESSION","direction":"BUY"}]
    if v>70:return [{"name":"RSI Overbought","category":"INDICATOR_SESSION","direction":"SELL"}]
    return []
