...
def _telegram_text(symbol,setup):
    ...
    regime=setup.get("regime") or {};regime_name=regime.get("regime") if isinstance(regime,dict) else regime
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        frames=_load_frames(symbol);m5=frames["5m"];ts=str(m5.iloc[-1].datetime);resolved=history.resolve_open_for_symbol(symbol,m5.to_dict("records"));history_rows=history.list_signals(days=3650,symbol=symbol,limit=1000);state=state_from_history(history_rows)
        ...
        regime=setup.get("regime") or {};regime_name=regime.get("regime") if isinstance(regime,dict) else regime
        print(f"[V12 TRACE] {symbol} regime={regime_name} allowed={','.join(setup.get('allowed_engines') or [])} final={setup.get('signal')} reason={','.join(setup.get('rejection_reasons') or [])}");print(f"[V12 TRACE] {symbol} {_decision_summary(setup)}",flush=True)
        active=history.active_for_symbol(symbol);signal=setup.get("signal");levels=setup.get("trade_levels") or {};valid=signal in ("BUY","SELL") and _levels_ready(levels,signal);setup["valid"]=valid;signal_id=f"V12-{symbol}-{ts.replace(':','').replace('-','').replace(' ','-')}-{signal if valid else 'NO_TRADE'}-{setup.get('trigger_id','NONE')}";setup["signal_id"]=signal_id
        if not valid:
        ...
        if active and not same_setup_active:
            active_signals=[]
            for r in active:
                active_signals.append({"signal_id":r["signal_id"],"direction":r["direction"],"strategy":json_load(r.get("payload_json"),"strategy"),"setup_id":json_load(r.get("payload_json"),"setup_id"),"entry":r["entry"],"sl":r["sl"],"tp":r["tp"]})
            return {"status":"active_signal_locked","reason":"DIFFERENT_SETUP_ALREADY_ACTIVE","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"active_signals":active_signals,"resolved_this_scan":resolved,"setup":setup,"live_orders_allowed":False}
