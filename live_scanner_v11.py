...
def _lse_frame(symbol,timeframe,points=200):
    ...
    now=datetime.now(timezone.utc); days=max(2,int(points*minutes/1440)+2)
    client=LSE(api_key=os.environ["LSE_API_KEY"])
    raw=client.candles(market,timeframe,start=(now-timedelta(days=days)).date().isoformat(),end=(now+timedelta(days=1)).date().isoformat(),limit=points,order="desc")
    ...
    maximum=20.0 if timeframe=="5m" else 45.0
    if age>maximum: raise RuntimeError(f"STALE_MARKET_DATA:{symbol}:{timeframe}:age={age:.1f}m")
    return frame

def _load_frames(symbol):
    points=max(100,int(os.getenv("LIVE_SIGNAL_HISTORY","200")))
    m5=_lse_frame(symbol,"5m",points); m15=_lse_frame(symbol,"15m",points)
    latest=m5.iloc[-1].datetime; m15=m15[m15.datetime<=latest].reset_index(drop=True)
...
def _telegram_text(symbol,setup):
    d=setup["signal"]; l=setup["trade_levels"]; m15=setup["m15_trend"]
    side="🟢 BUY — ซื้อ" if d=="BUY" else "🔴 SELL — ขาย"
...
def scan_once(symbol="BTC"):
    ...
    with _SCAN_LOCK:
        ...
        ts=str(m5.iloc[-1].datetime); setup.update({"candle_time":ts,"closed_candle":ts,"symbol":symbol,"engine_version":engine.ENGINE_VERSION,"live_orders_allowed":False})
        ...
        if not valid:
            recorded=history.record_no_trade({**setup,"signal":"NO_TRADE","result":"NO_TRADE","created_at":datetime.now(timezone.utc).isoformat()})
            return {"status":"no_trade","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":"NO_TRADE","recorded":recorded,"rejection_reasons":setup.get("rejection_reasons",[]),**setup}
        ...
        telegram=send_telegram(_telegram_text(symbol,setup))
        return {"status":"signal_sent" if telegram.get("success") else "signal_recorded_telegram_failed","engine_version":engine.ENGINE_VERSION,"symbol":symbol,"signal":signal,"strategy":setup.get("strategy"),"signal_id":signal_id,"recorded":True,"telegram":telegram,"telegram_alert_sent":bool(telegram.get("success")),"setup":setup,"live_orders_allowed":False}
